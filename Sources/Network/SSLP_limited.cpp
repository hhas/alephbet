...
#define SSLPINT_INSTANCE_TIMEOUT 20'000
...
template <class T>
void PacketCopyIn(unsigned char*& Ptr, const T& Value) {
    int Size = sizeof(T);
...
template <class T>
void PacketCopyOut(unsigned char*& Ptr, T& Value) {
    int Size = sizeof(T);
...
template <class T>
void PacketCopyInList(unsigned char*& Ptr, const T* List, int Count) {
...
template <class T>
void PacketCopyOutList(unsigned char*& Ptr, T* List, int Count) {
    int Size = Count * sizeof(T);
...
// caller should notify anyone interested that a new instance was found (and should refer to it by the returned
// pointer). if we already knew about the instance, returns NULL.  in any case, the caller may discard the
// ServiceInstance it passed us (but NOT any we return - that one's ours :) )
static struct SSLP_ServiceInstance* SSLPint_FoundAnInstance(struct SSLP_ServiceInstance* inInstance) {
    ...
    while (theCurrentFoundInstance != NULL) {
        if (inInstance->sslps_address.host == theCurrentFoundInstance->mInstance->sslps_address.host) {
            if (inInstance->sslps_address.port == theCurrentFoundInstance->mInstance->sslps_address.port) {
                ...
                if (strncmp(theCurrentFoundInstance->mInstance->sslps_name, inInstance->sslps_name,
                            SSLP_MAX_NAME_LENGTH)
                    != 0) {
                    // name changed - copy new name and notify
                    strncpy(theCurrentFoundInstance->mInstance->sslps_name, inInstance->sslps_name,
                            SSLP_MAX_NAME_LENGTH);
    ...
    if (theCurrentFoundInstance == NULL) {
        struct SSLP_ServiceInstance* theServiceInstance
                = (struct SSLP_ServiceInstance*)malloc(sizeof(struct SSLP_ServiceInstance));
...
void SSLPint_RemoveTimedOutInstances() {
...
// they passed us.
static struct SSLP_ServiceInstance* SSLPint_LostAnInstance(struct SSLP_ServiceInstance* inInstance) {
...
static void SSLPint_FlushAllFoundInstances() {
...
static void SSLPint_ReceivedPacket() {
        ...
        case SSLPP_MESSAGE_FIND: {
            ...
            if (sBehaviorsDesired & SSLPINT_RESPONDING) {
                ...
                if (strncmp(theReceivedPacket->sslpp_service_type,
                    ...
                    logNote("type mismatch (%s != %s)", theReceivedPacket->sslpp_service_type,
                            theResponsePacket->sslpp_service_type);
        ...
        case SSLPP_MESSAGE_HAVE: {
            ...
            if (sBehaviorsDesired & SSLPINT_LOCATING) {
                ...
                if (strncmp(theReceivedPacket->sslpp_service_type, theFindPacket->sslpp_service_type,
                    ...
                    strncpy(theReceivedInstance.sslps_type, theReceivedPacket->sslpp_service_type,
                            SSLP_MAX_TYPE_LENGTH);
                    strncpy(theReceivedInstance.sslps_name, theReceivedPacket->sslpp_service_name,
                            SSLP_MAX_NAME_LENGTH);
                    ...
                    if (theReturnedInstance != NULL) {
                        // It didn't know about this instance, and has returned a pointer to its (new) permanent copy.
                        // Spread the word.
                    ...
                    logNote("wrong service type (%s != %s)", theReceivedPacket->sslpp_service_type,
                            theFindPacket->sslpp_service_type);
...
// note that all externally-visible functions should be called by the main thread, including SSLP_Pump which does the
// actual work.
void SSLP_Locate_Service_Instances(const char* inServiceType,
    ...
    sFindPacket->address.host = 0xffff'ffff; // UDP_Broadcast ignores host-part anyway
    ...
    // bzero(theFindPacket->sslpp_service_name, sizeof(theFindPacket->sslpp_service_name));	// (service_name is unused
    // in FIND)
...
void SSLP_Stop_Locating_Service_Instances(const char* inServiceType) {
...
void SSLP_Allow_Service_Discovery(const struct SSLP_ServiceInstance* inServiceInstance) {
...
void SSLP_Hint_Service_Discovery(const struct SSLP_ServiceInstance* inServiceInstance, const IPaddress* inAddress) {
...
void SSLP_Disallow_Service_Discovery(const struct SSLP_ServiceInstance* inInstance) {
