...
#include "PlayerImage_sdl.hpp"
#include "network_dialogs.hpp" // for net_rank
#include "player.hpp"          // for MAXIMUM_PLAYER_NAME_LENGTH
...
class w_found_players : public w_list<prospective_joiner_info> {
  ...
  private:
    ...
    vector<prospective_joiner_info>
            hidden_players; // players we don't want displayed - may include some not in found_players.
    vector<prospective_joiner_info> listed_players; // {found_players} - {hidden_players} (keyed by particular found
                                                    // instance, not name nor address)
    ...
    void draw_item(vector<prospective_joiner_info>::const_iterator i, SDL_Surface* s, int16 x, int16 y, uint16 width,
                   bool selected) const;
};
...
class w_players_in_game2 : public widget {
  public:
    ...
    void set_graph_data(const net_rank* inRankings, int inNumRankings, int inSelectedPlayer, bool inClumpPlayersByTeam,
                        bool inDrawScoresNotCarnage);
  ...
  protected:
    ...
    void draw_bar_or_bars(SDL_Surface* s, size_t rank_index, int center_x, int maximum_value,
    ...
    void draw_bar_labels(SDL_Surface* s, const vector<bar_info>& inBarInfos,
    ...
    void draw_bar(SDL_Surface* s, int inCenterX, int inBarColorIndex, int inBarValue, int inMaxValue,
...
class w_entry_point_selector : public w_select_button {
  public:
    ...
    const entry_point& getEntryPoint() { return mEntryPoint; }
