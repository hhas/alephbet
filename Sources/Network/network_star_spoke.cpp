...
#include "crc.hpp"
#include "mytm.hpp"
#include "network_private.hpp" // kPROTOCOL_TYPE
...
extern void call_distribution_response_function_if_available(byte* inBuffer, uint16 inBufferSize,
                                                             int16 inDistributionType, uint8 inSendingPlayerIndex);
...
static CircularQueue<SpokeLossyByteStreamChunkDescriptor>
...
static void process_optional_message(AIStream& ps, IncomingGameDataPacketProcessingContext& context,
                                     uint16 inMessageType);
static bool spoke_tick();
static void send_packet();
static void send_identification_packet();

static inline NetworkPlayer_spoke& getNetworkPlayer(size_t inIndex) {
...
static OSErr send_frame_to_local_hub(DDPFramePtr frame, NetAddrBlock* address, short protocolType, short port) {
...
static inline void check_send_packet_to_hub() {
...
void spoke_initialize(const NetAddrBlock& inHubAddress, int32 inFirstTick, size_t inNumberOfPlayers,
                      WritableTickBasedActionQueue* const inPlayerQueues[], bool inPlayerConnected[],
                      size_t inLocalPlayerIndex, bool inHubIsLocal) {
...
void spoke_cleanup(bool inGraceful) {
...
int32 spoke_get_net_time() {
    ...
    return (sConnected ? sOutgoingFlags.getWriteTick() - theDelay
                       : getNetworkPlayer(sLocalPlayerIndex).mQueue->getWriteTick());
}

void spoke_distribute_lossy_streaming_bytes_to_everyone(int16 inDistributionType, byte* inBytes, uint16 inLength,
                                                        bool inExcludeLocalPlayer, bool onlySendToTeam) {
    ...
    for (size_t i = 0; i < sNetworkPlayers.size(); i++) {
        if ((i != sLocalPlayerIndex || !inExcludeLocalPlayer) && !sNetworkPlayers[i].mZombie
            && sNetworkPlayers[i].mConnected) {
            if (onlySendToTeam) {
...
void spoke_distribute_lossy_streaming_bytes(int16 inDistributionType, uint32 inDestinationsBitmask, byte* inBytes,
                                            uint16 inLength) {
    if (inLength > sOutgoingLossyByteStreamData.getRemainingSpace()) {
        logNoteNMT("spoke has insufficient buffer space for %hu bytes of outgoing lossy streaming type %hd; discarded",
                   inLength, inDistributionType);
        return;
    }

    if (sOutgoingLossyByteStreamDescriptors.getRemainingSpace() < 1) {
        logNoteNMT("spoke has exhausted descriptor buffer space; discarding %hu bytes of outgoing lossy streaming type "
                   "%hd",
                   inLength, inDistributionType);
        return;
    }
    ...
    logDumpNMT("spoke application decided to send %d bytes of lossy streaming type %d destined for players 0x%x",
               inLength, inDistributionType, inDestinationsBitmask);
...
void spoke_received_network_packet(DDPPacketBufferPtr inPacket) {
    ...
    try {
        ...
        if ((!sConnected || !sSpokeActive) && thePacketMagic != kPingRequestPacket
    ...
    if (ps.tellg() >= ps.maxg()) {
        ...
        for (size_t i = 0; i < sNetworkPlayers.size(); i++) {
            if (i != sLocalPlayerIndex
                && (sNetworkPlayers[i].mConnected || sNetworkPlayers[i].mNetDeadTick > theSmallestUnacknowledgedTick)) {
                weAreAlone = false;
                break;
            }
        }
            ...
            for (size_t i = 0; i < sNetworkPlayers.size(); i++) {
                ...
                if (i == sLocalPlayerIndex) {
                    ...
                    while (thePlayer.mQueue->getWriteTick() < theSmallestUnacknowledgedTick) {
                        logDumpNMT("enqueued NET_DEAD_ACTION_FLAG for player %d tick %d", i,
                                   thePlayer.mQueue->getWriteTick());
    ...
    while (ps.tellg() < ps.maxg()) {
        ...
        for (size_t i = 0; i < sNetworkPlayers.size(); i++) {
            ...
            if (i == sLocalPlayerIndex && !reflected_flags) {
                if (theSmallestUnreadTick == sSmallestUnreceivedTick
                    && theSmallestUnreadTick >= sSmallestRealGameTick) {
...
static void handle_timing_adjustment_message(AIStream& ps, IncomingGameDataPacketProcessingContext& context) {
    ...
    if (theAdjustment != sRequestedTimingAdjustment) {
        ...
        logTraceNMT("new timing adjustment message; requested: %d outstanding: %d", sRequestedTimingAdjustment,
...
static void handle_lossy_byte_stream_message(AIStream& ps, IncomingGameDataPacketProcessingContext& context) {
    ...
    if (theDataLength > sizeof(sScratchBuffer)) {
        logNoteNMT("received too many bytes (%d) of lossy streaming data type %d from player %d; truncating",
                   theDataLength, theDistributionType, theSendingPlayer);
    ...
    logDumpNMT("received %d bytes of lossy streaming type %d data from player %d", theDataLength, theDistributionType,
               theSendingPlayer);

    call_distribution_response_function_if_available(sScratchBuffer, theDataLength, theDistributionType,
                                                     theSendingPlayer);
}
...
static bool spoke_tick() {
    ...
    if (sConnected) {
        int32 theSilentTicksBeforeNetDeath = (sOutgoingFlags.getReadTick() >= sSmallestRealGameTick)
    ...
    if (sOutstandingTimingAdjustment <= 0) {
        ...
        while (theNumberOfFlagsToProvide > 0) {
            ...
            WritableTickBasedActionQueue& theTargetQueue
                    = sConnected ? ((sOutgoingFlags.getWriteTick() >= sSmallestRealGameTick)
        ...
        for (size_t i = 0; i < sNetworkPlayers.size(); i++) {
            ...
            if (!thePlayer.mZombie) {
                while (thePlayer.mQueue->getWriteTick() < theLocalPlayerWriteTick) {
                    logDumpNMT("enqueueing NET_DEAD_ACTION_FLAG for player %d tick %d", i,
                               thePlayer.mQueue->getWriteTick());
                    thePlayer.mQueue->enqueue(static_cast<action_flags_t>(NET_DEAD_ACTION_FLAG));
                }
            }
...
static void send_packet() {
    try {
        ...
        if (sOutgoingLossyByteStreamDescriptors.getCountOfElements() > 0) {
            ...
            uint16 theMessageLength
                    = theDescriptor.mLength + sizeof(theDescriptor.mType) + sizeof(theDescriptor.mDestinations);

            ps << (uint16)kSpokeToHubLossyByteStreamMessageType << theMessageLength << theDescriptor.mType
        ...
        logDumpNMT("preparing to send packet: ACK %d, flags [%d,%d)", sSmallestUnreceivedTick,
                   sOutgoingFlags.getReadTick(), sOutgoingFlags.getWriteTick());
...
int32 spoke_latency() {
    return (sDisplayLatencyCount >= TICKS_PER_SECOND)
                   ? sDisplayLatencyTicks * 1000 / TICKS_PER_SECOND / sDisplayLatencyBuffer.size()
                   : NetworkStats::invalid;
}

TickBasedActionQueue* spoke_get_unconfirmed_flags_queue() { return &sUnconfirmedFlags; }

int32 spoke_get_smallest_unconfirmed_tick() { return sSmallestUnconfirmedTick; }
...
static const char* sAttributeStrings[kNumInt32Attributes]
        = {"pregame_ticks_before_net_death", "ingame_ticks_before_net_death",
           //	"outgoing_flags_queue_size",
           "recovery_send_period", "timing_window_size", "timing_nth_element"};

static int32* sAttributeDestinations[kNumInt32Attributes]
        = {&sSpokePreferences.mPregameTicksBeforeNetDeath, &sSpokePreferences.mInGameTicksBeforeNetDeath,
           //	&sSpokePreferences.mOutgoingFlagsQueueSize,
           &sSpokePreferences.mRecoverySendPeriod, &sSpokePreferences.mTimingWindowSize,
...
void SpokeParsePreferencesTree(InfoTree prefs, std::string version) {
    for (size_t i = 0; i < kNumInt32Attributes; ++i) {
        ...
        if (prefs.read_attr(sAttributeStrings[i], value)) {
            ...
            if (value < min)
                logWarning("improper value %d for attribute %s of <spoke>; must be at least %d. using default of %d",
    ...
    if (sSpokePreferences.mTimingNthElement >= sSpokePreferences.mTimingWindowSize) {
        logWarning("value for <spoke> attribute %s (%d) must be less than value for %s (%d).  using %d",
                   sAttributeStrings[kTimingNthElementAttribute], sSpokePreferences.mTimingNthElement,
                   sAttributeStrings[kTimingWindowSizeAttribute], sSpokePreferences.mTimingWindowSize,
                   sSpokePreferences.mTimingWindowSize - 1);
...
InfoTree SpokePreferencesTree() {
    ...
    for (size_t i = 0; i < kNumInt32Attributes; ++i) root.put_attr(sAttributeStrings[i], *(sAttributeDestinations[i]));
    root.put_attr("adjust_timing", sSpokePreferences.mAdjustTiming);

    return root;
}

void DefaultSpokePreferences() {
