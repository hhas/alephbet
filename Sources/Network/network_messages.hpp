...
class AcceptJoinMessage : public SmallMessageHelper {
  public:
    ...
    AcceptJoinMessage* clone() const { return new AcceptJoinMessage(*this); }
...
class CapabilitiesMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kCAPABILITIES_MESSAGE
    };

    CapabilitiesMessage() : SmallMessageHelper() {}

    CapabilitiesMessage(const Capabilities& capabilities) : SmallMessageHelper() { mCapabilities = capabilities; }

    CapabilitiesMessage* clone() const { return new CapabilitiesMessage(*this); }
...
class ChangeColorsMessage : public SmallMessageHelper {
  public:
    ...
    ChangeColorsMessage* clone() const { return new ChangeColorsMessage(*this); }
...
class ClientInfoMessage : public SmallMessageHelper {
  public:
    ...
    ClientInfoMessage(int16 stream_id, const ClientChatInfo* clientChatInfo, int16 action)
        : mStreamID(stream_id), mAction(action), mClientChatInfo(*clientChatInfo) {}

    ClientInfoMessage* clone() const { return new ClientInfoMessage(*this); }
...
class RemoteHubReadyMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kREMOTE_HUB_READY_MESSAGE
    };
...
class HelloMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kHELLO_MESSAGE
    };

    HelloMessage() : SmallMessageHelper() {}

    HelloMessage(const std::string& version) : SmallMessageHelper(), mVersion(version) {}

    HelloMessage* clone() const { return new HelloMessage(*this); }
...
class RemoteHubHostResponseMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kREMOTE_HUB_RESPONSE_MESSAGE
    };
...
class RemoteHubHostConnectMessage : public HelloMessage {
  public:
    ...
    enum {
        kType = kREMOTE_HUB_REQUEST_MESSAGE
    };
...
class JoinerInfoMessage : public SmallMessageHelper {
  public:
    ...
    JoinerInfoMessage* clone() const { return new JoinerInfoMessage(*this); }

    prospective_joiner_info* info() { return &mInfo; }

    void info(const prospective_joiner_info* theInfo) { mInfo = *theInfo; }
...
// zips on deflate, unzips on inflate
{
  public:
    ...
    BigChunkOfZippedDataMessage(MessageTypeID inType, const Uint8* inBuffer = NULL, size_t inLength = 0)
        : BigChunkOfDataMessage(inType, inBuffer, inLength) {}
...
template <int messageType, class T>
class TemplatizedDataMessage : public T {
  public:
    ...
    TemplatizedDataMessage(const Uint8* inBuffer = NULL, size_t inLength = 0) : T(kType, inBuffer, inLength) {};

    TemplatizedDataMessage(const TemplatizedDataMessage& other) : T(other) {}

    TemplatizedDataMessage* clone() const { return new TemplatizedDataMessage(*this); }
};
...
class NetworkChatMessage : public SmallMessageHelper {
  public:
    ...
    NetworkChatMessage(const char* chatText = NULL, int16 senderID = 0, int16 target = 0, int16 targetID = -1)
    ...
    NetworkChatMessage* clone() const { return new NetworkChatMessage(*this); }
...
class NetworkStatsMessage : public SmallMessageHelper {
  public:
    ...
    NetworkStatsMessage* clone() const { return new NetworkStatsMessage(*this); }
...
class ServerWarningMessage : public SmallMessageHelper {
  public:
    ...
    ServerWarningMessage* clone() const { return new ServerWarningMessage(*this); }
...
class TopologyMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kTOPOLOGY_MESSAGE
    };

    TopologyMessage() : SmallMessageHelper() {}

    TopologyMessage(NetTopology* topology) : SmallMessageHelper() { mTopology = *topology; }

    TopologyMessage* clone() const { return new TopologyMessage(*this); }
...
class RemoteHubCommandMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kREMOTE_HUB_COMMAND_MESSAGE
    };
    ...
    RemoteHubCommandMessage* clone() const { return new RemoteHubCommandMessage(*this); }
...
struct Client {
    ...
    bool can_pregame_chat() {
        return ((state == _connected) || (state == _connected_but_not_yet_shown) || (state == _ungatherable)
                || (state == _joiner_didnt_accept) || (state == _awaiting_accept_join) || (state == _awaiting_map));
    }
