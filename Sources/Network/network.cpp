...
#include "map.hpp"         // for TICKS_PER_SECOND and "struct entry_point"
#include "mytm.hpp"        // ZZZ: both versions use mytm now
#include "preferences.hpp" // for network_preferences and environment_preferences

#include "network_lookup_sdl.hpp"
...
#include "NetworkGameProtocol.hpp"
...
const static NetworkStats sInvalidStats    = {NetworkStats::invalid, NetworkStats::invalid, NetworkStats::invalid, 0};
...
bool player_is_ignored(int player_index) { return (sIgnoredPlayers.find(player_index) != sIgnoredPlayers.end()); }
...
struct ignore_lua {
    void operator()(const std::string&) const { ToggleLuaMute(); }
};
...
Client::Client(std::shared_ptr<CommunicationsChannel> inChannel)
    ...
    mRemoteHubHostRequestMessageHandler.reset(
            newMessageHandlerMethod(this, &Client::handleRemoteHubHostConnectMessage));
...
void Client::drop() {
    ...
    if (state == _connected) { // (remove from the list of joinable players)
        ...
        if (i != topology->player_count) {
            for (; i < topology->player_count - 1; i++) { topology->players[i] = topology->players[i + 1]; }
...
bool Client::capabilities_indicate_player_is_gatherable(bool warn_joiner) {
    ...
    if (capabilities[Capabilities::kGameworld] < Capabilities::kGameworldVersion
        || (shapes_file_is_m1() && capabilities[Capabilities::kGameworldM1] < Capabilities::kGameworldM1Version)) {
        if (warn_joiner) {
            ServerWarningMessage serverWarningMessage(
                    expand_app_variables("The gatherer is using a new version of $appName$. You will not appear in the "
                                         "list of available players."),
                    ServerWarningMessage::kJoinerUngatherable);
    ...
    if (network_preferences->game_protocol == _network_game_protocol_star) {
        if (capabilities[Capabilities::kStar] == 0) {
            if (warn_joiner) {
                ServerWarningMessage serverWarningMessage(getcstr(s, strNETWORK_ERRORS, netWarnJoinerHasNoStar),
                                                          ServerWarningMessage::kJoinerUngatherable);
                channel->enqueueOutgoingMessage(serverWarningMessage);
            }
            return false;
            ...
            if (warn_joiner) {
                ServerWarningMessage serverWarningMessage(
                        expand_app_variables("The gatherer is using a newer version of $appName$. You will not appear "
                                             "in the list of available players."),
                        ServerWarningMessage::kJoinerUngatherable);
                channel->enqueueOutgoingMessage(serverWarningMessage);
            }
            return false;
        ...
        if (capabilities[Capabilities::kRing] == 0) {
            if (warn_joiner) {
                ServerWarningMessage serverWarningMessage(getcstr(s, strNETWORK_ERRORS, netWarnJoinerHasNoRing),
                                                          ServerWarningMessage::kJoinerUngatherable);
                channel->enqueueOutgoingMessage(serverWarningMessage);
            }
            return false;
    ...
    if (do_netscript) {
        if (capabilities[Capabilities::kLua] == 0) {
            if (warn_joiner) {
                char s[256];
                ServerWarningMessage serverWarningMessage(getcstr(s, strNETWORK_ERRORS, netWarnJoinerNoLua),
                                                          ServerWarningMessage::kJoinerUngatherable);
                channel->enqueueOutgoingMessage(serverWarningMessage);
            }
            return false;
            ...
            if (warn_joiner) {
                ServerWarningMessage serverWarningMessage(
                        "The gatherer is using a newer version of Lua (net script) that you do not have. You will not "
                        "appear in the list of available players.",
                        ServerWarningMessage::kJoinerUngatherable);
                channel->enqueueOutgoingMessage(serverWarningMessage);
            }
            return false;
    ...
    if (topology->game_data.net_game_type == _game_of_rugby) {
        if (capabilities[Capabilities::kRugby] == 0) {
            if (warn_joiner) {
                ServerWarningMessage serverWarningMessage(
                        expand_app_variables("The gatherer is using a newer version of $appName$ with different rugby "
                                             "scoring. You will not appear in the list of available players."),
                        ServerWarningMessage::kJoinerUngatherable);
                channel->enqueueOutgoingMessage(serverWarningMessage);
            }
            return false;
...
#endif // AB_NETWORK_STANDALONE_HUB
        } else {
            state = Client::_ungatherable;
        }
...
void Client::handleAcceptJoinMessage(AcceptJoinMessage* acceptJoinMessage, CommunicationsChannel*) {
    if (state == _awaiting_accept_join) {
        if (acceptJoinMessage->accepted()) {
            ...
            GameSessionMessage gameSessionMessage(reinterpret_cast<const uint8*>(gameSessionIdentifier.c_str()),
                                                  gameSessionIdentifier.size());
...
void Client::handleChangeColorsMessage(ChangeColorsMessage* changeColorsMessage, CommunicationsChannel* channel) {
    if (can_pregame_chat()) {
        ...
        if (client_chat_info[stream_id]) {
            ...
            ClientInfoMessage clientInfoMessage(stream_id, client_chat_info[stream_id],
                                                (int16)ClientInfoMessage::kUpdate);
            client_map_t::iterator it;
            for (it = connections_to_clients.begin(); it != connections_to_clients.end(); it++) {
                if (it->second->can_pregame_chat()) {
                    it->second->channel->enqueueOutgoingMessage(clientInfoMessage);
    ...
    if (state == _awaiting_map) {
        ...
        if (i != topology->player_count) {
            ...
            if (player->desired_color != changeColorsMessage->color() || player->team != changeColorsMessage->team()) {
...
void Client::handleRemoteHubHostConnectMessage(RemoteHubHostConnectMessage* message, CommunicationsChannel* channel) {
    channel->enqueueOutgoingMessage(
            RemoteHubHostResponseMessage(false)); // we already have a gatherer using us (the remote hub)
    state = Client::_disconnect;
}
...
void Client::handleChatMessage(NetworkChatMessage* netChatMessage, CommunicationsChannel*) {
    ...
    if (state == _ingame) {
        ...
        if (netChatMessage->target() == NetworkChatMessage::kTargetPlayers) {
            NetworkChatMessage chatMessage(netChatMessage->chatText(), getStreamIdFromChannel(channel.get()),
                                           NetworkChatMessage::kTargetPlayers);
            client_map_t::iterator it;
            for (it = connections_to_clients.begin(); it != connections_to_clients.end(); it++) {
            ...
            if (chatCallbacks) {
                for (int playerIndex = 0; playerIndex < topology->player_count; playerIndex++) {
                    if (topology->players[playerIndex].stream_id == getStreamIdFromChannel(channel.get())) {
                        if (player_is_ignored(playerIndex))
                            return;
                        chatCallbacks->ReceivedMessageFromPlayer(topology->players[playerIndex].player_data.name,
                                                                 netChatMessage->chatText());
                        return;
                    }
                }
            }
        ...
        if (netChatMessage->target() == NetworkChatMessage::kTargetClients) {
            NetworkChatMessage chatMessage(netChatMessage->chatText(), getStreamIdFromChannel(channel.get()),
                                           NetworkChatMessage::kTargetClients);
            client_map_t::iterator it;
            for (it = connections_to_clients.begin(); it != connections_to_clients.end(); it++) {
                if (it->second->can_pregame_chat()) {
                    it->second->channel->enqueueOutgoingMessage(chatMessage);
            ...
            if (chatCallbacks) {
                int stream_id = getStreamIdFromChannel(channel.get());
                if (client_chat_info[stream_id]) {
                    chatCallbacks->ReceivedMessageFromPlayer(client_chat_info[stream_id]->name.c_str(),
                                                             netChatMessage->chatText());
                } else {
    ...
    if (handlerState == netJoining) {
        ...
        if (capabilities[Capabilities::kGameworld] < Capabilities::kGameworldVersion
            ...
            || (network_preferences->game_protocol == _network_game_protocol_star
                && capabilities[Capabilities::kStar] < Capabilities::kStarVersion)) {
            // I'm not gatherable
            ...
            alert_user(expand_app_variables("The gatherer is using an old version of $appName$. You will not appear in "
            ...
            *client_chat_info[id] = *clientInfoMessage->info();
            ...
            if (use_remote_hub) {
                if (gatherCallbacks) {
                    ...
                    std::strncpy(joiner_info.name, clientInfoMessage->info()->name.c_str(),
                                 clientInfoMessage->info()->name.length());

                    if (!gatherCallbacks->JoiningPlayerDropped(
                                &joiner_info)) // we don't really know the joiner state here so we deduce it like this
                    {
    ...
    if (chatCallbacks) {
        if (netState == netActive) {
            ...
            if (client_chat_info[chatMessage->senderID()]) {
                chatCallbacks->ReceivedMessageFromPlayer(client_chat_info[chatMessage->senderID()]->name.c_str(),
                                                         chatMessage->chatText());
            } else {
                logAnomaly("chat message from %i, player not found", chatMessage->senderID());
            }
...
static void handleTopologyMessage(TopologyMessage* topologyMessage, CommunicationsChannel*) {
    if (netState == netWaiting) {
        *topology = *(topologyMessage->topology());
        ...
        logWarning("unexpected topology message received -- gatherer and joiner could disagree on topology! (netState "
                   "is %i)",
                   netState);
    }
}
...
static void handleGameSessionMessage(GameSessionMessage* gameSessionMessage, CommunicationsChannel*) {
    if (handlerState == netWaiting) {
        gameSessionIdentifier.assign(gameSessionMessage->buffer(),
                                     gameSessionMessage->buffer() + gameSessionMessage->length());
    } else {
...
void NetSetGatherCallbacks(GatherCallbacks* gc) { gatherCallbacks = gc; }

void NetSetChatCallbacks(ChatCallbacks* cc) { chatCallbacks = cc; }
    ...
    if (netState == netActive) {
        if (connection_to_server) {
            NetworkChatMessage chatMessage(
                    message.c_str(), 0,
                    NetworkChatMessage::kTargetPlayers); // gatherer will replace senderID with my ID
            connection_to_server->enqueueOutgoingMessage(chatMessage);
        } else {
            NetworkChatMessage chatMessage(message.c_str(), 0,
                                           NetworkChatMessage::kTargetPlayers); // gatherer stream ID is always 0
            client_map_t::iterator it;
            for (it = connections_to_clients.begin(); it != connections_to_clients.end(); it++) {
        ...
        NetworkChatMessage chatMessage(message.c_str(), 0,
                                       NetworkChatMessage::kTargetClients); // gatherer stream ID is always 0
        client_map_t::iterator it;
        for (it = connections_to_clients.begin(); it != connections_to_clients.end(); it++) {
            if (it->second->can_pregame_chat()) {
                it->second->channel->enqueueOutgoingMessage(chatMessage);
        ...
        NetworkChatMessage chatMessage(message.c_str(), 0,
                                       NetworkChatMessage::kTargetClients); // gatherer will replace senderID with my ID
        connection_to_server->enqueueOutgoingMessage(chatMessage);
    } else {
...
std::string InGameChatCallbacks::prompt() { return (std::string(player_preferences->name) + ":"); }
...
void NetSetDefaultInflater(CommunicationsChannel* channel) { channel->setMessageInflater(inflater); }
...
void NetExit(void) {
    ...
    {
        client_map_t::iterator it;
        for (it = connections_to_clients.begin(); it != connections_to_clients.end(); it++) delete (it->second);
        connections_to_clients.clear();
    }

    {
        client_chat_info_map_t::iterator it;
        for (it = client_chat_info.begin(); it != client_chat_info.end(); it++) delete (it->second);
...
std::weak_ptr<Pinger> NetGetPinger() { return std::weak_ptr<Pinger>(pinger); }
...
void NetRemovePinger() { pinger.reset(); }
...
void NetAddDistributionFunction(int16 inDataTypeID, NetDistributionProc inProc, bool inLossy) {
...
void NetRemoveDistributionFunction(int16 inDataTypeID) { distribution_info_map.erase(inDataTypeID); }
...
void NetDistributeInformation(short type, void* buffer, short buffer_size, bool send_to_self, bool only_send_to_team) {
    sCurrentGameProtocol->DistributeInformation(type, buffer, buffer_size, send_to_self, only_send_to_team);
}

short NetState(void) { return netState; }

// Game session identifiers allow the metaserver to
// identify which players join a particular game.

std::string NetSessionIdentifier(void) { return gameSessionIdentifier; }
...
void NetInitializeSessionIdentifier(void) {
    ...
    for (int i = 0; i < 16; i++) { gameSessionIdentifier += static_cast<char>(rand() % 256); }
}
...
bool NetConnectRemoteHub(const IPaddress& remote_hub_address) {
    ...
    auto response_message = std::unique_ptr<RemoteHubHostResponseMessage>(
            connection_to_server->receiveSpecificMessage<RemoteHubHostResponseMessage>(3000u, 3000u));
    ...
    if (!std::unique_ptr<RemoteHubReadyMessage>(
                connection_to_server->receiveSpecificMessage<RemoteHubReadyMessage>())) {
        connection_to_server->disconnect();
        return false;
    }
...
void NetSetCapabilities(const Capabilities* capabilities) { my_capabilities = *capabilities; }
...
bool NetGameJoin(void* player_data, short player_data_size, const char* host_addr_string) {
...
void NetCancelJoin(void) {
    assert(netState == netConnecting || netState == netJoining || netState == netWaiting || netState == netCancelled
           || netState == netJoinErrorOccurred);
}
...
void NetChangeColors(int16 color, int16 team) {
    ...
    if (netState == netWaiting) {
        ...
        if (player->desired_color != color || player->team != team) {
...
*/
void NetSetServerIdentifier(short identifier) { sServerPlayerIndex = identifier; }
...
void NetDDPPacketHandler(DDPPacketBufferPtr packet) { sCurrentGameProtocol->PacketHandler(packet); }
...
static void NetLocalAddrBlock(NetAddrBlock* address, short socketNumber) {
    ...
    address->host = 0x7f00'0001;  //!! XXX (ZZZ) yeah, that's really bad.
    address->port = socketNumber; // OTOH, I guess others are set up to "stuff" the address they actually saw for us
                                  // instead of this, anyway... right??  So maybe it's not that big a deal.......
}
...
static void NetUpdateTopology(void) {
    ...
    for (localPlayerIndex = 0; localPlayerIndex < topology->player_count; ++localPlayerIndex) {
        if (topology->players[localPlayerIndex].identifier == localPlayerIdentifier)
            break;
    }
#ifdef DEBUG
    if (localPlayerIndex == topology->player_count)
        fdprintf("couldn't find my identifier: %p", (void*)topology);
#endif
#endif
}

static bool NetSetSelfSend(bool on) { return false; }
...
void match_starts_with_existing_players(player_start_data* ioStartArray, short* ioStartCount) {
    ...
    for (p = 0; p < *ioStartCount; p++) { ioStartArray[p] = theOriginalStarts[startAssignedToPlayer[p]]; }
}
...
#endif
            if (success)
                NetDistributeGameDataToAllPlayers(wad, length, do_physics);

        } else { // wait for de damn map.

            if (use_remote_hub && get_game_state() == _change_level) // if the gatherer is using a remote hub, it has to
                                                                     // send it to the hub first
            {
                wad = (unsigned char*)get_map_for_net_transfer(entry);
                assert(wad);
                length = get_net_map_data_length(wad);
...
void SetNetscriptStatus(bool status) { do_netscript = status; }
...
OSErr NetDistributeGameDataToAllPlayers(byte* wad_buffer, int32 wad_length, bool do_physics,
    ...
    message_id         = remote_hub                      ? _connecting_to_remote_hub
                         : (topology->player_count == 2) ? (_distribute_map_single)
                                                         : (_distribute_map_multiple);
    physics_message_id = remote_hub                      ? _connecting_to_remote_hub
                         : (topology->player_count == 2) ? (_distribute_physics_single)
    ...
    if (physics_buffer) {
        if (zipCapableChannels.size()) {
            ...
            std::for_each(zipCapableChannels.begin(), zipCapableChannels.end(),
                          std::bind(&CommunicationsChannel::enqueueOutgoingMessage, std::placeholders::_1,
                                    *uninflatedMessage));
        }
        ...
        if (zipIncapableChannels.size()) {
            ...
            std::for_each(
                    zipIncapableChannels.begin(), zipIncapableChannels.end(),
                    std::bind(&CommunicationsChannel::enqueueOutgoingMessage, std::placeholders::_1, physicsMessage));
        }
    }

    {
        ...
        if (zipIncapableChannels.size()) {
            ...
            std::for_each(zipIncapableChannels.begin(), zipIncapableChannels.end(),
                          std::bind(&CommunicationsChannel::enqueueOutgoingMessage, std::placeholders::_1, mapMessage));
        }
    }
    ...
    if (do_netscript) {
        ...
        if (zipIncapableChannels.size()) {
            ...
            std::for_each(zipIncapableChannels.begin(), zipIncapableChannels.end(),
                          std::bind(&CommunicationsChannel::enqueueOutgoingMessage, std::placeholders::_1, luaMessage));
        }
    }

    {
        ...
        std::for_each(
                ...
                std::bind(&CommunicationsChannel::enqueueOutgoingMessage, std::placeholders::_1, endGameDataMessage));
    }

    CommunicationsChannel::multipleFlushOutgoingMessages(channels, false, 30'000, 30'000);
...
byte* NetReceiveGameData(bool do_physics) {
    ...
    std::unique_ptr<EndGameDataMessage> endGameDataMessage(
            connection_to_server->receiveSpecificMessage<EndGameDataMessage>((Uint32)60'000, (Uint32)30'000));
...
int32 NetGetNetTime(void) { return sCurrentGameProtocol->GetNetTime(); }

bool NetCheckWorldUpdate() { return sCurrentGameProtocol->CheckWorldUpdate(); }
...
void NetProcessMessagesInGame() {
    if (connection_to_server) {
        ...
        if (sCurrentGameProtocol == static_cast<NetworkGameProtocol*>(&sStarGameProtocol)
            && last_network_stats_send + network_stats_send_period < machine_tick_count()) {
...
bool NetCheckForNewJoiner(prospective_joiner_info& info, CommunicationsChannelFactory* server_override,
                          bool process_new_joiners) {
    auto actual_server = server_override ? server_override : server;
    CommunicationsChannel* new_joiner
            = process_new_joiners && actual_server ? actual_server->newIncomingConnection() : nullptr;
...
short NetUpdateJoinState(void) {
    ...
    if (newState != netPlayerAdded && newState != netPlayerDropped && newState != netPlayerChanged
        && newState != netChatMessageReceived && newState != netStartingResumeGame && newState != NONE)
        netState = newState;
...
static void NetDistributeTopology(short tag) {
    ...
    for (playerIndex = 0; playerIndex < topology->player_count; ++playerIndex) {
        ...
        if (topology->players[playerIndex].identifier != NONE && playerIndex != localPlayerIndex) {
            ...
            if (client)
                client->channel->enqueueOutgoingMessage(topologyMessage);
        }
    }
}

bool NetAllowCrosshair() {
    return (dynamic_world->player_count == 1 || (dynamic_world->game_information.cheat_flags & _allow_crosshair));
}

bool NetAllowTunnelVision() {
    return (dynamic_world->player_count == 1 || dynamic_world->game_information.cheat_flags & _allow_tunnel_vision);
}

bool NetAllowBehindview() {
    return (dynamic_world->player_count == 1 || dynamic_world->game_information.cheat_flags & _allow_behindview);
}
...
bool NetAllowSavingLevel() {
    return (dynamic_world->player_count == 1 || localPlayerIndex == sServerPlayerIndex
            || !(dynamic_world->game_information.cheat_flags & _disable_saving_level));
}

bool NetAllowOverlayMap() {
    return (dynamic_world->player_count == 1 || (dynamic_world->game_information.cheat_flags & _allow_overlay_map));
}
