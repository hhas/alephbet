...
StandaloneHub::~StandaloneHub() { NetExit(); }
...
bool StandaloneHub::SetupGathererGame(bool& gathering_done) {
    ...
    if (!NetGather(&_topology_message->topology()->game_data, sizeof(game_info),
                   &_topology_message->topology()->players->player_data, sizeof(player_info), !_physics_message,
                   false)) {
        return Reset();
    }
...
bool StandaloneHub::WaitForGatherer() {
    ...
    auto gatherer_request = std::unique_ptr<RemoteHubHostConnectMessage>(
            _gatherer->receiveSpecificMessage<RemoteHubHostConnectMessage>(3000u, 3000u));
    ...
    if (can_use_hub) {
        auto gatherer_capabilities = std::unique_ptr<CapabilitiesMessage>(
                _gatherer->receiveSpecificMessage<CapabilitiesMessage>(5000u, 5000u));
...
bool StandaloneHub::CheckGathererCapabilities(const Capabilities* capabilities) {
    ...
    return gatherer_capabilities[Capabilities::kStar] == Capabilities::kStarVersion
...
bool StandaloneHub::GatherJoiners() {
    while (!_gatherer_client.expired() && !_start_game_signal
           && machine_tick_count() - _start_check_timeout_ms < _gathering_timeout_ms) {
        ...
        while (auto message = client->receiveMessage()) {
            switch (message->type()) {
                case kTOPOLOGY_MESSAGE:
                    if (_topology_message)
                        delete message; // we don't expect to get a topology message while transiting to another level
                    else
