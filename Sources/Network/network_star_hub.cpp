...
/*
 *  The portion of the star game network protocol run on a single machine,
 *  perhaps (eventually) without an actual player on the machine - or even
 *  without most of the game, for that matter.
 */
...
#include "WindowedNthElementFinder.hpp"
#include "mytm.hpp"
#include "network_private.hpp"

#include "crc.hpp"
#include "player.hpp" // for masking out action flags triggers :(
...
static CircularQueue<HubLossyByteStreamChunkDescriptor>
        sOutgoingLossyByteStreamDescriptors(kLossyByteStreamDescriptorCount);
...
static bool player_provided_flags_from_tick_to_tick(size_t inPlayerIndex, int32 inFirstNewTick,
                                                    int32 inSmallestUnreceivedTick);
...
static inline NetworkPlayer_hub& getNetworkPlayer(size_t inIndex) {
    assert(inIndex < sNetworkPlayers.size());
    return sNetworkPlayers[inIndex];
}

static inline TickBasedActionQueue& getFlagsQueue(size_t inIndex) {
    assert(inIndex < sFlagsQueues.size());
    return sFlagsQueues[inIndex];
}

static inline TickBasedActionQueue& getLateFlagsQueue(size_t inIndex) {
    assert(inIndex < sFlagsQueues.size());
    return sLateFlagsQueues[inIndex];
}

static inline bool operator<(const NetAddrBlock& a, const NetAddrBlock& b) { return memcmp(&a, &b, sizeof(a)) < 0; }

static OSErr send_frame_to_local_spoke(DDPFramePtr frame, NetAddrBlock* address, short protocolType, short port) {
#ifndef AB_NETWORK_STANDALONE_HUB
...
static inline void check_send_packet_to_spoke() {
...
void hub_initialize(int32 inStartingTick, int inNumPlayers, const NetAddrBlock* const* inPlayerAddresses,
                    int inLocalPlayerIndex) {
...
#ifdef AB_NETWORK_STANDALONE_HUB
    if (sReferencePlayerIndex == NONE)
        sReferencePlayerIndex = 0; // we have no connected players at this point, but just in case
#endif
...
bool hub_is_active() { return sHubActive.load(); }

void hub_cleanup(bool inGraceful, int32 inSmallestPostGameTick) {
    if (sHubInitialized) {
        if (inGraceful) {
            // Signal our demise
...
void hub_received_network_packet(DDPPacketBufferPtr inPacket) {
    ...
    try {
        ...
        // Processing packets?
        if (!sHubActive && thePacketMagic != kPingRequestPacket && thePacketMagic != kPingResponsePacket)
            return;
...
static void hub_update_player_pregame_state(int inPlayerIndex, int16 state) {
    if (sNetworkPlayers[inPlayerIndex].mStats.pregame_state == NetworkStats::invalid
        && sSmallestUnsentTick < sSmallestRealGameTick) {
...
static void hub_received_ping_request(AIStream& ps, NetAddrBlock address) {
    ...
    } catch (...) { logWarningNMT("Caught exception while constructing/sending ping response packet"); }
...
static void hub_received_game_data_packet_v1(AIStream& ps, int inSenderIndex) {
    ...
    if (theSmallestUnacknowledgedTick > sSmallestIncompleteTick) {
        logAnomalyNMT("received ack from player %d for tick %d; have only sent up to %d", inSenderIndex,
                      theSmallestUnacknowledgedTick, sSmallestIncompleteTick);
        return;
    }
    ...
    //        int	theRedundantActionFlagsCount = std::min(theQueue.getWriteTick() - theStartTick,
    //        theActionFlagsCount);
    ...
    // Enqueue late flags
    int theLateActionFlagsCount = std::min(theQueue.getWriteTick() - theLateQueue.getWriteTick(),
                                           theActionFlagsCount - theRedundantActionFlagsCount);
    ...
    int theRemainingQueueSpace    = (sPlayerDataDisposition.getReadTick() < sSmallestRealGameTick
                                  && theQueue.size() > sHubPreferences.mPregameWindowSize)
                                            ? 0
                                            : theQueue.availableCapacity();
    ...
    if (thePlayer.mSmallestUnheardTick >= sSmallestRealGameTick
        && static_cast<int32>(thePlayer.mNthElementFinder.window_size()) != sHubPreferences.mInGameWindowSize)
    ...
    if (thePlayer.mOutstandingTimingAdjustment == 0 && thePlayer.mNthElementFinder.window_full()) {
        thePlayer.mOutstandingTimingAdjustment = thePlayer.mNthElementFinder.nth_smallest_element(
                (thePlayer.mSmallestUnheardTick >= sSmallestRealGameTick) ? sHubPreferences.mInGameNthElement
        ...
        if (thePlayer.mOutstandingTimingAdjustment != 0) {
            ...
            logTraceNMT("tick %d: asking player %d to adjust timing by %d", sSmallestIncompleteTick, inSenderIndex,
                        thePlayer.mOutstandingTimingAdjustment);
            ...
            if (debug_timing_adjustments && thePlayer.mSmallestUnheardTick >= sSmallestRealGameTick) {
                dout << sNetworkTicker << ": "
                     ...
                     << "O" << thePlayer.mOutstandingTimingAdjustment << " " << thePlayer.mStats.latency << "/"
                     << thePlayer.mStats.jitter << " "
                     << reinterpret_cast<player_info*>(NetGetPlayerData(inSenderIndex))->name << std::endl;
                dout << "S";
                for (int i = 0; i < 20; ++i) {
                    dout << std::setw(3) << thePlayer.mNthElementFinder.nth_smallest_element(i) << " ";
                }
                dout << "M";
                ...
                dout << "L";
                for (int i = 19; i >= 0; --i) {
                    dout << std::setw(3) << thePlayer.mNthElementFinder.nth_largest_element(i) << " ";
                }
                dout << std::endl;
            }
#endif
    ...
    if (theEnqueueableFlagsCount > 0) {
        ...
        bool shouldSend = player_provided_flags_from_tick_to_tick(
                inSenderIndex, theStartTick + theRedundantActionFlagsCount + theLateActionFlagsCount,
                theStartTick + theRedundantActionFlagsCount + theEnqueueableFlagsCount + theLateActionFlagsCount);
    ...
    for (int i = 0; i < sNetworkPlayers.size(); i++) {
        if (getFlagsQueue(i).getWriteTick() == sSmallestIncompleteTick) {
            ...
            if (sLaggingPlayersBitmask & (1 << i) && theLateQueue.getWriteTick() > theLateQueue.getReadTick()) {
                uint32 midpoint
                        = ((theLateQueue.getWriteTick() - theLateQueue.getReadTick()) / 2 + theLateQueue.getReadTick());
                ...
                while (theLateQueue.getReadTick() < midpoint) {
                    ...
                    triggerFlags |= (motionFlags
                                     & (_left_trigger_state | _right_trigger_state | _action_trigger_state
                                        | _cycle_weapons_forward | _cycle_weapons_backward | _toggle_map
                                        | _microphone_button | _swim));
                    theLateQueue.dequeue();
                }
                ...
                if (local_random() % 10 > 8)
                    sLastFlagsReceived[i] = 0;
            }
...
static bool player_provided_flags_from_tick_to_tick(size_t inPlayerIndex, int32 inFirstNewTick,
                                                    int32 inSmallestUnreceivedTick) {
    logTraceNMT("player_provided_flags_from_tick_to_tick(%d, %d, %d)", inPlayerIndex, inFirstNewTick,
                inSmallestUnreceivedTick);
    ...
    for (int i = inFirstNewTick; i < inSmallestUnreceivedTick; i++) {
        ...
        while (theLateQueue.getReadTick() < theLateQueue.getWriteTick()) theLateQueue.dequeue();
...
static void process_lossy_byte_stream_message(AIStream& ps, int inSenderIndex, uint16 inLength) {
    ...
    logDumpNMT("got %d bytes of lossy stream type %d from player %d for destinations 0x%x", theDescriptor.mLength,
               theDescriptor.mType, theDescriptor.mSender, theDescriptor.mDestinations);

    bool canEnqueue = true;

    if (sOutgoingLossyByteStreamDescriptors.getRemainingSpace() < 1) {
        logNoteNMT("no descriptor space remains; discarding (%uh) bytes of lossy streaming data of distribution type "
                   "%hd from player %hu destined for 0x%lx",
                   theDescriptor.mLength, theDescriptor.mType, theDescriptor.mSender, theDescriptor.mDestinations);
        canEnqueue = false;
    }
    ...
    if (theDescriptor.mLength > sOutgoingLossyByteStreamData.getRemainingSpace()) {
        logNoteNMT("insufficient buffer space for %uh bytes of lossy streaming data of distribution type %hd from "
                   "player %hu destined for 0x%lx; discarded",
                   theDescriptor.mLength, theDescriptor.mType, theDescriptor.mSender, theDescriptor.mDestinations);
        canEnqueue = false;
    }
...
static void make_player_netdead(int inPlayerIndex) {
    ...
    player_provided_flags_from_tick_to_tick(inPlayerIndex, getFlagsQueue(inPlayerIndex).getWriteTick(),
                                            sPlayerDataDisposition.getWriteTick());
...
static bool hub_tick() {
    ...
    bool shouldSend = false;
    for (size_t i = 0; i < sNetworkPlayers.size(); i++) {
        int theSilentTicksBeforeNetDeath = (sNetworkPlayers[i].mSmallestUnacknowledgedTick < sSmallestRealGameTick)
                                                   ? sHubPreferences.mPregameTicksBeforeNetDeath
                                                   : sHubPreferences.mInGameTicksBeforeNetDeath;
        if (sNetworkPlayers[i].mConnected
            && sNetworkTicker - sNetworkPlayers[i].mLastNetworkTickHeard > theSilentTicksBeforeNetDeath) {
            make_player_netdead(i);
            shouldSend = true;
        }
        // if this guy's last ACK was longer ago than the queues have space to store things, I guess dump him
        else if (i != sLocalPlayerIndex && sNetworkPlayers[i].mConnected
                 && sNetworkPlayers[i].mSmallestUnacknowledgedTick >= sSmallestRealGameTick
                 && (sNetworkPlayers[sReferencePlayerIndex].mSmallestUnacknowledgedTick
                     - sNetworkPlayers[i].mSmallestUnacknowledgedTick)
                            >= kFlagsQueueSize) {
            {
                logWarningNMT("Disconnecting player %i for late ACKs (last ACK %i, reference ACK %i", i,
                              sNetworkPlayers[i].mSmallestUnacknowledgedTick,
                              sNetworkPlayers[sReferencePlayerIndex].mSmallestUnacknowledgedTick);
                make_player_netdead(i);
                shouldSend = true;
    ...
    if (sHubPreferences.mBandwidthReduction && sPlayerDataDisposition.getReadTick() >= sSmallestRealGameTick) {
        if (sHubPreferences.mMinimumSendPeriod >= sHubPreferences.mSendPeriod
            && sSmallestIncompleteTick < sPlayerDataDisposition.getWriteTick()) {
            ...
            if (sNetworkTicker - sLastRealUpdate >= sHubPreferences.mMinimumSendPeriod) {
                ...
                for (int i = 0; i < sNetworkPlayers.size(); i++) {
                    if (i != sLocalPlayerIndex && sNetworkPlayers[i].mConnected
                        && sSmallestRealGameTick > sNetworkPlayers[i].mNetDeadTick) {
        ...
        if (shouldSend)
            ...
            if (sNetworkTicker > sLastNetworkTickSent
                && (sNetworkTicker - sLastNetworkTickSent) >= sHubPreferences.mRecoverySendPeriod)
                send_packets();
        }
    }
    check_send_packet_to_spoke();
            ...
            if (i != sLocalPlayerIndex) {
                ...
                if (thePlayer.mConnected) {
                    if (thePlayer.mLatencyBuffer.size()) {
                        double average = accumulate(thePlayer.mLatencyBuffer.begin(), thePlayer.mLatencyBuffer.end(), 0)
                                         / thePlayer.mLatencyBuffer.size();
                        double squares = accumulate(thePlayer.mLatencyBuffer.begin(), thePlayer.mLatencyBuffer.end(), 0,
                                                    add_squares);
    ...
    for (int i = 0; i < sNetworkPlayers.size(); ++i) {
        ...
        if (i != sLocalPlayerIndex) {
            if (thePlayer.mConnected) {
                if (thePlayer.mLatencyBuffer.size()) {
                    int32 samples
                            = std::min(thePlayer.mLatencyBuffer.size(), static_cast<size_t>(kDisplayLatencyWindow));
                    int32 latency_ticks      = std::max(thePlayer.mLatencyTicks,
                                                        ((sNetworkTicker - thePlayer.mLastNetworkTickHeard) * samples));
        ...
        if (thePlayer.mConnected && thePlayer.mAddressKnown) {
            ...
            try {
                ...
                if (thePlayer.mOutstandingTimingAdjustment != 0) {
                    ...
                    ps << (uint16)kTimingAdjustmentMessageType << adjustment;
                }

                // Netdead players?
                for (size_t j = 0; j < sNetworkPlayers.size(); j++) {
                    if (thePlayer.mSmallestUnacknowledgedTick <= sNetworkPlayers[j].mNetDeadTick) {
                        ps << (uint16)kPlayerNetDeadMessageType << (uint8)j // dead player index
                ...
                if (haveLossyData && ((theDescriptor.mDestinations & (((uint32)1) << i)) != 0)) {
                    logDumpNMT("packet to player %d will contain %d bytes of lossy byte stream type %d from player %d",
                               i, theDescriptor.mLength, theDescriptor.mType, theDescriptor.mSender);
                    // In AStreams, sizeof(packed scalar) == sizeof(unpacked scalar)
                    uint16 theMessageLength
                            = sizeof(theDescriptor.mType) + sizeof(theDescriptor.mSender) + theDescriptor.mLength;

                    ps << (uint16)kHubToSpokeLossyByteStreamMessageType << theMessageLength << theDescriptor.mType
                ...
                if (sHubPreferences.mBandwidthReduction
                    && sPlayerDataDisposition.getReadTick() >= sSmallestRealGameTick) {
                    ...
                    if (sNetworkTicker - thePlayer.mLastRecoverySend >= effectiveLatency) {
                        ...
                        endTick   = (startTick + maxTicks < sSmallestIncompleteTick) ? startTick + maxTicks
                                                                                     : sSmallestIncompleteTick;
                    } else {
                ...
                for (int32 tick = startTick; tick < endTick && !reflectFlags; tick++) {
                    if (sPlayerReflectedFlags.peek(tick) & (1 << i))
                        reflectFlags = true;
                }
                ...
                hdr << (uint16)(reflectFlags ? kHubToSpokeGameDataPacketWithSpokeFlagsV1Magic
                                             : kHubToSpokeGameDataPacketV1Magic);
...
const NetworkStats& hub_stats(int player_index) { return getNetworkPlayer(player_index).mStats; }
...
void HubParsePreferencesTree(InfoTree prefs, std::string version) {
    for (size_t i = 0; i < kNumAttributes; ++i) {
        ...
        if (prefs.read_attr(sAttributeStrings[i], value)) {
            ...
            if (value < min)
                logWarning("improper value %d for attribute %s of <hub>; must be at least %d. using default of %d",
                           value, sAttributeStrings[i], min, *(sAttributeDestinations[i]));
            else
    ...
    if (sHubPreferences.mPregameNthElement >= sHubPreferences.mPregameWindowSize) {
        logWarning("value for <hub> attribute %s (%d) must be less than value for %s (%d).  using %d",
                   sAttributeStrings[kPregameNthElementAttribute], sHubPreferences.mPregameNthElement,
                   sAttributeStrings[kPregameWindowSizeAttribute], sHubPreferences.mPregameWindowSize,
                   sHubPreferences.mPregameWindowSize - 1);
    ...
    if (sHubPreferences.mInGameNthElement >= sHubPreferences.mInGameWindowSize) {
        logWarning("value for <hub> attribute %s (%d) must be less than value for %s (%d).  using %d",
                   sAttributeStrings[kInGameNthElementAttribute], sHubPreferences.mInGameNthElement,
                   sAttributeStrings[kInGameWindowSizeAttribute], sHubPreferences.mInGameWindowSize,
                   sHubPreferences.mInGameWindowSize - 1);
...
InfoTree HubPreferencesTree() {
    ...
    for (size_t i = 0; i < kNumAttributes; ++i) root.put_attr(sAttributeStrings[i], *(sAttributeDestinations[i]));
    root.put_attr("use_bandwidth_reduction", sHubPreferences.mBandwidthReduction);

    return root;
}

void DefaultHubPreferences() {
    for (size_t i = 0; i < kNumAttributes; i++) *(sAttributeDestinations[i]) = sDefaultHubPreferences[i];
