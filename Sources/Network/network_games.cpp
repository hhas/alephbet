...
#include "cseries.hpp"
#include "game_window.hpp" // for mark_player_network_stats_as_dirty
#include "items.hpp"
...
long get_team_net_ranking(short team, short* kills, short* deaths, bool game_is_over) {
    ...
    *deaths      = team_damage_taken[team].kills + team_monster_damage_taken[team].kills;
    ...
    for (int i = 0; i < NUMBER_OF_TEAM_COLORS; i++) { total_monster_damage += team_monster_damage_given[i].damage; }
...
short get_network_compass_state(short player_index) {
    ...
    if (use_lua_compass[player_index]) {
        ...
        switch (GET_GAME_TYPE()) {
            ...
            // Benad
            case _game_of_defense:
                if (get_polygon_data(get_player_data(player_index)->supporting_polygon_index)->type
                    == _polygon_is_hill) {
    ...
    if (beacon) {
        ...
        angle theta                  = NORMALIZE_ANGLE(get_object_data(player->object_index)->facing
            ...
            case _game_of_capture_the_flag:
                ...
                for (player_index = 0; player_index < dynamic_world->player_count; ++player_index) {
                    ...
                    if ((polygon->type == _polygon_is_base && polygon->permutation == player->team)
                        || ((dynamic_world->game_information.kill_limit == 819)
                            && (polygon->type == _polygon_is_hill))) {
            ...
            case _game_of_rugby:
                ...
                for (player_index = 0; player_index < dynamic_world->player_count; ++player_index) {
                    ...
                    if (player_has_ball(player_index, SINGLE_BALL_COLOR)) {
                        ...
                        else if (polygon->type == _polygon_is_base && polygon->permutation == player->team
                                 && (!PLAYER_IS_DEAD(player))) {
                            /* Ditch the ball.. (it will be recreated by the timer..) */
                            destroy_players_ball(player_index);
                            dynamic_world->game_player_index = NONE;
...
void calculate_player_rankings(struct player_ranking_data* rankings) {
    ...
    for (player_index = 0; player_index < dynamic_world->player_count; ++player_index) {
        ...
        temporary_copy[player_index].ranking      = get_player_net_ranking(player_index, &kills, &deaths, false);
    }
        ...
        switch (GET_GAME_TYPE()) {
            ...
            case _game_of_tag:
                ...
                for (player_index = 0; player_index < dynamic_world->player_count; ++player_index) {
                    ...
                    if (player->total_damage_given.kills - player->damage_taken[player_index].kills
...
void get_network_joined_message(char* buffer, short game_type) {
    ...
    switch (game_type) {
        ...
        case _game_of_cooperative_play:
            ...
            break;
        case _game_of_capture_the_flag:
            format_word = _capture_the_flag;
            break;
        case _game_of_rugby:
            format_word = _rugby;
            break;
        case _game_of_king_of_the_hill:
            format_word = _king_of_the_hill;
            break;
        case _game_of_kill_man_with_ball:
            format_word = _kill_the_man_with_the_ball;
            break;
        case _game_of_defense:
            format_word = _defender_offender;
            break;
        case _game_of_tag:
            format_word = _tag;
            break;
