...
void w_found_players::unlist_player(const prospective_joiner_info& player) {
    ...
    auto player_index
            = std::distance(listed_players.begin(), std::find(listed_players.begin(), listed_players.end(), player));
    ...
    // If the element deleted was the top item or before the top item, shift view up an item to compensate (if there is
    // anything "up").
...
void w_found_players::draw_item(vector<prospective_joiner_info>::const_iterator i, SDL_Surface* s, int16 x, int16 y,
...
enum {
    ...
    kPostgameTopMargin = 190,   // For postgame layout without chat window, we can use a lot more space.  (use 70 to
                                // coexist with full chat UI)
    ...
    kUseLegendThreshhold
            = 5, // with this many players or more, use legend for kills/deaths rather than print at bar labels
...
w_players_in_game2::w_players_in_game2(bool inPostgameLayout)
    : widget(MESSAGE_WIDGET), displaying_actual_information(false), postgame_layout(inPostgameLayout),
      draw_carnage_graph(false), num_valid_net_rankings(0), selected_player(NONE), clump_players_by_team(false),
      draw_scores_not_carnage(false) {
    rect.w = kWPIG2Width;
...
w_players_in_game2::~w_players_in_game2() { clear_vector(); }
        ...
        if (clump_players_by_team) {
            for (size_t i = 0; i < num_valid_net_rankings; i++) {
                if (std::abs(x - get_wide_spaced_center_offset(rect.x, rect.w, i, num_valid_net_rankings))
                    ...
                    if (element_clicked_callback != NULL)
                        element_clicked_callback(this, clump_players_by_team, draw_carnage_graph,
                                                 draw_scores_not_carnage, i, net_rankings[i].color);
            ...
            for (size_t i = 0; i < num_valid_net_rankings; i++) {
                if (std::abs(x - get_close_spaced_center_offset(rect.x, rect.w, i, num_valid_net_rankings))
                    ...
                    if (element_clicked_callback != NULL)
                        element_clicked_callback(this, clump_players_by_team, draw_carnage_graph,
                                                 draw_scores_not_carnage, i, net_rankings[i].player_index);
...
void w_players_in_game2::draw_player_icon(SDL_Surface* s, size_t rank_index, int center_x) const {
...
void w_players_in_game2::draw_player_icons_separately(SDL_Surface* s) const {
    if (draw_carnage_graph) {
...
void w_players_in_game2::draw_player_icons_clumped(SDL_Surface* s) const {
    ...
    for (size_t i = 0; i < num_valid_net_rankings; i++) {
        ...
        for (size_t j = 0; j < theNumberOfPlayersOnThisTeam; j++) {
            int player_center_x
                    = get_close_spaced_center_offset(team_left_x, width_per_team, j, theNumberOfPlayersOnThisTeam);
...
void w_players_in_game2::draw_player_names_separately(SDL_Surface* s, TextLayoutHelper& ioTextLayoutHelper) const {
    ...
    for (size_t i = 0; i < theNumPlayers; i++) {
        int center_x = get_close_spaced_center_offset(rect.x, rect.w, i, theNumPlayers);
        const player_entry2* theEntry
                = draw_carnage_graph ? &player_entries[net_rankings[i].player_index] : &player_entries[i];
        ...
        draw_text(s, theEntry->player_name, name_x, name_y, theEntry->name_pixel_color, font, style | styleShadow);
    }
}

void w_players_in_game2::draw_player_names_clumped(SDL_Surface* s, TextLayoutHelper& ioTextLayoutHelper) const {
    // Now let's draw the names.  Let's take care to offset names vertically if they would
    // overlap (or come too close as defined by kNameMargin), so it's more readable.
...
void w_players_in_game2::draw_bar_or_bars(SDL_Surface* s, size_t rank_index, int center_x, int maximum_value,
                                          vector<bar_info>& outBarInfos) const {
    // Draw score bar
    if (draw_scores_not_carnage) {
        bar_info theBarInfo;
        ...
        if (rank_index == selected_player) {
            ...
            theKillsFormat  = num_valid_net_rankings >= kUseLegendThreshhold
                                      ? "%d"
                                      : TS_GetCString(strNET_STATS_STRINGS, strKILLS_STRING);
            theDeathsFormat = num_valid_net_rankings >= kUseLegendThreshhold
                                      ? "%d"
                                      : TS_GetCString(strNET_STATS_STRINGS, strDEATHS_STRING);
            ...
            // Put shorter bar_info in vector first so its label doesn't "leapfrog" the taller bar label in case of
            // conflict. Don't put "0"s into the vector.
...
void w_players_in_game2::draw_bars_separately(SDL_Surface* s, vector<bar_info>& outBarInfos) const {
...
void w_players_in_game2::draw_bars_clumped(SDL_Surface* s, vector<bar_info>& outBarInfos) const {
    // Find the largest value we'll be drawing, so we know how to scale our bars.
    int theMaxValue = find_maximum_bar_value();
...
void w_players_in_game2::draw_carnage_totals(SDL_Surface* s) const {
    for (size_t i = 0; i < num_valid_net_rankings; i++) {
        int center_x;
        if (clump_players_by_team)
        ...
        int thePlayerCarnageScore
                = (selected_player == i) ? -net_rankings[i].kills : net_rankings[i].kills - net_rankings[i].deaths;
...
void w_players_in_game2::draw_carnage_legend(SDL_Surface* s) const {
    ...
    uint32 thePixelColor
            = SDL_MapRGB(s->format, theMiddleColor.red >> 8, theMiddleColor.green >> 8, theMiddleColor.blue >> 8);
...
void w_players_in_game2::draw_bar_labels(SDL_Surface* s, const vector<bar_info>& inBarInfos,
                                         TextLayoutHelper& ioTextLayoutHelper) const {
    ...
    for (size_t i = 0; i < theNumberOfLabels; i++) {
        ...
        int theBestY = ioTextLayoutHelper.reserveSpaceFor(theTextX - kNameMargin / 2, theStringWidth + kNameMargin,
                                                          theBarInfo.top_y - 1, font->get_line_height());

        draw_text(s, theBarInfo.label_text.c_str(), theTextX, theBestY, theBarInfo.pixel_color, font,
                  style | styleShadow);
    }
} // draw_bar_labels
...
void w_players_in_game2::draw_bar(SDL_Surface* s, int inCenterX, int inBarColorIndex, int inBarValue, int inMaxValue,
                                  bar_info& outBarInfo) const {
    if (inBarValue != 0) {
        ...
        // "- 1" leaves room for shadow style.  Leave two line-heights so a kills and deaths at the top of widget
        // resolve (thanks to TextLayoutHelper) and still have space to live.
...
void w_entry_point_selector::validateEntryPoint() {
...
void w_entry_point_selector::gotSelectedCallback(void* arg) { ((w_entry_point_selector*)arg)->gotSelected(); }

void w_entry_point_selector::gotSelected() {
    if (mEntryPoints.size() > 1) {
        ...
        sprintf(temporary, "%zu %s levels available", mEntryPoints.size(),
...
void w_entry_point_selector::event(SDL_Event& e) {
    if (e.type == SDL_KEYDOWN) {
        if (e.key.keysym.sym == SDLK_LEFT || e.key.keysym.sym == SDLK_RIGHT) {
            ...
            if (theNumberOfEntryPoints > 1) {
                ...
                mCurrentIndex = (mCurrentIndex + theNumberOfEntryPoints + theDesiredOffset) % theNumberOfEntryPoints;
