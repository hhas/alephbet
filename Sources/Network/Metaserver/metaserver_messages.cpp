...
#include "shell.hpp" // get_player_color :(
...
static const char* sRoomNames[] = {"Crows Bridge",
                                   "Otter Ferry",
                                   "White Falls",
                                   "Silvermines",
                                   "Shoal",
...
static void write_padded_bytes(AOStream& inStream, const char* inBytes, size_t inByteCount, size_t inTargetLength) {
    ...
    for (size_t i = 0; i < theZeroCount; i++) { inStream << static_cast<uint8>('\0'); }
}
...
static void write_string(AOStream& inStream, const string& inString) { write_string(inStream, inString.c_str()); }
...
static const string read_string(AIStream& in) {
    string result;
    int8 c;
    in >> c;
    while (c != '\0') {
...
static const string read_padded_string(AIStream& in, size_t length) {
    vector<char> temp;
...
void write_player_aux_data(AOStream& out, string name, const string& team, bool away, const string& away_message) {
    uint8 unused8 = 0;
    ...
    out << kPlayerIcon << unused8 << (uint16)(away ? kSTATE_AWAY : kSTATE_AWAKE) << primaryColor[0] << primaryColor[1]
        << primaryColor[2] << unused16 << secondaryColor[0] << secondaryColor[1] << secondaryColor[2] << unused16
        << orderIndex << kAlephBetClientVersion;
...
void LoginAndPlayerInfoMessage::reallyDeflateTo(AOStream& thePacket) const {
    ...
    thePacket << platform_type << metaserver_version << flags << user_id << max_authentication << thePlayerDataSize;
...
ostream& operator<<(ostream& out, const RoomDescription& roomDesc) {
    out << roomDesc.roomName() << " : " << roomDesc.roomType() << " : " << roomDesc.playerCount() << " : "
        << roomDesc.gameCount();
    return out;
}
...
void RemoteHubRequestMessage::reallyDeflateTo(AOStream& out) const { write_string(out, mVersion); }
...
bool IDAndLimitMessage::reallyInflateFrom(AIStream& inStream) {
    ...
    inStream >> m_playerID >> m_playerLimit >> ignore16;
...
PrivateMessage::PrivateMessage(uint32 inSenderID, const string& inSenderName, uint32 inSelectedID,
                               const string& inMessage)
    : m_senderID(inSenderID), m_selectedID(inSelectedID), m_internalType(0), m_flags(kDirectedBit),
      m_senderName(inSenderName), m_message(inMessage) {
...
void PrivateMessage::reallyDeflateTo(AOStream& thePacket) const {
    ...
    thePacket << m_selectedID << echo << m_internalType << size;

    thePacket.write(m_color, 3);

    thePacket << colorFlags << m_flags << unused16 << m_senderID << m_selectedID;
...
bool PrivateMessage::reallyInflateFrom(AIStream& inStream) {
    ...
    inStream >> player_id >> echo >> m_internalType >> size;

    inStream.read(m_color, 3);

    inStream >> colorFlags >> m_flags >> unused16 >> m_senderID >> m_selectedID;
...
void ChatMessage::reallyDeflateTo(AOStream& thePacket) const {
    ...
    thePacket << m_internalType << size;

    thePacket.write(m_color, 3);

    thePacket << colorFlags << m_flags << unused16 << m_senderID << destinationPlayerID;
...
bool ChatMessage::reallyInflateFrom(AIStream& inStream) {
    ...
    inStream >> m_internalType >> size;

    inStream.read(m_color, 3);

    inStream >> colorFlags >> m_flags >> unused16 >> m_senderID >> destinationPlayerID;
...
MetaserverPlayerInfo::MetaserverPlayerInfo(AIStream& inStream) : m_target(false) {
    ...
    inStream >> m_verb >> m_adminFlags >> ignore32 >> m_playerID >> m_roomID >> ignore8 >> m_rank >> m_playerDataSize
            >> ignore32 >> m_icon >> ignore8 >> m_status;
...
ostream& operator<<(ostream& out, const MetaserverPlayerInfo& info) {
    return (out << info.m_verb << "; " << info.m_adminFlags << "; " << info.m_playerID << "; " << info.m_roomID << "; "
                << info.m_rank << "; " << info.m_playerDataSize << "; " << (uint16)info.m_icon << "; "
                << (uint16)info.m_status << "; " << info.m_name << "; " << info.m_team << endl
                << endl);
}

bool PlayerListMessage::reallyInflateFrom(AIStream& inStream) {
    while (inStream.maxg() > inStream.tellg()) m_players.push_back(MetaserverPlayerInfo(inStream));
...
void CreateGameMessage::reallyDeflateTo(AOStream& thePacket) const {
    thePacket << m_gamePort << m_remoteHubId << m_description;
}
...
bool LoginSuccessfulMessage::reallyInflateFrom(AIStream& inStream) {
    ...
    inStream >> m_userID >> m_order >> unused >> m_token;

    return true;
}

bool SetPlayerDataMessage::reallyInflateFrom(AIStream& inStream) { return true; }
...
AIStream& operator>>(AIStream& stream, GameDescription& desc) {
    ...
    stream >> unknown16 >> desc.m_type >> options >> desc.m_timeLimit >> desc.m_mapChecksum >> desc.m_difficulty
            >> desc.m_maxPlayers >> teamRandomSeed >> maxTeams >> randomSeed >> planningTime >> unused32 >> unused32
            >> unknown16 >> pluginFlag;
    ...
    if (pluginFlag & 0x1) {
        ...
        if (pluginFlag & 0x2) {
            ...
            stream >> desc.m_gameOptions >> desc.m_cheatFlags >> desc.m_killLimit;
    ...
    stream >> clientVersion >> unknown32 >> status >> desc.m_numPlayers >> action;
...
AOStream& operator<<(AOStream& stream, const GameDescription& desc) {
    ...
    const int kPluginListSize = 512;
    uint32 clientVersion      = 0xc136'e436;
    uint32 unknown32          = 0;
    uint16 status             = 0 | (desc.m_closed ? 0x0010 : 0x0000) | (desc.m_running ? 0x0001 : 0x0000);
    uint8 action              = 0;
...
static const char* difficultyLevelString[] = {"Kindergarten", "Easy", "Normal", "Major Damage", "Total Carnage"};

ostream& operator<<(ostream& stream, const GameDescription& desc) {
    stream << desc.m_name << " : " << desc.m_mapName << " (" << desc.m_mapChecksum
           << ") : " << gameTypeString[desc.m_type] << " : " << difficultyLevelString[desc.m_difficulty] << " : "
           << desc.m_timeLimit << " : " << static_cast<uint16>(desc.m_numPlayers) << "/" << desc.m_maxPlayers << " : "
           << (desc.m_closed ? "Closed" : "Open") << " : " << (desc.m_running ? "Running" : "Not Running") << " : "
...
string GameListMessage::GameListEntry::format_for_chat(const string& player_name) const {
    ...
    if (m_description.m_timeLimit && !(m_description.m_timeLimit == INT32_MAX || m_description.m_timeLimit == -1)) {
        message << m_description.m_timeLimit / 60 / TICKS_PER_SECOND << " minutes of |i";
    }
    message << m_description.m_mapName << ",|p " << game_string();
...
AIStream& operator>>(AIStream& stream, GameListMessage::GameListEntry& entry) {
    ...
    stream >> entry.m_gameID >> entry.m_ipAddress[0] >> entry.m_ipAddress[1] >> entry.m_ipAddress[2]
            >> entry.m_ipAddress[3] >> entry.m_port >> entry.m_verb >> entry.m_gameEnable >> entry.m_timeRemaining
            >> entry.m_hostPlayerID >> entry.m_len >> unused32 >> unused32 >> unused16 >> entry.m_description;

    return stream;
}

static ostream& printDottedDecimal(ostream& stream, const uint8* ip) {
    stream << static_cast<unsigned int>(ip[0]) << "." << static_cast<unsigned int>(ip[1]) << "."
...
std::ostream& operator<<(std::ostream& stream, const GameListMessage::GameListEntry& entry) {
    stream << static_cast<uint16>(entry.m_verb) << " : " << entry.m_gameID << " : ";

    printDottedDecimal(stream, entry.m_ipAddress);

    stream << ":" << entry.m_port << " : " << static_cast<uint16>(entry.m_gameEnable) << " : " << entry.m_timeRemaining
           << " : " << entry.m_hostPlayerID << " : " << entry.m_len << " : " << entry.m_description;
...
void StartGameMessage::reallyDeflateTo(AOStream& thePacket) const {
    uint32 unused32 = 0;
    thePacket << m_gameTimeInSeconds << unused32 << unused32;
}
