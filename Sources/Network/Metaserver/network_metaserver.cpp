...
void MetaserverClient::handleUnexpectedMessage(Message* inMessage, CommunicationsChannel* inChannel) {
...
void MetaserverClient::handleBroadcastMessage(BroadcastMessage* inMessage, CommunicationsChannel* inChannel) {
    if (m_notificationAdapter)
        m_notificationAdapter->receivedBroadcastMessage(inMessage->message());
}

void MetaserverClient::handleChatMessage(ChatMessage* message, CommunicationsChannel* inChannel) {
    if (message->internalType() == 0 && m_notificationAdapter) {
        std::string realSenderName = remove_formatting(message->senderName());
        if (realSenderName[0] == '\260')
            realSenderName.erase(realSenderName.begin());
        if (network_preferences->mute_metaserver_guests && starts_with(realSenderName, "Guest "))
            return;
        if (s_ignoreNames.find(realSenderName) != s_ignoreNames.end())
            return;

        if (message->directed())
            m_notificationAdapter->receivedPrivateMessage(message->senderName(), message->senderID(),
                                                          message->message());
        else
...
void MetaserverClient::handleKeepAliveMessage(Message* inMessage, CommunicationsChannel* inChannel) {
    inChannel->enqueueOutgoingMessage(KeepAliveMessage());
}

void MetaserverClient::handlePlayerListMessage(PlayerListMessage* inMessage, CommunicationsChannel* inChannel) {
...
const std::vector<GameListMessage::GameListEntry> MetaserverClient::gamesInRoomUpdate(bool reset_ping) {
    ...
    if (auto pinger = NetGetPinger().lock()) {
        ...
        for (auto& game : games) {
            if (game.running())
                continue;
            if (!reset_ping && ping_games.find(game.id()) != ping_games.end())
                continue;
        ...
        for (auto& game : games) {
            ...
            if (ping_games.find(game.id()) != ping_games.end()
                && ping_responses.find(ping_games[game.id()]) != ping_responses.end()) {
...
void MetaserverClient::handleRoomListMessage(RoomListMessage* inMessage, CommunicationsChannel* inChannel) {
    m_rooms = inMessage->rooms();
}

void MetaserverClient::handleRemoteHubListMessage(RemoteHubListMessage* inMessage, CommunicationsChannel* inChannel) {
    m_remoteHubServers = inMessage->servers();
}

void MetaserverClient::handleGameListMessage(GameListMessage* inMessage, CommunicationsChannel* inChannel) {
...
MetaserverClient::MetaserverClient()
    : m_channel(new CommunicationsChannel()), m_inflater(new MessageInflater()), m_dispatcher(new MessageDispatcher()),
      m_loginDispatcher(new MessageDispatcher()), m_notificationAdapter(NULL), m_notifiedOfDisconnected(false),
...
void MetaserverClient::connect(const std::string& serverName, uint16 port, const std::string& userName,
                               const std::string& userPassword, bool use_remote_hub) {
    try {
        ...
        if (dynamic_cast<SaltMessage*>(theSaltOrAcceptMessage.get()) != 0) {
            ...
            if (theSaltMessage->encryptionType() == SaltMessage::kPlaintextEncryption) {
                ...
                for (size_t i = 1; i < sizeof(theKey); i++) { theKey[i] = theKey[i] ^ theKey[i - 1]; }
        ...
        std::unique_ptr<LoginSuccessfulMessage> theLoginSuccessfulMessage(
                m_channel->receiveSpecificMessageOrThrow<LoginSuccessfulMessage>());

        //	std::unique_ptr<SetPlayerDataMessage>
        //theSetPlayerDataMessage(m_channel->receiveSpecificMessageOrThrow<SetPlayerDataMessage>());

        std::unique_ptr<RoomListMessage> theRoomListMessage(
                m_channel->receiveSpecificMessageOrThrow<RoomListMessage>());
        ...
        if (use_remote_hub) {
            ...
            std::unique_ptr<RemoteHubListMessage> theRemoteHubListMessage(
                    m_channel->receiveSpecificMessageOrThrow<RemoteHubListMessage>());
        ...
        std::unique_ptr<IDAndLimitMessage> theIDAndLimitMessage(
                m_channel->receiveSpecificMessageOrThrow<IDAndLimitMessage>());
...
bool MetaserverClient::isConnected() const { return m_channel->isConnected(); }

void MetaserverClient::disconnect() {
...
void MetaserverClient::pumpAll() { for_each(s_instances.begin(), s_instances.end(), mem_fn(&MetaserverClient::pump)); }

void MetaserverClient::sendChatMessage(const std::string& message) {
    if (message == ".available" || message == ".avail") {
        ...
        if (m_notificationAdapter) {
            ...
            for (vector<GameListMessage::GameListEntry>::iterator it = games.begin(); it != games.end(); ++it) {
                ...
                string player_name;
                for (vector<MetaserverPlayerInfo>::iterator player_it = players.begin(); player_it != players.end();
                     ++player_it) {
...
void MetaserverClient::sendPrivateMessage(MetaserverPlayerInfo::IDType id, const std::string& message) {
    m_channel->enqueueOutgoingMessage(PrivateMessage(m_playerID, m_playerName, id, message));
}

void MetaserverClient::announceGame(uint16 gamePort, const GameDescription& description, uint16 remoteHubId) {
...
void MetaserverClient::announcePlayersInGame(uint8 players) {
...
void MetaserverClient::announceGameStarted(int32 gameTimeInSeconds) {
...
void MetaserverClient::announceGameReset() { m_channel->enqueueOutgoingMessage(ResetGameMessage()); }

void MetaserverClient::announceGameDeleted() {
    if (m_gameAnnounced) {
...
void MetaserverClient::syncGames() { m_channel->enqueueOutgoingMessage(SyncGamesMessage()); }
...
void MetaserverClient::setPlayerName(const std::string& name) { m_playerName = name; }

void MetaserverClient::setPlayerTeamName(const std::string& name) { m_teamName = name; }
