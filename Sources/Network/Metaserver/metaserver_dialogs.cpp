...
#include "game_wad.hpp" // embedded physics/lua detection!!?!?!!
#include "map.hpp"      // for _force_unique_teams!?!
#include "metaserver_dialogs.hpp"
#include "network_metaserver.hpp"
#include "network_private.hpp" // GAME_PORT
...
const IPaddress run_network_metaserver_ui() { return MetaserverClientUi::Create()->GetJoinAddressByRunning(); }
...
void setupAndConnectClient(MetaserverClient& client, bool use_remote_hub) {
    ...
    if (network_preferences->check_for_updates && !user_informed) {
        ...
        if (status == Update::UpdateAvailable) {
            ...
            placer->dual_add(new w_static_text(expand_app_variables("An update for $appName$ is available.").c_str()),
                             d);
#ifdef MAC_APP_STORE
        ...
        if (status != Update::CheckingForUpdate)
            user_informed = true;
    }
#endif

    client.setPlayerTeamName("");
    client.connect(A1_METASERVER_HOST, 6321, network_preferences->metaserver_login,
                   network_preferences->metaserver_password, use_remote_hub);
}
...
void GameAvailableMetaserverAnnouncer::Start(int32 time_limit) { gMetaserverClient->announceGameStarted(time_limit); }

extern void PlayInterfaceButtonSound(short);

void GlobalMetaserverChatNotificationAdapter::playersInRoomChanged(
        const std::vector<MetaserverPlayerInfo>& playerChanges) {
...
void GlobalMetaserverChatNotificationAdapter::gamesInRoomChanged(
        const std::vector<GameListMessage::GameListEntry>& gameChanges) {
...
void GlobalMetaserverChatNotificationAdapter::receivedChatMessage(const std::string& senderName, uint32 senderID,
...
void GlobalMetaserverChatNotificationAdapter::receivedPrivateMessage(const std::string& senderName, uint32 senderID,
                                                                     const std::string& message) {
    ColoredChatEntry e;
...
const IPaddress MetaserverClientUi::GetJoinAddressByRunning() {
    ...
    m_gamesInRoomWidget->SetItemSelectedCallback(
            std::bind(&MetaserverClientUi::GameSelected, this, std::placeholders::_1));
    m_playersInRoomWidget->SetItemSelectedCallback(
            std::bind(&MetaserverClientUi::PlayerSelected, this, std::placeholders::_1));
...
void MetaserverClientUi::GameSelected(GameListMessage::GameListEntry game) {
    if (gMetaserverClient->game_target() == game.id()) {
        if (machine_tick_count() - m_lastGameSelected < 333
            && (!game.running() && Scenario::instance()->IsCompatible(game.m_description.m_scenarioID))) {
            JoinClicked();
        } else {
...
void MetaserverClientUi::MuteClicked() { gMetaserverClient->ignore(gMetaserverClient->player_target()); }
