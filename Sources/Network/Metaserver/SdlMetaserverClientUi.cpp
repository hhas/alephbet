...
#include "interface.hpp" // set_drawing_clip_rectangle()
#include "metaserver_dialogs.hpp"
#include "network_dialog_widgets_sdl.hpp" // chat_history widget
...
class SdlMetaserverClientUi : public MetaserverClientUi {
  public:
    ...
    SdlMetaserverClientUi() : m_disconnected(false) {
        ...
        games_in_room_w
                = new w_games_in_room(std::bind(&SdlMetaserverClientUi::GameSelected, this, std::placeholders::_1), 320,
    ...
    void InfoClicked() {
        ...
        if (game) {
            ...
            if (TS_GetCString(kDifficultyLevelsStringSetID, game->m_description.m_difficulty)) {
                table->dual_add(new w_static_text(
                                        TS_GetCString(kDifficultyLevelsStringSetID, game->m_description.m_difficulty)),
                                info_dialog);
            } else {
                table->add(new w_spacer(), true);
            }
            ...
            if (game->m_description.m_hasGameOptions && (game->m_description.m_gameOptions & _game_has_kill_limit)) {
                ...
                if (game->m_description.m_timeLimit
                    && !(game->m_description.m_timeLimit == INT32_MAX || game->m_description.m_timeLimit == -1)) {
                    char minutes[32];
...
std::unique_ptr<MetaserverClientUi> MetaserverClientUi::Create() { return std::make_unique<SdlMetaserverClientUi>(); }
