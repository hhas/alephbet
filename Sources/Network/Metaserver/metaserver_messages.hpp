...
#include "AStream.hpp"
#include "Scenario.hpp" // for scenario name and ID
#include "network.hpp"  // for network protocol ID
#include <SDL_net.h>
...
struct GameDescription {
    ...
    GameDescription()
        : m_type(0), m_timeLimit(0), m_mapChecksum(0), m_difficulty(0), m_maxPlayers(8), m_teamsAllowed(false),
          m_closed(false), m_running(false), m_numPlayers(1), m_name("Untitled Game"), m_mapName("Unspecified Map"),
          m_scenarioID(Scenario::instance()->GetID()), m_networkSetupProtocolID(kNetworkSetupProtocolID),
          m_scenarioName(Scenario::instance()->GetName()), m_scenarioVersion(Scenario::instance()->GetVersion()),
          m_hasGameOptions(false), m_gameOptions(0), m_cheatFlags(0), m_killLimit(0), m_latency(UINT16_MAX) {}
};
...
class LoginAndPlayerInfoMessage : public SmallMessageHelper {
  public:
    ...
    LoginAndPlayerInfoMessage* clone() const { return new LoginAndPlayerInfoMessage(*this); }
...
class SetPlayerModeMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kCLIENT_PLAYERMODE
    };

    MessageTypeID type() const { return kType; }

    SetPlayerModeMessage* clone() const { return new SetPlayerModeMessage(*this); }
...
class SaltMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kPlaintextEncryption,
        kBraindeadSimpleEncryption,
        kHTTPSEncryption = 4
    };
    ...
    SaltMessage* clone() const { return new SaltMessage(*this); }
...
class LocalizationMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kCLIENT_LOCALIZE
    };

    MessageTypeID type() const { return kType; }

    LocalizationMessage* clone() const { return new LocalizationMessage(*this); }
...
class RoomListMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kSERVER_ROOMLIST
    };

    MessageTypeID type() const { return kType; }

    RoomListMessage* clone() const { return new RoomListMessage(*this); }
...
class RemoteHubListMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = KSERVER_REMOTEHUBLIST
    };

    MessageTypeID type() const { return kType; }

    RemoteHubListMessage* clone() const { return new RemoteHubListMessage(*this); }
...
class RemoteHubRequestMessage : public SmallMessageHelper {
  public:
    ...
    RemoteHubRequestMessage* clone() const { return new RemoteHubRequestMessage(*this); }
...
class RoomLoginMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType      = kCLIENT_ROOM_LOGIN,
        kKeyLength = 16
    };

    MessageTypeID type() const { return kType; }

    RoomLoginMessage* clone() const { return new RoomLoginMessage(*this); }
...
class NameAndTeamMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kCLIENT_NAME_TEAM
    };

    MessageTypeID type() const { return kType; }

    NameAndTeamMessage* clone() const { return new NameAndTeamMessage(*this); }

    NameAndTeamMessage(const std::string& name, const std::string& team)
        : m_name(name), m_team(team), m_away(false), m_away_message("") {}

    NameAndTeamMessage(const std::string& name, const std::string& team, bool away, const std::string& away_message)
        : m_name(name), m_team(team), m_away(away), m_away_message(away_message) {}
...
class IDAndLimitMessage : public SmallMessageHelper {
  public:
    ...
    IDAndLimitMessage* clone() const { return new IDAndLimitMessage(*this); }
...
class DenialMessage : public SmallMessageHelper {
  public:
    ...
    DenialMessage* clone() const { return new DenialMessage(*this); }
...
class BroadcastMessage : public SmallMessageHelper {
  public:
    ...
    BroadcastMessage* clone() const { return new BroadcastMessage(*this); }
...
class PrivateMessage : public SmallMessageHelper {
  public:
    ...
    PrivateMessage(uint32 inSenderID, const std::string& inSenderName, uint32 inSelectedID,
    ...
    PrivateMessage* clone() const { return new PrivateMessage(*this); }
...
class ChatMessage : public SmallMessageHelper {
  public:
    ...
    ChatMessage* clone() const { return new ChatMessage(*this); }
...
class MetaserverPlayerInfo {
  public:
    ...
    static bool sort(const MetaserverPlayerInfo& a, const MetaserverPlayerInfo& b) {
        return (a.m_adminFlags == b.m_adminFlags)
                       ? ((a.m_status == b.m_status) ? a.playerID() < b.playerID() : a.m_status < b.m_status)
                       : a.m_adminFlags > b.m_adminFlags;
    }
    ...
    static const IDType IdNone = 0xffff'ffff;
...
class PlayerListMessage : public SmallMessageHelper {
  public:
    ...
    PlayerListMessage* clone() const { return new PlayerListMessage(*this); }
...
class CreateGameMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kCLIENT_CREATEGAME
    };

    MessageTypeID type() const { return kType; }

    CreateGameMessage(uint16 gamePort, const GameDescription& description, uint16 remoteHubId)
        : m_gamePort(gamePort), m_description(description), m_remoteHubId(remoteHubId) {}

    CreateGameMessage* clone() const { return new CreateGameMessage(*this); }
...
class LoginSuccessfulMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kSERVER_LOGINSUCCESS
    };

    MessageTypeID type() const { return kType; }

    LoginSuccessfulMessage* clone() const { return new LoginSuccessfulMessage(*this); }
...
class SetPlayerDataMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kSERVER_SETPLAYERDATA
    };

    MessageTypeID type() const { return kType; }

    SetPlayerDataMessage* clone() const { return new SetPlayerDataMessage(*this); }
...
class GameListMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kSERVER_GAMELIST
    };

    MessageTypeID type() const { return kType; }

    GameListMessage* clone() const { return new GameListMessage(*this); }
...
class StartGameMessage : public SmallMessageHelper {
  public:
    ...
    enum {
        kType = kCLIENT_STARTGAME
    };

    MessageTypeID type() const { return kType; }

    StartGameMessage* clone() const { return new StartGameMessage(*this); }
