...
#include "network_dialogs.hpp"
#include "cseries.hpp"
#include "game_wad.hpp" // get_map_file
#include "map.hpp"
#include "metaserver_dialogs.hpp" // GameAvailableMetaserverAnnouncer
#include "network.hpp"
#include "network_games.hpp"
#include "network_messages.hpp"
#include "preferences.hpp"
#include "shell.hpp"
#include "wad.hpp" // jkvw: for read_wad_file_checksum
#include <functional>
#include <map>
// For LAN netgame location services
#include "SSLP_API.h"
#include "network_private.hpp" // actually just need "network_dialogs_private.h"
#include <sstream>
...
static const string get_sslp_service_type() { return kNetworkSetupProtocolID; }
...
GathererAvailableAnnouncer::~GathererAvailableAnnouncer() { SSLP_Disallow_Service_Discovery(&mServiceInstance); }
...
JoinerSeekingGathererAnnouncer::JoinerSeekingGathererAnnouncer(bool shouldSeek) : mShouldSeek(shouldSeek) {
    if (mShouldSeek)
        SSLP_Locate_Service_Instances(get_sslp_service_type().c_str(), found_gatherer_callback, lost_gatherer_callback,
...
static uint16 network_gather_remote_hub() {
    ...
    if (auto pinger = NetGetPinger().lock()) // there is a problem if it fails
    {
        ...
        for (const auto& remote_hub_latency : ordered_remote_hubs) {
            ...
            if (hub_get_minimum_send_period() && latency > hub_get_minimum_send_period() * 33)
                continue;
...
bool network_gather(bool inResumingGame, bool& outUseRemoteHub) {
    ...
    if (network_game_setup(&myPlayerInfo, &myGameInfo, inResumingGame, advertiseOnMetaserver, outUpnpPortForward,
        ...
        if (NetEnter(outUseRemoteHub)) {
            ...
            if (NetGather(&myGameInfo, sizeof(game_info), (void*)&myPlayerInfo, sizeof(myPlayerInfo), inResumingGame,
                          outUpnpPortForward)) {
                ...
                if (advertiseOnMetaserver) {
                    try {
                        ...
                        if (e.code() == MetaserverClient::LoginDeniedException::BadUserOrPassword) {
                            strncpy(message,
                                    "Login denied: bad username or password. Your game could not be advertised on the "
                                    "Internet.",
                                    1024);
                        } else if (e.code() == MetaserverClient::LoginDeniedException::UserAlreadyLoggedIn) {
                            strncpy(message,
                                    "Login denied: that user is already logged in. Your game could not be advertised "
                                    "on the Internet.",
                                    1024);
                        } else if (e.code() == MetaserverClient::LoginDeniedException::AccountAlreadyLoggedIn) {
                            strncpy(message,
                                    "Login denied: that account is already logged in. Your game could not be "
                                    "advertised on the Internet.",
                                    1024);
                        } else if (e.code() == MetaserverClient::LoginDeniedException::RoomFull) {
                            strncpy(message,
                                    "Login denied: room full! Your game could not be advertised on the Internet.",
                                    1024);
                        } else if (e.code() == MetaserverClient::LoginDeniedException::AccountLocked) {
                            strncpy(message,
                                    "Login denied: your account is locked. Your game could not be advertised on the "
                                    "Internet.",
                                    1024);
                        } else {
                            sprintf(message,
                                    "There was a problem connecting to the server that tracks Internet games (%s). "
                ...
                gather_success = gather_success
                                 && (!outUseRemoteHub || NetGameJoin(&myPlayerInfo, sizeof(myPlayerInfo), nullptr))
                                 && GatherDialog::Create(outUseRemoteHub)->GatherNetworkGameByRunning();
...
void GatherDialog::update_ungathered_widget() {
    ...
    for (std::map<int, prospective_joiner_info>::iterator it = m_ungathered_players.begin();
         it != m_ungathered_players.end(); ++it)
...
void GatherDialog::JoinedPlayerChanged(const prospective_joiner_info* player) { m_pigWidget->redraw(); }
...
void GatherDialog::ReceivedMessageFromPlayer(const char* player_name, const char* message) {
    ColoredChatEntry e;
...
void JoinDialog::getJoinAddressFromMetaserver() {
    ...
    try {
        ...
        if (result.host != 0) {
            ...
            std::ostringstream s;
            s << (uint16)hostBytes[0] << '.' << (uint16)hostBytes[1] << '.' << (uint16)hostBytes[2] << '.'
              << (uint16)hostBytes[3];
        ...
        if (e.code() == MetaserverClient::LoginDeniedException::BadUserOrPassword) {
            ...
            sprintf(message,
                    "There was a problem connecting to the server that tracks Internet games (%s). Please try again "
                    "later.",
                    e.what());
        }
...
bool network_game_setup(player_info* player_information, game_info* game_information, bool ResumingGame,
    ...
    if (SetupNetgameDialog::Create()->SetupNetworkGameByRunning(player_information, game_information, ResumingGame,
                                                                outAdvertiseGameOnMetaserver, outUpnpPortForward,
                                                                outUseRemoteHub)) {
        write_preferences();
        return true;
    } else {
...
class TimerInt32Pref : public Bindable<int> {
  public:
    ...
    virtual int bind_export() { return m_pref / (60 * TICKS_PER_SECOND); }

    virtual void bind_import(int value) { m_pref = static_cast<int32>(value) * (60 * TICKS_PER_SECOND); }
...
class LimitTypePref : public Bindable<int> {
  public:
    ...
    LimitTypePref(bool& untimed_pref, uint16& options_pref, uint16 kill_limit_mask)
        : m_untimed(untimed_pref), m_kill_limited(options_pref, kill_limit_mask) {}
...
class GametypePref : public Bindable<int> {
  public:
    ...
    virtual int bind_export() { return ((m_pref < 5) ? m_pref : m_pref - 1); }

    virtual void bind_import(int value) { m_pref = ((value < 5) ? value : value + 1); }
...
class LatencyTolerancePref : public Bindable<int> {
  public:
    ...
    virtual int bind_export() { return (m_pref == 0) ? 5 : (m_pref - 1); }

    virtual void bind_import(int value) { m_pref = (value == 5) ? 0 : (value + 1); }
...
static const vector<string> make_entry_vector(int32 entry_flags) {
    ...
    while (get_indexed_entry_point(&ep, &index, entry_flags)) result.push_back(string(ep.level_name));
...
bool SetupNetgameDialog::SetupNetworkGameByRunning(player_info* player_information, game_info* game_information,
                                                   bool resuming_game, bool& outAdvertiseGameOnMetaserver,
                                                   bool& outUpnpPortForward, bool& outUseRemoteHub) {
    int32 entry_flags;
    ...
    if (resuming_game) {
        ...
        theAdjustedPreferences.game_is_untimed
                = (dynamic_world->game_information.game_time_remaining > 7 * 24 * 3600 * TICKS_PER_SECOND);
        ...
        if (dynamic_world->player_count == 1 && GET_GAME_TYPE() == _game_of_kill_monsters) {
            ...
            theAdjustedPreferences.game_options
                    |= _live_network_stats; // single-player game doesn't, and they probably want it
        }
    ...
    network_preferences_data* active_network_preferences
            = resuming_game ? &theAdjustedPreferences : network_preferences;
    ...
    LimitTypePref limitTypePref(active_network_preferences->game_is_untimed, active_network_preferences->game_options,
                                _game_has_kill_limit);
    ...
    if (Run()) {
        ...
        vassert(updates_per_packet > 0 && update_latency >= 0 && updates_per_packet < 16,
                csprintf(temporary, "You idiot! updates_per_packet = %d, update_latency = %d", updates_per_packet,
                         update_latency));
        ...
        game_information->initial_random_seed
                = resuming_game ? dynamic_world->random_seed : (uint16)machine_tick_count();
        ...
        //	strncpy(network_preferences->netscript_file, theNetscriptFile.GetPath(),
        //sizeof(network_preferences->netscript_file));
        //}
...
bool SetupNetgameDialog::informationIsAcceptable() {
    ...
    if (information_is_acceptable) {
        ...
        information_is_acceptable
                = get_indexed_entry_point(&ep, &index, get_entry_point_flags_for_game_type(m_old_game_type));
    }
...
void menu_index_to_level_entry(short menu_index, int32 entry_flags, struct entry_point* entry) {
    short i, map_index;

    map_index = 0;
    for (i = 0; i <= menu_index; i++) { get_indexed_entry_point(entry, &map_index, entry_flags); }
...
short find_graph_mode(NetgameOutcomeData& outcome, short* index) {
    short value;
    ...
    if (value < dynamic_world->player_count) {
        ...
        if (theValueAfterPlayers >= 0)
            theValueAfterPlayers++;
        if (theValueAfterPlayers >= 3)
            theValueAfterPlayers++;
...
void calculate_rankings(struct net_rank* ranks, short num_players) {
    short player_index;
    ...
    for (player_index = 0; player_index < num_players; ++player_index) {
        ...
        ranks[player_index].game_ranking
                = get_player_net_ranking(player_index, &ranks[player_index].kills, &ranks[player_index].deaths, true);
...
int rank_compare(void const* r1, void const* r2) {
...
int team_rank_compare(void const* rank1, void const* rank2) {
    return ((struct net_rank const*)rank2)->ranking - ((struct net_rank const*)rank1)->ranking;
}

// (ZZZ annotation:) Game-specific score comparison for sorting.
int score_rank_compare(void const* rank1, void const* rank2) {
    return ((struct net_rank const*)rank2)->game_ranking - ((struct net_rank const*)rank1)->game_ranking;
}

// (ZZZ annotation:) Graph of player's killing performance vs each other player
void draw_player_graph(NetgameOutcomeData& outcome, short index) {
...
void draw_team_graph(NetgameOutcomeData& outcome, short team_index) {
...
void draw_totals_graph(NetgameOutcomeData& outcome) {
...
void draw_team_totals_graph(NetgameOutcomeData& outcome) {
...
void draw_total_scores_graph(NetgameOutcomeData& outcome) {
    struct net_rank ranks[MAXIMUM_NUMBER_OF_PLAYERS];
...
void draw_team_total_scores_graph(NetgameOutcomeData& outcome) {
...
void update_carnage_summary(NetgameOutcomeData& outcome, struct net_rank* ranks, short num_players, short suicide_index,
...
void draw_new_graph(NetgameOutcomeData& outcome) {
    short graph_type;
    short index;
...
short calculate_max_kills(size_t num_players) {
    short max_kills = 0;
    size_t i, j;
...
void get_net_color(short index, RGBColor* color) {
    switch (index) {
        case _suicide_color:
        ...
        case _death_color:
        case _score_color:
            color->red = color->green = color->blue = 60'000;
            break;
        default:
            assert(false);
            break;
...
void draw_names(DialogPtr& dialog, struct net_rank* ranks, short number_of_bars, short which_player) {
...
void draw_kill_bars(DialogPtr& dialog, struct net_rank* ranks, short num_players, short suicide_index, bool do_totals,
                    bool friendly_fire) {
...
void draw_score_bars(DialogPtr& dialog, struct net_rank* ranks, short bar_count) {
    // We don't actually draw here - we just pass the data along to the widget, and it will take care of the rest.
    w_players_in_game2* wpig2 = dynamic_cast<w_players_in_game2*>(dialog->get_widget_by_id(iDAMAGE_STATS));
...
static void respond_to_element_clicked(w_players_in_game2* inWPIG2, bool inTeam, bool inGraph, bool inScore,
                                       size_t inDrawIndex, int inPlayerIndexOrTeamColor) {
...
static void respond_to_graph_type_change(w_select* inGraphMenu) {
...
static void send_text_fake(w_text_entry* te) {
    assert(te != NULL);
...
std::unique_ptr<GatherDialog> GatherDialog::Create(bool remote_hub_mode) {
...
class SdlJoinDialog : public JoinDialog {
  public:
    ...
    SdlJoinDialog() : m_tabs(0) {
        ...
        if ((m & KMOD_ALT) || (m & KMOD_GUI))
            skipToMetaserver = !skipToMetaserver;
...
std::unique_ptr<JoinDialog> JoinDialog::Create() { return std::unique_ptr<JoinDialog>(new SdlJoinDialog); }
...
class SdlSetupNetgameDialog : public SetupNetgameDialog {
  public:
    ...
    virtual bool Run() { return (m_dialog.run() == 0); }
    ...
    virtual bool allLevelsAllowed() { return false; }

    virtual void unacceptableInfo() { play_dialog_sound(DIALOG_ERROR_SOUND); }
...
std::unique_ptr<SetupNetgameDialog> SetupNetgameDialog::Create() {
...
static const char* sTestingNames[] = {"Doctor Burrito", "Carnage Asada", "Bongo Bob",      "The Napalm Man",
                                      "Kissy Monster",  "lala",          "Prof. Windsurf", "-ZED-"};
