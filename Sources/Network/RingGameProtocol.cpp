...
#include "Logging.hpp"
#include "interface.hpp" // process_action_flags
...
#undef NETWORK_USE_RECENT_FLAGS // if the game stalls, use flags at the end of the stall, not the more stale ones at the
                                // beginning
...
// Note that this is based on the idea that NQT will run, on average, n times in n periods - it just may be a little
// more jittery than we'd like.  In the SDL version this is true.  In the 'classic' Mac version, well, I'm not sure what
// the Time Manager does if it misses a deadline.  If it runs its task an extra time, this is the way to go. If it
// shrugs and the call for that period is lost forever, it should NOT use this mechanism. In the spirit of leaving
// things alone if they work, I'll leave this just as an IPring thing for now.
#ifdef NETWORK_FAUX_QUEUE
static volatile uint32 faux_queue_due; // packet handler increments each time it smears
static volatile uint32
        faux_queue_paid; // NQT increments this each time it throws away action_flags instead of queueing them
#endif
...
// simultaneously burping :) ) queue up a bunch of action_flags as normal in NQT while they wait.  (Note that this also
// applies to a situation where network congestion causes a burp "between" stations - e.g. we have to retransmit due to
...
enum {
    kAdaptiveLatencyWindowSize
            = 3 * TICKS_PER_SECOND,   // number of rings we look at for deciding whether to change latency
...
// Hmm, now not only am I concerned about the concept of "net time", but I'm worried that the faux-queue idea I
// implemented before is now going to interfere with the adaptive latency scheme, or vice-versa.  I mean, if adaptive
// latency decides that we need another action_flag per packet, we're probably going to catch some non-servers unawares,
// and they'll have to smear to meet our requirement (which is fine) - but then when they run their NetQueueingTask,
// they'll try to pay back for the smear, which means they'll have to smear again next time... it's not the end of the
// world, but it defeats the purpose of calling NQT periodically instead of just smearing every time we need data. OK OK
// I convinced myself.  Disabling FAUX_QUEUE.  That didn't help with erratic server scheduling anyway.
...
// Same thing.  net_time is 3, there are 3 flags in local_queue, and player_queue is empty.  dynamic_world is oddly
// still. The ring packet comes in!  It has a required_flags of 1, and every player's data in the packet has 1 flag in
// it. The server's net time was 1 when the packet was sent.  *We set our local net_time to 1.*  We move a local_flag
// into the packet and pass the ring on.  net_time is 1, there are 2 flags in local_queue, and player_queue has 1. NQT
// queues a local flag.  net_time is 2.  There are 3 flags in local_queue. dynamic_world updates to time 1 - it could
// try to go to 2, but there's only 1 flag.  player_queue is now empty. NQT queues a local flag.  net_time is 3.  There
// are 4 flags in local_queue.  player_queue is empty.  dynamic_world is stopped. NQT queues a local flag.  net_time
// is 4.  There are 5 flags in local_queue.  player_queue is empty.  dynamic_world is stopped. The ring packet comes in!
// required_flags is 3; every player has 3 flags.  The server's net_time was 4. Our net_time agrees with the server's.
// We pack up 3 local_flags for the packet and pass it on. net_time = 4, |local_queue| = 2, |player_queue| = 3, DWTC
// = 1. dynamic_world wants to update to time 4, and CAN - it voraciously consumes all 3 player_flags. net_time = 4,
// |local_queue| = 2, |player_queue| = 0, DWTC = 4. NQT: net_time = 5, |local_queue| = 3, |player_queue| = 0, DWTC = 4.
...
// Options are to speculatively update dynamic_world, thus letting DWTC track net_time (but without "real data" to go
// on), (HARD), or to delay the engine's view of net_time by some amount (EASY), introducing even more latency but
// smoothing out the frames.

// How do we decide to raise or lower the amount of delay?
// Should be based on, when a packet comes in, how many player_flags there are?  How DWTC compares to (net_time -
// delay)?
...
bool RingGameProtocol::Enter(short* inNetStatePtr) {
    bool success = false;
...
bool RingGameProtocol::Sync(NetTopology* inTopology, int32 inSmallestGameTick, int inLocalPlayerIndex,
 ...
 */
bool RingGameProtocol::UnSync(bool inGraceful, int32 inSmallestPostgameTick) {
    bool success = true;
    uint32 ticks;
...
void RingGameProtocol::DistributeInformation(short type, void* buffer, short buffer_size, bool send_to_self, bool) {
    ...
    memcpy(distributionFrame->data + sizeof(NetPacketHeader_NET), &distribution_header_NET,
           sizeof(NetDistributionPacket_NET));
    memcpy(distributionFrame->data + sizeof(NetPacketHeader_NET)
                   + sizeof(NetDistributionPacket_NET) /*- 2*sizeof(byte)*/,
           buffer, buffer_size);
...
void RingGameProtocol::PacketHandler(DDPPacketBufferPtr packet) {
                    ...
                    if (sRingPreferences.mAcceptPacketsFromAnyone
                        ...
                        if (header->sequence == status->lastValidRingSequence + 1) {
                            ...
                            if (header->sequence <= status->lastValidRingSequence) {
                                ...
                                //								fdprintf("early ack (%d>%d);g", header->sequence,
                                //status->lastValidRingSequence);
                                assert(false);
                            }
                        }
                    }
                        ...
                        if (sRingPreferences.mAcceptPacketsFromAnyone
                            ...
                            if (header->sequence <= status->lastValidRingSequence) {
#ifdef DEBUG_NET
                                {
                                    ...
                                    NetPacket* packet_data = &packet_data_storage;
                                    NetPacket_NET* packet_data_NET
                                            = (NetPacket_NET*)(packet->datagramData + sizeof(NetPacketHeader_NET));

                                    netcpy(packet_data, packet_data_NET);

                                    // NetPacketPtr packet_data= (NetPacketPtr)
                                    // (packet->datagramData+sizeof(NetPacketHeader));
                                ...
                                //							fdprintf("late ring (%d<=%d);g", header->sequence,
                                //status->lastValidRingSequence);
                            } // sequence <= lastValidRingSequence
                            else {
                                /* on-time or early ring packet */
                                //							fdprintf("Got ring.;g");
                                //							fdprintf("on-time ring %p (%d bytes);dm #%d #%d;g", packet,
                                //packet->datagramSize, packet->datagramData, packet->datagramSize);
                                ...
                                NetProcessIncomingBuffer(packet->datagramData + sizeof(NetPacketHeader_NET),
                                                         packet->datagramSize - sizeof(NetPacketHeader_NET),
                                                         header->sequence);
                            }
                        } // came from expected source
                        /* Note that I ignore packets from an unknown source. There's a valid reason that we could get
                         * them. Imagine a ring with 3+ players. A->B->C->...->A. Player B sends to C, and crashes
                         * before getting the ack. but it's not a fatal crash. (macsbug warning...) C sends an ack
                         * though and forwards the packet. within 2 seconds, A drops B from the game. B recovers from
                         * the crash and says "Whoa! I didn't get an ack from C, let's resend". Then C gets a packet
                         * from the wrong person. (Note that time didn't pass for B while in macsbug, that's why he
                         * didn't just drop c from the game) */
                        /* Come to think of it, B could have had a fatal crash. He goes into macsbug with an assert,
                         * then when he comes out, the network code hasn't been halted quite yet, so everything still
                         * happens for a moment. */
                        else {
#ifdef DEBUG_NET
                            net_stats.packets_from_the_unknown++;
#endif
                            //						fdprintf("packet from unknown source %8x!=%8x.;g;",
                            //*((long*)&packet->sourceAddress), *((long*)&status->downringAddress));
                        }
                    } // accept_ring_packets
                    break;
...
static void NetProcessLossyDistribution(void* buffer) {
    ...
    if (packet_data->first_player_index != status->upringPlayerIndex) {
        ...
        // fill in data
        distributionFrame->data_size
                = sizeof(NetPacketHeader_NET) + sizeof(NetDistributionPacket_NET) + packet_data->data_size;
        ...
        memcpy(distributionFrame->data + sizeof(NetPacketHeader_NET), buffer,
...
/*
 ...
 this function queues flags from remote players, adds the local player’s latest command (thus
                                             modifying the buffer in place), calls NetBuildRingPacket to set up
 ringFrame based on this new data and then returns.
 */
...
static void NetProcessIncomingBuffer(void* buffer, int32 buffer_size, int32 sequence) {
    ...
    // ZZZ: this is the only spot we sample/adjust our adaptive_latency_2 (or 3): when we've received a valid ring
    // packet. We sample the server's required_action_flags (set to its SizeofLocalQueue before sent) as that should be
    // a good indicator of actual ring latency.
    ...
    netcpy(&packet_data->action_flags[0], (uint32*)(((char*)buffer) + sizeof(NetPacket_NET)),
    ...
    switch (packet_data->ring_packet_type) {
        case typeSYNC_RING_PACKET:
        case typeTIME_RING_PACKET:
            NetBuildRingPacket(ringFrame, (unsigned char*)packet_data, NetPacketSize(packet_data),
...
static void NetAddFlagsToPacket(NetPacketPtr packet) {
    ...
    vwarn(packet->required_action_flags >= 0 && packet->required_action_flags <= MAXIMUM_UPDATES_PER_PACKET,
          csprintf(temporary, "the server asked for %d flags.  bastard.  fucking ram doubler.",
    ...
    for (player_index = 0; player_index < localPlayerIndex; player_index++) {
        vwarn(packet->action_flag_count[player_index] >= -1
    ...
    if (packet->required_action_flags != packet->action_flag_count[localPlayerIndex]
        ...
        //		memcpy(action_flags + packet->required_action_flags, action_flags +
        //packet->action_flag_count[localPlayerIndex], count * sizeof(uint32));
        memmove(action_flags + packet->required_action_flags,
                action_flags + packet->action_flag_count[localPlayerIndex], count * sizeof(uint32));
...
/*
 ...
 ----------------------
    ...
    --> sequence to acknowledge		// ZZZ note: always status->lastValidRingSequence EXCEPT when acking a late ring
 packet.
...
static void NetSendRingPacket(DDPFramePtr frame) {
    ...
    status->clearToForwardRing = false; /* will not be set until we receive the next valid ring packet but will be
                                           irrelevant if serverCanForwardRing is true */
    ...
    if (reinstall) {
...
#endif
        {
            ...
            {   // ZZZ: this control path is taken if the ring is waiting for us.  The packet we received is in
...
#ifdef DEBUG_NET
                        //						fdprintf("I Server got a normal packet, at zero.  unsyncing... (%d);g",
                        //net_stats.action_flags_processed);
                        net_stats.server_unsyncing++;
#endif
                        packet_data->ring_packet_type = typeUNSYNC_RING_PACKET;
                    } else {
#ifdef DEBUG_NET
                        //						fdprintf("I Server got a normal packet & net was coming down required
                        //flags at 0. (%d);g", net_stats.action_flags_processed);
                ...
                NetBuildRingPacket(ringFrame, (byte*)packet_data, NetPacketSize(packet_data),
                                   status->lastValidRingSequence + 1);
...
static void update_adaptive_latency(int measurement, int tick) {
...
static short NetAdjustUpringAddressUpwards(void) {
    ...
    for (nextPlayerIndex = 0; nextPlayerIndex < topology->player_count; nextPlayerIndex++) {
        ...
        if (address->host == status->upringAddress.host && address->port == status->upringAddress.port) {
            break;
        }
    }
...
static void drop_upring_player(void) {
...
#ifdef DEBUG_NET
    //	fdprintf("Dropping upring- Attempting to delete upring (node %d) from ring. muhaha.;g",
    //status->upringAddress.aNode);
    ...
    if (flag_count > 0) {
        ...
        memmove(action_flags, action_flags + packet_data->action_flag_count[oldNextPlayerIndex],
                flag_count * sizeof(uint32));
        // memcpy(action_flags, action_flags + packet_data->action_flag_count[oldNextPlayerIndex], flag_count *
        // sizeof(uint32)); BlockMove(action_flags + packet_data->action_flag_count[oldNextPlayerIndex], 	action_flags,
        //sizeof(long)*flag_count);
    }
...
int32 RingGameProtocol::GetNetTime(void) {
...
bool RingGameProtocol::CheckWorldUpdate() {
...
void NetPrintInfo(void) {
    ...
    fdprintf("Unknown packets: %d Upring Drops: %d Rebuilt server Tags: %d;g", net_stats.packets_from_the_unknown,
             net_stats.upring_drops, net_stats.rebuilt_server_tag);
    fdprintf("Late Rings: Sync: %d Time: %d Normal: %d Unsync: %d Dead: %d;g", net_stats.late_sync_rings,
             net_stats.late_time_rings, net_stats.late_rings, net_stats.late_unsync_rings, net_stats.late_dead_rings);
    fdprintf("---Retries: Sync: %d Time: %d Normal: %d Unsync: %d Dead: %d;g", net_stats.sync_retry_count,
             net_stats.time_retry_count, net_stats.retry_count, net_stats.unsync_retry_count,
             net_stats.dead_retry_count);
    fdprintf("Ontime Ack: Sync: %d Time: %d Normal: %d Unsync: %d Dead: %d Late: %d;g", net_stats.sync_ontime_acks,
             net_stats.time_ontime_acks, net_stats.ontime_acks, net_stats.unsync_ontime_acks,
             net_stats.dead_ontime_acks, net_stats.late_acks);
    if (localPlayerIndex == status->server_player_index) {
        fdprintf("Server: Req to zero: %d Unsyncing: %d Returned: %d;g", net_stats.server_set_required_flags_to_zero,
                 net_stats.server_unsyncing, net_stats.unsync_returned_to_sender);
    }
    fdprintf("Packets w/zero flags: %d Spurious Unsyncs: %d;g", net_stats.packets_with_zero_flags,
             net_stats.spurious_unsyncs);
    fdprintf("Assumed control: Normal: %d Retry: %d (Server bailed early: %d);g", net_stats.assuming_control,
             net_stats.assuming_control_on_retry, net_stats.server_bailing_early);
...
static void process_packet_buffer_flags(void* buffer, int32 buffer_size, short packet_tag) {
    ...
    */
    ...
    if (!status->iAmTheServer || status->canForwardRing) {
        status->canForwardRing = false;
        if (status->iAmTheServer) { // (ZZZ) This is the (server) control path if there is enough data available to send
                                    // on the ring
            ...
            if (*sNetStatePtr == netComingDown) {
                ...
                if (packet_data->required_action_flags == 0) {
#ifdef DEBUG_NET
                    //					fdprintf("Server got a final packet & net was coming down (changed) type: %d
                    //(%d);g",  packet_data->ring_packet_type, net_stats.action_flags_processed);
                    net_stats.server_unsyncing++;
#endif
                    packet_data->ring_packet_type = typeUNSYNC_RING_PACKET;
                } else {
#ifdef DEBUG_NET
                    //					fdprintf("Server got a packet & net was coming down (changed) (%d) current:
                    //%d;g",  net_stats.action_flags_processed, packet_data->required_action_flags);
                    net_stats.server_set_required_flags_to_zero++;
#endif
                    packet_data->required_action_flags = 0;
                }
...
/*  pointer arithmetic. */
static void process_flags(NetPacket* packet_data) {
...
void record_profile(int req_action_flags) {
...
void RingGameProtocol::ParsePreferencesTree(InfoTree prefs, std::string version) {
