...
MusicPlayer::MusicPlayer(std::shared_ptr<StreamDecoder> decoder, MusicParameters parameters)
    : AudioPlayer(decoder->Rate(), decoder->IsStereo(), decoder->GetAudioFormat()) {
...
int MusicPlayer::GetNextData(uint8* data, int length) {
    ...
    if (dataSize == length || !parameters.Get().loop)
        return dataSize;
    decoder->Rewind();
