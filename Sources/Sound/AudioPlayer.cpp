...
AudioPlayer::AudioPlayer(int rate, bool stereo, AudioFormat audioFormat) { Init(rate, stereo, audioFormat); }
...
void AudioPlayer::ResetSource() {
    ...
    for (auto& buffer : audio_source->buffers) { buffer.second = false; }
}
