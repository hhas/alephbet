...
struct SoundStereo {
    ...
    bool operator==(const SoundStereo& other) const {
        return std::tie(is_panning, gain_global, gain_right, gain_left)
               == std::tie(other.is_panning, other.gain_global, other.gain_right, other.gain_left);
    }

    bool operator!=(const SoundStereo& other) const { return !(*(this) == other); }
};
...
struct SoundBehavior {
    ...
    bool operator==(const SoundBehavior& other) const {
        return std::tie(distance_reference, distance_max, rolloff_factor, max_gain, high_frequency_gain)
               == std::tie(other.distance_reference, other.distance_max, other.rolloff_factor, other.max_gain,
                           other.high_frequency_gain);
    }

    bool operator!=(const SoundBehavior& other) const { return !(*(this) == other); }
};
...
class SoundPlayer : public AudioPlayer {
  public:
    ...
    SoundPlayer(const Sound& sound,
                const SoundParameters& parameters); // Must not be used outside OpenALManager (public for make_shared)
