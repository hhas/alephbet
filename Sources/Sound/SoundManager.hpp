...
class SoundManager {
  public:
    ...
    std::shared_ptr<SoundPlayer> PlaySound(short sound_index, world_location3d* source, short identifier,
                                           _fixed pitch = _normal_frequency);
    ...
    void AddOneAmbientSoundSource(ambient_sound_data* ambient_sounds, world_location3d* source,
                                  world_location3d* listener, short ambient_sound_index, short absolute_volume);
    ...
    static float From_db(float db, bool music = false) {
        return db <= (SoundManager::MINIMUM_VOLUME_DB / (music ? 2 : 1)) ? 0 : std::pow(10.f, db / 20.f);
    }

    struct Parameters {
        static const int DEFAULT_RATE    = 44'100;
  ...
  private:
    ...
    std::shared_ptr<SoundPlayer> GetSoundPlayer(short identifier, short source_identifier,
                                                bool sound_identifier_only = false) const;
    std::shared_ptr<SoundPlayer> UpdateExistingPlayer(const Sound& sound, const SoundParameters& soundParameters,
...
typedef void (*add_ambient_sound_source_proc_ptr)(ambient_sound_data* ambient_sounds, world_location3d* source,
