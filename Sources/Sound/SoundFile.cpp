...
SoundHeader::SoundHeader() : SoundInfo(), data_offset(0), signed_8bits(false) {}
...
bool SoundHeader::UnpackStandardSystem7Header(BIStreamBE& header) {
    try {
        ...
        return true;
    } catch (const basic_bstream::failure& e) { return false; }
}
...
bool SoundHeader::UnpackExtendedSystem7Header(BIStreamBE& header) {
    try {
        ...
        if ((loop_end - loop_start >= 4) && ((loop_start % bytes_per_frame) || (loop_end % bytes_per_frame))) {
            logWarning("loop_start=%i and loop_end=%i but bytes_per_frame=%i; interpreting as frame offsets",
...
std::shared_ptr<SoundData> SoundHeader::LoadData(BIStreamBE& s) {
    ...
    } catch (const basic_bstream::failure& e) { p.reset(); }

    return p;
}

void SoundHeader::ConvertSignedToUnsignedByte(uint8* data, int length) {
    for (int i = 0; i < length; i++) { data[i] = static_cast<int8>(data[i]) + 128; }
}
...
SoundDefinition::SoundDefinition()
    : sound_code(0), behavior_index(1), flags(0), chance(0), low_pitch(0), high_pitch(0), permutations(1),
      permutations_played(0), group_offset(0), single_length(0), total_length(0), last_played(0) {}
...
bool SoundDefinition::Unpack(OpenedFile& SoundFile) {
    ...
    for (int i = 0; i < sound_offsets.size(); i++) { header >> sound_offsets[i]; }
...
bool SoundDefinition::Load(OpenedFile& SoundFile, bool LoadPermutations) {
    ...
    for (int i = 0; i < sounds.size(); i++) {
        if (!SoundFile.SetPosition(group_offset + sound_offsets[i]) || !sounds[i].Load(SoundFile)) {
            sounds.clear();
            return false;
        }
    }
...
bool M2SoundFile::Open(FileSpecifier& SoundFileSpec) {
    ...
    if (!SoundFileSpec.Open(*sound_file, false))
        return false;
    ...
    if ((version != 0 && version != 1) || tag != FOUR_CHARS_TO_INT('s', 'n', 'd', '2') || sound_count < 0
        || source_count < 0) {
        return false;
    }
    ...
    for (int source = 0; source < source_count; ++source) {
        for (int i = 0; i < sound_count; ++i) { sound_definitions[source][i].Load(*sound_file, true); }
    }
...
void M2SoundFile::Close() { sound_definitions.clear(); }
...
SoundDefinition* M1SoundFile::GetSoundDefinition(int, int sound_index) {
    if (resource_file.Check('s', 'n', 'd', ' ', sound_index)) {
        ...
        if (it == definitions.end()) {
            ...
            while (resource_file.Check('s', 'n', 'd', ' ', sound_index + definition.permutations)
                   && definition.permutations < MAXIMUM_PERMUTATIONS_PER_SOUND) {
