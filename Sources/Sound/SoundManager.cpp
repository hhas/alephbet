...
class SoundMemoryManager {
  public:
    ...
    bool IsLoaded(short index) { return m_entries.count(index); }
...
void SoundManager::TestVolume(float db, short sound_index) {
    if (active) {
        ...
        while (sound && sound->IsActive()) yield();
        OpenALManager::Get()->SetMasterVolume(From_db(parameters.volume_db));
    }
}
...
void SoundManager::LoadSounds(short* sounds, short count) {
    for (short i = 0; i < count; i++) { LoadSound(sounds[i]); }
}
...
// A sound is identified as unique with sound index + source index, NONE is considered as a valid source index (local
// sounds) The flag sound_identifier_only must be used to know if there is a sound playing with a specific identifier
// without caring of the source
std::shared_ptr<SoundPlayer> SoundManager::GetSoundPlayer(short identifier, short source_identifier,
                                                          bool sound_identifier_only) const {
    ...
    std::copy_if(sound_players.begin(), sound_players.end(), std::back_inserter(matchingPlayers),
                 [identifier](const std::shared_ptr<SoundPlayer> player) {
    ...
    if (!sound_identifier_only) {
        ...
        auto matchingSourcePlayer = std::find_if(matchingPlayers.begin(), matchingPlayers.end(),
                                                 [source_identifier](const std::shared_ptr<SoundPlayer> player) {
                                                     return player->GetSourceIdentifier() == source_identifier;
                                                 });

        if (matchingSourcePlayer == matchingPlayers.end() && matchingPlayers.size() >= MAX_SOUNDS_FOR_SOURCE) {
            matchingPlayer = *std::min_element(
                    matchingPlayers.begin(), matchingPlayers.end(),
                    [](const std::shared_ptr<SoundPlayer>& a, const std::shared_ptr<SoundPlayer>& b) {
                        return a->GetPriority() < b->GetPriority();
                    });
        } else {
            matchingPlayer = matchingSourcePlayer != matchingPlayers.end() ? *matchingSourcePlayer
                                                                           : std::shared_ptr<SoundPlayer>();
        }
    }
...
std::shared_ptr<SoundPlayer>
        SoundManager::PlaySound(short sound_index, world_location3d* source,
    ...
    if (source) {
        ...
        parameters.dynamic_source_location3d
                = (this->parameters.flags & _dynamic_tracking_flag) && identifier != NONE ? source : nullptr;
...
std::shared_ptr<SoundPlayer> SoundManager::DirectPlaySound(short sound_index, angle direction, short volume,
                                                           _fixed pitch) {
    ...
    if (direction != NONE && listener) {
        ...
        AngleAndVolumeToStereoVolume(direction - listener->yaw, volume, &variables.right_volume,
                                     &variables.left_volume);
        parameters.stereo_parameters.is_panning  = true;
    ...
    for (auto& soundPlayer : sound_players) {
        ...
        if (parameters.loop && SoundPlayer::Simulate(parameters) <= 0) {
            ...
            if (!parameters.is_2d) {
                ...
                parameters.obstruction_flags
                        = GetSoundObstructionFlags(parameters.identifier, &parameters.source_location3d);
                updateParameters = updateParameters || obstruction_flags != parameters.obstruction_flags;
            } else if (parameters.stereo_parameters.is_panning
                       && parameters.source_identifier != NONE) { // only occurs when 3D sounds is disabled
...
void SoundManager::UpdateListener() {
    if (!active || !(parameters.flags & _3d_sounds_flag))
        return;
    auto listener = _sound_listener_proc();
    if (listener && *listener != OpenALManager::Get()->GetListener())
...
static sound_behavior_definition* get_sound_behavior_definition(const short sound_behavior_index) {
...
static short distance_to_volume(SoundDefinition* definition, world_distance distance, uint16 flags) {
    struct sound_behavior_definition* behavior = get_sound_behavior_definition(definition->behavior_index);
    // LP change: idiot-proofing
    if (!behavior)
        return 0; // Silence
    ...
    if (distance <= depth_curve->maximum_volume_distance) {
        ...
        if (distance > depth_curve->minimum_volume_distance) {
            ...
            volume = depth_curve->minimum_volume
                     - ((depth_curve->minimum_volume - depth_curve->maximum_volume)
                        * (depth_curve->minimum_volume_distance - distance))
...
static ambient_sound_definition* get_ambient_sound_definition(const short ambient_sound_index) {
    return GetMemberWithBounds(ambient_sound_definitions, ambient_sound_index, NUMBER_OF_AMBIENT_SOUND_DEFINITIONS);
}

void SoundManager::AddOneAmbientSoundSource(ambient_sound_data* ambient_sounds, world_location3d* source,
                                            world_location3d* listener, short ambient_sound_index,
    ...
    // LP change: idiot-proofing
    if (!definition || definition->sound_code == NONE)
        return;
    ...
    for (i = 0, ambient = ambient_sounds; i < MAXIMUM_PROCESSED_AMBIENT_SOUNDS; ++i, ++ambient) {
        if (SLOT_IS_USED(ambient)) {
            if (ambient->sound_index == sound_index)
                break;
        } else {
...
SoundManager::Parameters::Parameters()
    : volume_db(DEFAULT_SOUND_LEVEL_DB),
      flags(_more_sounds_flag | _dynamic_tracking_flag | _ambient_sound_flag | _16bit_sound_flag), rate(DEFAULT_RATE),
      samples(DEFAULT_SAMPLES), music_db(DEFAULT_MUSIC_LEVEL_DB),
      video_export_volume_db(DEFAULT_VIDEO_EXPORT_VOLUME_DB), channel_type(ChannelType::_stereo) {}
...
void SoundManager::SetStatus(bool active) {
    if (!initialized || active == this->active)
        return;
    ...
    if (active) {
        ...
        AudioParameters audio_parameters = {parameters.rate,
                                            parameters.samples,
...
std::shared_ptr<SoundPlayer> SoundManager::UpdateExistingPlayer(const Sound& sound,
                                                                const SoundParameters& soundParameters,
    ...
    if (soundParameters.flags & _sound_does_not_self_abort)
        return std::shared_ptr<SoundPlayer>();

    auto existingPlayer = GetSoundPlayer(soundParameters.identifier, soundParameters.source_identifier,
                                         !(parameters.flags & _3d_sounds_flag)
                                                 || (soundParameters.flags & _sound_cannot_be_restarted));

    if (existingPlayer) {

        if (soundParameters.soft_rewind
            || (!(soundParameters.flags & _sound_cannot_be_restarted)
                && (existingPlayer->CanFastRewind(soundParameters)
                    || simulatedVolume + abortAmplitudeThreshold
                               > SoundPlayer::Simulate(existingPlayer->GetParameters())))) {
            existingPlayer->AskRewind(
                    soundParameters, sound); // we found one, we won't create another player but rewind this one instead
        }
    }
...
std::shared_ptr<SoundPlayer> SoundManager::ManageSound(const Sound& sound, const SoundParameters& parameters) {
    ...
    if (returnedPlayer)
        sound_players.insert(returnedPlayer);
    return returnedPlayer;
}
...
short SoundManager::GetRandomSoundPermutation(short sound_index) {
    ...
    if (!(definition->permutations > 0))
        return 0;

    if (parameters.flags & _more_sounds_flag) {
        if ((definition->permutations_played & ((1 << definition->permutations) - 1))
            == ((1 << definition->permutations) - 1))
...
static void add_one_ambient_sound_source(struct ambient_sound_data* ambient_sounds, world_location3d* source,
                                         world_location3d* listener, short sound_index, short absolute_volume) {
...
void SoundManager::UpdateAmbientSoundSources() {
    ...
    for (auto sound = ambient_sound_players.begin(); sound != ambient_sound_players.end();
         sound++) { // we are playing a sound that shouldn't be played anymore, stop the looping sound
        bool found        = false;
    ...
    for (short i = 0; i < MAXIMUM_PROCESSED_AMBIENT_SOUNDS; i++) {
        if (SLOT_IS_FREE(&ambient_sounds[i]))
            continue;
        ...
        if (soundPlayer && soundPlayer->HasRewind()) {
            ...
            parameters.stereo_parameters.gain_left
                    = ambient_sounds[i].variables.left_volume * 1.f / MAXIMUM_SOUND_VOLUME;
            parameters.stereo_parameters.gain_right
                    = ambient_sounds[i].variables.right_volume * 1.f / MAXIMUM_SOUND_VOLUME;
...
void SoundManager::CalculateSoundVariables(short sound_index, world_location3d* source, SoundVolumes& variables) {
    ...
    if (source && listener) {
        ...
        if (dx || dy) {
            ...
            AngleAndVolumeToStereoVolume(arctangent(dx, dy) - listener->yaw, variables.volume, &variables.right_volume,
                                         &variables.left_volume);
        } else {
...
void SoundManager::CalculateInitialSoundVariables(short sound_index, world_location3d* source,
                                                  SoundVolumes& variables) {
    SoundDefinition* definition = GetSoundDefinition(sound_index);
    if (!definition)
        return;
...
void parse_mml_sounds(const InfoTree& root) {
    ...
    if (!original_ambient_sound_definitions) {
        original_ambient_sound_definitions = (struct ambient_sound_definition*)malloc(
                sizeof(struct ambient_sound_definition) * NUMBER_OF_AMBIENT_SOUND_DEFINITIONS);
        assert(original_ambient_sound_definitions);
        for (int i = 0; i < NUMBER_OF_AMBIENT_SOUND_DEFINITIONS; i++)
            original_ambient_sound_definitions[i] = ambient_sound_definitions[i];
    }
    if (!original_random_sound_definitions) {
        original_random_sound_definitions = (struct random_sound_definition*)malloc(
                sizeof(struct random_sound_definition) * NUMBER_OF_RANDOM_SOUND_DEFINITIONS);
