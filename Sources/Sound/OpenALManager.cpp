...
bool OpenALManager::Init(const AudioParameters& parameters) {
    ...
    if (instance) { // Don't bother recreating all the OpenAL context if nothing changed for it
        if (parameters.hrtf != instance->audio_parameters.hrtf || parameters.rate != instance->audio_parameters.rate
            || parameters.channel_type != instance->audio_parameters.channel_type
            || parameters.sample_frame_size != instance->audio_parameters.sample_frame_size) {
...
#undef LOAD_PROC
            ...
            logError("ALC_SOFT_loopback extension is not supported"); // Should never be the case as long as >=
                                                                      // OpenAL 1.14
            return false;
        }
    }
...
void OpenALManager::ProcessAudioQueue() {
    ...
    while (audio_players_shared.pop_nonblocking(audioPlayer)) { audio_players_queue.push_back(audioPlayer); }
    ...
    for (int i = 0; i < audio_players_queue.size(); i++) {
        ...
        audio_players_queue.push_back(
                audio); // We have just processed a part of the data for you, now wait your next turn
    }
}
...
void OpenALManager::UpdateListener() {
    ...
    ALfloat position[] = {(float)(listener.point.x) / WORLD_ONE, (float)(listener.point.z) / WORLD_ONE,
                          (float)(listener.point.y) / WORLD_ONE};

    ALfloat velocity[] = {(float)listener.velocity.i / WORLD_ONE, (float)listener.velocity.k / WORLD_ONE,
...
std::shared_ptr<MusicPlayer> OpenALManager::PlayMusic(std::shared_ptr<StreamDecoder> decoder,
                                                      MusicParameters parameters) {
    if (!process_audio_active)
        return std::shared_ptr<MusicPlayer>();
...
std::shared_ptr<StreamPlayer> OpenALManager::PlayStream(CallBackStreamPlayer callback, int length, int rate,
                                                        bool stereo, AudioFormat audioFormat) {
    if (!process_audio_active)
        return std::shared_ptr<StreamPlayer>();
...
std::unique_ptr<AudioPlayer::AudioSource> OpenALManager::PickAvailableSource(const AudioPlayer& audioPlayer) {
    if (sources_pool.empty()) {
        const auto& victimPlayer
                = *std::min_element(audio_players_queue.begin(), audio_players_queue.end(),
...
void OpenALManager::StopAllPlayers() {
    ...
    std::shared_ptr<AudioPlayer> audioPlayer;
    while (audio_players_shared.pop_nonblocking(audioPlayer)) { audioPlayer->is_active = false; }
...
bool OpenALManager::OpenDevice() {
    ...
    if (openal_rendering_format) {
        ALCint attrs[] = {
                ...
                openal_rendering_format,
                ALC_FORMAT_CHANNELS_SOFT,
                mapping_sdl_openal_channel.at(audio_parameters.channel_type),
                ALC_FREQUENCY,
                audio_parameters.rate,
                ALC_HRTF_SOFT,
                audio_parameters.hrtf,
                0,
        };
...
bool OpenALManager::GenerateSources() {
    ...
    for (auto source_id : sources_id) {
        ...
        for (int i = 0; i < num_buffers; i++) { audioSource.buffers.insert({buffers_id[i], false}); }
...
void OpenALManager::MixerCallback(void* usr, uint8* stream, int len) {
    ...
    int frameSize = manager->sdl_audio_specs_obtained.channels
                    * SDL_AUDIO_BITSIZE(manager->sdl_audio_specs_obtained.format) / 8;
...
void OpenALManager::CleanEverything() {
    ...
    while (!sources_pool.empty()) {
        ...
        for (auto const& buffer : audioSource->buffers) { alDeleteBuffers(1, &buffer.first); }
