...
SoundPlayer::SoundPlayer(const Sound& sound, const SoundParameters& parameters)
    : AudioPlayer(sound.header.rate >> 16, sound.header.stereo,
                  sound.header.audio_format) { // since header.rate is on 16.16 format
    this->sound = sound;
    Init(parameters);
}
...
float SoundPlayer::Simulate(const SoundParameters& soundParameters) {
    if (soundParameters.is_2d && !soundParameters.stereo_parameters.is_panning)
        return 1; // ofc we play all 2d sounds without stereo panning
    if (soundParameters.stereo_parameters.is_panning)
        return soundParameters.stereo_parameters.gain_global;

    const auto& listener
            = OpenALManager::Get()->GetListener(); // if we don't have a listener on a 3d sound, there is a problem
    ...
    const bool obstruction = (soundParameters.obstruction_flags & _sound_was_obstructed)
                             || (soundParameters.obstruction_flags & _sound_was_media_obstructed);
    const bool muffled = soundParameters.obstruction_flags & _sound_was_media_muffled;

    const auto& behaviorParameters
            = obstruction && muffled   ? sound_obstructed_and_muffled_behavior_parameters[soundParameters.behavior]
    ...
    float volume = behaviorParameters.distance_reference
                   / (behaviorParameters.distance_reference
                      + behaviorParameters.rolloff_factor * (distance - behaviorParameters.distance_reference));
...
bool SoundPlayer::CanRewind(int baseTick) const {
    ...
    if (rewindParameters.soft_rewind)
        return true;
    auto rewindTime = OpenALManager::Get()->IsBalanceRewindSound() || !CanFastRewind(rewindParameters)
                              ? rewind_time
                              : fast_rewind_time;
...
bool SoundPlayer::CanFastRewind(const SoundParameters& soundParameters) const {
    return (soundParameters.source_identifier != NONE
            || (soundParameters.is_2d && !soundParameters.stereo_parameters.is_panning))
...
void SoundPlayer::Rewind() {
    ...
    if (!CanRewind(start_tick)
        && (current_index_data > 0 || IsPlaying())) { // if a sound hasn't started playing yet, it's ok to replace it
        rewind_signal = false;
        return;
    }
...
bool SoundPlayer::LoadParametersUpdates() {
    ...
    if (softStop && sound_transition.allow_transition)
        return true;
    ...
    while (rewind_parameters.Consume(soundParameters)) {
        ...
        if (priority <= rewindLastPriority)
            continue;
    ...
    if (rewindLastPriority > 0)
        rewind_parameters.Set(bestRewindParameters);
...
SetupALResult SoundPlayer::SetUpALSourceIdle() {
    ...
    if (soundParameters.is_2d) {
        ...
        if (soundParameters.stereo_parameters.is_panning) {
            auto pan = (acosf(std::min(soundParameters.stereo_parameters.gain_left, 1.f))
                        + asinf(std::min(soundParameters.stereo_parameters.gain_right, 1.f)))
                       / ((float)M_PI); // average angle in [0,1]
...
float SoundPlayer::ComputeParameterForTransition(float targetParameter, float currentParameter, int currentTick) const {
    float computedParameter = std::max((targetParameter - currentParameter)
                                                       * std::min((currentTick - sound_transition.start_transition_tick)
                                                                          / (float)smooth_volume_transition_time_ms,
                                                                  1.f)
                                               + currentParameter,
                                       0.f);
    return targetParameter > currentParameter ? std::min(targetParameter, computedParameter)
                                              : std::max(targetParameter, computedParameter);
}
...
SoundBehavior SoundPlayer::ComputeVolumeForTransition(const SoundBehavior& targetSoundBehavior) {
    ...
    if (sound_transition.allow_transition && targetSoundBehavior != sound_transition.current_sound_behavior) {
        ...
        computedSoundBehavior.distance_max = ComputeParameterForTransition(
                targetSoundBehavior.distance_max, sound_transition.current_sound_behavior.distance_max, currentTick);
        computedSoundBehavior.distance_reference = ComputeParameterForTransition(
                targetSoundBehavior.distance_reference, sound_transition.current_sound_behavior.distance_reference,
                currentTick);
        computedSoundBehavior.max_gain = ComputeParameterForTransition(
                targetSoundBehavior.max_gain, sound_transition.current_sound_behavior.max_gain, currentTick);
        computedSoundBehavior.rolloff_factor
                = ComputeParameterForTransition(targetSoundBehavior.rolloff_factor,
                                                sound_transition.current_sound_behavior.rolloff_factor, currentTick);
        computedSoundBehavior.high_frequency_gain = ComputeParameterForTransition(
                targetSoundBehavior.high_frequency_gain, sound_transition.current_sound_behavior.high_frequency_gain,
                currentTick);
        sound_transition.start_transition_tick = currentTick;
    }
...
float SoundPlayer::ComputeVolumeForTransition(float targetVolume) {
    ...
    if (sound_transition.allow_transition
        && std::abs(targetVolume - sound_transition.current_volume) > smooth_volume_transition_threshold) {
...
SetupALResult SoundPlayer::SetUpALSource3D() {
    ...
    alSourcei(audio_source->source_id, AL_DIRECT_FILTER,
              OpenALManager::Get()->GetLowPassFilter(finalBehaviorParameters.high_frequency_gain));
...
int SoundPlayer::GetNextData(uint8* data, int length) {
    ...
    if (remainingDataLength <= 0)
        return LoopManager(data, length);
    int returnedDataLength = std::min(remainingDataLength, length);
    auto& sound_data       = sound.Get().data;
    std::copy(sound_data->data() + current_index_data, sound_data->data() + current_index_data + returnedDataLength,
              data);
    current_index_data += returnedDataLength;
    if (returnedDataLength < length && parameters.Get().loop)
        return returnedDataLength + LoopManager(data + returnedDataLength, length - returnedDataLength);
