...
Movie::Movie()
    : moviefile(""), temp_surface(NULL), av(NULL), encodeThread(NULL), encodeReady(NULL),
#ifdef HAVE_OPENGL
      frameBufferObject(nullptr),
#endif
      fillReady(NULL), stillEncoding(0) {
    av = new libav_vars_t;
...
bool Movie::IsRecording() { return (moviefile.length() > 0); }
...
bool Movie::Setup() {
    ...
    temp_surface = SDL_CreateRGBSurface(SDL_SWSURFACE, view_rect.w, view_rect.h, 32, 0x00ff'0000, 0x0000'ff00,
                                        0x0000'00ff, 0);

    if (temp_surface == NULL) {
        ThrowUserError("Could not create SDL surface");
        return false;
    }
    ...
    if (!av->ffmpeg_file) {
        ThrowUserError("Could not create ffmpeg file: " + std::string(SDL_ffmpegGetError()));
        return false;
    }
                      ...
                      // recommendations
    {
        ...
        else if (view_rect.h >= 1440)
            bitrate = 16 * 1024 * 1024;
        else if (view_rect.h >= 1080)
            bitrate = 8 * 1024 * 1024;
        else if (view_rect.h >= 720)
            bitrate = 5 * 1024 * 1024;
        else if (view_rect.h >= 480)
            bitrate = 5 * 1024 * 1024 / 2;
        else
            bitrate = 1024 * 1024;
    ...
    if (!video_stream) {
        ThrowUserError("Could not add video stream: " + std::string(SDL_ffmpegGetError()));
        return false;
    }

    auto audio_stream = SDL_ffmpegAddAudioStream(av->ffmpeg_file, codec);
    if (!audio_stream) {
        ThrowUserError("Could not add audio stream: " + std::string(SDL_ffmpegGetError()));
        return false;
    }

    if (SDL_ffmpegSelectVideoStream(av->ffmpeg_file, video_stream->id) == -1) {
        ThrowUserError("Could not select video stream: " + std::string(SDL_ffmpegGetError()));
        return false;
    }
    if (SDL_ffmpegSelectAudioStream(av->ffmpeg_file, audio_stream->id) == -1) {
        ThrowUserError("Could not select audio stream: " + std::string(SDL_ffmpegGetError()));
        return false;
    }

    av->audio_frame = SDL_ffmpegCreateAudioFrame(av->ffmpeg_file, 0);
    if (!av->audio_frame) {
        ThrowUserError("Could not create audio frame");
        return false;
    }

    if (avformat_write_header(av->ffmpeg_file->_ffmpeg, 0) < 0) {
        ThrowUserError("Could not write header");
        return false;
    }

    av->audio_fifo = av_fifo_alloc(262'144);
    if (!av->audio_fifo) {
        ThrowUserError("Could not allocate audio fifo");
        return false;
    }

    // set up our threads and intermediate storage
    videobuf.resize(view_rect.w * view_rect.h * 4 + 10'000);
    audiobuf.resize(2 * in_bps * OpenALManager::Get()->GetFrequency() / fps);

    // TODO: fixme!
    if (OpenALManager::Get()->GetFrequency() % fps != 0) {
        ThrowUserError("Audio buffer size is non-integer; try lowering FPS target");
        return false;
    }
    ...
    stillEncoding = true;
    if (!encodeReady || !fillReady) {
        ThrowUserError("Could not create movie thread semaphores");
        return false;
    }

    encodeThread = SDL_CreateThread(Movie_EncodeThread, "MovieSetup_encodeThread", this);
    if (!encodeThread) {
        ThrowUserError("Could not create movie encoding thread");
        return false;
    }
...
long Movie::GetCurrentAudioTimeStamp() {
    return IsRecording() && av->inited && av->ffmpeg_file->audioStream ? av->ffmpeg_file->audioStream->lastTimeStamp
                                                                       : 0;
}
...
void Movie::AddFrame(FrameType ftype) {
    ...
    if (!MainScreenIsOpenGL()) {
        ...
        GLint fbx = viewportDimensions.x, fby = viewportDimensions.y, fbWidth = viewportDimensions.w,
              fbHeight = viewportDimensions.h;
        ...
        glBlitFramebufferEXT(fbx, fby, fbWidth + fbx, fbHeight + fby, view_rect.x, view_rect.y, view_rect.w,
                             view_rect.h, GL_COLOR_BUFFER_BIT, GL_LINEAR);
        ...
        glReadPixels(view_rect.x, view_rect.y, view_rect.w, view_rect.h, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV,
                     &videobuf.front());
        frameBufferObject->deactivate();

        // Copy pixel buffer (which is upside-down) to surface
        for (int y = 0; y < view_rect.h; y++)
            memcpy((uint8*)temp_surface->pixels + temp_surface->pitch * y,
                   &videobuf.front() + view_rect.w * 4 * (view_rect.h - y - 1), view_rect.w * 4);
    }
#endif
