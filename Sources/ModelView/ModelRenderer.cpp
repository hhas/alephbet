...
void ModelRenderer::Render(Model3D& Model, ModelRenderShader* Shaders, int NumShaders, int NumSeparableShaders,
                           bool Use_Z_Buffer) {
    if (NumShaders <= 0)
        return;
    if (!Shaders)
...
void ModelRenderer::SetupRenderPass(Model3D& Model, ModelRenderShader& Shader) {
    ...
    if (Shader.LightingCallback && !Model.Normals.empty() && TEST_FLAG(Shader.Flags, ExtLight)) {
        ...
        Shader.LightingCallback(Shader.LightingCallbackData, NumVerts, Model.NormBase(), Model.PosBase(),
                                &ExtLightColors[0]);
        ...
        if (!Model.Colors.empty() && TEST_FLAG(Shader.Flags, Colored)) {
            ...
            if (NumCPlanes == 3) {
                for (size_t k = 0; k < NumCValues; k++, ExtColorPtr++, ColorPtr++) (*ExtColorPtr) *= (*ColorPtr);
