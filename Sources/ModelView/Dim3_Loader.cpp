...
static void parse_dim3(const InfoTree& root, Model3D& Model) {
    ...
    for (const InfoTree& bound_box : root.children_named("Bound_Box")) { parse_bounding_box(bound_box, Model); }
    for (const InfoTree& view_box : root.children_named("View_Box")) { parse_bounding_box(view_box, Model); }
    ...
    for (const InfoTree& poses : root.children_named("Poses")) {
        for (const InfoTree& pose : poses.children_named("Pose")) {
                ...
                for (const InfoTree& bone : bones.children_named("Bone")) {
                    ...
                    if (bone.read_attr("rot", tempstr)) {
                        ...
                        if (sscanf(tempstr.c_str(), "%f,%f,%f", &in_angle[0], &in_angle[1], &in_angle[2]) == 3) {
                            for (int c = 0; c < 3; ++c) data.Angles[c] = GetAngle(-in_angle[c]);
                        }
                    }
                    float tempfloat;
            ...
            for (size_t b = 0; b < read_frame.size(); ++b) Model.Frames.push_back(read_frame[b]);
...
bool LoadModel_Dim3(FileSpecifier& Spec, Model3D& Model, int WhichPass) {
    ...
    try {
        ...
        for (const InfoTree& root : fileroot.children_named("Model")) { parse_dim3(root, Model); }
    ...
    // Set these up now
    if (Model.InverseVSIndices.empty())
        Model.BuildInverseVSIndices();
    ...
    if (!Normals.empty()) {
        ...
        for (unsigned k = 0; k < Model.VtxSources.size(); k++) {
            ...
            for (int p = P0; p < P1; p++) {
                ...
                for (int c = 0; c < 3; c++) DestNorm[c] = Norm[c];
            }
        }
        // All done with them
    ...
    if (BoneIndices.empty() && !Model.Bones.empty()) {
        ...
        // Reorder the bones
        for (size_t ib = 0; ib < NumBones; ib++) SortedBones[BoneIndices[ib]] = Model.Bones[ib];
