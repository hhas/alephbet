...
static void FindFrameTransform(Model3D_Transform& T, Model3D_Frame& Frame, GLfloat MixFrac, Model3D_Frame& AddlFrame);
static void FindBoneTransform(Model3D_Transform& T, Model3D_Bone& Bone, Model3D_Frame& Frame, GLfloat MixFrac,
...
static bool NormalizeNormal(GLfloat* Normal) {
    GLfloat NormalSqr = Normal[0] * Normal[0] + Normal[1] * Normal[1] + Normal[2] * Normal[2];
...
void Model3D::CalculateTangents() {
    ...
    for (GLushort i = 0; i < VertIndices.size(); i += 3) {
        ...
        vec3 T((t2 * x1 - t1 * x2) * r, (t2 * y1 - t1 * y2) * r, (t2 * z1 - t1 * z2) * r);
        vec3 B((s1 * x2 - s2 * x1) * r, (s1 * y2 - s2 * y1) * r, (s1 * z2 - s2 * z1) * r);
        if ((s1 * t2 - s2 * t1) == 0.0) {
            T = (v3 - v1).norm();
            B = (v2 - v1).norm();
        }
...
void Model3D::AdjustNormals(int NormalType, float SmoothThreshold) {
    ...
    switch (NormalType) {
        ...
        default:
            // Normalize
            for (unsigned k = 0; k < Normals.size() / 3; k++) NormalizeNormal(&Normals[3 * k]);
            break;
            ...
            // The really interesting stuff
            {
                ...
                for (unsigned k = 0; k < NumVerts; k++) {
                    ...
                    if (NumVertPolys > 0 && PerVertexNormalList[k].Flag)
                        PerVertexNormalList[k].Flag = sqrt(Variances[k] / NumVertPolys) <= SmoothThreshold;
                }
                ...
                for (unsigned k = 0; k < NumVerts; k++) {
                    ...
                    if (VSPresent) {
                        for (unsigned c = 0; c < NumVertPolys; c++) *(NewS++) = *OldS;
                    }
                    if (PVN.Flag) {
                        GLfloat* VP = PVN.Vec;
                        *(NewN++)   = *(VP++);
    ...
    switch (NormalType) {
        ...
        case CounterclockwiseSide: {
            ...
            for (unsigned k = 0; k < Normals.size(); k++) *(NormalPtr++) *= -1;
        }
    }
...
void Model3D::BuildTrigTables() { build_trig_tables(); }
...
void Model3D::BuildInverseVSIndices() {
    ...
    for (vector<GLushort>::iterator VSI_Iter = VtxSrcIndices.begin(); VSI_Iter < VtxSrcIndices.end(); VSI_Iter++)
        InvVSIPointers[*VSI_Iter]++;

    // Find the positions from the counts
    GLushort PtrSum = 0;
    for (vector<GLushort>::iterator IVP_Iter = InvVSIPointers.begin(); IVP_Iter < InvVSIPointers.end(); IVP_Iter++) {
    ...
    for (unsigned k = 0; k < VtxSrcIndices.size(); k++) InverseVSIndices[InvVSIPointers[VtxSrcIndices[k]]++] = k;
    ...
    for (vector<GLushort>::iterator IVP_Iter = InvVSIPointers.end() - 1; IVP_Iter > InvVSIPointers.begin();
         IVP_Iter--) {
        *IVP_Iter = *(IVP_Iter - 1);
    }
    InvVSIPointers[0] = 0;
...
// Frame case
bool Model3D::FindPositions_Frame(bool UseModelTransform, GLshort FrameIndex, GLfloat MixFrac, GLshort AddlFrameIndex) {
    ...
    for (size_t ib = 0; ib < NumBones; ib++)
        FindBoneTransform(BoneMatrices[ib], Bones[ib], FramePtr[ib], MixFrac, AddlFramePtr[ib]);
...
bool Model3D::FindPositions_Sequence(bool UseModelTransform, GLshort SeqIndex, GLshort FrameIndex, GLfloat MixFrac,
...
static void FindFrameTransform(Model3D_Transform& T, Model3D_Frame& Frame, GLfloat MixFrac, Model3D_Frame& AddlFrame) {
    ...
    if (MixFrac != 0) {
        ...
        for (int ic = 0; ic < 3; ic++) T.M[ic][3] = FrameOfst[ic] + MixFrac * (AddlFrameOfst[ic] - FrameOfst[ic]);
    } else {
        for (int ic = 0; ic < 3; ic++) T.M[ic][3] = FrameOfst[ic];
    }
}
...
static void FindBoneTransform(Model3D_Transform& T, Model3D_Bone& Bone, Model3D_Frame& Frame, GLfloat MixFrac,
    ...
    for (int ic = 0; ic < 3; ic++) T.M[ic][3] += BonePos[ic] - ScalarProd(T.M[ic], BonePos);
}
...
static void TMatMultiply(Model3D_Transform& Res, Model3D_Transform& A, Model3D_Transform& B) {
    ...
    for (int i = 0; i < 3; i++) {
        GLfloat Sum = 0;
        for (int k = 0; k < 3; k++) Sum += A.M[i][k] * B.M[k][3];
