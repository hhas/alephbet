...
char* GetVertIndxSet(char* Buffer, short& Presence, short& PosIndx, short& TCIndx, short& NormIndx);
...
bool LoadModel_Wavefront(FileSpecifier& Spec, Model3D& Model) {
    ...
    while (MoreLines) {
        ...
        if ((RestOfLine = CompareToKeyword("v")) != NULL) // Vertex position
        {
            ...
            for (int k = 0; k < 3; k++) Positions.push_back(Position[k]);
        } else if ((RestOfLine = CompareToKeyword("vt")) != NULL) // Vertex texture coordinate
        {
            ...
            for (int k = 0; k < 2; k++) TxtrCoords.push_back(TxtrCoord[k]);
        } else if ((RestOfLine = CompareToKeyword("vn")) != NULL) // Vertex normal
        {
            ...
            for (int k = 0; k < 3; k++) Normals.push_back(Normal[k]);
        }
        /*
    ...
    if (!(WhatsPresent & Present_Position))
        return false;
    ...
    for (unsigned k = 0; k < VertIndxSets.size() / 4; k++) {
        ...
        if (PosIndx < 0 || PosIndx >= int(Positions.size())) {
            logError("ERROR: Out of range vertex position: %u: %d (0,%lu) in %s", k, PosIndx,
                     (unsigned long)Positions.size() - 1, Path);
            AllInRange = false;
        }
        ...
        if (WhatsPresent & Present_TxtrCoord) {
            ...
            if (TCIndx < 0 || TCIndx >= int(TxtrCoords.size())) {
                logError("ERROR: Out of range vertex position: %u: %d (0,%lu) in %s", k, TCIndx,
                         (unsigned long)(TxtrCoords.size() - 1), Path);
                AllInRange = false;
            }
        } else
        ...
        if (WhatsPresent & Present_Normal) {
            ...
            if (NormIndx < 0 || NormIndx >= int(Normals.size())) {
                logError("ERROR: Out of range vertex position: %u: %d (0,%lu) in %s", k, NormIndx,
                         (unsigned long)(Normals.size() - 1), Path);
                AllInRange = false;
            }
        } else
    ...
    for (unsigned k = 0; k < VertIndxRefs.size(); k++) VertIndxRefs[k] = k;
    ...
    for (unsigned k = 0; k < VertIndxRefs.size(); k++) {
        ...
        // Load the positions
        {
            ...
            for (int m = 0; m < 3; m++) Model.Positions.push_back(*(PosPtr++));
        }

        // Load the texture coordinates
        if (WhatsPresent & Present_TxtrCoord) {
            GLfloat* TCPtr = &TxtrCoords[2 * TCIndx];
            for (int m = 0; m < 2; m++) Model.TxtrCoords.push_back(*(TCPtr++));
        }

        // Load the normals
        if (WhatsPresent & Present_Normal) {
            GLfloat* NormPtr = &Normals[3 * NormIndx];
            for (int m = 0; m < 3; m++) Model.Normals.push_back(*(NormPtr++));
        }
...
char* CompareToKeyword(const char* Keyword) {
    ...
    if (InputLine.size() < KWLen)
        return NULL;

    for (unsigned k = 0; k < KWLen; k++)
        if (InputLine[k] != Keyword[k])
            return NULL;
