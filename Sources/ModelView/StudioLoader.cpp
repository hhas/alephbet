...
bool LoadModel_Studio(FileSpecifier& Spec, Model3D& Model) {
    ...
    if (!ReadChunkHeader(OFile, ChunkHeader))
        return false;
    ...
    if (!ReadContainer(OFile, ChunkHeader, ReadMaster))
        return false;
...
bool SkipChunk(OpenedFile& OFile, ChunkHeaderData& ChunkHeader) {
    ...
    if (!OFile.SetPosition(Location + DataSize))
        return false;
    return true;
}

// Generic container-chunk reader
bool ReadContainer(OpenedFile& OFile, ChunkHeaderData& ChunkHeader, bool (*ContainerCallback)(OpenedFile&, int32)) {
    ...
    if (!ContainerCallback(OFile, ChunkEnd))
        return false;
...
static bool ReadMaster(OpenedFile& OFile, int32 ParentChunkEnd) {
    ...
    while (Location < ParentChunkEnd) {
        ...
        switch (ChunkHeader.ID) {
            case EDITOR:
                if (!ReadContainer(OFile, ChunkHeader, ReadEditor))
                    return false;
                break;

            default:
                if (!SkipChunk(OFile, ChunkHeader))
                    return false;
        }
...
static bool ReadEditor(OpenedFile& OFile, int32 ParentChunkEnd) {
    ...
    while (Location < ParentChunkEnd) {
        ...
        switch (ChunkHeader.ID) {
            case OBJECT:
                if (!ReadContainer(OFile, ChunkHeader, ReadObject))
                    return false;
                break;
...
static bool ReadObject(OpenedFile& OFile, int32 ParentChunkEnd) {
    ...
    while (Location < ParentChunkEnd) {
        ...
        switch (ChunkHeader.ID) {
            case TRIMESH:
                if (!ReadContainer(OFile, ChunkHeader, ReadTrimesh))
                    return false;
                break;
...
static bool ReadTrimesh(OpenedFile& OFile, int32 ParentChunkEnd) {
    ...
    while (Location < ParentChunkEnd) {
        ...
        switch (ChunkHeader.ID) {
            case VERTICES:
                if (!LoadChunk(OFile, ChunkHeader))
                    return false;
                LoadVertices();
                break;
            ...
            case FACE_DATA:
                if (!ReadContainer(OFile, ChunkHeader, ReadFaceData))
                    return false;
                break;
...
void LoadFloats(int NVals, uint8* Stream, GLfloat* Floats) {
    ...
    for (int k = 0; k < NVals; k++, FloatPtr++) {
        ...
        for (int c = 0; c < 4; c++) DestPtr[c] = SrcPtr[c];
    }
}
