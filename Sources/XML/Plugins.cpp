...
bool Plugin::compatible() const {
    ...
    for (std::vector<ScenarioInfo>::const_iterator it = required_scenarios.begin(); it != required_scenarios.end();
         ++it) {
        if ((it->name.empty() || it->name == scenName) && (it->scenario_id.empty() || it->scenario_id == scenID)
...
bool Plugin::valid() const {
    ...
    if (!environment_preferences->use_solo_lua && Plugins::instance()->mode() == Plugins::kMode_Solo)
...
void Plugins::load_shapes_patches(bool is_opengl) {
    ...
    for (std::vector<Plugin>::iterator it = m_plugins.begin(); it != m_plugins.end(); ++it) {
        if (it->valid()) {
            ...
            for (std::vector<ShapesPatch>::iterator shapes_patch = it->shapes_patches.begin();
                 shapes_patch != it->shapes_patches.end(); ++shapes_patch) {
...
static int utf8_to_int(const std::string& s) {
    ...
    if (mac_roman.size() == 4) {
        return FOUR_CHARS_TO_INT(mac_roman[0], mac_roman[1], mac_roman[2], mac_roman[3]);
    } else {
        return 0;
    }
}
    ...
    if (file_name.Open(file)) {
        ...
        if (file.Read(data_size, &file_data[0])) {
            ...
            try {
                ...
                if (root.read_attr("hud_lua", Data.hud_lua) && !plugin_file_exists(Data, Data.hud_lua))
                    Data.hud_lua = "";
...
void Plugins::enumerate() {
    ...
    for (std::vector<DirectorySpecifier>::const_iterator it = data_search_path.begin(); it != data_search_path.end();
         ++it) {
...
bool Plugins::get_resource(uint32_t type, int id, LoadedResource& rsrc) {
    for (auto it = m_plugins.rbegin(); it != m_plugins.rend(); ++it) {
        if (it->enabled && it->compatible() && it->get_resource(m_map_checksum, type, id, rsrc)) {
            return true;
        }
    }
...
void Plugins::set_map_checksum(uint32_t checksum) {
    ...
    while (!m_search_paths.empty()) { m_search_paths.pop(); }
...
void Plugins::validate() {
    ...
    for (std::vector<Plugin>::reverse_iterator rit = m_plugins.rbegin(); rit != m_plugins.rend(); ++rit) {
        ...
        if (!rit->enabled || !rit->compatible() || !rit->allowed() || (found_solo_lua && rit->solo_lua.size())
            || (found_hud_lua && rit->hud_lua.size()) || (found_stats_lua && rit->stats_lua.size())
    ...
    for (std::vector<Plugin>::reverse_iterator rit = m_plugins.rbegin(); rit != m_plugins.rend(); ++rit) {
        ...
        if (!rit->enabled || !rit->compatible() || !rit->allowed() || (rit->solo_lua.size())
            || (found_hud_lua && rit->hud_lua.size()) || (found_stats_lua && rit->stats_lua.size())
            || (found_theme && rit->theme.size())) {
            rit->overridden = true;
            continue;
