            ...
            case LevelScriptCommand::LoadScreen: {
                if (Cmd.FileSpec.size() > 0) {
                    if (Cmd.L || Cmd.T || Cmd.R || Cmd.B) {
                        OGL_LoadScreen::instance()->Set(Cmd.FileSpec.c_str(), Cmd.Stretch, Cmd.Scale, Cmd.L, Cmd.T,
                                                        Cmd.R - Cmd.L, Cmd.B - Cmd.T);
...
void parse_mml_default_levels(const InfoTree& root) {
    ...
    for (const InfoTree& child : root.children_named("random_order")) { child.read_attr("on", ls_ptr->RandomOrder); }
...
void parse_levels_xml(InfoTree root) {
    ...
    for (const InfoTree& child : root.children_named("end")) { parse_level_commands(child, LevelScriptHeader::End); }
