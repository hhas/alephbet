...
void QuickSaveImageCache::clear() {
    m_images.clear();
    for (cache_iter_t it = m_used.begin(); it != m_used.end(); ++it) { SDL_FreeSurface(it->second); }
    m_used.clear();
}
...
class w_saves : public w_list_base {
  public:
    ...
    void update_selected(QuickSave& save) {
        m_saves[get_selection()] = save;
        dirty                    = true;
    }
...
void w_saves::click(int x, int y) {
    if (x == 0 && y == 0 && active) {
        ...
        if (num_items > 0)
            item_selected();
    } else if (x >= trough_rect.x && x < trough_rect.x + trough_rect.w && y >= thumb_y && y <= thumb_y + thumb_height) {
...
void w_saves::item_selected() { get_owning_dialog()->quit(0); }
...
static void dialog_export(void* arg) {
    ...
    dstFile += "unused.sgaA";
    char prompt[256];
    if (dstFile.WriteDialog(_typecode_savegame, getcstr(prompt, strPROMPTS, _save_replay_prompt),
                            utf8_to_mac_roman(name).c_str())) {
...
static bool build_map_preview(std::ostringstream& ostream) {
    ...
    SDL_Surface* surface = SDL_CreateRGBSurface(SDL_SWSURFACE, r.w, r.h, 32, 0xff'0000, 0x00'ff00, 0x00'00ff, 0);
    if (!surface)
        return false;
...
void create_updated_save(QuickSave& save) {
    ...
    if (save.save_file.Open(currentFile)) {
        if (read_wad_header(currentFile, &header)) {
            ...
            if (game_wad)
                game_wad_length = calculate_wad_length(&header, game_wad);
        ...
        if (open_wad_file_for_writing(TempFile, SaveFile)) {
            if (write_wad_header(SaveFile, &header)) {
                ...
                if (write_wad(SaveFile, &header, game_wad, offset)) {
                    ...
                    if (new_meta_wad) {
                        set_indexed_directory_offset_and_length(&header, entries, 1, offset, meta_wad_length,
...
bool create_quick_save(void) {
    ...
    if (save.ticks < 60 * TICKS_PER_MINUTE)
        sprintf(fmt_ticks, "%d:%02d", save.ticks / TICKS_PER_MINUTE, (save.ticks / TICKS_PER_SECOND) % 60);
    else
        sprintf(fmt_ticks, "%d:%02d:%02d", save.ticks / (60 * TICKS_PER_MINUTE), (save.ticks / TICKS_PER_MINUTE) % 60,
    ...
    if (file_name.Open(file)) {
        if (read_wad_header(file, &header)) {
            ...
            if (wad) {
                ...
                } catch (const InfoTree::ini_error& e) { return false; }
...
void QuickSaves::clear() { m_saves.clear(); }

bool most_recent_dir_entry(const dir_entry& a, const dir_entry& b) { return a.date > b.date; }
