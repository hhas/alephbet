  ...
  public:
    ...
    explicit InfoTreeFileStream(FileSpecifier path, bool write = false) {
        if (!(write ? path.OpenForWritingText(f) : path.Open(f))) {
            ...
            const auto msg
                    = std::string("couldn't open '") + path.GetPath() + "' for " + mode + " (error " + code + ")";
...
static void write_indented_xml(std::ostream& dest, const pt::iptree& src) {
    using settings_t = pt::xml_writer_settings<std::conditional<BOOST_VERSION >= 105'600, std::string, char>::type>;
...
void InfoTree::save_xml(std::ostringstream& stream) const { write_indented_xml(stream, *this); }
...
void InfoTree::save_ini(std::ostringstream& stream) const { pt::write_ini<pt::iptree>(stream, *this); }
...
void InfoTree::put_cstr(std::string key, std::string cstr) { put(key, mac_roman_to_utf8(cstr)); }

void InfoTree::put_attr_cstr(std::string key, std::string cstr) { put_attr(key, mac_roman_to_utf8(cstr)); }

static const float ColorToTree = 1 / static_cast<float>(65'535);
static const float TreeToColor = static_cast<float>(65'535);
...
template <typename T>
bool _get_color(const InfoTree* tree, T& color) {
...
template <typename T>
InfoTree _make_color(const T& color) {
    InfoTree ctree;
...
template <typename T>
InfoTree _make_color(const T& color, size_t index) {
    InfoTree ctree;
...
bool InfoTree::read_color(RGBColor& color) const { return _get_color(this, color); }

bool InfoTree::read_color(rgb_color& color) const { return _get_color(this, color); }

void InfoTree::add_color(std::string path, const RGBColor& color) { add_child(path, _make_color(color)); }

void InfoTree::add_color(std::string path, const RGBColor& color, size_t index) {
    add_child(path, _make_color(color, index));
}

void InfoTree::add_color(std::string path, const rgb_color& color) { add_child(path, _make_color(color)); }
