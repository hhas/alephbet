...
class basic_bstream {
  public:
    ...
    std::streambuf* rdbuf(std::streambuf* new_sb) {
        std::streambuf* old = sb_;
        sb_                 = new_sb;
        return old;
    }
...
class BIStreamBE : public BIStream {
  public:
    ...
    BIStream& operator>>(uint8& value) { return BIStream::operator>>(value); }

    BIStream& operator>>(int8& value) { return BIStream::operator>>(value); }
...
class BOStreamBE : public BOStream {
  public:
    ...
    BOStream& operator<<(uint8 value) { return BOStream::operator<<(value); }

    BOStream& operator<<(int8 value) { return BOStream::operator<<(value); }
