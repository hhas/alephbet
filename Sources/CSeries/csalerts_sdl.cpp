...
bool system_alert_choose_scenario(char* chosen_dir) {
    ...
    bi.ulFlags        = BIF_RETURNONLYFSDIRS | BIF_NEWDIALOGSTYLE | 0x0000'0200; // no "New Folder" button
...
void alert_user(const char* message, short severity) {
    if (!MainScreenVisible()) {
        SDL_ShowSimpleMessageBox(severity == infoError ? SDL_MESSAGEBOX_WARNING : SDL_MESSAGEBOX_ERROR,
                                 severity == infoError ? "Warning" : "Error", message, NULL);
    } else {
        dialog d;
...
bool alert_choose_scenario(char* chosen_dir) { return system_alert_choose_scenario(chosen_dir); }
