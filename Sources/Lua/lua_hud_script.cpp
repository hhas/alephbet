...
class LuaHUDState {
  public:
    ...
    LuaHUDState() : running_(false), inited_(false), num_scripts_(0) { state_.reset(luaL_newstate(), lua_close); }
  ...
  public:
    ...
    void SetSearchPath(const std::string& path) { L_Set_Search_Path(State(), path); }
...
void LuaHUDState::RegisterFunctions() { Lua_HUDObjects_register(State()); }
...
bool LuaHUDState::Run() {
    ...
    // Call 'em
    for (int i = 0; i < num_scripts_; ++i) result = result || lua_pcall(State(), 0, LUA_MULTRET, 0);
...
bool LuaHUDRunning() { return (hud_state && hud_state->Running()); }
...
void SetLuaHUDScriptSearchPath(const std::string& directory) { hud_state->SetSearchPath(directory); }
