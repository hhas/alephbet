...
int Lua_Camera_Path_Points_New(lua_State* L) {
    ...
    world_point3d point = {static_cast<world_distance>(lua_tonumber(L, 2) * WORLD_ONE),
...
static int Lua_Camera_Valid(int16 index) { return index >= 0 && index < lua_cameras.size(); }
...
static int16 Lua_Cameras_Length() { return lua_cameras.size(); }
...
static int Lua_Player_Items_Set(lua_State* L) {
    ...
    if (new_item_count < item_count) {
        if (definition->item_kind == _ball) {
            ...
            if (accounting) {
                for (int i = 0; i < real_difference; ++i) object_was_just_destroyed(_object_is_item, item_type);
            }
        }
...
static int Lua_ExternalVelocity_Get_I(lua_State* L) {
    lua_pushnumber(L, (double)get_player_data(Lua_ExternalVelocity::Index(L, 1))->variables.external_velocity.i
                              / WORLD_ONE);
    return 1;
}

static int Lua_ExternalVelocity_Get_J(lua_State* L) {
    lua_pushnumber(L, (double)get_player_data(Lua_ExternalVelocity::Index(L, 1))->variables.external_velocity.j
                              / WORLD_ONE);
    return 1;
}

static int Lua_ExternalVelocity_Get_K(lua_State* L) {
    lua_pushnumber(L, (double)get_player_data(Lua_ExternalVelocity::Index(L, 1))->variables.external_velocity.k
                              / WORLD_ONE);
    return 1;
}
...
void LuaTexturePaletteClear() { lua_texture_palette.clear(); }

int LuaTexturePaletteSize() { return lua_texture_palette.size(); }
...
int LuaTexturePaletteSelected() { return lua_texture_palette_selected; }
...
static int Lua_Texture_Palette_Slot_Set_Collection(lua_State* L) {
    ...
    lua_texture_palette[index].shape
            = BUILD_DESCRIPTOR(collection_index, GET_DESCRIPTOR_SHAPE(lua_texture_palette[index].shape));
    return 0;
}
...
static int Lua_Texture_Palette_Slot_Set_Texture(lua_State* L) {
    ...
    lua_texture_palette[index].shape
            = BUILD_DESCRIPTOR(GET_DESCRIPTOR_COLLECTION(lua_texture_palette[index].shape), shape_index);
    return 0;
}
...
Lua_Player_Weapon_Trigger* Lua_Player_Weapon_Trigger::Push(lua_State* L, int16 player_index, int16 weapon_index,
                                                           int16 index) {
...
int16 Lua_Player_Weapon_Trigger::WeaponIndex(lua_State* L, int index) {
    ...
    if (!t)
        luaL_typerror(L, index, Lua_Player_Weapon_Trigger_Name);
    return t->m_weapon_index;
}

static int Lua_Player_Weapon_Trigger_Get_Rounds(lua_State* L) {
    short rounds = get_player_weapon_ammo_count(Lua_Player_Weapon_Trigger::PlayerIndex(L, 1),
...
int Lua_Player_Accelerate(lua_State* L) {
    ...
    accelerate_player(player->monster_index, static_cast<int>(vertical_velocity * WORLD_ONE),
                      static_cast<int>(direction / AngleConvert), static_cast<int>(velocity * WORLD_ONE));
    return 0;
}
...
int Lua_Player_Find_Action_Key_Target(lua_State* L) {
    ...
    short object_index = find_action_key_target(Lua_Player::Index(L, 1), MAXIMUM_ACTIVATION_RANGE, &target_type,
                                                film_profile.find_action_key_target_has_side_effects);
...
int Lua_Player_Find_Target(lua_State* L) {
    ...
    uint16 flags = translate_projectile(0, &origin, old_polygon, &destination, &new_polygon, player->monster_index,
    ...
    while (!(flags & _projectile_hit)) {
        ...
        flags = translate_projectile(0, &origin, old_polygon, &destination, &new_polygon, player->monster_index,
                                     &obstruction_index, &line_index, true, NONE);
    }
...
extern void instantiate_physics_variables(struct physics_constants* constants, struct physics_variables* variables,
                                          short player_index, bool first_time, bool take_action);
...
int Lua_Player_Position(lua_State* L) {
    ...
    instantiate_physics_variables(get_physics_constants_for_model(static_world->physics_model, 0), &player->variables,
...
bool Lua_Player_Valid(int16 index) { return index >= 0 && index < dynamic_world->player_count; }
...
int16 Lua_Players_Length() { return dynamic_world->player_count; }
...
static int Lua_Game_Set_Over(lua_State* L) {
    if (lua_isnil(L, 2))
        ...
        game_end_condition = lua_toboolean(L, 2) ? _game_end_now_condition : _game_no_end_condition;
    return 0;
}
...
static const char* compatibility_script
        = ""
          "function accelerate_player(player, vertical_velocity, direction, velocity) "
          "Players[player]:accelerate(direction, velocity, vertical_velocity) end\n"
          ...
          "function add_item(player, item_type) Players[player].items[item_type] = Players[player].items[item_type] + "
          "1 end\n"
          "function add_path_angle(camera, yaw, pitch, time) Cameras[camera].path_angles:new(yaw, pitch, time) end\n"
          "function add_path_point(camera, polygon, x, y, z, time) Cameras[camera].path_points:new(x, y, z, polygon, "
          "time) end\n"
          "function award_kills(player, slain_player, amount) if player == -1 then Players[slain_player].deaths = "
          "Players[slain_player].deaths + amount else Players[player].kills[slain_player] = "
          "Players[player].kills[slain_player] + amount end end\n"
          "function add_to_player_external_velocity(player, x, y, z) Players[player].external_velocity.i = "
          "Players[player].external_velocity.i + x Players[player].external_velocity.j = "
          "Players[player].external_velocity.j + y Players[player].external_velocity.k = "
          "Players[player].external_velocity.k + z end\n"
          ...
          "function destroy_ball(player) for i in ItemTypes() do if i.ball then Players[player].items[i] = 0 end end "
          "end\n"
          "function fade_music(duration) if duration then Music.fade(duration * 60 / 1000) else Music.fade(60 / 1000) "
          "end end\n"
          "function get_game_difficulty() return Game.difficulty.index end\n"
          "function get_game_type() return Game.type.index end\n"
          "function get_kills(player, slain_player) if player == -1 then return Players[slain_player].deaths else "
          "return Players[player].kills[slain_player] end end\n"
          ...
          "function get_player_external_velocity(player) return Players[player].external_velocity.i * 1024, "
          "Players[player].external_velocity.j * 1024, Players[player].external_velocity.k * 1024 end\n"
          "function get_player_internal_velocity(player) return Players[player].internal_velocity.forward * 65536, "
          "Players[player].internal_velocity.perpendicular * 65536 end\n"
          ...
          "function get_player_powerup_duration(player, powerup) if powerup == _powerup_invisibility then return "
          "Players[player].invisibility_duration elseif powerup == _powerup_invincibility then return "
          "Players[player].invincibility_duration elseif powerup == _powerup_infravision then return "
          "Players[player].infravision_duration elseif powerup == _powerup_extravision then return "
          "Players[player].extravision_duration end end\n"
          "function get_player_team(player) return Players[player].team.index end\n"
          "function get_player_weapon(player) if Players[player].weapons.current then return "
          "Players[player].weapons.current.index else return nil end end\n"
          "function get_points(player) return Players[player].points end\n"
          "function global_random() return Game.global_random() end\n"
          "function inflict_damage(player, amount, type) if (type) then Players[player]:damage(amount, type) else "
          "Players[player]:damage(amount) end end\n"
          ...
          "function player_media(player) if Players[player].head_below_media then return "
          "Players[player].polygon.media.index else return nil end end\n"
          ...
          "function remove_item(player, item_type) if Players[player].items[item_type] > 0 then "
          "Players[player].items[item_type] = Players[player].items[item_type] - 1 end end\n"
          "function screen_fade(player, fade) if fade then Players[player]:fade_screen(fade) else for p in Players() "
          "do p:fade_screen(player) end end end\n"
          "function screen_print(player, message) if message then if Players[player] then "
          "Players[player]:print(message) end else Players.print(player) end end\n"
          ...
          "function set_kills(player, slain_player, amount) if player == -1 then Players[slain_player].deaths = amount "
          "else Players[player].kills[slain_player] = amount end end\n"
          "function set_life(player, shield) Players[player].energy = shield end\n"
          "function set_lua_compass_beacon(player, x, y) Players[player].compass.x = x Players[player].compass.y = y "
          "end\n"
          "function set_lua_compass_state(player, state) if state > 15 then Players[player].compass.beacon = true "
          "state = state % 16 else Players[player].compass.beacon = false end if state > 7 then "
          "Players[player].compass.se = true state = state - 8 else Players[player].compass.se = false end if state > "
          "3 then Players[player].compass.sw = true state = state - 4 else Players[player].compass.sw = false end if "
          "state > 1 then Players[player].compass.ne = true state = state - 2 else Players[player].compass.ne = false "
          "end if state == 1 then Players[player].compass.nw = true else Players[player].compass.nw = false end end\n"
          "function set_motion_sensor_state(player, state) Players[player].motion_sensor_active = state end\n"
          "function set_overlay_color(overlay, color) for p in Players() do if p.local_ then p.overlays[overlay].color "
          "= color end end end\n"
          "function set_overlay_icon(overlay, icon) for p in Players() do if p.local_ then p.overlays[overlay].icon = "
          "icon end end end\n"
          "function set_overlay_icon_by_color(overlay, color) for p in Players() do if p.local_ then "
          "p.overlays[overlay]:fill_icon(color) end end end\n"
          "function set_overlay_text(overlay, text) for p in Players() do if p.local_ then p.overlays[overlay].text = "
          "text end end end\n"
          "function set_oxygen(player, oxygen) Players[player].oxygen = oxygen end\n"
          "function set_player_angle(player, yaw, pitch) Players[player].yaw = yaw Players[player].pitch = pitch + "
          "360.0 end\n"
          "function set_player_color(player, color) Players[player].color = color end\n"
          "function set_player_external_velocity(player, x, y, z) Players[player].external_velocity.i = x / 1024 "
          "Players[player].external_velocity.j = y / 1024 Players[player].external_velocity.k = z / 1024 end\n"
          "function set_player_position(player, x, y, z, polygon) Players[player]:position(x, y, z, polygon) end\n"
          "function set_player_powerup_duration(player, powerup, duration) if powerup == _powerup_invisibility then "
          "Players[player].invisibility_duration = duration elseif powerup == _powerup_invincibility then "
          "Players[player].invincibility_duration = duration elseif powerup == _powerup_infravision then "
          "Players[player].infravision_duration = duration elseif powerup == _powerup_extravision then "
          "Players[player].extravision_duration = duration end end\n"
          ...
          "function use_lua_compass(player, state) if state ~= nil then Players[player].compass.lua = state else for p "
          "in Players() do p.compass.lua = player end end end\n"
