...
#include "interpolated_world.hpp"
#include "items.hpp"
...
int GetLuaScoringMode() { return game_scoring_mode; }

bool mute_lua = false;

// Steal all this stuff
extern void ShootForTargetPoint(bool ThroughWalls, world_point3d& StartPosition, world_point3d& EndPosition,
                                short& Polygon);
...
class LuaState {
  ...
  public:
    ...
    LuaState() : running_(false), num_scripts_(0) { state_.reset(luaL_newstate(), lua_close); }
  ...
  public:
    ...
    virtual void SetSearchPath(const std::string& path) { L_Set_Search_Path(State(), path); }
  ...
  public:
    ...
    void MonsterDamaged(short monster_index, short aggressor_monster_index, int16 damage_type, short damage_amount,
                        short projectile_index);
    void PlayerDamaged(short player_index, short aggressor_player_index, short aggressor_monster_index,
                       int16 damage_type, short damage_amount, short projectile_index);
    void ProjectileDetonated(short type, short owner_index, short polygon, world_point3d location, uint16_t flags,
...
void LuaState::MonsterDamaged(short monster_index, short aggressor_monster_index, int16 damage_type,
                              short damage_amount, short projectile_index) {
...
void LuaState::PlayerDamaged(short player_index, short aggressor_player_index, short aggressor_monster_index,
                             int16 damage_type, short damage_amount, short projectile_index) {
...
void LuaState::ProjectileDetonated(short type, short owner_index, short polygon, world_point3d location, uint16_t flags,
                                   int16_t obstruction_index, int16_t line_index) {
...
void L_Set_Nonlocal_Overlays(lua_State* L, bool value) { SetScriptHUDNonlocal(value); }

bool L_Get_Nonlocal_Overlays(lua_State* L) { return IsScriptHUDNonlocal(); }
...
static const char* compatibility_triggers
        = ""
          ...
          "Triggers.start_refuel = function(class, player, side) if start_refuel then start_refuel(class.index, "
          "player.index) end end\n"
          "Triggers.end_refuel = function(class, player, side) if end_refuel then end_refuel(class.index, "
          "player.index) end end\n"
          "Triggers.tag_switch = function(tag, player) if tag_switch then tag_switch(tag.index, player.index) end end\n"
          "Triggers.light_switch = function(light, player) if light_switch then light_switch(light.index, "
          "player.index) end end\n"
          "Triggers.platform_switch = function(platform, player) if platform_switch then "
          "platform_switch(platform.index, player.index) end end\n"
          "Triggers.terminal_enter = function(terminal, player) if terminal_enter then terminal_enter(terminal.index, "
          "player.index) end end\n"
          "Triggers.terminal_exit = function(terminal, player) if terminal_exit then terminal_exit(terminal.index, "
          "player.index) end end\n"
          "Triggers.pattern_buffer = function(side, player) if pattern_buffer then "
          "pattern_buffer(side.control_panel.permutation, player.index) end end\n"
          ...
          "Triggers.platform_activated = function(polygon) if platform_activated then "
          "platform_activated(polygon.index) end end\n"
          "Triggers.player_revived = function(player) if player_revived then player_revived(player.index) end end\n"
          "Triggers.player_killed = function(player, aggressor, action, projectile) if player_killed then if aggressor "
          "then aggressor_index = aggressor.index else aggressor_index = -1 end if projectile then projectile_index = "
          "projectile.index else projectile_index = -1 end player_killed(player.index, aggressor_index, action.index, "
          "projectile_index) end end\n"
          "Triggers.monster_killed = function(monster, aggressor, projectile) if monster_killed then if aggressor then "
          "aggressor_index = aggressor.index else aggressor_index = -1 end if projectile then projectile_index = "
          "projectile.index else projectile_index = -1 end monster_killed(monster.index, aggressor_index, "
          "projectile_index) end end\n"
          "Triggers.player_damaged = function(player, aggressor_player, aggressor_monster, type, amount, projectile) "
          "if player_damaged then if aggressor_player then aggressor_player_index = aggressor_player.index else "
          "aggressor_player_index = -1 end if aggressor_monster then aggressor_monster_index = aggressor_monster.index "
          "else aggressor_monster_index = -1 end if projectile then projectile_index = projectile.index else "
          "projectile_index = -1 end player_damaged(player.index, aggressor_player_index, aggressor_monster_index, "
          "type.index, amount, projectile_index) end end\n"
          "Triggers.projectile_detonated = function(type, owner, polygon, x, y, z) if projectile_detonated then if "
          "owner then owner_index = owner.index else owner_index = -1 end projectile_detonated(type.index, "
          "owner_index, polygon.index, x, y, z) end end\n"
...
bool LuaState::Run() {
    ...
    for (int i = 0; i < num_scripts_ - 1; ++i) lua_insert(State(), -(num_scripts_ - i));
...
void L_Error(const char* inMessage) {
    if (!mute_lua)
...
void L_Dispatch(const UnaryFunction& f) {
    for (state_map::iterator it = states.begin(); it != states.end(); ++it) { f(it->second); }
}
...
void L_Call_Init(bool fRestoringSaved) {
    if (LuaRunning()) {
        ...
        lua_random_generator.jcong
                = (static_cast<uint32>(global_random()) << 16) + static_cast<uint32>(global_random());
...
void L_Call_Cleanup() { L_Dispatch(std::bind(&LuaState::Cleanup, std::placeholders::_1)); }
...
void L_Call_PostIdle() { L_Dispatch(std::bind(&LuaState::PostIdle, std::placeholders::_1)); }
...
void L_Call_Player_Killed(short player_index, short aggressor_player_index, short action, short projectile_index) {
    L_Dispatch(std::bind(&LuaState::PlayerKilled, std::placeholders::_1, player_index, aggressor_player_index, action,
                         projectile_index));
}

void L_Call_Monster_Killed(short monster_index, short aggressor_player_index, short projectile_index) {
    L_Dispatch(std::bind(&LuaState::MonsterKilled, std::placeholders::_1, monster_index, aggressor_player_index,
                         projectile_index));
}

void L_Call_Monster_Damaged(short monster_index, short aggressor_monster_index, int16 damage_type, short damage_amount,
                            short projectile_index) {
    L_Dispatch(std::bind(&LuaState::MonsterDamaged, std::placeholders::_1, monster_index, aggressor_monster_index,
                         damage_type, damage_amount, projectile_index));
}

void L_Call_Player_Damaged(short player_index, short aggressor_player_index, short aggressor_monster_index,
                           int16 damage_type, short damage_amount, short projectile_index) {
    L_Dispatch(std::bind(&LuaState::PlayerDamaged, std::placeholders::_1, player_index, aggressor_player_index,
                         aggressor_monster_index, damage_type, damage_amount, projectile_index));
}

void L_Call_Projectile_Detonated(short type, short owner_index, short polygon, world_point3d location, uint16_t flags,
                                 int16_t obstruction_index, int16_t line_index) {
    L_Dispatch(std::bind(&LuaState::ProjectileDetonated, std::placeholders::_1, type, owner_index, polygon, location,
                         flags, obstruction_index, line_index));
}
...
int L_Player_Control(lua_State* L) {
    ...
    *variablesptr                       = variables;
    ...
    if (DoAction) {
        for (int i = 1; i < value; i++) action_flags[i] = action_flags[0];
...
#ifdef HAVE_OPENGL
    for (int i = 0; i < OGL_NUMBER_OF_FOG_TYPES; i++) { PreLuaFogState[i] = *OGL_GetFogData(i); }
#endif
...
#ifdef HAVE_OPENGL
    for (int i = 0; i < OGL_NUMBER_OF_FOG_TYPES; i++) { *OGL_GetFogData(i) = PreLuaFogState[i]; }
#endif
...
bool RunLuaScript() {
    ...
    lua_random_local_generator.jcong
            = (static_cast<uint32>(local_random()) << 16) + static_cast<uint32>(local_random());

    bool running = false;
    for (state_map::iterator it = states.begin(); it != states.end(); ++it) { running |= it->second->Run(); }
...
void ResetPassedLua() { PassedLuaState.clear(); }
...
void ResetLuaMute() { mute_lua = false; }
...
void UpdateLuaCameras() {
    ...
    for (std::vector<lua_camera>::iterator it = lua_cameras.begin(); it != lua_cameras.end(); ++it) {
        ...
        if (point_index >= 0
            && it->time_elapsed - it->path.last_point_time >= it->path.path_points[point_index].delta_time) {
        ...
        if (angle_index >= 0
            && it->time_elapsed - it->path.last_angle_time >= it->path.path_angles[angle_index].delta_time) {
    ...
    for (std::vector<lua_camera>::iterator it = lua_cameras.begin(); it != lua_cameras.end(); ++it) {
        ...
        if (angle_index >= 0 && angle_index < static_cast<short>(it->path.path_angles.size())) {
            if (static_cast<size_t>(angle_index) == it->path.path_angles.size() - 1) {
                ...
                world_view->yaw   = normalize_angle(static_cast<short>(FindLinearValue(
                        it->path.path_angles[angle_index].yaw, it->path.path_angles[angle_index + 1].yaw,
                        it->path.path_angles[angle_index].delta_time, it->time_elapsed - it->path.last_angle_time)));
                world_view->pitch = normalize_angle(static_cast<short>(FindLinearValue(
                        it->path.path_angles[angle_index].pitch, it->path.path_angles[angle_index + 1].pitch,
                        it->path.path_angles[angle_index].delta_time, it->time_elapsed - it->path.last_angle_time)));
            }
        }
        ...
        if (point_index >= 0 && point_index < static_cast<short>(it->path.path_points.size())) {
            if (static_cast<size_t>(point_index) == it->path.path_points.size() - 1) {
                ...
                world_view->origin     = FindLinearValue(
                        it->path.path_points[point_index].point, it->path.path_points[point_index + 1].point,
                        it->path.path_points[point_index].delta_time, it->time_elapsed - it->path.last_point_time);
...
bool LuaPlayerCanWieldWeapons(short player_index) { return (can_wield_weapons[player_index] || !LuaRunning()); }

int GetLuaGameEndCondition() { return game_end_condition; }
