...
static inline bool powerOfTwo(int32 x) { return !((x - 1) & x); }

char Lua_MonsterClass_Name[] = "monster_class";
typedef L_Enum<Lua_MonsterClass_Name, int32> Lua_MonsterClass;

bool Lua_MonsterClass_Valid(int32 index) { return index >= 1 && index <= _class_yeti && powerOfTwo(index); }
...
static bool Lua_MonsterType_Valid(int16 index) { return index >= 0 && index < NUMBER_OF_MONSTER_TYPES; }
...
static int Lua_MonsterType_Set_Random_Chance(lua_State* L) {
    if (lua_isnumber(L, 2)) {
        monster_placement_info[Lua_MonsterType::Index(L, 1)].random_chance
                = static_cast<uint16>(lua_tonumber(L, 2) * UINT16_MAX + 0.5);
    } else {
...
int Lua_Monster_Accelerate(lua_State* L) {
    ...
    accelerate_monster(monster_index, static_cast<int>(vertical_velocity * WORLD_ONE),
...
extern int32 monster_pathfinding_cost_function(short source_polygon_index, short line_index,
...
int Lua_Monster_Move_By_Path(lua_State* L) {
    ...
    monster->path = new_path((world_point2d*)&object->location, object->polygon, &destination, polygon_index,
                             3 * definition->radius, monster_pathfinding_cost_function, &path);
    if (monster->path == NONE) {
...
static int Lua_Monster_Set_Visible(lua_State* L) {
    ...
    if (monster->action == _monster_is_teleporting_out)
        return 0;
...
static const char* compatibility_script
        = ""
          ...
          "function damage_monster(monster, damage, type) if type then Monsters[monster]:damage(damage, type) else "
          "Monsters[monster]:damage(damage) end end\n"
          "function deactivate_monster(monster) Monsters[monster].active = false end\n"
          "function get_monster_action(monster) if Monsters[monster].action then return Monsters[monster].action.index "
          "else return -1 end end\n"
          "function get_monster_enemy(monster_type, enemy_type) return MonsterTypes[monster_type].enemies[enemy_type] "
          "end\n"
          "function get_monster_friend(monster_type, friend_type) return "
          "MonsterTypes[monster_type].friends[friend_type] end\n"
          "function get_monster_facing(monster) return Monsters[monster].facing * 512 / 360 end\n"
          "function get_monster_immunity(monster, damage_type) return MonsterTypes[monster].immunities[damage_type] "
          "end\n"
          "function get_monster_item(monster) if MonsterTypes[monster].item then return "
          "MonsterTypes[monster].item.index else return -1 end end\n"
          "function get_monster_mode(monster) if Monsters[monster].mode then return Monsters[monster].mode.index else "
          "return -1 end end\n"
          "function get_monster_polygon(monster) return Monsters[monster].polygon.index end\n"
          "function get_monster_position(monster) return Monsters[monster].x * 1024, Monsters[monster].y * 1024, "
          "Monsters[monster].z * 1024 end\n"
          ...
          "function new_monster(type, poly, facing, height, x, y) if (x and y) then m = Monsters.new(x, y, height / "
          "1024, poly, type) elseif (height) then m = Monsters.new(Polygons[poly].x, Polygons[poly].y, height / 1024, "
          "poly, type) else m = Monsters.new(Polygons[poly].x, Polygons[poly].y, 0, poly, type) end if m then return "
          "m.index else return -1 end end\n"
          "function set_monster_enemy(monster_type, enemy_type, hostile) "
          "MonsterTypes[monster_type].enemies[enemy_type] = hostile end\n"
          "function set_monster_friend(monster_type, friend_type, friendly) "
          "MonsterTypes[monster_type].friends[friend_type] = friendly end\n"
          "function set_monster_immunity(monster, damage, immune) MonsterTypes[monster].immunities[damage] = immune "
          "end\n"
          "function set_monster_item(monster, item) if item == -1 then MonsterTypes[monster].item = nil else "
          "MonsterTypes[monster].item = item end end\n"
