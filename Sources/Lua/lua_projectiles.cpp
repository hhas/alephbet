...
static bool Lua_ProjectileType_Valid(int32 index) { return (index >= 0 && index < NUMBER_OF_PROJECTILE_TYPES); }
...
const char* compatibility_script
        = ""
          "function get_projectile_angle(index) local elevation = Projectiles[index].elevation if elevation < 0.0 then "
          "elevation = elevation + 360 end return Projectiles[index].facing, elevation end\n"
          "function get_projectile_damage_type(index) return Projectiles[index].type.damage.type.index end\n"
          "function get_projectile_owner(index) if Projectiles[index].owner then return Projectiles[index].owner.index "
          "end end\n"
          "function get_projectile_position(index) return Projectiles[index].polygon.index, Projectiles[index].x, "
          "Projectiles[index].y, Projectiles[index].z end\n"
          "function get_projectile_target(index) if Projectiles[index].target then return "
          "Projectiles[index].target.index end end\n"
          ...
          "function set_projectile_angle(index, yaw, pitch) Projectiles[index].facing = yaw "
          "Projectiles[index].elevation = pitch end\n"
          "function set_projectile_owner(index, owner) Projectiles[index].owner = owner end\n"
          "function set_projectile_position(index, polygon, x, y, z) Projectiles[index]:position(x, y, z, polygon) "
          "end\n"
