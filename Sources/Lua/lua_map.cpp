...
#include "interface.hpp" // get_game_state
#include "lightsource.hpp"
#include "lua_map.hpp"
#include "lua_monsters.hpp"
#include "lua_objects.hpp"
#include "lua_player.hpp"
#include "lua_templates.hpp"
#include "map.hpp"
#include "media.hpp"
#include "network.hpp" // game_info
...
static bool Lua_Endpoint_Valid(int16 index) { return index >= 0 && index < EndpointList.size(); }
...
static bool Lua_Line_Valid(int16 index) { return index >= 0 && index < LineList.size(); }
...
bool Lua_Platform_Valid(int16 index) { return index >= 0 && index < dynamic_world->platform_count; }

template <int flag_bit>
int Lua_Platform_Get_Dynamic_Flag(lua_State* L) {
...
template <int flag_bit>
int Lua_Platform_Set_Dynamic_Flag(lua_State* L) {
    if (!lua_isboolean(L, 2))
...
template <int flag_bit>
int Lua_Platform_Get_Static_Flag(lua_State* L) {
    platform_data* platform = get_platform_data(Lua_Platform::Index(L, 1));
...
template <int flag_bit>
int Lua_Platform_Set_Static_Flag(lua_State* L) {
    if (!lua_isboolean(L, 2))
        return luaL_error(L, "platform: incorrect argument type");
...
static int Lua_Platform_Set_Tag(lua_State* L) {
    ...
    if (lua_isnumber(L, 2)) {
        ...
        return luaL_error(L, "tag: incorrect argument type");
...
const luaL_Reg Lua_Platform_Get[] = {
        {"activates_adjacent_platforms_at_each_level",
         Lua_Platform_Get_Static_Flag<_platform_activates_adjacent_platforms_at_each_level>                                          },
        {"activates_adjacent_platforms_when_activating",
         Lua_Platform_Get_Static_Flag<_platform_activates_adjacent_platforms_when_activating>                                        },
        {"activates_adjacent_platforms_when_deactivating",
         Lua_Platform_Get_Static_Flag<_platform_activates_adjacent_platforms_when_deactivating>                                      },
        ...
        {"deactivates_adjacent_platforms_when_activating",
         Lua_Platform_Get_Static_Flag<_platform_deactivates_adjacent_platforms_when_activating>                                      },
        {"deactivates_adjacent_platforms_when_deactivating",
         Lua_Platform_Get_Static_Flag<_platform_deactivates_adjacent_platforms_when_deactivating>                                    },
        ...
        {"reverses_direction_when_obstructed",
         Lua_Platform_Get_Static_Flag<_platform_reverses_direction_when_obstructed>                                                  },
...
const luaL_Reg Lua_Platform_Set[] = {
        {"activates_adjacent_platforms_at_each_level",
         Lua_Platform_Set_Static_Flag<_platform_activates_adjacent_platforms_at_each_level>                                          },
        {"activates_adjacent_platforms_when_activating",
         Lua_Platform_Set_Static_Flag<_platform_activates_adjacent_platforms_when_activating>                                        },
        {"activates_adjacent_platforms_when_deactivating",
         Lua_Platform_Set_Static_Flag<_platform_activates_adjacent_platforms_when_deactivating>                                      },
        ...
        {"deactivates_adjacent_platforms_when_activating",
         Lua_Platform_Set_Static_Flag<_platform_deactivates_adjacent_platforms_when_activating>                                      },
        {"deactivates_adjacent_platforms_when_deactivating",
         Lua_Platform_Set_Static_Flag<_platform_deactivates_adjacent_platforms_when_deactivating>                                    },
        ...
        {"reverses_direction_when_obstructed",
         Lua_Platform_Set_Static_Flag<_platform_reverses_direction_when_obstructed>                                                  },
...
int16 Lua_Platforms_Length() { return dynamic_world->platform_count; }

char Lua_Polygon_Floor_Name[] = "polygon_floor";

static int Lua_Polygon_Floor_Get_Collection(lua_State* L) {
    Lua_Collection::Push(L, GET_COLLECTION(GET_DESCRIPTOR_COLLECTION(
                                    get_polygon_data(Lua_Polygon_Floor::Index(L, 1))->floor_texture)));
    return 1;
}
...
static int Lua_Polygon_Ceiling_Get_Collection(lua_State* L) {
    Lua_Collection::Push(L, GET_COLLECTION(GET_DESCRIPTOR_COLLECTION(
                                    get_polygon_data(Lua_Polygon_Ceiling::Index(L, 1))->ceiling_texture)));
    return 1;
}
...
int Lua_Polygon_Check_Collision(lua_State* L) {
    if (!lua_isnumber(L, 2) || !lua_isnumber(L, 3) || !lua_isnumber(L, 4) || !lua_isnumber(L, 6) || !lua_isnumber(L, 7)
        || !lua_isnumber(L, 8) || (lua_gettop(L) >= 9 && !(lua_isnil(L, 9) || lua_isboolean(L, 9)))
    ...
    world_point3d origin = {static_cast<world_distance>(lua_tonumber(L, 2) * WORLD_ONE),
                            static_cast<world_distance>(lua_tonumber(L, 3) * WORLD_ONE),
                            static_cast<world_distance>(lua_tonumber(L, 4) * WORLD_ONE)};

    world_point3d destination = {static_cast<world_distance>(lua_tonumber(L, 6) * WORLD_ONE),
    ...
    world_distance distance
            = distance2d(reinterpret_cast<world_point2d*>(&origin), reinterpret_cast<world_point2d*>(&destination));
    ...
    projectile_definition->flags = _usually_pass_transparent_side | _sometimes_pass_transparent_side
    ...
    for (auto i = 0; i < chunks && !(flags & _projectile_hit); ++i) {
        ...
        p1 = {static_cast<world_distance>(origin.x + dx * (i + 1) / chunks),
              static_cast<world_distance>(origin.y + dy * (i + 1) / chunks),
              static_cast<world_distance>(origin.z + dz * (i + 1) / chunks)};

        flags = translate_projectile(0, &p0, old_polygon, &p1, &new_polygon, owner, &obstruction_index, &line_index,
                                     true, NONE);
    }
...
int Lua_Polygon_Contains(lua_State* L) {
    ...
    lua_pushboolean(L,
                    point_in_polygon(polygon_index, &p) && z >= polygon->floor_height && z <= polygon->ceiling_height);
    return 1;
}
...
static bool Lua_Polygon_Valid(int16 index) { return index >= 0 && index < dynamic_world->polygon_count; }
...
int16 Lua_Polygons_Length() { return dynamic_world->polygon_count; }
...
static int Lua_Side_ControlPanel_Get_UsesItem(lua_State* L) {
    control_panel_definition* definition
            = get_control_panel_definition(get_side_data(Lua_Side_ControlPanel::Index(L, 1))->control_panel_type);
...
static int Lua_Side_ControlPanel_Set_UsesItem(lua_State*) { return 0; }
...
static int Lua_Primary_Side_Get_Collection(lua_State* L) {
    Lua_Collection::Push(L, GET_COLLECTION(GET_DESCRIPTOR_COLLECTION(
                                    get_side_data(Lua_Primary_Side::Index(L, 1))->primary_texture.texture)));
    return 1;
}
...
static int Lua_Primary_Side_Set_Collection(lua_State* L) {
    ...
    side->primary_texture.texture
            = BUILD_DESCRIPTOR(collection_index, GET_DESCRIPTOR_SHAPE(side->primary_texture.texture));
...
static int Lua_Primary_Side_Set_Texture_Index(lua_State* L) {
    ...
    side->primary_texture.texture
            = BUILD_DESCRIPTOR(GET_DESCRIPTOR_COLLECTION(side->primary_texture.texture), shape_index);
    update_line_redundancy(side->line_index);
    return 0;
}
...
static int Lua_Secondary_Side_Get_Collection(lua_State* L) {
    Lua_Collection::Push(L, GET_COLLECTION(GET_DESCRIPTOR_COLLECTION(
                                    get_side_data(Lua_Secondary_Side::Index(L, 1))->secondary_texture.texture)));
    return 1;
}
...
static int Lua_Secondary_Side_Set_Collection(lua_State* L) {
    ...
    side_data* side = get_side_data(side_index);
    side->secondary_texture.texture
            = BUILD_DESCRIPTOR(collection_index, GET_DESCRIPTOR_SHAPE(side->secondary_texture.texture));
    update_line_redundancy(side->line_index);
    return 0;
}
...
static int Lua_Secondary_Side_Set_Texture_Index(lua_State* L) {
    ...
    side->secondary_texture.texture
            = BUILD_DESCRIPTOR(GET_DESCRIPTOR_COLLECTION(side->secondary_texture.texture), shape_index);
    update_line_redundancy(side->line_index);
    return 0;
}
...
static int Lua_Transparent_Side_Get_Collection(lua_State* L) {
    Lua_Collection::Push(L, GET_COLLECTION(GET_DESCRIPTOR_COLLECTION(
                                    get_side_data(Lua_Transparent_Side::Index(L, 1))->transparent_texture.texture)));
    return 1;
}
...
static int Lua_Transparent_Side_Get_Texture_Index(lua_State* L) {
    lua_pushnumber(L,
                   GET_DESCRIPTOR_SHAPE(get_side_data(Lua_Transparent_Side::Index(L, 1))->transparent_texture.texture));
    return 1;
}
...
static int Lua_Transparent_Side_Set_Collection(lua_State* L) {
    ...
    side_data* side = get_side_data(side_index);
    side->transparent_texture.texture
            = BUILD_DESCRIPTOR(collection_index, GET_DESCRIPTOR_SHAPE(side->transparent_texture.texture));
    update_line_redundancy(side->line_index);
    return 0;
}
...
static int Lua_Transparent_Side_Set_Texture_Index(lua_State* L) {
    ...
    side->transparent_texture.texture
            = BUILD_DESCRIPTOR(GET_DESCRIPTOR_COLLECTION(side->transparent_texture.texture), shape_index);
    update_line_redundancy(side->line_index);
    return 0;
}
...
static int Lua_Side_Set_Control_Panel(lua_State* L) {
    ...
    if (lua_toboolean(L, 2) != (side->flags & _side_is_control_panel)) {
        ...
        side->flags              &= ~(_control_panel_status | _side_is_control_panel | _side_is_repair_switch
                         | _side_is_destructive_switch | _side_is_lighted_switch | _side_switch_can_be_destroyed
                         | _side_switch_can_only_be_hit_by_projectiles);
...
static bool Lua_Side_Valid(int16 index) { return index >= 0 && index < SideList.size(); }
...
int Lua_Sides_New(lua_State* L) {
    ...
    if ((line->clockwise_polygon_owner == polygon_index && line->clockwise_polygon_side_index != NONE)
        || (line->counterclockwise_polygon_owner == polygon_index && line->counterclockwise_polygon_side_index != NONE))
...
int16 Lua_Light_State::LightIndex(lua_State* L, int index) {
    ...
    if (!t)
        luaL_typerror(L, index, Lua_Light_State_Name);
...
static int Lua_Light_State_Set_Delta_Intensity(lua_State* L) {
    ...
    lighting_function_specification* spec
            = get_light_function_spec(Lua_Light_State::LightIndex(L, 1), Lua_Light_State::Index(L, 1));
...
bool Lua_Light_Valid(int16 index) { return index >= 0 && index < MAXIMUM_LIGHTS_PER_MAP; }
...
static int Lua_Tag_Get_Active(lua_State* L) {
    ...
    for (platform_index = 0, platform = platforms; platform_index < dynamic_world->platform_count && !changed;
...
static bool Lua_Terminal_Valid(int16 index) { return index >= 0 && index < number_of_terminal_texts(); }

char Lua_Terminals_Name[] = "Terminals";

static int16 Lua_Terminals_Length() { return number_of_terminal_texts(); }
...
static bool Lua_Media_Valid(int16 index) { return index >= 0 && index < MAXIMUM_MEDIAS_PER_MAP; }
...
bool Lua_Annotation_Valid(int16 index) { return index >= 0 && index < MapAnnotationList.size(); }
...
int16 Lua_Annotations_Length() { return MapAnnotationList.size(); }
...
static int Lua_Fog_Color_Get_R(lua_State* L) {
    lua_pushnumber(L, (float)(OGL_GetFogData(Lua_Fog_Color::Index(L, 1))->Color.red) / 65'535);
    return 1;
}

static int Lua_Fog_Color_Get_G(lua_State* L) {
    lua_pushnumber(L, (float)(OGL_GetFogData(Lua_Fog_Color::Index(L, 1))->Color.green) / 65'535);
    return 1;
}

static int Lua_Fog_Color_Get_B(lua_State* L) {
    lua_pushnumber(L, (float)(OGL_GetFogData(Lua_Fog_Color::Index(L, 1))->Color.blue) / 65'535);
    return 1;
}
...
static int Lua_Fog_Color_Set_R(lua_State* L) {
    ...
    OGL_GetFogData(Lua_Fog_Color::Index(L, 1))->Color.red = PIN(int(65'535 * color + 0.5), 0, 65'535);
    return 0;
}
...
static int Lua_Fog_Color_Set_G(lua_State* L) {
    ...
    float color                                             = static_cast<float>(lua_tonumber(L, 2));
    OGL_GetFogData(Lua_Fog_Color::Index(L, 1))->Color.green = PIN(int(65'535 * color + 0.5), 0, 65'535);
    return 0;
}
...
static int Lua_Fog_Color_Set_B(lua_State* L) {
    ...
    float color                                            = static_cast<float>(lua_tonumber(L, 2));
    OGL_GetFogData(Lua_Fog_Color::Index(L, 1))->Color.blue = PIN(int(65'535 * color + 0.5), 0, 65'535);
    return 0;
}
...
static const char* compatibility_script
        = ""
          "function annotations() local i = 0 local n = # Annotations return function() if i < n then a = "
          "Annotations[i] i = i + 1 if a.polygon then p = a.polygon.index else p = -1 end return a.text, p, a.x, a.y "
          "else return nil end end end\n"
          "function get_all_fog_attributes() local attributes = { OGL_Fog_AboveLiquid = { Depth = Level.fog.depth, "
          "Color = { red = Level.fog.color.r, blue = Level.fog.color.b, green = Level.fog.color.g }, IsPresent = "
          "Level.fog.present, AffectsLandscapes = Level.fog.affects_landscapes }, OGL_Fog_BelowLiquid = { Depth = "
          "Level.underwater_fog.depth, Color = { red = Level.underwater_fog.color.r, green = "
          "Level.underwater_fog.color.g, blue = Level.underwater_fog.color.b, }, IsPresent = "
          "Level.underwater_fog.present, AffectsLandscapes = Level.underwater_fog.affects_landscapes } } return "
          "attributes end\n"
          ...
          "function get_platform_ceiling_height(polygon) if Polygons[polygon].platform then return "
          "Polygons[polygon].platform.ceiling_height end end\n"
          "function get_platform_floor_height(polygon) if Polygons[polygon].platform then return "
          "Polygons[polygon].platform.floor_height end end\n"
          "function get_platform_index(polygon) if Polygons[polygon].platform then return "
          "Polygons[polygon].platform.index else return -1 end end\n"
          "function get_polygon_media(polygon) if Polygons[polygon].media then return Polygons[polygon].media.index "
          "else return -1 end end\n"
          "function get_platform_monster_control(polygon) if Polygons[polygon].platform then return "
          "Polygons[polygon].platform.monster_controllable end end\n"
          "function get_platform_movement(polygon) if Polygons[polygon].platform then return "
          "Polygons[polygon].platform.extending end end\n"
          "function get_platform_player_control(polygon) if Polygons[polygon].platform then return "
          "Polygons[polygon].platform.player_controllable end end\n"
          "function get_platform_speed(polygon) if Polygons[polygon].platform then return "
          "Polygons[polygon].platform.speed * 1024 end end\n"
          "function get_platform_state(polygon) if Polygons[polygon].platform then return "
          "Polygons[polygon].platform.active end end\n"
          ...
          "function get_terminal_text_number(poly, line) if Lines[line].ccw_polygon == Polygons[poly] then s = "
          "Lines[line].ccw_side elseif Lines[line].cw_polygon == Polygons[poly] then s = Lines[line].cw_side else "
          "return line end if s.control_panel and s.control_panel.type.class == 8 then return "
          "s.control_panel.permutation else return line end end\n"
          "function get_underwater_fog_affects_landscapes() return Level.underwater_fog.affects_landscapes end\n"
          "function get_underwater_fog_color() return Level.underwater_fog.color.r, Level.underwater_fog.color.g, "
          "Level.underwater_fog.color.b end\n"
          ...
          "function select_monster(type, poly) for m in Polygons[poly]:monsters() do if m.type == type and m.visible "
          "and (m.action < 6 or m.action > 9) then return m.index end end return nil end\n"
          "function set_all_fog_attributes(t) Level.fog.depth = t.OGL_Fog_AboveLiquid.Depth Level.fog.present = "
          "t.OGL_Fog_AboveLiquid.IsPresent Level.fog.affects_landscapes = t.OGL_Fog_AboveLiquid.AffectsLandscapes "
          "Level.fog.color.r = t.OGL_Fog_AboveLiquid.Color.red Level.fog.color.g = t.OGL_Fog_AboveLiquid.Color.green "
          "Level.fog.color.b = t.OGL_Fog_AboveLiquid.Color.blue Level.underwater_fog.depth = "
          "t.OGL_Fog_BelowLiquid.Depth Level.underwater_fog.present = t.OGL_Fog_BelowLiquid.IsPresent "
          "Level.underwater_fog.affects_landscapes = t.OGL_Fog_BelowLiquid.AffectsLandscapes "
          "Level.underwater_fog.color.r = t.OGL_Fog_BelowLiquid.Color.red Level.underwater_fog.color.g = "
          "t.OGL_Fog_BelowLiquid.Color.green Level.underwater_fog.color.b = t.OGL_Fog_BelowLiquid.Color.blue end\n"
          "function set_fog_affects_landscapes(affects_landscapes) Level.fog.affects_landscapes = affects_landscapes "
          "end\n"
          ...
          "function set_platform_ceiling_height(polygon, height) if Polygons[polygon].platform then "
          "Polygons[polygon].platform.ceiling_height = height end end\n"
          "function set_platform_floor_height(polygon, height) if Polygons[polygon].platform then "
          "Polygons[polygon].platform.floor_height = height end end\n"
          "function set_platform_monster_control(polygon, control) if Polygons[polygon].platform then "
          "Polygons[polygon].platform.monster_controllable = control end end\n"
          "function set_platform_movement(polygon, movement) if Polygons[polygon].platform then "
          "Polygons[polygon].platform.extending = movement end end\n"
          "function set_platform_player_control(polygon, control) if Polygons[polygon].platform then "
          "Polygons[polygon].platform.player_controllable = control end end\n"
          "function set_platform_speed(polygon, speed) if Polygons[polygon].platform then "
          "Polygons[polygon].platform.speed = speed / 1024 end end\n"
          "function set_platform_state(polygon, state) if Polygons[polygon].platform then "
          "Polygons[polygon].platform.active = state end end\n"
          ...
          "function set_polygon_media(polygon, media) if media == -1 then Polygons[polygon].media = nil else "
          "Polygons[polygon].media = media end end\n"
          ...
          "function set_terminal_text_number(poly, line, id) if Lines[line].ccw_polygon == Polygons[poly] then s = "
          "Lines[line].ccw_side elseif Lines[line].cw_polygon == Polygons[poly] then s = Lines[line].cw_side else "
          "return end if s.control_panel and s.control_panel.type.class == 8 then s.control_panel.permutation = id end "
          "end\n"
          "function set_underwater_fog_affects_landscapes(affects_landscapes) Level.underwater_fog.affects_landscapes "
          "= affects_landscapes end\n"
          "function set_underwater_fog_color(r, g, b) Level.underwater_fog.color.r = r Level.underwater_fog.color.g = "
          "g Level.underwater_fog.color.b = b end\n"
