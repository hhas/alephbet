...
static float Lua_HUDColor_Get_R(lua_State* L, int pos) { return Lua_HUDColor_Lookup(L, pos, 1, "r", "red"); }

static float Lua_HUDColor_Get_G(lua_State* L, int pos) { return Lua_HUDColor_Lookup(L, pos, 2, "g", "green"); }

static float Lua_HUDColor_Get_B(lua_State* L, int pos) { return Lua_HUDColor_Lookup(L, pos, 3, "b", "blue"); }

static float Lua_HUDColor_Get_A(lua_State* L, int pos) { return Lua_HUDColor_Lookup(L, pos, 4, "a", "alpha"); }
...
int Lua_Images_New(lua_State* L) {
    ...
    if (!lua_isnil(L, -1)) {
        ...
        Image_Blitter* blitter
                = (get_screen_mode()->acceleration != _no_acceleration) ? new OGL_Blitter() : new Image_Blitter();
#else
...
float Lua_Font::Scale(lua_State* L, int index) {
    ...
    if (!t)
        luaL_typerror(L, index, Lua_Font_Name);
...
int Lua_Font_Draw_Text(lua_State* L) {
    ...
    Lua_HUDInstance()->draw_text(Lua_Font::Object(L, 1), str, x, y, r, g, b, a, Lua_Font::Scale(L, 1));
    return 0;
}
...
Lua_HUDPlayer_Weapon_Trigger_Bullet* Lua_HUDPlayer_Weapon_Trigger_Bullet::Push(lua_State* L, int16 weapon_index,
                                                                               int16 index) {
...
int16 Lua_HUDPlayer_Weapon_Trigger_Bullet::WeaponIndex(lua_State* L, int index) {
    ...
    if (!t)
        luaL_typerror(L, index, Lua_HUDPlayer_Weapon_Trigger_Bullet_Name);
...
Lua_HUDPlayer_Weapon_Trigger_Energy* Lua_HUDPlayer_Weapon_Trigger_Energy::Push(lua_State* L, int16 weapon_index,
                                                                               int16 index) {
...
int16 Lua_HUDPlayer_Weapon_Trigger_Energy::WeaponIndex(lua_State* L, int index) {
    ...
    if (!t)
        luaL_typerror(L, index, Lua_HUDPlayer_Weapon_Trigger_Energy_Name);
    return t->m_weapon_index;
}
...
int16 Lua_HUDPlayer_Weapon_Trigger::WeaponIndex(lua_State* L, int index) {
    ...
    if (!t)
        luaL_typerror(L, index, Lua_HUDPlayer_Weapon_Trigger_Name);
    return t->m_weapon_index;
}

static int Lua_HUDPlayer_Weapon_Trigger_Get_Rounds(lua_State* L) {
    short rounds = get_player_weapon_ammo_count(current_player_index, Lua_HUDPlayer_Weapon_Trigger::WeaponIndex(L, 1),
...
static int Lua_HUDPlayer_Weapon_Trigger_Get_Total_Rounds(lua_State* L) {
    short rounds = get_player_weapon_ammo_maximum(current_player_index, Lua_HUDPlayer_Weapon_Trigger::WeaponIndex(L, 1),
                                                  Lua_HUDPlayer_Weapon_Trigger::Index(L, 1));
    lua_pushnumber(L, rounds);
    return 1;
}
...
static int Lua_HUDPlayer_Weapon_Trigger_Get_Ammo_Type(lua_State* L) {
    int16 t = get_player_weapon_ammo_type(current_player_index, Lua_HUDPlayer_Weapon_Trigger::WeaponIndex(L, 1),
                                          Lua_HUDPlayer_Weapon_Trigger::Index(L, 1));
...
static int Lua_HUDPlayer_Weapon_Trigger_Get_Weapon_Drawn(lua_State* L) {
    bool t = get_player_weapon_drawn(current_player_index, Lua_HUDPlayer_Weapon_Trigger::WeaponIndex(L, 1),
                                     Lua_HUDPlayer_Weapon_Trigger::Index(L, 1));
    lua_pushboolean(L, t);
    return 1;
}
...
#define strWEAPON_NAME_LIST 137
        ...
        for (item_index = BALL_ITEM_BASE; item_index < BALL_ITEM_BASE + MAXIMUM_NUMBER_OF_PLAYERS; ++item_index) {
            if (current_player->items[item_index] > 0)
                break;
        }
...
static bool Lua_HUDTexturePalette_Slot_Valid(int16 index) { return index >= 0 && index < LuaTexturePaletteSize(); }
...
static int Lua_HUDPlayer_Velocity_Get_Forward(lua_State* L) {
    double diff_x
            = (current_player->variables.position.x - current_player->variables.last_position.x) / (double)FIXED_ONE;
    double diff_y
            = (current_player->variables.position.y - current_player->variables.last_position.y) / (double)FIXED_ONE;
...
static int Lua_HUDPlayer_Velocity_Get_Vertical(lua_State* L) {
    lua_pushnumber(L, (current_player->variables.position.z - current_player->variables.last_position.z)
                              / (double)FIXED_ONE);
    return 1;
}
...
static bool Lua_HUDGame_Player_Valid(int16 index) { return index >= 0 && index < dynamic_world->player_count; }
...
static int Lua_HUDLighting_Get_Ambient(lua_State* L) {
    lua_pushnumber(L, get_light_intensity(get_polygon_data(world_view->origin_polygon_index)->floor_lightsource_index)
                              / 65535.f);
    return 1;
}
...
int Lua_HUDObjects_register(lua_State* L) {
    ...
    Lua_HUDPlayer_Weapon_Trigger_Bullet::Valid
            = Lua_HUDPlayer_Weapon_Trigger_Bullet::ValidRange((int)_secondary_weapon + 1);

    Lua_HUDPlayer_Weapon_Trigger_Energy::Register(L, Lua_HUDPlayer_Weapon_Trigger_Energy_Get);
    Lua_HUDPlayer_Weapon_Trigger_Energy::Valid
            = Lua_HUDPlayer_Weapon_Trigger_Energy::ValidRange((int)_secondary_weapon + 1);
