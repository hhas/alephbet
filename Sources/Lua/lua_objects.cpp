...
static bool Lua_ItemKind_Valid(int32 index) { return index >= 0 && index <= NUMBER_OF_ITEM_TYPES; }
...
static int Lua_ItemType_Get_Maximum_Inventory(lua_State* L) {
    ...
    lua_pushnumber(L, get_item_definition_external(Lua_ItemType::Index(L, 1))
                              ->get_maximum_count_per_player(is_m1, difficulty_level));
    return 1;
}
...
static int Lua_ItemType_Set_Random_Chance(lua_State* L) {
    if (lua_isnumber(L, 2)) {
        get_placement_info()[Lua_ItemType::Index(L, 1)].random_chance
...
static bool Lua_ItemType_Valid(int32 index) { return index >= 0 && index < NUMBER_OF_DEFINED_ITEMS; }
...
static bool Lua_SceneryType_Valid(int32 index) { return index >= 0 && index <= NUMBER_OF_SCENERY_DEFINITIONS; }
...
int Lua_Sounds_Length() { return INT16_MAX; }
...
static const char* compatibility_script
        = ""
          ...
          "function new_item(type, polygon, height) if (height) then return Items.new(Polygons[polygon].x, "
          "Polygons[polygon].y, height / 1024, polygon, type).index else return Items.new(Polygons[polygon].x, "
          "Polygons[polygon].y, 0, polygon, type).index end end\n";
