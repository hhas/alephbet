...
bool is_applesingle(SDL_RWops* f, bool rsrc_fork, int32& offset, int32& length) {
    ...
    if (id != 0x0005'1600 || version != 0x0002'0000)
        return false;
...
void set_external_resources_file(FileSpecifier& f) { f.Open(ExternalResources); }

void close_external_resources() { ExternalResources.Close(); }

/*
 *  Initialize resource management
 */

void initialize_resources(void) { atexit(close_external_resources); }
...
bool res_file_t::read_map(void) {
    ...
    logDump("resource header: data offset %d, map_offset %d, data_size %d, map_size %d", data_offset, map_offset,
            data_size, map_size);

    // Verify integrity of resource header
    if (data_offset >= file_size || map_offset >= file_size || data_offset + data_size > file_size
        || map_offset + map_size > file_size) {
    ...
    for (int i = 0; i < num_types; i++) {
        ...
        for (int j = 0; j < num_refs; j++) {
            ...
            uint32 rsrc_data_offset = data_offset + (SDL_ReadBE32(f) & 0x00ff'ffff);
...
SDL_RWops* open_res_file_from_rwops(SDL_RWops* f) {
    if (f) {
...
static SDL_RWops* open_res_file_from_path(const char* inPath) {
...
size_t count_1_resources(uint32 type) { return (*cur_res_file_t)->count_resources(type); }
...
void res_file_t::get_resource_id_list(uint32 type, vector<int>& ids) const {
    ...
    if (i != types.end()) {
        ...
        for (j = i->second.begin(); j != end; j++) ids.push_back(j->first);
    }
}
...
bool has_1_resource(uint32 type, int id) { return (*cur_res_file_t)->has_resource(type, id); }
