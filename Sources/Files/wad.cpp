...
#include "interface.hpp" // for strERRORS
#include "tags.hpp"
#include "wad.hpp"
...
static bool read_indexed_directory_data(OpenedFile& OFile, struct wad_header* header, short index,
                                        struct directory_entry* entry);
static int32 calculate_raw_wad_length(struct wad_header* file_header, uint8* wad);
static bool read_indexed_wad_from_file_into_buffer(OpenedFile& OFile, struct wad_header* header, short index,
...
static bool size_of_indexed_wad(OpenedFile& OFile, struct wad_header* header, short index, int32* length);
...
struct wad_data* read_indexed_wad_from_file(OpenedFile& OFile, struct wad_header* header, short index, bool read_only) {
    ...
    // if(file_id>=0) /* NOT a union wadfile... */
    {
        if (size_of_indexed_wad(OFile, header, index, &length)) {
            ...
            raw_wad = BetweenLevels ? (uint8*)level_transition_malloc(padded_length) : (uint8*)malloc(padded_length);
...
bool wad_file_has_parent_checksum(FileSpecifier& File, uint32 parent_checksum) {
    // fileref file_id;
    bool has_checksum = false;
    struct wad_header header;
...
int32 get_size_of_directory_data(struct wad_header* header) {
    ...
    return (header->wad_count * (header->application_specific_directory_data_size + base_entry_size));
}
...
int32 calculate_wad_length(struct wad_header* file_header, struct wad_data* wad) {
    ...
    for (ii = 0; ii < wad->tag_count; ++ii) { running_length += wad->tag_data[ii].length + header_length; }

    return running_length;
}

/* ------------ Transfer type functions */
#define CURRENT_FLAT_MAGIC_COOKIE (0xDEAD'DEAD)
        ...
        if (success) {
            ...
            if (size_of_indexed_wad(OFile, &header, wad_index, &length)) {
                ...
                if (data) {
                    ...
                    success = read_indexed_wad_from_file_into_buffer(OFile, &header, wad_index, buffer, &length);
...
void dump_wad(struct wad_data* wad) {
    ...
    for (index = 0; index < wad->tag_count; ++index) {
        assert(tag);
        dprintf("Tag: %x data: %p length: %d offset: %d", tag->tag, tag->data, tag->length, tag->offset);
        tag++;
    }
    dprintf("---End of Dump---");
}
...
bool create_wadfile(FileSpecifier& File, Typecode Type) { return File.Create(Type); }
...
void close_wad_file(OpenedFile& File) { File.Close(); }
...
static bool read_indexed_wad_from_file_into_buffer(
        OpenedFile& OFile, struct wad_header* header, short index, void* buffer,
        ...
        if (raw_length) {
            ...
            if (wad->tag_data) {
                ...
                for (index = 0; index < tag_count; ++index) {
                    ...
                    /* Copy the data.. */
                    memcpy(wad->tag_data[index].data, raw_wad_entry_header + entry_header_size,
