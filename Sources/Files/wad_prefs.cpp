...
void w_write_preferences_file(void) {
    ...
    if (open_wad_file_for_writing(prefInfo->PrefsFile, PrefsFile)) {
        ...
        fill_default_wad_header(prefInfo->PrefsFile, CURRENT_WADFILE_VERSION, CURRENT_PREF_WADFILE_VERSION, 1, 0l,
                                &header);
        ...
        if (write_wad_header(PrefsFile, &header)) {
            ...
            if (write_wad(PrefsFile, &header, prefInfo->wad, offset)) {
                ...
                if (write_wad_header(PrefsFile, &header) && write_directorys(PrefsFile, &header, &entry)) {
                    /* Success! */
                } else {
                    assert(error_pending());
                }
