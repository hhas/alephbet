...
/* ---------- code */
void set_physics_file(FileSpecifier& File) { PhysicsFileSpec = File; }

void set_to_default_physics_file(void) {
    get_default_physics_spec(PhysicsFileSpec);
    //	dprintf("Set to: %d %d %.*s", physics_file.vRefNum, physics_file.parID, physics_file.name[0],
    //physics_file.name+1);
}
...
#define M1_PHYSICS_MAGIC_COOKIE (0xDEAF'DEAF)
...
void* get_network_physics_buffer(int32* physics_length) {
    if (physics_file_is_m1()) {
        ...
        uint8* data  = NULL;
        OpenedFile PhysicsFile;
        if (PhysicsFileSpec.Open(PhysicsFile) && PhysicsFile.GetLength(*physics_length)) {
            *physics_length += 8;
...
uint32_t get_physics_file_checksum() { return calculate_crc_for_file(PhysicsFileSpec); }

/* --------- local code */
static struct wad_data* get_physics_wad_data(bool* bungie_physics) {
    struct wad_data* wad = NULL;

    //	dprintf("Open is: %d %d %.*s", physics_file.vRefNum, physics_file.parID, physics_file.name[0],
    //physics_file.name+1);
