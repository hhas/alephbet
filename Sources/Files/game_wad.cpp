...
#include "computer_interface.hpp" // for loading/saving terminal state.
#include "editor.hpp"
...
#include "motion_sensor.hpp" // ZZZ for reset_motion_sensor()
...
static void allocate_map_for_counts(size_t polygon_count, size_t side_count, size_t endpoint_count, size_t line_count);
...
static void complete_loading_level(short* _map_indexes, size_t map_index_count, uint8* _platform_data,
                                   size_t platform_data_count, uint8* actual_platform_data,
                                   size_t actual_platform_data_count, short version);
...
/* ------------------------ Net functions */
int32 get_net_map_data_length(void* data) { return get_flat_data_length(data); }
...
/* This sucks, beavis. */
void complete_loading_level(short* _map_indexes, size_t map_index_count, uint8* _platform_data,
                            size_t platform_data_count, uint8* actual_platform_data, size_t actual_platform_data_count,
                            short version) {
...
short get_player_starting_location_and_facing(short team, short index, struct object_location* location) {
    ...
    if (location)
        vassert(done, csprintf(temporary, "Tried to place: %d only %d starting pts.", index, count));
...
bool new_game(short number_of_players, bool network, struct game_data* game_information,
    ...
    if (success) {
        ...
        for (i = 0; i < number_of_players; ++i) {
            ...
            player_index           = new_player(player_start_information[i].team, player_start_information[i].color,
                                                player_start_information[i].identifier, flags);
...
void allocate_map_for_counts(size_t polygon_count, size_t side_count, size_t endpoint_count, size_t line_count) {
...
void load_points(uint8* points, size_t count) {
    size_t loop;
...
void load_lines(uint8* lines, size_t count) {
...
void load_sides(uint8* sides, size_t count, short version) {
    size_t loop;
...
void load_polygons(uint8* polys, size_t count, short version) {
    size_t loop;
...
void load_lights(uint8* _lights, size_t count, short version) {
...
void load_annotations(uint8* annotations, size_t count) {
...
void load_map_info(uint8* map_info) {
    unpack_static_data(map_info, static_world, 1);
    static_world->ball_in_play = false;
}

void load_media(uint8* _medias, size_t count) {
...
void load_ambient_sound_images(uint8* data, size_t count) {
...
void load_random_sound_images(uint8* data, size_t count) {
...
void get_current_saved_game_name(FileSpecifier& File) { File = revert_game_data.SavedGame; }
            ...
            if (write_wad_header(SaveFile, &header)) {
                ...
                if (wad) {
                    ...
                    if (write_wad(SaveFile, &header, wad, offset)) {
                        ...
                        if (meta_wad) {
                            set_indexed_directory_offset_and_length(&header, entries, 1, offset, wad_length,
                                                                    SAVE_GAME_METADATA_INDEX);
...
bool process_map_wad(struct wad_data* wad, bool restoring_game, short version) {
    ...
    assert(version == MARATHON_INFINITY_DATA_VERSION || version == MARATHON_TWO_DATA_VERSION
    ...
    if (version == MARATHON_ONE_DATA_VERSION) {
        ...
        static_world->environment_flags |= _environment_glue_m1 | _environment_ouch_m1 | _environment_song_index_m1
                                           | _environment_terminals_stop_time | _environment_activation_ranges
                                           | _environment_m1_weapons;
    }
    ...
    if (restoring_game) {
        ...
        complete_loading_level((short*)map_index_data, map_index_count, data, count, platform_structures,
...
void load_terminal_data(uint8* data, size_t length) {
...
save_game_data export_data[] = {
        ...
        {
         LIGHTSOURCE_TAG, SIZEOF_static_light_data,
         true, },
    ...
    if (wad) {
        ...
        for (size_t loop = 0; loop < PlatformList.size(); ++loop) {
            ...
            if (PLATFORM_COMES_FROM_CEILING(platform)) {
                auto new_ceiling_height = PLATFORM_IS_INITIALLY_EXTENDED(platform) ? platform->minimum_ceiling_height
            ...
            PolygonList[platform->polygon_index].ceiling_height
                    = PolygonListCopy[platform->polygon_index].ceiling_height;
        }
...
FileSpecifier& get_map_file() { return MapFileSpec; }
