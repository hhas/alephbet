...
extern DirectorySpecifier local_data_dir, preferences_dir, saved_games_dir, quick_saves_dir, image_cache_dir,
        recordings_dir;
...
bool OpenedFile::IsOpen() { return f != NULL; }
...
std::streamsize opened_file_device::read(char* s, std::streamsize n) { return SDL_RWread(f.GetRWops(), s, 1, n); }
...
std::streampos opened_file_stream::seekoff(std::streamoff off, std::ios_base::seekdir dir,
...
std::streampos const_byte_stream::seekoff(std::streamoff off, std::ios_base::seekdir dir,
                                          std::ios_base::openmode which) {
    ...
    setg(this->eback(), this->eback() + target_offset, this->egptr());
    return target_offset;
}

std::streamsize const_byte_stream::showmanyc() { return this->egptr() - this->gptr(); }
...
std::streampos mutable_byte_stream::seekoff(std::streamoff off, std::ios_base::seekdir dir,
                                            std::ios_base::openmode which) {
    ...
    switch (which) {
        ...
        case std::ios::out: {
            ...
            setp(this->eback() + target_offset, this->epptr());
            return target_offset;
        }
    }
}

std::streamsize mutable_byte_stream::showmanyc() { return this->egptr() - this->gptr(); }
...
bool LoadedResource::IsLoaded() { return p != NULL; }
...
size_t LoadedResource::GetLength() { return size; }
...
bool OpenedResourceFile::IsOpen() { return f != NULL; }
...
not_map:;
    ...
    // Check for Shapes file
    {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            if (status_flags != 0 || (offset != NONE && (offset >= file_length || offset + length > file_length))
...
void FileSpecifier::SetToLocalDataDir() { name = local_data_dir.name; }

// Set to preferences directory
void FileSpecifier::SetToPreferencesDir() { name = preferences_dir.name; }

// Set to saved games directory
void FileSpecifier::SetToSavedGamesDir() { name = saved_games_dir.name; }

// Set to newer saved games directory
void FileSpecifier::SetToQuickSavesDir() { name = quick_saves_dir.name; }

// Set to image cache directory
void FileSpecifier::SetToImageCacheDir() { name = image_cache_dir.name; }

// Set to recordings directory
void FileSpecifier::SetToRecordingsDir() { name = recordings_dir.name; }
...
void FileSpecifier::SetTempName(const FileSpecifier& other) { name = other.name + "^"; }
...
void FileSpecifier::FromDirectory(DirectorySpecifier& Dir) { name = Dir.name; }
...
bool FileSpecifier::ReadZIP(vector<string>& vec) {
    ...
    for (ZZIP_DIRENT entry; zzip_dir_read(zip, &entry);) vec.emplace_back(entry.d_name);
...
class w_directory_browsing_list : public w_list<dir_entry> {
  public:
    ...
    w_directory_browsing_list(const FileSpecifier& inStartingDirectory, dialog* inParentDialog,
                              const string& inStartingFile)
        : w_list<dir_entry>(entries, 400, 15, 0), parent_dialog(inParentDialog),
          current_directory(inStartingDirectory) {
        refresh_entries();
        if (entries.size() != 0)
    ...
    void draw_item(vector<dir_entry>::const_iterator i, SDL_Surface* s, int16 x, int16 y, uint16 width,
                   bool selected) const {
        y += font->get_ascent();
        ...
        if (i->is_directory) {
            ...
            draw_text(s, theName.c_str(), x, y,
                      selected ? get_theme_color(ITEM_WIDGET, ACTIVE_STATE)
                               : get_theme_color(ITEM_WIDGET, DEFAULT_STATE),
                      font, style, true);
        } else {
            char date[256];
            ...
            if (time_info) {
                ...
                draw_text(s, date, x + width - date_width, y,
                          selected ? get_theme_color(ITEM_WIDGET, ACTIVE_STATE)
                                   : get_theme_color(ITEM_WIDGET, DEFAULT_STATE),
                          font, style);
                set_drawing_clip_rectangle(0, x, s->h, x + width - date_width - 4);
            }
            draw_text(s, FileSpecifier::HideExtension(i->name).c_str(), x, y,
                      selected ? get_theme_color(ITEM_WIDGET, ACTIVE_STATE)
                               : get_theme_color(ITEM_WIDGET, DEFAULT_STATE),
                      font, style, true);
        }
  ...
  private:
    ...
    struct most_recent {
        bool operator()(const dir_entry& a, const dir_entry& b) { return a.date > b.date; }
    };
...
const char* sort_by_labels[] = {"name", "date", 0};
...
class FileDialog {
  ...
  private:
    ...
    void on_up() { m_list_w->move_up_a_level(); }
};
...
class ReadFileDialog : public FileDialog {
  public:
    ...
    FileSpecifier GetFile() { return m_list_w->get_file(); }

  private:

    void on_file_selected() { m_dialog.quit(0); }
...
class WriteFileDialog : public FileDialog {
  public:
    ...
    WriteFileDialog(FileSpecifier dir, Typecode type, const char* prompt, const char* default_name)
        : FileDialog(), m_prompt(prompt), m_default_name(default_name), m_extension(0) {
        if (!m_prompt) {
            switch (type) {
                case _typecode_savegame:
                    m_prompt = "SAVE GAME";
    ...
    std::string GetFilename() { return m_name_w->get_text(); }
...
ScopedSearchPath::ScopedSearchPath(const DirectorySpecifier& dir) : d{dir} {
