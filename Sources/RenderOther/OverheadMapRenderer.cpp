...
void OverheadMapClass::Render(overhead_map_data& Control) {
    ...
    assert(ConfigPtr);
    if (ConfigPtr->ShowAliens)
        GET_GAME_OPTIONS() |= _overhead_map_shows_monsters;
    if (ConfigPtr->ShowItems)
        GET_GAME_OPTIONS() |= _overhead_map_shows_items;
    if (ConfigPtr->ShowProjectiles)
        GET_GAME_OPTIONS() |= _overhead_map_shows_projectiles;

    if (Control.mode == _rendering_checkpoint_map)
        generate_false_automap(Control.origin_polygon_index);
            ...
            if (!POLYGON_IS_DETACHED(polygon)) {
                ...
                switch (polygon->type) {
                    case _polygon_is_platform:
                        color = PLATFORM_IS_SECRET(get_platform_data(polygon->permutation)) ? _polygon_color
                ...
                if (polygon->media_index != NONE) {
                    ...
                    if (media) {
                        if (media->height >= polygon->floor_height) {
                            switch (media->type) {
                                case _media_water:
                                    color = _polygon_water_color;
                                    break;
                                case _media_lava:
                                    color = _polygon_lava_color;
                                    break;
                                case _media_goo:
                                    color = _polygon_goo_color;
                                    break;
                                // LP change: separated sewage and JjaroGoo
                                case _media_sewage:
                                    color = _polygon_sewage_color;
                                    break;
                                case _media_jjaro:
                                    color = _polygon_jjaro_color;
                                    break;
                            }
                        }
                    }
    ...
    for (i = 0; i < dynamic_world->line_count; ++i) {
        ...
        if (LINE_IS_IN_AUTOMAP(i)) {
            if ((line->clockwise_polygon_owner != NONE
                 && TEST_STATE_FLAG(line->clockwise_polygon_owner, _polygon_on_automap))
                || (line->counterclockwise_polygon_owner != NONE
                    && TEST_STATE_FLAG(line->counterclockwise_polygon_owner, _polygon_on_automap))) {
                struct polygon_data* clockwise_polygon = line->clockwise_polygon_owner == NONE
                                                                 ? NULL
                                                                 : get_polygon_data(line->clockwise_polygon_owner);
                struct polygon_data* counterclockwise_polygon
                        = line->counterclockwise_polygon_owner == NONE
                                  ? NULL
                    ...
                    if (LINE_IS_LANDSCAPED(line)) {
                        if ((!clockwise_polygon || clockwise_polygon->floor_transfer_mode != _xfer_landscape)
                            && (!counterclockwise_polygon
                                || counterclockwise_polygon->floor_transfer_mode != _xfer_landscape)) {
    ...
    if (Control.mode != _rendering_checkpoint_map) {
                ...
                if (!OBJECT_IS_INVISIBLE(object)) {
                    ...
                    switch (GET_OBJECT_OWNER(object)) {
                        case _object_is_monster: {
                            ...
                            if (MONSTER_IS_PLAYER(monster)) {
                                struct player_data* player
                                        = get_player_data(monster_index_to_player_index(object->permutation));

                                if ((GET_GAME_OPTIONS() & _overhead_map_is_omniscient)
                                    || local_player->team == player->team) {
                        ...
                        case _object_is_garbage:
                            ...
                            switch (ConfigPtr->dead_monster_displays[GET_COLLECTION(
                                    GET_DESCRIPTOR_COLLECTION(object->shape))]) {
                                case _civilian_thing:
                                    thing_type = _civilian_thing;
                                    break;
        ...
        for (i = 0; i < dynamic_world->initial_objects_count; ++i) {
            ...
            if (saved_object->type == _saved_goal && saved_object->location.x == Control.origin.x
                && saved_object->location.y == Control.origin.y) {
    ...
    if (Control.mode == _rendering_game_map)
        draw_map_name(Control, static_world->level_name);
    if (Control.mode == _rendering_checkpoint_map)
...
void OverheadMapClass::transform_endpoints_for_overhead_map(struct overhead_map_data& Control) {
    world_distance x0 = Control.origin.x, y0 = Control.origin.y;
    int xoff = Control.left + Control.half_width, yoff = Control.top + Control.half_height;
    short scale = Control.scale;
    short i;
    ...
    for (i = 0; i < dynamic_world->endpoint_count; ++i) {
        ...
        if (endpoint->transformed.x >= Control.left && endpoint->transformed.y >= Control.top
...
static void add_poly_to_false_automap(short polygon_index) {
    ...
    for (int i = 0; i < polygon->vertex_count; ++i) ADD_LINE_TO_AUTOMAP(polygon->line_indexes[i]);
    ADD_POLYGON_TO_AUTOMAP(polygon_index);
}

void OverheadMapClass::generate_false_automap(short polygon_index) {
    int32 automap_line_buffer_size, automap_polygon_buffer_size;

    automap_line_buffer_size
            = (dynamic_world->line_count / 8 + ((dynamic_world->line_count % 8) ? 1 : 0)) * sizeof(byte);
    automap_polygon_buffer_size
...
void OverheadMapClass::replace_real_automap(void) {
    if (saved_automap_lines) {
        int32 automap_line_buffer_size
                = (dynamic_world->line_count / 8 + ((dynamic_world->line_count % 8) ? 1 : 0)) * sizeof(byte);
    ...
    if (saved_automap_polygons) {
        int32 automap_polygon_buffer_size
                = (dynamic_world->polygon_count / 8 + ((dynamic_world->polygon_count % 8) ? 1 : 0)) * sizeof(byte);
...
int32 OverheadMapClass::false_automap_cost_proc(short source_polygon_index, short line_index,
                                                short destination_polygon_index, void* caller_data) {
    ...
    if (destination_polygon->type == _polygon_is_platform) {
        ...
        if (PLATFORM_IS_DOOR(platform)) {
            if (PLATFORM_IS_SECRET(platform))
                cost = -1;
        }
    }
