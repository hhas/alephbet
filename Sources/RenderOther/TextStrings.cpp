 ...
 *  Implementation of MacOS STR# resource replacement
 */
...
bool TS_IsPresent(short ID) { return StringSetRoot.count(ID); }
...
void TS_DeleteStringSet(short ID) { StringSetRoot.erase(ID); }

// Deletes all of the stringsets
void TS_DeleteAllStrings() { StringSetRoot.clear(); }
...
static size_t DeUTF8(const char* InString, size_t InLen, char* OutString, size_t OutMaxLen) {
    ...
    for (size_t ic = 0; ic < InLen; ic++) {
        ...
        if (NumExtra == 0) {
            ...
            uint8 oc         = (uc >= 0x20 && uc != 0x7f && uc < 0x1'0000) ? unicode_to_mac_roman((uint16)uc) : '$';
