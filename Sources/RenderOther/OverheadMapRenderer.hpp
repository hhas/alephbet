...
class OverheadMapClass {
    ...
    static int32 false_automap_cost_proc(short source_polygon_index, short line_index, short destination_polygon_index,
                                         void* caller_data);
  ...
  protected:
    ...
    virtual void draw_polygon(short vertex_count, short* vertices, rgb_color& color) {}
    ...
    virtual void draw_player(world_point2d& center, angle facing, rgb_color& color, short shrink, short front,
                             short rear, short rear_theta) {}

    virtual void draw_text(world_point2d& location, rgb_color& color, char* text, FontSpecifier& FontData,
                           short justify) {}

    virtual void set_path_drawing(rgb_color& color) {}

    virtual void draw_path(short step, // 0: first point
  ...
  private:
    ...
    void draw_player(world_point2d* center, angle facing, short color, short scale) {
        ...
        draw_player(*center, facing, PlayerColor, OVERHEAD_MAP_MAXIMUM_SCALE - scale, EntityDef.front, EntityDef.rear,
                    EntityDef.rear_theta);
    }

    void draw_annotation(world_point2d* location, short color, char* text, short scale) {
        if (!(color >= 0 && color < NUMBER_OF_ANNOTATION_DEFINITIONS))
            return;
        if (!(scale >= OVERHEAD_MAP_MINIMUM_SCALE && scale <= OVERHEAD_MAP_MAXIMUM_SCALE))
            return;
    ...
    void draw_map_name(overhead_map_data& Control, char* name) {
    ...
    void set_path_drawing() { set_path_drawing(ConfigPtr->path_color); }
