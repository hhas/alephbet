...
void initialize_screen_drawing(void) {
    ...
    /* load the font stuff. */
    for (loop = 0; loop < NUMBER_OF_INTERFACE_FONTS; ++loop) { InterfaceFonts[loop].Init(); }
}
...
void _draw_screen_shape(shape_descriptor shape_id, screen_rectangle* destination, screen_rectangle* source) {
    ...
    SDL_Rect dst_rect = {destination->left, destination->top, destination->right - destination->left,
...
template <class T>
inline static int draw_glyph(uint8 c, int x, int y, T* p, int pitch, int clip_left, int clip_top, int clip_right,
                             int clip_bottom, uint32 pixel, const sdl_font_info* font, bool oblique) {
...
template <class T>
inline static int draw_text(const uint8* text, size_t length, int x, int y, T* p, int pitch, int clip_left,
                            int clip_top, int clip_right, int clip_bottom, uint32 pixel, const sdl_font_info* font,
                            uint16 style) {
    ...
    while (length--) {
        ...
        if (style & styleUnderline) {
            for (int i = 0; i < width; i++) p[y * pitch / sizeof(T) + x + i] = pixel;
        }
...
int sdl_font_info::_draw_text(SDL_Surface* s, const char* text, size_t length, int x, int y, uint32 pixel, uint16 style,
                              bool) const {
    ...
    switch (s->format->BytesPerPixel) {
        case 1:
            width = ::draw_text((const uint8*)text, length, x, y, (pixel8*)s->pixels, s->pitch, clip_left, clip_top,
                                clip_right, clip_bottom, pixel, this, style);
            break;
        case 2:
            width = ::draw_text((const uint8*)text, length, x, y, (pixel16*)s->pixels, s->pitch, clip_left, clip_top,
                                clip_right, clip_bottom, pixel, this, style);
            break;
        case 4:
            width = ::draw_text((const uint8*)text, length, x, y, (pixel32*)s->pixels, s->pitch, clip_left, clip_top,
                                clip_right, clip_bottom, pixel, this, style);
            break;
    }
    if (SDL_MUSTLOCK(s)) {
...
int ttf_font_info::_draw_text(SDL_Surface* s, const char* text, size_t length, int x, int y, uint32 pixel, uint16 style,
                              bool utf8) const {
    int clip_top, clip_bottom, clip_left, clip_right;
    if (draw_clip_rect_active) {
        clip_top    = draw_clip_rect.top;
    ...
    if (s == MainScreenSurface())
        MainScreenUpdateRect(x, y - TTF_FontAscent(get_ttf(style)), text_width(text, style, utf8),
                             TTF_FontHeight(get_ttf(style)));
...
void _draw_screen_text(const char* text, screen_rectangle* destination, short flags, short font_id, short text_color) {
    ...
    if (flags & _wrap_text) {
        ...
        if (count != len) {
            ...
            memcpy(remaining_text_to_draw, text_to_draw + last_non_printing_character + 1,
...
TextSpec* _get_font_spec(short font_index) { return &NullSpec; }
...
template <class T>
static inline void draw_thin_line_noclip(T* p, int pitch, const world_point2d* v1, const world_point2d* v2,
                                         uint32 pixel) {
