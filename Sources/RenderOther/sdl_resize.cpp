...
static inline void Resample(SDL_Surface* src, SDL_Surface* dst, int filter) {
    ...
    for (int i = 0; i < src->h; i++) temp[i] = new Uint32[dst->w];
    ...
    for (int x = 0; x < dst->w; ++x) {
        ...
        for (int y = 0; y < src->h; y++) {
            for (size_t n = 0; n < nmax; ++n) {
                if (y == 0) { // only come up with the contribution list once per column.
                ...
                for (int c = 0; c < 4; c++) point[c] += p[c] * contribution_x[n];
            }
    ...
    for (int y = 0; y < dst->h; ++y) {
        ...
        for (int x = 0; x < dst->w; x++) {
            for (size_t n = 0; n < nmax; ++n) {
                ...
                for (int c = 0; c < 4; c++) point[c] += p[c] * contribution_y[n];
            }
    ...
    for (int i = 0; i < src->h; i++) delete[] temp[i];
    delete[] temp;
}
...
SDL_Surface* SDL_Resize(SDL_Surface* src, float factor, bool free, int filter) {
    if (factor > 100.0f)
        factor = 100.0f; // let's be reasonable...
    int new_w = (int)(src->w * factor), new_h = (int)(src->h * factor);
    if (new_w < 1)
        new_w = 1;
    if (new_h < 1)
        new_h = 1;
...
SDL_Surface* SDL_Resize(SDL_Surface* src, int new_w, int new_h, bool free, int filter) {
    ...
    Uint32 rmask = 0x0000'00ff, gmask = 0x0000'ff00, bmask = 0x00ff'0000, amask = 0xff00'0000;
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
    rmask = 0xff00'0000;
    gmask = 0x00ff'0000;
    bmask = 0x0000'ff00;
