...
enum {
    ...
    _scenario_file_delta16 = 10'000,
    _scenario_file_delta32 = 20'000
};
...
static int uncompress_picture(const uint8* src, int row_bytes, uint8* dst, int dst_pitch, int depth, int height,
                              int pack_type) {
...
no_packing:
    ...
    if (depth < 8) {
        ...
        int x_max = row_bytes;
        while (x_max * 8 / depth > orig_dst_pitch) x_max--;
        ...
        switch (opcode) {
            ...
            case 0x009b: { // Direct CopyBits with clipping region
                ...
                switch (pixel_size) {
                    ...
                    case 8:
                        Rmask = Gmask = Bmask = 0;
                        surface_depth         = 8; // SDL surfaces must be at least 8 bits depth, so we expand 1/2/4-bit
                                                   // pictures to 8-bit
                        break;
                    case 16:
                        Rmask         = 0x7c00;
                ...
                if (done)
                    break;
                SDL_Surface* bm
                        = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, surface_depth, Rmask, Gmask, Bmask, 0);
                if (bm == NULL) {
                    done = true;
                    break;
                }
                ...
                // 5. graphics data
                int data_size = uncompress_picture((uint8*)rsrc.GetPointer() + SDL_RWtell(p), row_bytes,
                                                   (uint8*)bm->pixels, bm->pitch, pixel_size, height, pack_type);
                if (data_size < 0) {
                    done = true;
                    break;
                }
            ...
            case 0x8200: { // Compressed QuickTime image (we only handle JPEG compression)
                ...
                if (codec_type != FOUR_CHARS_TO_INT('j', 'p', 'e', 'g')) {
                    fprintf(stderr, "Unsupported codec type %c%c%c%c\n", codec_type >> 24, codec_type >> 16,
                            codec_type >> 8, codec_type);
                    done = true;
                    break;
                }
                SDL_RWseek(
                        p, 36,
                        SEEK_CUR); // resvd1/resvd2/dataRefIndex/version/revisionLevel/vendor/temporalQuality/spatialQuality/width/height/hRes/vRes
                ...
                if (!s) {
                    s.reset(SDL_CreateRGBSurface(SDL_SWSURFACE, pic_width, pic_height, 32,
                                                 ...
                                                 0x0000'00ff, 0x0000'ff00, 0x00ff'0000, 0xff00'0000
#else
                                                 0xff00'0000, 0x00ff'0000, 0x0000'ff00, 0x0000'00ff
#endif
                                                 ));
                    if (!s) {
                        done = true;
...
template <class T>
static void rescale(T* src_pixels, int src_pitch, T* dst_pixels, int dst_pitch, int width, int height, uint32 dx,
                    uint32 dy) {
...
SDL_Surface* rescale_surface(SDL_Surface* s, int width, int height) {
    ...
    SDL_Surface* s2 = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, s->format->BitsPerPixel, s->format->Rmask,
...
template <class T>
static void tile(T* src_pixels, int src_pitch, T* dst_pixels, int dst_pitch, int src_width, int src_height,
...
static void draw_picture(LoadedResource& rsrc) { draw_picture_surface(picture_to_surface(rsrc)); }
...
void scroll_full_screen_pict_resource_from_scenario(int pict_resource_number, bool text_block) {
    ...
    if (scroll_horizontal || scroll_vertical) {
        ...
        SDL_Rect src_rect = {0, 0, scroll_horizontal ? screen_width : picture_width,
                             scroll_vertical ? screen_height : picture_height};
...
void set_scenario_images_file(FileSpecifier& file) { ScenarioFile.open_file(file); }

void unset_scenario_images_file() { ScenarioFile.close_file(); }

void set_shapes_images_file(FileSpecifier& file) { ShapesImagesFile.open_file(file); }
...
void set_external_resources_images_file(FileSpecifier& file) {
    ...
    if (!file.Exists() || !ExternalResourcesFile.open_file(file)) {
        ...
        if ((!file.Exists() || !ExternalResourcesFile.open_file(file)) && !ImagesFile.is_open()) {
            alert_bad_extra_file();
        }
    }
}

void set_sounds_images_file(FileSpecifier& file) { SoundsImagesFile.open_file(file); }
...
bool image_file_t::open_file(FileSpecifier& file) {
    ...
    if (!file.Open(rsrc_file)) {
        ...
        if (!open_wad_file_for_reading(file, wad_file) || !read_wad_header(wad_file, &wad_hdr)) {
...
bool image_file_t::is_open(void) { return rsrc_file.IsOpen() || wad_file.IsOpen(); }
...
bool image_file_t::has_pict(int id) {
    return has_rsrc(FOUR_CHARS_TO_INT('P', 'I', 'C', 'T'), FOUR_CHARS_TO_INT('P', 'I', 'C', 'T'), id)
           || has_rsrc(FOUR_CHARS_TO_INT('P', 'I', 'C', 'T'), FOUR_CHARS_TO_INT('p', 'i', 'c', 't'), id);
}
...
bool get_picture_resource_from_images(int base_resource, LoadedResource& PictRsrc) {
    ...
    if (!found && ImagesFile.is_open())
        found = ImagesFile.get_pict(
                ImagesFile.determine_pict_resource_id(base_resource, _images_file_delta16, _images_file_delta32),
                PictRsrc);
...
static void create_m1_menu_surfaces(void) {
    ...
    if (PlatformIsLittleEndian()) {
        s.reset(SDL_CreateRGBSurface(SDL_SWSURFACE, 640, 480, 32, 0x0000'00ff, 0x0000'ff00, 0x00ff'0000, 0));
    } else {
        s.reset(SDL_CreateRGBSurface(SDL_SWSURFACE, 640, 480, 32, 0xff00'0000, 0x00ff'0000, 0x0000'ff00, 0));
    }
    if (!s)
        return;
...
bool get_picture_resource_from_scenario(int base_resource, LoadedResource& PictRsrc) {
    ...
    if (!found && ScenarioFile.is_open()) {
        auto id = ScenarioFile.determine_pict_resource_id(base_resource, _scenario_file_delta16,
                                                          _scenario_file_delta32);
        found   = Plugins::instance()->get_resource(FOUR_CHARS_TO_INT('P', 'I', 'C', 'T'), id, PictRsrc);
        if (!found) {
            found = ScenarioFile.get_pict(ScenarioFile.determine_pict_resource_id(base_resource, _scenario_file_delta16,
                                                                                  _scenario_file_delta32),
                                          PictRsrc);
        }
    }
...
bool image_file_t::make_rsrc_from_pict(void* data, size_t length, LoadedResource& rsrc, void* clut_data,
                                       size_t clut_length) {
    if (length < 10)
        return false;
    ...
    if (depth == 8) {
        q[0]  = 0x00;
        q[1]  = 0x98; // PackBitsRect
        q    += 2;
    } else {
        q[0]  = 0x00;
        q[1]  = 0x9a; // DirectBitsRect
        q    += 6;    // skip pmBaseAddr
    }
