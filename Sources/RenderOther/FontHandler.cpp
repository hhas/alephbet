...
void FontSpecifier::Update() {
    ...
    if (Info) {
        ...
        for (int k = 0; k < 256; k++) Widths[k] = char_width(k, Info, Style);
    } else
...
int FontSpecifier::TextWidth(const char* text) {
    ...
    if (!text)
        return width;
    while ((c = *text++) != 0) width += Widths[static_cast<unsigned char>(c)];
    return width;
}
...
void FontSpecifier::OGL_Reset(bool IsStarting) {
    ...
    int widths_p[256];
    for (int i = 0; i < 256; i++) { widths_p[i] = Widths[i] + 2 * Pad; }
    // Now for the totals and dimensions
    int TotalWidth = 0;
    for (int k = 0; k < 256; k++) TotalWidth += widths_p[k];
    ...
    SDL_Surface* FontSurface
            = SDL_CreateRGBSurface(SDL_SWSURFACE, TxtrWidth, TxtrHeight, 32, 0xff'0000, 0x00'ff00, 0x00'00ff, 0);
    if (FontSurface == NULL)
        return;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE_ALPHA, TxtrWidth, TxtrHeight, 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE,
                 OGL_Texture);
    ...
    for (int k = 0; k <= LastLine; k++) {
        ...
        for (int m = 0; m < CharCounts[k]; m++) {
            ...
            OGL_RenderTexturedRect(0, -ascent_p, Width, descent_p + ascent_p, Left, Top, Right, Bottom);
...
void FontSpecifier::OGL_ResetFonts(bool IsStarting) {
    ...
    if (IsStarting) {
        for (it = m_font_registry->begin(); it != m_font_registry->end(); ++it) (*it)->OGL_Reset(IsStarting);
    } else {
        for (it = m_font_registry->begin(); it != m_font_registry->end(); it = m_font_registry->begin())
