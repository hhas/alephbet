...
#include "HUDRenderer_Lua.hpp"
#include "Movie.hpp"
#include "lua_hud_script.hpp"
...
// void initialize_screen(struct screen_mode_data *mode, bool ShowFreqDialog)
{
    ...
    if (!screen_initialized) {
        ...
        Intro_Buffer = SDL_CreateRGBSurface(SDL_SWSURFACE, 640, 480, 32, pixel_format_32.Rmask, pixel_format_32.Gmask,
                                            pixel_format_32.Bmask, 0);
        Intro_Buffer_corrected = SDL_CreateRGBSurface(SDL_SWSURFACE, 640, 480, 32, pixel_format_32.Rmask,
                                                      pixel_format_32.Gmask, pixel_format_32.Bmask, 0);
        ...
        if (1) {
            ...
            for (std::vector<std::pair<int, int>>::const_iterator it = common_modes.begin(); it != common_modes.end();
                 ++it) {
                if (it->first <= m_modes[0].first && it->second <= m_modes[0].second
                    && !(it->first == m_modes[0].first && it->second == m_modes[0].second)) {
        ...
        if (graphics_preferences->screen_mode.width <= m_modes[0].first
            && graphics_preferences->screen_mode.height <= m_modes[0].second) {
            // sort it into the list
            for (std::vector<std::pair<int, int>>::iterator it = m_modes.begin(); it != m_modes.end(); ++it) {
                if (graphics_preferences->screen_mode.width >= it->first
                    && graphics_preferences->screen_mode.height >= it->second) {
                    if (graphics_preferences->screen_mode.width != it->first
                        || graphics_preferences->screen_mode.height != it->second) {
                        m_modes.insert(it, std::pair<int, int>(graphics_preferences->screen_mode.width,
        ...
        // validate them here
        if (Screen::instance()->FindMode(graphics_preferences->screen_mode.width,
                                         graphics_preferences->screen_mode.height)
            < 0) {
...
int Screen::height() { return MainScreenLogicalHeight(); }

int Screen::width() { return MainScreenLogicalWidth(); }

float Screen::pixel_scale() { return MainScreenPixelScale(); }

int Screen::window_height() { return std::max(static_cast<short>(480), screen_mode.height); }

int Screen::window_width() { return std::max(static_cast<short>(640), screen_mode.width); }

bool Screen::hud() { return screen_mode.hud; }

bool Screen::lua_hud() { return screen_mode.hud && LuaHUDRunning(); }

bool Screen::openGL() { return screen_mode.acceleration != _no_acceleration; }

bool Screen::fifty_percent() { return screen_mode.height == 160; }

bool Screen::seventyfive_percent() { return screen_mode.height == 240; }
...
SDL_Rect Screen::OpenGLViewPort() { return m_viewport_rect; }
...
void Screen::scissor_screen_to_rect(SDL_Rect& r) {
    ...
    if (MainScreenIsOpenGL()) {
        ...
        glScissor(m_viewport_rect.x + (r.x * m_viewport_rect.w / m_ortho_rect.w),
                  ...
                  r.w * m_viewport_rect.w / m_ortho_rect.w, r.h * m_viewport_rect.h / m_ortho_rect.h);
    }
#endif
}
...
static void reallocate_world_pixels(int width, int height) {
    ...
    //	world_pixels = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, f->BitsPerPixel, f->Rmask, f->Gmask, f->Bmask,
    //f->Amask);
    switch (bit_depth) {
        case 8:
            ...
            break;
        case 16:
            world_pixels = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 16, pixel_format_16.Rmask,
                                                pixel_format_16.Gmask, pixel_format_16.Bmask, 0);
            break;
        default:
            world_pixels = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 32, pixel_format_32.Rmask,
                                                pixel_format_32.Gmask, pixel_format_32.Bmask, 0);
            break;
    }
    ...
    if (world_pixels == NULL)
        ...
        world_pixels_corrected = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, world_pixels->format->BitsPerPixel,
                                                      world_pixels->format->Rmask, world_pixels->format->Gmask,
                                                      world_pixels->format->Bmask, 0);
}
...
static void reallocate_map_pixels(int width, int height) {
    ...
    Map_Buffer = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, main_surface->format->BitsPerPixel,
                                      main_surface->format->Rmask, main_surface->format->Gmask,
                                      main_surface->format->Bmask, 0);
    if (Map_Buffer == NULL)
        alert_out_of_memory();
    if (map_is_translucent()) {
...
bool map_is_translucent(void) { return (screen_mode.translucent_map && NetAllowOverlayMap()); }
...
static void change_screen_mode(int width, int height, int depth, bool nogl, bool force_menu, bool force_resize_hud) {
    ...
    //		main_texture = SDL_CreateTexture(main_render, pixel_format_32.format, SDL_TEXTUREACCESS_STREAMING,
    //vmode_width, vmode_height);
    //	}
    //	main_surface = SDL_CreateRGBSurface(SDL_SWSURFACE, vmode_width, vmode_height, 32, pixel_format_32.Rmask,
    //pixel_format_32.Gmask, pixel_format_32.Bmask, 0);
    ...
    if (need_mode_change(sdl_width, sdl_height, vmode_width, vmode_height, depth, nogl)) {
        ...
        main_screen = SDL_CreateWindow(get_application_name().c_str(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                                       sdl_width, sdl_height, flags);

        bool context_created = false;
#ifdef HAVE_OPENGL
        if (main_screen == NULL && !nogl && screen_mode.acceleration != _no_acceleration
            && Get_OGL_ConfigureData().Multisamples > 0) {
            // retry with multisampling off
            SDL_GL_SetAttribute(SDL_GL_MULTISAMPLEBUFFERS, 0);
            SDL_GL_SetAttribute(SDL_GL_MULTISAMPLESAMPLES, 0);
            main_screen = SDL_CreateWindow(get_application_name().c_str(), SDL_WINDOWPOS_CENTERED,
                                           SDL_WINDOWPOS_CENTERED, sdl_width, sdl_height, flags);
            if (main_screen)
        ...
        if (main_screen == NULL && (flags & SDL_WINDOW_FULLSCREEN_DESKTOP)) {
            ...
            main_screen      = SDL_CreateWindow(get_application_name().c_str(), SDL_WINDOWPOS_CENTERED,
                                                SDL_WINDOWPOS_CENTERED, vmode_width, vmode_height, tempflags);
            if (main_screen) {
        ...
        if (main_screen == NULL && (flags & SDL_WINDOW_OPENGL)) {
            ...
            main_screen      = SDL_CreateWindow(get_application_name().c_str(), SDL_WINDOWPOS_CENTERED,
                                                SDL_WINDOWPOS_CENTERED, sdl_width, sdl_height, tempflags);
            if (main_screen) {
                screen_mode.acceleration = graphics_preferences->screen_mode.acceleration = _no_acceleration;
            }
        }
    ...
    screen_rectangle* term_rect = get_interface_rectangle(_terminal_screen_rect);
    Term_Buffer = SDL_CreateRGBSurface(SDL_SWSURFACE, RECTANGLE_WIDTH(term_rect), RECTANGLE_HEIGHT(term_rect), 32,
                                       pixel_format_32.Rmask, pixel_format_32.Gmask, pixel_format_32.Bmask,
    ...
    if (in_game && (screen_mode.hud && (prev_width != main_surface->w || prev_height != main_surface->h))
        || force_resize_hud) {
        L_Call_HUDResize();
    }
}
...
bool get_auto_resolution_size(short* w, short* h, struct screen_mode_data* mode) {
    if (screen_mode.auto_resolution) {
        ...
        if (!screen_mode.fullscreen && ((width > 640) || (height > 480))
...
void update_world_view_camera() {
    ...
    if (!use_cameras) {
        world_view->show_weapons_in_hand = !ChaseCam_GetPosition(world_view->origin, world_view->origin_polygon_index,
...
void render_screen(short ticks_elapsed) {
    ...
    if (screen_mode.acceleration == _no_acceleration && (MapIsTranslucent || Screen::instance()->lua_hud()))
        clear_screen_margin();
...
#endif
        SDL_Rect rect = {(Screen::instance()->window_rect().w - ViewRect.w) / 2,
                         (Screen::instance()->window_rect().h - ViewRect.h) / 2, 0, 0};
    ...
    if (screen_mode.acceleration != _no_acceleration) {
        ...
        // Update world window
        if (!world_view->terminal_mode_active && (!world_view->overhead_map_active || MapIsTranslucent))
        ...
        if (update_full_screen || Screen::instance()->lua_hud()) {
            MainScreenUpdateRect(0, 0, 0, 0);
        } else if ((!world_view->overhead_map_active || MapIsTranslucent) && !world_view->terminal_mode_active) {
...
static inline void quadruple_surface(const T* src, int src_pitch, T* dst, int dst_pitch, const SDL_Rect& dst_rect,
    ...
    bool overlay_active = world_view->overhead_map_active && map_is_translucent();
    ...
    while (height-- > 0) {
        if (every_other_line) {
            if (overlay_active) {
                ...
                for (int x = 0; x < width; x++) { dst[x * 2] = dst[x * 2 + 1] = src[x]; }
            }
        } else {
            for (int x = 0; x < width; x++) {
                T p        = src[x];
...
static void apply_gamma(SDL_Surface* src, SDL_Surface* dst) {
    ...
    for (size_t i = 0; i < numpixels; ++i) {
        ...
        dst_px = (((dst_r >> drl) << drs) & drm) | (((dst_g >> dgl) << dgs) & dgm) | (((dst_b >> dbl) << dbs) & dbm);
...
static inline bool pixel_formats_equal(SDL_PixelFormat* a, SDL_PixelFormat* b) {
    return (a->BytesPerPixel == b->BytesPerPixel && a->Rmask == b->Rmask && a->Gmask == b->Gmask
            && a->Bmask == b->Bmask);
}
...
static void update_screen(SDL_Rect& source, SDL_Rect& destination, bool hi_rez, bool every_other_line) {
    ...
    if (hi_rez) {
        ...
        switch (s->format->BytesPerPixel) {
            case 1:
                quadruple_surface((pixel8*)s->pixels, s->pitch, (pixel8*)main_surface->pixels, main_surface->pitch,
                                  destination, every_other_line);
                break;
            case 2:
                quadruple_surface((pixel16*)s->pixels, s->pitch, (pixel16*)main_surface->pixels, main_surface->pitch,
                                  destination, every_other_line);
                break;
            case 4:
                quadruple_surface((pixel32*)s->pixels, s->pitch, (pixel32*)main_surface->pixels, main_surface->pitch,
                                  destination, every_other_line);
                break;
        }
...
void initialize_gamma(void) {
    if (!default_gamma_inited) {
        ...
        for (int i = 0; i < 256; ++i) { default_gamma_r[i] = default_gamma_g[i] = default_gamma_b[i] = i << 8; }
...
static inline void draw_pattern_rect(T* p, int pitch, uint32 pixel, const SDL_Rect& r) {
    ...
    for (int y = 0; y < r.h; y++) {
        for (int x = y & 1; x < r.w; x += 2) p[x] = pixel;
        p += pitch / sizeof(T);
    }
}
...
void validate_world_window(void) { RequestDrawingTerm(); }
...
bool MainScreenVisible() { return (main_screen != NULL); }

int MainScreenLogicalWidth() { return (main_surface ? main_surface->w : 0); }

int MainScreenLogicalHeight() { return (main_surface ? main_surface->h : 0); }
...
float MainScreenPixelScale() { return MainScreenPixelWidth() / static_cast<float>(MainScreenWindowWidth()); }

bool MainScreenIsOpenGL() { return (main_screen && !main_render); }

void MainScreenSwap() { SDL_GL_SwapWindow(main_screen); }
...
SDL_Surface* MainScreenSurface() { return main_surface; }
