...
#include "map.hpp" // for TICKS_PER_SECOND
#include "screen.hpp"
...
#define FADES_RANDOM() \
    ((fades_random_seed & 1) ? (fades_random_seed = (fades_random_seed >> 1) ^ 0xb400) : (fades_random_seed >>= 1))

/* ---------- types */

typedef void (*fade_proc)(struct color_table* original_color_table, struct color_table* animated_color_table,
                          struct rgb_color* color, _fixed transparency);
...
static void tint_color_table(struct color_table* original_color_table, struct color_table* animated_color_table,
                             struct rgb_color* color, _fixed transparency);
static void randomize_color_table(struct color_table* original_color_table, struct color_table* animated_color_table,
                                  struct rgb_color* color, _fixed transparency);
static void negate_color_table(struct color_table* original_color_table, struct color_table* animated_color_table,
                               struct rgb_color* color, _fixed transparency);
static void dodge_color_table(struct color_table* original_color_table, struct color_table* animated_color_table,
                              struct rgb_color* color, _fixed transparency);
static void burn_color_table(struct color_table* original_color_table, struct color_table* animated_color_table,
                             struct rgb_color* color, _fixed transparency);
static void soft_tint_color_table(struct color_table* original_color_table, struct color_table* animated_color_table,
                                  struct rgb_color* color, _fixed transparency);
...
static struct fade_definition fade_definitions[NUMBER_OF_FADE_TYPES] = {
  ...
  */
        ...
        {tint_color_table,      {65'535, 0, 0},            (3 * FIXED_ONE) / 4, 0,         MACHINE_TICKS_PER_SECOND / 4,       0,                 0}, /* _fade_red */
        {tint_color_table,      {65'535, 0, 0},            FIXED_ONE,           0,         (3 * MACHINE_TICKS_PER_SECOND) / 4, 0,                 0}, /* _fade_big_red */
        {tint_color_table,      {0, 65'535, 0},            FIXED_ONE_HALF,      0,         MACHINE_TICKS_PER_SECOND / 4,       0,                 0}, /* _fade_bonus */
        {tint_color_table,      {65'535, 65'535, 50'000},  FIXED_ONE,           0,         MACHINE_TICKS_PER_SECOND / 3,       0,                 0}, /* _fade_bright
  */
        {tint_color_table,      {65'535, 65'535, 50'000},  FIXED_ONE,           0,         4 * MACHINE_TICKS_PER_SECOND,       0,                 1}, /* _fade_long_bright
  */
        {tint_color_table,      {65'535, 65'535, 0},       FIXED_ONE,           0,         MACHINE_TICKS_PER_SECOND / 2,       0,                 0}, /* _fade_yellow */
        {tint_color_table,      {65'535, 65'535, 0},       FIXED_ONE,           0,         MACHINE_TICKS_PER_SECOND,           0,                 0}, /* _fade_big_yellow */
        {tint_color_table,
         {215 * 256, 107 * 256, 65'535},
         (3 * FIXED_ONE) / 4,
         0,                                                                                MACHINE_TICKS_PER_SECOND / 4,
         0,                                                                                                                                       0}, /* _fade_purple */
        {tint_color_table,
         {169 * 256, 65'535, 224 * 256},
         (3 * FIXED_ONE) / 4,
         0,                                                                                MACHINE_TICKS_PER_SECOND / 2,
         0,                                                                                                                                       0}, /* _fade_cyan */
        {tint_color_table,      {65'535, 65'535, 65'535},  FIXED_ONE_HALF,      0,         MACHINE_TICKS_PER_SECOND / 4,       0,                 0}, /* _fade_white
  */
        {tint_color_table,      {65'535, 65'535, 65'535},  FIXED_ONE,           0,         MACHINE_TICKS_PER_SECOND / 2,       0,                 0}, /* _fade_big_white
  */
        {tint_color_table,      {65'535, 32'768, 0},       FIXED_ONE,           0,         MACHINE_TICKS_PER_SECOND / 4,       0,                 0}, /* _fade_orange */
        {tint_color_table,      {65'535, 32'768, 0},       FIXED_ONE / 4,       0,         3 * MACHINE_TICKS_PER_SECOND,       0,                 0}, /* _fade_long_orange
  */
        {tint_color_table,      {0, 65'535, 0},            3 * FIXED_ONE / 4,   0,         MACHINE_TICKS_PER_SECOND / 2,       0,                 0}, /* _fade_green */
        {tint_color_table,      {65'535, 0, 65'535},       FIXED_ONE / 4,       0,         3 * MACHINE_TICKS_PER_SECOND,       0,                 0}, /* _fade_long_green
  */

        {randomize_color_table, {0, 0, 0},                 FIXED_ONE,           0,         (3 * MACHINE_TICKS_PER_SECOND) / 8, 0,                 0}, /* _fade_static */
        {negate_color_table,    {65'535, 65'535, 65'535},  FIXED_ONE,           0,         MACHINE_TICKS_PER_SECOND / 2,       0,                 0}, /* _fade_negative
  */
        {negate_color_table,    {65'535, 65'535, 65'535},  FIXED_ONE,           0,         (3 * MACHINE_TICKS_PER_SECOND) / 2, 0,                 0}, /* _fade_big_negative
  */
        {negate_color_table,
         {0, 65'535, 0},
         FIXED_ONE,                                                             0,
         MACHINE_TICKS_PER_SECOND / 2,
         _random_transparency_flag,                                                                                                               0}, /* _fade_flicker_negative */
        {dodge_color_table,     {0, 65'535, 0},            FIXED_ONE,           0,         (3 * MACHINE_TICKS_PER_SECOND) / 4, 0,                 0}, /* _fade_dodge_purple
  */
        {burn_color_table,      {0, 65'535, 65'535},       FIXED_ONE,           0,         MACHINE_TICKS_PER_SECOND,           0,                 0}, /* _fade_burn_cyan */
        {dodge_color_table,     {0, 0, 65'535},            FIXED_ONE,           0,         (3 * MACHINE_TICKS_PER_SECOND) / 2, 0,                 0}, /* _fade_dodge_yellow
  */
        {burn_color_table,      {0, 65'535, 0},            FIXED_ONE,           0,         2 * MACHINE_TICKS_PER_SECOND,       0,                 0}, /* _fade_burn_green */

        {soft_tint_color_table, {137 * 256, 0, 137 * 256}, FIXED_ONE,           0,         2 * MACHINE_TICKS_PER_SECOND,       0,                 0}, /* _fade_tint_purple
  */
        {soft_tint_color_table, {0, 0, 65'535},            FIXED_ONE,           0,         2 * MACHINE_TICKS_PER_SECOND,       0,                 0}, /* _fade_tint_blue */
        {soft_tint_color_table, {65'535, 16'384, 0},       FIXED_ONE,           0,         2 * MACHINE_TICKS_PER_SECOND,       0,                 0}, /* _fade_tint_orange
  */
        {soft_tint_color_table, {32'768, 65'535, 0},       FIXED_ONE,           0,         2 * MACHINE_TICKS_PER_SECOND,       0,                 0}, /* _fade_tint_gross
  */
...
struct fade_effect_definition fade_effect_definitions[NUMBER_OF_FADE_EFFECT_TYPES] = {
        ...
        {_fade_tint_jjaro,  3 * FIXED_ONE / 4},
 /* _effect_under_jjaro */  // LP addition
        {_fade_tint_green,  3 * FIXED_ONE / 4}, /* _effect_under_goo */
};

static float actual_gamma_values[NUMBER_OF_GAMMA_LEVELS] = {1.3F,  1.15F,
                                                            1.0F, // default
                                                            0.95F, 0.90F, 0.85F, 0.77F, 0.70F};

/* ---------- private prototypes */

static fade_definition* get_fade_definition(const short index);

static fade_effect_definition* get_fade_effect_definition(const short index);

static void recalculate_and_display_color_table(short type, _fixed transparency,
                                                struct color_table* original_color_table,
                                                struct color_table* animated_color_table, bool fade_active);
...
// Accessors:
fade_definition* get_fade_definition(const short index) {
    return GetMemberWithBounds(fade_definitions, index, NUMBER_OF_FADE_TYPES);
}

fade_effect_definition* get_fade_effect_definition(const short index) {
...
bool update_fades(bool game_in_progress) {
    if (FADE_IS_ACTIVE(fade)) {
        ...
        short phase;
        if (game_in_progress) {
            phase = (dynamic_world->tick_count - fade->last_update_game_tick) * MACHINE_TICKS_PER_SECOND
                    / TICKS_PER_SECOND;
        } else {
        ...
        if (phase >= definition->period) {
            ...
            transparency = definition->initial_transparency
                           + (phase * (definition->final_transparency - definition->initial_transparency))
                                     / definition->period;
            if (definition->flags & _random_transparency_flag)
                transparency += FADES_RANDOM() % (definition->final_transparency - transparency);
        }

        recalculate_and_display_color_table(fade->type, transparency, fade->original_color_table,
                                            fade->animated_color_table, FADE_IS_ACTIVE(fade));
...
void SetFadeEffectDelay(int _FadeEffectDelay) { FadeEffectDelay = _FadeEffectDelay; }
...
void start_fade(short type) { explicit_start_fade(type, world_color_table, visible_color_table, true); }
...
void explicit_start_fade(short type, struct color_table* original_color_table, struct color_table* animated_color_table,
    ...
    if (FADE_IS_ACTIVE(fade)) {
        ...
        // LP change: idiot-proofing
        if (old_definition) {
            if (old_definition->priority > definition->priority)
                do_fade = false;
            if (fade->type == type) {
                short phase;
                if (game_in_progress) {
    ...
    if (do_fade) {
        ...
        recalculate_and_display_color_table(type, definition->initial_transparency, original_color_table,
                                            animated_color_table, definition->period);
...
void stop_fade(void) {
    if (FADE_IS_ACTIVE(fade)) {
        ...
        recalculate_and_display_color_table(fade->type, definition->final_transparency, fade->original_color_table,
                                            fade->animated_color_table, false);

        SET_FADE_ACTIVE_STATUS(fade, false);
    }
}

bool fade_finished(void) { return FADE_IS_ACTIVE(fade) ? false : true; }
...
float get_actual_gamma_adjust(short gamma_level) { return actual_gamma_values[gamma_level]; }

bool fade_blacked_screen(void) {
    return (!FADE_IS_ACTIVE(fade)
            && (last_fade_type == _start_cinematic_fade_in || last_fade_type == _cinematic_fade_out));
}
...
static void recalculate_and_display_color_table(short type, _fixed transparency,
    ...
    if (fade->fade_effect_type != NONE) {
        ...
        definition->proc(original_color_table, animated_color_table, &definition->color,
                         effect_definition->transparency);
...
static void tint_color_table(struct color_table* original_color_table, struct color_table* animated_color_table,
    ...
    for (i = 0; i < original_color_table->color_count; ++i, ++adjusted, ++unadjusted) {
        adjusted->red = unadjusted->red
                        + (((color->red - unadjusted->red) * adjusted_transparency)
                           >> (FIXED_FRACTIONAL_BITS - ADJUSTED_TRANSPARENCY_DOWNSHIFT));
        adjusted->green = unadjusted->green
                          + (((color->green - unadjusted->green) * adjusted_transparency)
                             >> (FIXED_FRACTIONAL_BITS - ADJUSTED_TRANSPARENCY_DOWNSHIFT));
        adjusted->blue = unadjusted->blue
                         + (((color->blue - unadjusted->blue) * adjusted_transparency)
                            >> (FIXED_FRACTIONAL_BITS - ADJUSTED_TRANSPARENCY_DOWNSHIFT));
    }
}
...
static void dodge_color_table(struct color_table* original_color_table, struct color_table* animated_color_table,
    ...
    for (i = 0; i < original_color_table->color_count; ++i, ++adjusted, ++unadjusted) {
        ...
        component = 0xffff - (int32(1LL * (color->red ^ 0xffff) * unadjusted->red) >> FIXED_FRACTIONAL_BITS)
                    - transparency,
        adjusted->red = CEILING(component, unadjusted->red);
        component     = 0xffff - (int32(1LL * (color->green ^ 0xffff) * unadjusted->green) >> FIXED_FRACTIONAL_BITS)
                    - transparency,
        adjusted->green = CEILING(component, unadjusted->green);
        component       = 0xffff - (int32(1LL * (color->blue ^ 0xffff) * unadjusted->blue) >> FIXED_FRACTIONAL_BITS)
                    - transparency,
...
static void burn_color_table(struct color_table* original_color_table, struct color_table* animated_color_table,
    ...
    for (i = 0; i < original_color_table->color_count; ++i, ++adjusted, ++unadjusted) {
        ...
        component       = (int32(1LL * color->red * unadjusted->red) >> FIXED_FRACTIONAL_BITS) + transparency,
        adjusted->red   = CEILING(component, unadjusted->red);
        component       = (int32(1LL * color->green * unadjusted->green) >> FIXED_FRACTIONAL_BITS) + transparency,
        adjusted->green = CEILING(component, unadjusted->green);
        component       = (int32(1LL * color->blue * unadjusted->blue) >> FIXED_FRACTIONAL_BITS) + transparency,
        adjusted->blue  = CEILING(component, unadjusted->blue);
    }
}
...
static void soft_tint_color_table(struct color_table* original_color_table, struct color_table* animated_color_table,
    ...
    for (i = 0; i < original_color_table->color_count; ++i, ++adjusted, ++unadjusted) {
        ...
        adjusted->red = unadjusted->red
                        + (((((color->red * intensity) >> (FIXED_FRACTIONAL_BITS - ADJUSTED_TRANSPARENCY_DOWNSHIFT))
                             - unadjusted->red)
                            * adjusted_transparency)
                           >> (FIXED_FRACTIONAL_BITS - ADJUSTED_TRANSPARENCY_DOWNSHIFT));
        adjusted->green = unadjusted->green
                          + (((((color->green * intensity) >> (FIXED_FRACTIONAL_BITS - ADJUSTED_TRANSPARENCY_DOWNSHIFT))
                               - unadjusted->green)
                              * adjusted_transparency)
                             >> (FIXED_FRACTIONAL_BITS - ADJUSTED_TRANSPARENCY_DOWNSHIFT));
        adjusted->blue = unadjusted->blue
                         + (((((color->blue * intensity) >> (FIXED_FRACTIONAL_BITS - ADJUSTED_TRANSPARENCY_DOWNSHIFT))
                              - unadjusted->blue)
                             * adjusted_transparency)
                            >> (FIXED_FRACTIONAL_BITS - ADJUSTED_TRANSPARENCY_DOWNSHIFT));
    }
...
void reset_mml_faders() {
    if (original_fade_definitions) {
        for (int i = 0; i < NUMBER_OF_FADE_TYPES; i++) fade_definitions[i] = original_fade_definitions[i];
...
void parse_mml_faders(const InfoTree& root) {
    ...
    if (!original_fade_definitions) {
        original_fade_definitions
                = (struct fade_definition*)malloc(sizeof(struct fade_definition) * NUMBER_OF_FADE_TYPES);
        assert(original_fade_definitions);
        for (int i = 0; i < NUMBER_OF_FADE_TYPES; i++) original_fade_definitions[i] = fade_definitions[i];
    }

    if (!original_fade_effect_definitions) {
        original_fade_effect_definitions = (struct fade_effect_definition*)malloc(sizeof(struct fade_effect_definition)
    ...
    for (const InfoTree& ftree : root.children_named("fader")) {
        ...
        for (const InfoTree& color : ftree.children_named("color")) color.read_color(def.color);
    }
