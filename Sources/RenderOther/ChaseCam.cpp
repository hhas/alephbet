...
bool ChaseCam_CanExist() { return !TEST_FLAG(GetChaseCamData().Flags, _ChaseCam_NeverActive); }

// All these functions return the chase cam's state (true: active; false: inactive)
bool ChaseCam_IsActive() {
    if (!NetAllowBehindview())
        return false;
...
//
/*static*/ void ShootForTargetPoint(bool ThroughWalls, world_point3d& StartPosition, world_point3d& EndPosition,
                                    short& Polygon) {
    // Prevent short-integer wraparound
    const short HALF_SHORT_MIN = SHRT_MIN >> 1;
    const short HALF_SHORT_MAX = SHRT_MAX >> 1;
    if (StartPosition.x >= HALF_SHORT_MAX && EndPosition.x < HALF_SHORT_MIN)
        EndPosition.x = SHRT_MAX;
    if (StartPosition.x < HALF_SHORT_MIN && EndPosition.x >= HALF_SHORT_MAX)
        EndPosition.x = SHRT_MIN;
    if (StartPosition.y >= HALF_SHORT_MAX && EndPosition.y < HALF_SHORT_MIN)
        EndPosition.y = SHRT_MAX;
    if (StartPosition.y < HALF_SHORT_MIN && EndPosition.y >= HALF_SHORT_MAX)
        EndPosition.y = SHRT_MIN;
    ...
    while (true) {
        ...
        if (EndPosition.z <= height) {
            ...
            if (find_line_crossed_leaving_polygon(Polygon, (world_point2d*)&StartPosition,
        ...
        // Which line crossed?
        line_index = find_line_crossed_leaving_polygon(Polygon, (world_point2d*)&StartPosition,
                                                       (world_point2d*)&EndPosition);
        if (line_index != NONE) {
...
bool ChaseCam_Update() {
    ...
    if (!_ChaseCam_IsReset) {
        ...
        CC_Position.x
                = CC_PosUpdate(ChaseCam.Damping, ChaseCam.Spring, CC_Position.x, CC_Position_1.x, CC_Position_2.x);
        CC_Position.y
                = CC_PosUpdate(ChaseCam.Damping, ChaseCam.Spring, CC_Position.y, CC_Position_1.y, CC_Position_2.y);
        CC_Position.z
                = CC_PosUpdate(ChaseCam.Damping, ChaseCam.Spring, CC_Position.z, CC_Position_1.z, CC_Position_2.z);
    }
