...
#include "ProFontAO.h"
#include "preferences.hpp" // smooth_font
...
class style_token_iterator {
    ...
    style_token_iterator(const std::string_view& source_string, std::string_view::const_iterator position)
        ...
        if (ending != source_string.cend()) {
            if (*position == '|') {
                ...
                while (ending != source_string.cend() && *ending != '|') { ++ending; }
            }
        }
  ...
  public:
    ...
    bool operator==(const style_token_iterator& other) const { return position == other.position; }
...
class style_tokenizer {
  ...
  public:
    ...
    style_tokenizer(const std::string_view& source_string) : source_string(source_string) {}

    style_token_iterator cbegin() const { return style_token_iterator(source_string, source_string.cbegin()); }

    style_token_iterator cend() const { return style_token_iterator(source_string, source_string.cend()); }
};
} // namespace
...
void initialize_fonts(bool last_chance) {
    ...
    for (int j = 0; j < NUMBER_OF_BUILTIN_FONTS; ++j) builtin_fonts[builtin_fontspecs[j].name] = builtin_fontspecs[j];
    ...
    while (num_assoc--) {
        ...
        if (size == spec.size) {
            ...
            if (info->rsrc.IsLoaded()) {
                ...
                // printf(" first %d, last %d, max_kern %d, rect_w %d, rect_h %d, ascent %d, descent %d, leading %d,
                ...
                // Set table pointers
                int table_size
                        = info->last_character - info->first_character + 3; // Tables contain 2 additional entries
...
font_info* load_font(const TextSpec& spec) {
    ...
    if (spec.normal != "") {
        ...
        if (font) {
            ...
            info->m_line_height = std::max({TTF_FontLineSkip(font), TTF_FontHeight(font), height});
            ...
            if (font) {
                ...
                if (font) {
                    ...
                    if (font) {
                        ...
                        assert(font);
                        info->m_styles[styleBold | styleItalic] = font;
                        info->m_keys[styleBold | styleItalic]
                                = ttf_font_key_t(file, styleBold | styleItalic, spec.size);
                    }
                }
            }
...
void unload_font(font_info* info) { info->_unload(); }
...
uint16 sdl_font_info::_text_width(const char* text, uint16 style, bool) const {
    int width = 0;
    char c;
    while ((c = *text++) != 0) width += char_width(c, style);
    assert(0 <= width);
...
int font_info::draw_styled_text(SDL_Surface* s, const std::string& text, size_t length, int x, int y, uint32 pixel,
                                uint16 style, bool utf8) const {
    int width = 0;
    ...
    for (auto it = tokenizer.cbegin(); it != tokenizer.cend(); ++it) {
        if (is_style_token(*it)) {
            ...
            if (style & styleShadow) {
                _draw_text(s, it->data(), it->size(), x + width + 1, y + 1, SDL_MapRGB(s->format, 0x0, 0x0, 0x0), style,
                           utf8);
            }
...
int font_info::draw_text(SDL_Surface* s, const char* text, size_t length, int x, int y, uint32 pixel, uint16 style,
                         bool utf8) const {
    if (style & styleShadow) {
