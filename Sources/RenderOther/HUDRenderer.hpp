...
#define SET_INVENTORY_DIRTY_STATE(p, v)                                 \
    ((void)((v) ? ((p)->interface_flags |= (uint16)INVENTORY_DIRTY_BIT) \
                : ((p)->interface_flags &= (uint16)~INVENTORY_DIRTY_BIT)))

#define INTERFACE_IS_DIRTY(p) ((p)->interface_flags & INTERFACE_DIRTY_BIT)
#define SET_INTERFACE_DIRTY_STATE(p, v) \
    ((v) ? ((p)->interface_flags |= INTERFACE_DIRTY_BIT) : (p)->interface_flags &= ~INTERFACE_DIRTY_BIT)
...
#define MAXIMUM_WEAPON_INTERFACE_DEFINITIONS \
    (sizeof(weapon_interface_definitions) / sizeof(struct weapon_interface_data))
...
class HUD_Class {
  ...
  protected:
    ...
    void draw_bar(screen_rectangle* rectangle, short actual_height, shape_descriptor top_piece,
                  shape_descriptor full_bar, shape_descriptor background_piece);
    void draw_ammo_display_in_panel(short trigger_id);
    void draw_inventory_item(char* text, short count, short offset, bool erase_first, bool valid_in_this_environment);
