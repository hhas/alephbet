...
void OGL_Blitter::_LoadTextures() {
    ...
    if (PlatformIsLittleEndian()) {
        t.reset(SDL_CreateRGBSurface(SDL_SWSURFACE, m_tile_width, m_tile_height, 32, 0x0000'00ff, 0x0000'ff00,
                                     0x00ff'0000, 0xff00'0000));
    } else {
        t.reset(SDL_CreateRGBSurface(SDL_SWSURFACE, m_tile_width, m_tile_height, 32, 0xff00'0000, 0x00ff'0000,
    ...
    for (int y = 0; y < v_rects; y++) {
        for (int x = 0; x < h_rects; x++) {
            ...
            for (int row = m_rects[i].h; row < m_tile_height; ++row) {
                uint32* curRow = static_cast<uint32*>(t->pixels) + (row * m_tile_width);
                for (int col = 0; col < m_tile_width; ++col) { curRow[col] = lastRow[col] & rgb_mask; }
            }
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_tile_width, m_tile_height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                         t->pixels);
...
void OGL_Blitter::StopTextures() {
    ...
    for (it = m_blitter_registry->begin(); it != m_blitter_registry->end(); it = m_blitter_registry->begin())
        (*it)->_UnloadTextures();
}

OGL_Blitter::~OGL_Blitter() { Unload(); }

int OGL_Blitter::ScreenWidth() { return MainScreenLogicalWidth(); }

int OGL_Blitter::ScreenHeight() { return MainScreenLogicalHeight(); }

void OGL_Blitter::BoundScreen(bool in_game) { alephbet::Screen::instance()->bound_screen(in_game); }

void OGL_Blitter::WindowToScreen(int& x, int& y, bool in_game) { alephbet::Screen::instance()->window_to_screen(x, y); }
...
void OGL_Blitter::Draw(const Image_Rect& dst, const Image_Rect& raw_src) {
    ...
    for (int i = 0; i < m_rects.size(); i++) {
        if (src.x > m_rects[i].x + m_rects[i].w || src.x + src.w < m_rects[i].x || src.y > m_rects[i].y + m_rects[i].h
