...
#include "lua_script.hpp" // texture palette
    ...
    if (!LuaTexturePaletteSize()) {
        ...
        for (int i = 0; i < LuaTexturePaletteSize(); ++i) {
            if (LuaTexturePaletteTexture(i) != UNONE)
                DrawTexture(LuaTexturePaletteTexture(i), LuaTexturePaletteTextureType(i), (i % cols) * size + x_offset,
                            320 + y_offset + (i / cols) * size, size - 1);
        }
...
void HUD_Class::update_suit_energy(short time_elapsed) {
    ...
    if (time_elapsed == NONE || (interface_state.shield_is_dirty)) {
        ...
        if (current_player->suit_energy > 2 * PLAYER_MAXIMUM_SUIT_ENERGY) {
            ...
            if (current_player->suit_energy < 0)
                actual_width = 0;
        }

        draw_bar(shield_rect, actual_width, BUILD_DESCRIPTOR(_collection_interface, bar_top_shape_id),
...
void HUD_Class::update_suit_oxygen(short time_elapsed) {
    ...
    if (((delay_time -= time_elapsed) < 0) || time_elapsed == NONE || interface_state.oxygen_is_dirty) {
        ...
        draw_bar(oxygen_rect, actual_width, BUILD_DESCRIPTOR(_collection_interface, _oxygen_bar_right),
            ...
            if (definition->multi_weapon) {
                if (definition->multiple_unusable_shape != UNONE) {
                    ...
                    if (definition->weapon_panel_shape != UNONE)
                        DrawShapeAtXY(definition->weapon_panel_shape, definition->standard_weapon_panel_left,
...
#define strWEAPON_NAME_LIST 137
                ...
                for (item_index = BALL_ITEM_BASE; item_index < BALL_ITEM_BASE + MAXIMUM_NUMBER_OF_PLAYERS;
                     ++item_index) {
            ...
            DrawText(weapon_name, &source, _center_horizontal | _center_vertical | _wrap_text, _weapon_name_font,
...
void HUD_Class::update_inventory_panel(bool force_redraw) {
    ...
    if (INVENTORY_IS_DIRTY(current_player) || force_redraw) {
        ...
        if (item_type != _network_statistics) {
            ...
            calculate_player_item_array(current_player_index, item_type, section_items, section_counts, &section_count);
        }
        ...
        if (item_type == _network_statistics) {
                ...
                switch (GET_GAME_TYPE()) {
                    ...
                    case _game_of_tag:
                        ...
                        for (player_index = 0; player_index < dynamic_world->player_count; ++player_index) {
                            ...
                            int kills_left
                                    = dynamic_world->game_information.kill_limit
                                      - (player->total_damage_given.kills - player->damage_taken[player_index].kills);
                            if (kills_left < kill_limit)
                                kill_limit = kills_left;
                        }
                        char kills_left[16];
            ...
            for (loop = 0; loop < dynamic_world->player_count; ++loop) {
                ...
                DrawText(player->name, &dest_rect, _center_vertical, _interface_font,
                         PLAYER_COLOR_BASE_INDEX + player->color);

                /* Now draw the ranking_text */
                dest_rect.right += width;
                dest_rect.left   = dest_rect.right - width;
                DrawText(temporary, &dest_rect, _center_vertical, _interface_font,
                         PLAYER_COLOR_BASE_INDEX + player->color);
            }
        } else
...
void HUD_Class::draw_bar(screen_rectangle* rectangle, short width, shape_descriptor top_piece,
                         shape_descriptor full_bar, shape_descriptor background_texture) {
    ...
    if (width > 2 * TOP_OF_BAR_WIDTH) {
        DrawShapeAtXY(top_piece, rectangle->left + width - TOP_OF_BAR_WIDTH, rectangle->top);
    } else {
        ...
        if (current_ammo_data->type != _unused_interface_data && ammunition_count != NONE) {
            if (current_ammo_data->type == _uses_energy) {
                ...
                for (row = 0; row < (ammunition_count / current_ammo_data->ammo_across); ++row) {
                    DrawShapeAtXY(current_ammo_data->bullet, x, y);
                ...
                if (partial_row_count) {
                    ...
                    if (current_ammo_data->right_to_left) {
                        ...
                        destination.right
                                = destination.left
                                  + (current_ammo_data->ammo_across - partial_row_count) * current_ammo_data->delta_x;
                        source = destination;
                        offset_rect(&source, -source.left, -source.top);
                        ...
                        destination.left = x, destination.top = y;
                        destination.right
                                = x + (current_ammo_data->ammo_across - partial_row_count) * current_ammo_data->delta_x;
                        destination.bottom = y + current_ammo_data->delta_y;
                        source             = destination;
                        offset_rect(&source, -source.left, -source.top);
                        DrawShape(current_ammo_data->empty_bullet, &destination, &source);
                ...
                for (row = 0; row < (max - ammunition_count) / current_ammo_data->ammo_across; ++row) {
                    DrawShapeAtXY(current_ammo_data->empty_bullet, x, y);
                    y += current_ammo_data->delta_y;
                }
            }
        }
...
void HUD_Class::draw_inventory_item(char* text, short count, short offset, bool erase_first,
                                    bool valid_in_this_environment) {
...
void HUD_Class::draw_player_name(void) {
    ...
    DrawText(player->name, player_name_rect, _center_vertical | _center_horizontal, _player_name_font,
