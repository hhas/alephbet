...
static void bitmap_window_copy(struct bitmap_definition* source, struct bitmap_definition* destination, short x0,
                               short y0, short x1, short y1);
...
void initialize_motion_sensor(shape_descriptor mount, shape_descriptor virgin_mounts, shape_descriptor aliens,
                              shape_descriptor friends, shape_descriptor enemies, shape_descriptor compasses,
                              short side_length) {
    mount_shape         = mount;
    ...
    for (int i = 0; i < MAXIMUM_MOTION_SENSOR_ENTITIES; i++) { entities[i].flags = 0; }
        ...
        for (monster_index = 0, monster = monsters; monster_index < MAXIMUM_MONSTERS_PER_MAP;
            ...
            if (SLOT_IS_USED(monster) && (MONSTER_IS_PLAYER(monster) || MONSTER_IS_ACTIVE(monster))) {
                ...
                world_distance distance
                        = guess_distance2d((world_point2d*)&object->location, (world_point2d*)&owner_object->location);
...
void HUD_Class::draw_network_compass(void) {
    ...
    if (difference & _network_compass_nw)
        draw_or_erase_unclipped_shape(36, 36, compass_shapes, (new_state & _network_compass_nw) != 0);
    if (difference & _network_compass_ne)
        draw_or_erase_unclipped_shape(61, 36, compass_shapes + 1, (new_state & _network_compass_ne) != 0);
    if (difference & _network_compass_se)
        draw_or_erase_unclipped_shape(61, 61, compass_shapes + 3, (new_state & _network_compass_se) != 0);
    if (difference & _network_compass_sw)
        draw_or_erase_unclipped_shape(36, 61, compass_shapes + 2, (new_state & _network_compass_sw) != 0);
...
void erase_all_entity_blips(void) {
    ...
    for (entity_index = 0, entity = entities; entity_index < MAXIMUM_MOTION_SENSOR_ENTITIES; ++entity_index, ++entity) {
        if (SLOT_IS_USED(entity)) {
            ...
            memmove(entity->visible_flags + 1, entity->visible_flags,
                    (NUMBER_OF_PREVIOUS_LOCATIONS - 1) * sizeof(bool));
            memmove(entity->previous_points + 1, entity->previous_points,
                    (NUMBER_OF_PREVIOUS_LOCATIONS - 1) * sizeof(point2d));
            ...
            if (!SLOT_IS_BEING_REMOVED(entity)) {
                ...
                if (object->transfer_mode != _xfer_invisibility && object->transfer_mode != _xfer_subtle_invisibility
                    && (!(static_world->environment_flags & _environment_magnetic)
                        || !((dynamic_world->tick_count + 4 * monster->object_index) & FLICKER_FREQUENCY))) {
...
void HUD_Class::draw_all_entity_blips(void) {
    ...
    for (intensity = NUMBER_OF_PREVIOUS_LOCATIONS - 1; intensity >= 0; --intensity) {
        for (entity_index = 0, entity = entities; entity_index < MAXIMUM_MOTION_SENSOR_ENTITIES;
             ++entity_index, ++entity) {
            if (SLOT_IS_USED(entity)) {
        ...
        for (entity_index = 0, entity = entities; entity_index < MAXIMUM_MOTION_SENSOR_ENTITIES;
            ...
            if (SLOT_IS_USED(entity)) {
                if (entity->visible_flags[intensity]) {
                    ...
                    add_entity_blip(display_type, intensity, entity->previous_points[intensity].x * MOTION_SENSOR_SCALE,
...
void HUD_SW_Class::draw_or_erase_unclipped_shape(short x, short y, shape_descriptor shape, bool draw) {
    ...
    draw ? unclipped_solid_sprite_copy(blip, mount, x, y)
...
void HUD_OGL_Class::draw_entity_blip(point2d* location, shape_descriptor shape) {
    ...
    DrawShapeAtXY(shape, x + c_x - (blip->width >> 1), y + c_y - (blip->height >> 1), true);
    DisableClipPlane();
}
...
static short find_or_add_motion_sensor_entity(short monster_index) {
    ...
    for (entity_index = 0, entity = entities; entity_index < MAXIMUM_MOTION_SENSOR_ENTITIES; ++entity_index, ++entity) {
        if (SLOT_IS_USED(entity)) {
            if (entity->monster_index == monster_index)
                break;
        } else {
...
static void clipped_transparent_sprite_copy(struct bitmap_definition* source, struct bitmap_definition* destination,
    ...
    if (y0 + height > destination->height)
        height = destination->height - y0;
    if (y0 < 0) {
        y       = -y0;
        height += y0;
    }
...
static shape_descriptor get_motion_sensor_entity_shape(short monster_index) {
    ...
    if (MONSTER_IS_PLAYER(monster)) {
        ...
        shape = ((player->team == owner->team && !(GET_GAME_OPTIONS() & _force_unique_teams))
                 || GET_GAME_TYPE() == _game_of_cooperative_play)
                        ? friendly_shapes
                        : enemy_shapes;
    } else {
        /*
...
void reset_mml_motion_sensor() {
    ...
    if (OriginalMonsterDisplays) {
        for (int i = 0; i < NUMBER_OF_MONSTER_TYPES; i++) MonsterDisplays[i] = OriginalMonsterDisplays[i];
...
void parse_mml_motion_sensor(const InfoTree& root) {
    ...
    if (!original_motion_sensor_settings) {
        original_motion_sensor_settings
                = (struct motion_sensor_definition*)malloc(sizeof(struct motion_sensor_definition));
    ...
    if (!OriginalMonsterDisplays) {
        ...
        for (int i = 0; i < NUMBER_OF_MONSTER_TYPES; i++) OriginalMonsterDisplays[i] = MonsterDisplays[i];
    }
