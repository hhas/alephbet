...
static bool shape_is_motion_blip(short collection, short frame_index) {
    if (shapes_file_is_m1()) {
        return (collection == 0
                && ((frame_index >= 23 && frame_index <= 34) || (frame_index >= 46 && frame_index <= 51)));
    }
    return (collection == 0 && (frame_index >= 12 && frame_index <= 29));
}

Shape_Blitter::Shape_Blitter(short collection, short frame_index, short texture_type, short clut_index)
    : m_coll(BUILD_COLLECTION(collection, clut_index)), m_frame(frame_index), m_type(texture_type), m_surface(NULL),
      m_scaled_surface(NULL), tint_color_r(1.0), tint_color_g(1.0), tint_color_b(1.0), tint_color_a(1.0),
      rotation(0.0) {
...
float Shape_Blitter::Width() { return m_scaled_src.w; }

float Shape_Blitter::Height() { return m_scaled_src.h; }

int Shape_Blitter::UnscaledWidth() { return m_src.w; }

int Shape_Blitter::UnscaledHeight() { return m_src.h; }
...
void Shape_Blitter::OGL_Draw(const Image_Rect& dst) {
    ...
    if (Wanting_sRGB && TMgr.TextureType != OGL_Txtr_WeaponsInHand
        && TMgr.TextureType != OGL_Txtr_HUD) // also not hud just in case
    {
    ...
    if (m_type == Shape_Texture_Interface) {
        ...
        OGL_RenderTexturedRect(dst.x, dst.y, dst.w, dst.h, U_Offset, V_Offset, U_Offset + U_Scale, V_Offset + V_Scale);
        ...
        OGL_RenderTexturedRect(dst.x, dst.y, dst.w, dst.h, V_Offset, U_Offset, V_Offset + V_Scale, U_Offset + U_Scale);
    } else {
        ...
        GLfloat texcoords[8] = {GLfloat(U_Offset),           GLfloat(V_Offset),
        ...
        GLfloat vertices[8]  = {dst.x, dst.y, dst.x + dst.w, dst.y, dst.x + dst.w, dst.y + dst.h, dst.x, dst.y + dst.h};
...
SDL_Surface* flip_surface(SDL_Surface* s, int width, int height) {
    ...
    for (int y = 0; y < height; y++) {
        int yi = height - y - 1;
        for (int x = 0; x < byteWidth; x++) { dst_pixels[yi * byteWidth + x] = src_pixels[y * byteWidth + x]; }
    }
...
void Shape_Blitter::SDL_Draw(SDL_Surface* dst_surface, const Image_Rect& dst) {
    ...
    if (!m_surface) {
        byte* pixelsOut = NULL;
        SDL_Surface* tmp
                = get_shape_surface(m_frame, m_coll, &pixelsOut, (m_type == Shape_Texture_Interface) ? -1.0 : 1.0);
        if (!tmp)
            return;
