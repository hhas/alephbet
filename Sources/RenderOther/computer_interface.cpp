...
#include "interface.hpp"   // for the error strings.
#include "lightsource.hpp" // for tagged lightsources
#include "map.hpp"
#include "overhead_map.hpp"
#include "platforms.hpp" // for tagged platforms
#include "player.hpp"
#include "screen.hpp"
...
#define SET_TERMINAL_IS_DIRTY(term, v) \
    ((void)((v) ? ((term)->flags |= _terminal_is_dirty) : ((term)->flags &= ~_terminal_is_dirty)))
...
struct terminal_groupings {
    ...
    int16 permutation; /* checkpoint id for chkpt, level id for _briefing, movie id for movie, sound id for sound,
                          soundtrack id for soundtrack */
    int16 start_index;
    int16 length;
...
/* ------------ private prototypes */
static player_terminal_data* get_player_terminal_data(short player_index);

static void draw_logon_text(terminal_text_t* terminal_text, short current_group_index, short logon_shape_id);
static void draw_computer_text(Rect* bounds, terminal_text_t* terminal_text, short current_group_index,
                               short current_line);
static void _draw_computer_text(char* base_text, short start_index, Rect* bounds, terminal_text_t* terminal_text,
                                short current_line);
static short find_group_type(terminal_text_t* data, short group_type);
...
static struct terminal_groupings* get_indexed_grouping(terminal_text_t* data, short index);
static struct text_face_data* get_indexed_font_changes(terminal_text_t* data, short index);
...
static void draw_terminal_borders(struct player_terminal_data* terminal_data, bool after_other_terminal_stuff);
...
static void present_checkpoint_text(terminal_text_t* terminal_text, short current_group_index, short current_line);
static bool find_checkpoint_location(short checkpoint_index, world_point2d* location, short* polygon_index);
static void set_text_face(struct text_face_data* text_face);
static void draw_line(char* base_text, short start_index, short end_index, Rect* bounds, terminal_text_t* terminal_text,
                      short* text_face_start_index, short line_number);
...
static void display_picture_with_text(struct player_terminal_data* terminal_data, Rect* bounds,
                                      terminal_text_t* terminal_text, short current_lien);
...
static void goto_terminal_group(short player_index, terminal_text_t* terminal_text, short new_group_index);
...
#include "joystick.hpp" // for AO_SCANCODE_BASE_JOYSTICK_BUTTON
...
player_terminal_data* get_player_terminal_data(short player_index) {
    struct player_terminal_data* player_terminal
            = GetMemberWithBounds(player_terminals, player_index, MAXIMUM_NUMBER_OF_PLAYERS);
...
void initialize_terminal_manager(void) {
    ...
    for (auto i = 0; i < MAXIMUM_NUMBER_OF_PLAYERS; ++i) { player_terminals[i].state = _no_terminal_state; }
    /*
        ...
        switch (terminal_data->state) {
            case _reading_terminal:
                ...
                switch (current_group->type) {
                    case _logon_group:
                    case _logoff_group:
                        draw_logon_text(terminal_text, terminal_data->current_group, current_group->permutation);
                        break;
                    ...
                    case _pict_group:
                        display_picture_with_text(terminal_data, &bounds, terminal_text, terminal_data->current_line);
                        break;
...
static void draw_logon_text(terminal_text_t* terminal_text, short current_group_index, short logon_shape_id) {
    ...
    short width;
    struct terminal_groupings* current_group = get_indexed_grouping(terminal_text, current_group_index);
    // LP change: just in case...
    if (!current_group)
        return;
    ...
    if (current_group->flags & _group_is_marathon_1) {
        ...
        _draw_screen_text(temporary, (screen_rectangle*)&picture_bounds, _center_vertical | _center_horizontal,
    ...
    // width = text_width(base_text + current_group->start_index, current_group->length, terminal_font,
    // _get_font_spec(_computer_interface_font)->style);
...
static void draw_computer_text(Rect* bounds, terminal_text_t* terminal_text, short current_group_index,
    ...
    _draw_computer_text(base_text, current_group_index, bounds, terminal_text, current_line);
}

/* Returns true if current_line> end of text.. (for phase change) */
static void _draw_computer_text(char* base_text, short group_index, Rect* bounds, terminal_text_t* terminal_text,
                                short current_line) {
    bool done = false;
    short start_index;
    ...
    for (index = 0; index < current_line; ++index) {
        /* Calculate one line.. */
        if (!calculate_line(base_text, RECTANGLE_WIDTH(bounds), start_index,
                            current_group->start_index + current_group->length, &end_index)) {
            if (end_index > current_group->start_index + current_group->length) {
                // dprintf("Start: %d Length: %d End: %d;g", current_group->start_index, current_group->length,
                // end_index); dprintf("Width: %d", RECTANGLE_WIDTH(bounds));
    ...
    if (!done) {
        ...
        if (last_text_index == NONE) {
            ...
            struct text_face_data* font_face = get_indexed_font_changes(terminal_text, last_text_index);
            // LP change: just in case...
            if (!font_face)
                return;
            text_face = *font_face;
        ...
        for (index = 0; !done && index < terminal_text->lines_per_page; ++index) {
            ...
            if (!calculate_line(base_text, RECTANGLE_WIDTH(bounds), start_index,
                                current_group->start_index + current_group->length, &end_index)) {
                // dprintf("draw calculate line: %d start: %d end: %d text: %x length: %d lti: %d", index, start_index,
                // end_index, base_text, current_group->length, last_text_index);
                if (end_index > current_group->start_index + current_group->length) {
                    // dprintf("Start: %d Length: %d End: %d;g", current_group->start_index, current_group->length,
                    // end_index); dprintf("Width: %d", RECTANGLE_WIDTH(bounds));
                    end_index = current_group->start_index + current_group->length;
                ...
                assert(end_index <= current_group->start_index + current_group->length);
                draw_line(base_text, start_index, end_index, bounds, terminal_text, &last_text_index, index);
                start_index = end_index;
            } else {
                /* End of text. */
                //				done= true;
...
static void draw_line(char* base_text, short start_index, short end_index, Rect* bounds, terminal_text_t* terminal_text,
    ...
    if (text_index < terminal_text->font_changes.size()) {
        do {
            ...
            if (!face_data)
                return;
            if (face_data->index < start_index)
                text_index++;
    ...
    while (!done) {
        ...
        xpos += draw_text(/*world_pixels*/ draw_surface, base_text + current_start, current_end - current_start, xpos,
                          bounds->top + line_height * (line_number + FUDGE_FACTOR), current_pixel, terminal_font,
                          current_style);
...
static void teleport_to_level(short level_number, int16_t delay_before_teleport) {
    ...
    if (s) {
        ...
        if (RECTANGLE_WIDTH(&bounds) <= RECTANGLE_WIDTH(&screen_bounds)
            ...
            if (RECTANGLE_HEIGHT(&bounds) - RECTANGLE_HEIGHT(&screen_bounds)
                ...
                adjusted_width
                        = RECTANGLE_HEIGHT(&screen_bounds) * RECTANGLE_WIDTH(&bounds) / RECTANGLE_HEIGHT(&bounds);
                ...
                adjusted_height
                        = RECTANGLE_WIDTH(&screen_bounds) * RECTANGLE_HEIGHT(&bounds) / RECTANGLE_WIDTH(&bounds);
                bounds = screen_bounds;
        ...
        *frame = bounds;
        ...
        draw_text(/*world_pixels*/ draw_surface, temporary, bounds.left + (RECTANGLE_WIDTH(&bounds) - width) / 2,
                  bounds.top + RECTANGLE_HEIGHT(&bounds) / 2,
                  SDL_MapRGB(/*world_pixels*/ draw_surface->format, 0xff, 0xff, 0xff), font, styleNormal);
    }
}
...
static terminal_text_t* get_indexed_terminal_data(short id) {
    if (id < 0 || id >= int(map_terminal_text.size())) {
        ...
        if (id == resource_terminal_id) {
            ...
            if (rsrc.IsLoaded()) {
                resource_terminal.reset(
                        compile_marathon_terminal(reinterpret_cast<char*>(rsrc.GetPointer()), rsrc.GetLength()));
...
static void encode_text(terminal_text_t* terminal_text) {
    ...
    for (int i = 0; i < length % 4; i++) *p++ ^= 0xfe;
...
static void draw_terminal_borders(struct player_terminal_data* terminal_data, bool after_other_terminal_stuff) {
    ...
    if (!after_other_terminal_stuff) {
        ...
        border.left  += LABEL_INSET;
        border.right -= LABEL_INSET;
...
static bool previous_terminal_group(short player_index, terminal_text_t* terminal_text) {
    ...
    if (terminal_data->state == _reading_terminal) {
        ...
        do {
            ...
            if (!use_new_group)
                done = true; // this fixes perma-loop freeze
        } while (!done);
...
static void next_terminal_group(short player_index, terminal_text_t* terminal_text) {
    ...
    if (terminal_data->current_group == NONE) {
        ...
        if (terminal_data->current_group == static_cast<int16>(terminal_text->groupings.size())
            && terminal_text->groupings.size() && (terminal_text->groupings[0].flags & _group_is_marathon_1)) {
            // Marathon 1 fallback
    ...
    switch (current_group->type) {
        ...
        case _pict_group:
            ...
            if (dynamic_world->player_count > 1) {
                ...
                calculate_bounds_for_text_box(current_group->flags, &text_bounds);
                terminal_data->maximum_line = count_total_lines(
                        get_text_base(terminal_text), RECTANGLE_WIDTH(&text_bounds), current_group->start_index,
                        current_group->start_index + current_group->length);

                if (film_profile.page_up_past_full_width_term_pict && terminal_data->maximum_line == 0) {
        ...
        case _information_group:
            ...
            if (dynamic_world->player_count > 1) {
                ...
                terminal_data->maximum_line = count_total_lines(get_text_base(terminal_text), RECTANGLE_WIDTH(&bounds),
...
static void get_date_string(char* date_string, short flags) {
    ...
    if (flags & _group_is_marathon_1) {
        ...
        seconds  = 800'070'137; // Wednesday, May 10, 1995 1:42:17
...
static void handle_reading_terminal_keys(short player_index, int32 action_flags) {
    ...
    if (!aborted && (initial_group != terminal->current_group || initial_line != terminal->current_line)) {
...
static void calculate_maximum_lines_for_groups(struct terminal_groupings* groups, short group_count, char* text_base);

class MarathonTerminalCompiler {
  public:

    MarathonTerminalCompiler(char* text, short length)
        : terminal(new terminal_text_t), in_buffer(text, length), in(&in_buffer) {
...
terminal_text_t* MarathonTerminalCompiler::Compile() {
    ...
    calculate_maximum_lines_for_groups(&terminal->groupings[0], terminal->groupings.size(),
                                       reinterpret_cast<char*>(terminal->text.data()));
...
void MarathonTerminalCompiler::BuildUnfinishedGroup() {
    ...
    if (information_group.type) {
        ...
        terminal->groupings.insert(terminal->groupings.end(), information_checkpoints.begin(),
    ...
    if (unfinished_group.type) {
        ...
        terminal->groupings.insert(terminal->groupings.end(), unfinished_checkpoints.begin(),
    ...
    for (index = 0; index < group_count; ++index) {
        switch (groups[index].type) {
            ...
            case _pict_group: {
                ...
                groups[index].maximum_line_count
                        = count_total_lines(text_base, RECTANGLE_WIDTH(&text_bounds), groups[index].start_index,
...
static char* get_text_base(terminal_text_t* data) { return (char*)data->text.data(); }
...
static short calculate_lines_per_page(void) {
    ...
    if (!film_profile.calculate_terminal_lines_correctly) {
        bounds = get_term_rectangle(_terminal_screen_rect);
        lines_per_page
                = (RECTANGLE_HEIGHT(&bounds) - 2 * BORDER_HEIGHT) / _get_font_line_height(_computer_interface_font);
...
static size_t packed_terminal_length(const terminal_text_t& t) {
    return SIZEOF_static_preprocessed_terminal_data + t.groupings.size() * SIZEOF_terminal_groupings
           + t.font_changes.size() * SIZEOF_text_face_data + t.text.size();
}
