...
Image_Blitter::Image_Blitter()
    : m_surface(NULL), m_disp_surface(NULL), m_scaled_surface(NULL), tint_color_r(1.0), tint_color_g(1.0),
      tint_color_b(1.0), tint_color_a(1.0), rotation(0.0) {
...
bool Image_Blitter::Load(const ImageDescriptor& image) {
    ...
    if (PlatformIsLittleEndian()) {
        s = SDL_CreateRGBSurfaceFrom(const_cast<uint32*>(image.GetBuffer()), image.GetWidth(), image.GetHeight(), 32,
                                     image.GetWidth() * 4, 0x0000'00ff, 0x0000'ff00, 0x00ff'0000, 0xff00'0000);
    } else {
        s = SDL_CreateRGBSurfaceFrom(const_cast<uint32*>(image.GetBuffer()), image.GetWidth(), image.GetHeight(), 32,
                                     image.GetWidth() * 4, 0xff00'0000, 0x00ff'0000, 0x0000'ff00, 0x0000'00ff);
    }
    if (!s)
        return false;
    bool ret = Load(*s);
...
bool Image_Blitter::Load(const SDL_Surface& s, const SDL_Rect& src) {
    ...
    if (PlatformIsLittleEndian()) {
        m_surface = SDL_CreateRGBSurface(SDL_SWSURFACE, m_src.w, m_src.h, 32, 0x0000'00ff, 0x0000'ff00, 0x00ff'0000,
                                         0xff00'0000);
    } else {
        m_surface = SDL_CreateRGBSurface(SDL_SWSURFACE, m_src.w, m_src.h, 32, 0xff00'0000, 0x00ff'0000, 0x0000'ff00,
                                         0x0000'00ff);
    }
    if (!m_surface)
        return false;
...
bool Image_Blitter::Loaded() { return (m_surface != NULL); }
...
float Image_Blitter::Width() { return m_scaled_src.w; }

float Image_Blitter::Height() { return m_scaled_src.h; }

int Image_Blitter::UnscaledWidth() { return m_src.w; }

int Image_Blitter::UnscaledHeight() { return m_src.h; }
...
Image_Blitter::~Image_Blitter() { Unload(); }
