...
class FpsCounter {
  public:
    ...
    FpsCounter() : next_update_{clock::now() + update_time}, sum_{0}, count_{0}, fps_{0} {}
...
static bool parseicon(const char* p, size_t rem, unsigned char palette[1024], int& numcolors,
...
void seticon(int player, int idx, unsigned char palette[1024], unsigned char graphic[256]) {
    ...
    srf = SDL_CreateRGBSurfaceFrom(ScriptHUDElements[player][idx].icon, 16, 16, 32, 64, 0xFF << 8, 0xFF << 16,
                                   0xFF << 24, 0xFF);
#else
    srf = SDL_CreateRGBSurfaceFrom(ScriptHUDElements[player][idx].icon, 16, 16, 32, 64, 0xFF << 16, 0xFF << 8, 0xFF,
                                   0xFF << 24);
#endif
#ifdef HAVE_OPENGL
    if (OGL_IsActive()) {
...
bool IsScriptHUDNonlocal() { return nonlocal_script_hud; }

void SetScriptHUDNonlocal(bool nonlocal) { nonlocal_script_hud = nonlocal; }
...
void SetScriptHUDText(int player, int idx, const char* text) {
    ...
    if (!text)
        ScriptHUDElements[player][idx].text.clear();
    else
        ScriptHUDElements[player][idx].text = text;
}
...
bool SetScriptHUDIcon(int player, int idx, const char* text, size_t rem) {
    ...
    idx    %= MAXIMUM_NUMBER_OF_SCRIPT_HUD_ELEMENTS;
    if (text) {
        if (!icon::parseicon(text, rem, palette, numcolors, graphic))
            return false;
        icon::seticon(player, idx, palette, graphic);
    } else
        ScriptHUDElements[player][idx].isicon = false;
    return true;
}
...
void reset_messages() {
    ...
    for (int i = 0; i < NumScreenMessages; i++) Messages[i].ExpirationTime = machine_tick_count();
...
bool game_window_is_full_screen(void) { return !alephbet::Screen::instance()->hud(); }
...
/*static*/ void DisplayText(short BaseX, short BaseY, const char* Text, unsigned char r = 0xff, unsigned char g = 0xff,
    ...
    if ((OGL_MapActive || !world_view->overhead_map_active) && !world_view->terminal_mode_active)
        if (OGL_RenderText(BaseX, BaseY, Text, r, g, b))
            return;
#endif

    draw_text(DisplayTextDest, Text, BaseX + 1, BaseY + 1, SDL_MapRGB(world_pixels->format, 0x00, 0x00, 0x00),
              DisplayTextFont, DisplayTextStyle);
    draw_text(DisplayTextDest, Text, BaseX, BaseY, SDL_MapRGB(world_pixels->format, r, g, b), DisplayTextFont,
              DisplayTextStyle);
}

/*static*/ void DisplayTextCursor(SDL_Surface* s, short BaseX, short BaseY, const char* Text, short Offset,
                                  unsigned char r = 0xff, unsigned char g = 0xff, unsigned char b = 0xff) {
    ...
    // activate only in the main view, and also if OpenGL is being used for the overhead map
    if ((OGL_MapActive || !world_view->overhead_map_active) && !world_view->terminal_mode_active)
        if (OGL_RenderTextCursor(cursor_rect, r, g, b))
            return;
#endif
...
uint16 DisplayTextWidth(const char* Text) { return text_width(Text, DisplayTextFont, DisplayTextStyle); }
...
static void update_fps_display(SDL_Surface* s) {
    if (displaying_fps && !player_in_terminal_mode(current_player_index)) {
        ...
        if (!fps_counter.ready()) {
            ...
            if (latency > -1)
                sprintf(ms, "(%i ms)", std::min(latency, 10'000));
            else
                ms[0] = '\0';
...
static void DisplayMessages(SDL_Surface* s) {
    ...
    short Y           = Y0 + LineSpacing;
    if (ShowPosition)
        Y += 6 * LineSpacing; // Make room for the position data
    /* SB */
    short view = nonlocal_script_hud ? local_player_index : current_player_index;
    for (int i = 0; i < MAXIMUM_NUMBER_OF_SCRIPT_HUD_ELEMENTS; ++i) {
        if (!ScriptHUDElements[view][i].text.empty()) {
            short x2 = X, sk = Font.TextWidth("AAAAAAAAAAAAAA"), icon_skip, icon_drop;
            ...
            for (i = 0; i < MAXIMUM_NUMBER_OF_SCRIPT_HUD_ELEMENTS; ++i) {
                ...
                DisplayText(x2, Y + (ScriptHUDElements[view][i].isicon ? icon_drop : 0),
                            ScriptHUDElements[view][i].text.c_str(), color.r, color.g, color.b);
                x2 += sk;
                if (ScriptHUDElements[view][i].isicon)
                    x2 -= icon_skip;
            }
    ...
    for (int k = NumScreenMessages - 1; k >= 0; k--) {
        ...
        while (Which < 0) Which += NumScreenMessages;
...
static void DisplayScores(SDL_Surface* s) {
    if (!game_is_networked || !ShowScores)
        return;
    ...
    int Y = std::max(text_margins.top + (s->h - text_margins.bottom - H) / 2, Font.LineSpacing * NumScreenMessages)
            + Font.LineSpacing;
...
void RequestDrawingHUD() { HUD_RenderRequest = true; }

// This is for requesting the drawing of the Terminal;
// this is done because its drawing is now done when the main display is drawn
void RequestDrawingTerm() { Term_RenderRequest = true; }
