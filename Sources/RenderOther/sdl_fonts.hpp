...
class font_info {
  ...
  public:
    ...
    int draw_text(SDL_Surface* s, const char* text, size_t length, int x, int y, uint32 pixel, uint16 style,
    ...
    int draw_styled_text(SDL_Surface* s, const std::string& text, size_t length, int x, int y, uint32 pixel,
                         uint16 initial_style, bool utf = false) const;
  ...
  protected:
    ...
    virtual int _draw_text(SDL_Surface* s, const char* text, size_t length, int x, int y, uint32 pixel, uint16 style,
                           bool utf8) const
            = 0;
...
class sdl_font_info : public font_info {
  ...
  public:
    ...
    sdl_font_info()
        : first_character(0), last_character(0), ascent(0), descent(0), leading(0), pixmap(NULL), ref_count(0) {}

    virtual ~sdl_font_info() {
        if (pixmap)
            free(pixmap);
    }
