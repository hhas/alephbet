...
static inline void SetColor(rgb_color& Color) {
    if (map_is_translucent())
        glColor4us(Color.red, Color.green, Color.blue, 32'767);
    else
...
void OverheadMap_OGL_Class::end_overall() { glEnableClientState(GL_TEXTURE_COORD_ARRAY); }
...
void OverheadMap_OGL_Class::end_polygons() { DrawCachedPolygons(); }

void OverheadMap_OGL_Class::DrawCachedPolygons() {
    glDrawElements(GL_TRIANGLES, PolygonCache.size(), GL_UNSIGNED_SHORT, PolygonCache.data());
    PolygonCache.clear();
}
...
void OverheadMap_OGL_Class::draw_line(short* vertices, rgb_color& color, short pen_size) {
    // Test whether the line parameters have changed
    bool AreColorsEqual      = ColorsEqual(color, SavedColor);
    bool AreLinesEquallyWide = (pen_size == SavedPenSize);

    // If any change, then draw the cached lines with the *old* parameters
    if (!AreColorsEqual || !AreLinesEquallyWide)
        DrawCachedLines();
...
void OverheadMap_OGL_Class::end_lines() { DrawCachedLines(); }
...
void OverheadMap_OGL_Class::draw_thing(world_point2d& center, rgb_color& color, short shape, short radius) {
    ...
    switch (shape) {
        ...
        case _circle_thing: {
            GLfloat ft           = 0.1f;
            GLfloat ht           = ft * 0.5f;
            GLfloat vertices[36] = {-0.30f - ht, -0.75f,      -0.30f + ht, -0.75f + ft, +0.30f + ht, -0.75f,
                                    +0.30f - ht, -0.75f + ft, +0.75f - ft, -0.30f + ht, +0.75f,      -0.30f - ht,
                                    +0.75f - ft, +0.30f - ht, +0.75f,      +0.30f + ht, +0.30f - ht, +0.75f - ft,
                                    +0.30f + ht, +0.75f,      -0.30f + ht, +0.75f - ft, -0.30f - ht, +0.75f,
                                    -0.75f + ft, +0.30f - ht, -0.75f,      +0.30f + ht, -0.75f + ft, -0.30f + ht,
                                    -0.75f,      -0.30f - ht, -0.30f - ht, -0.75f,      -0.30f + ht, -0.75f + ft};
...
void OverheadMap_OGL_Class::draw_player(world_point2d& center, angle facing, rgb_color& color, short shrink,
                                        short front, short rear, short rear_theta) {
    SetColor(color);
...
void OverheadMap_OGL_Class::draw_text(world_point2d& location, rgb_color& color, char* text, FontSpecifier& FontData,
                                      short justify) {
...
void OverheadMap_OGL_Class::set_path_drawing(rgb_color& color) { SetColor(color); }

void OverheadMap_OGL_Class::draw_path(short step, // 0: first point
