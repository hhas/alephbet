...
class PlayerImage {
  public:
    ...
    PlayerImage()
        : mLegsView(NONE), mTorsoView(NONE), mLegsColor(NONE), mTorsoColor(NONE), mLegsAction(NONE), mTorsoAction(NONE),
          mPseudoWeapon(NONE), mLegsFrame(NONE), mTorsoFrame(NONE), mLegsBrightness(1.0f), mTorsoBrightness(1.0f),
          mTiny(false),
    ...
    void setTorsoView(int16 inView) {
        if (mTorsoView != inView) {
            mTorsoView  = inView;
            mTorsoDirty = true;
        }
    }

    void setView(int16 inView) {
        setLegsView(inView);
        setTorsoView(inView);
    }
    ...
    void setLegsColor(int16 inColor) {
        if (mLegsColor != inColor) {
            mLegsColor = inColor;
            mLegsDirty = true;
        }
    }
    ...
    void setTorsoColor(int16 inColor) {
        if (mTorsoColor != inColor) {
            mTorsoColor = inColor;
            mTorsoDirty = true;
        }
    }
    ...
    void setColor(int16 inColor) {
        setLegsColor(inColor);
        setTorsoColor(inColor);
    }

    // Leg actions: _player_running, etc. from player.h
    void setLegsAction(int16 inAction) {
        if (mLegsAction != inAction) {
            mLegsAction = inAction;
            mLegsDirty  = true;
        }
    }

    // Torso actions: _shape_weapon_firing, etc. from weapons.h
    void setTorsoAction(int16 inAction) {
        if (mTorsoAction != inAction) {
            mTorsoAction = inAction;
            mTorsoDirty  = true;
        }
    }
    ...
    void setPseudoWeapon(int16 inWeapon) {
        if (mPseudoWeapon != inWeapon) {
            mPseudoWeapon = inWeapon;
            mTorsoDirty   = true;
        }
    }

    // Frame: numbered 0 through...?  (depends on action, whether torso or legs, etc.)
    void setLegsFrame(int16 inFrame) {
        if (mLegsFrame != inFrame) {
            mLegsFrame = inFrame;
            mLegsDirty = true;
        }
    }

    void setTorsoFrame(int16 inFrame) {
        if (mTorsoFrame != inFrame) {
            mTorsoFrame = inFrame;
            mTorsoDirty = true;
        }
    }
    ...
    void setLegsBrightness(float inBrightness) {
        if (mLegsBrightness != inBrightness) {
            mLegsBrightness = inBrightness;
            mLegsDirty      = true;
        }
    }

    void setTorsoBrightness(float inBrightness) {
        if (mTorsoBrightness != inBrightness) {
            mTorsoBrightness = inBrightness;
            mTorsoDirty      = true;
        }
    }

    void setBrightness(float inBrightness) {
        setLegsBrightness(inBrightness);
        setTorsoBrightness(inBrightness);
    }

    // Tiny (quarter-sized): true or false
    void setTiny(bool inTiny) {
        if (mTiny != inTiny) {
            mTiny       = inTiny;
            mTorsoDirty = true;
            mLegsDirty  = true;
    ...
    bool canDrawSomething() {
        updateDrawingInfo();
        return mLegsValid || mTorsoValid;
    }
