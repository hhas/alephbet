...
class CircularQueue {
  public:
    ...
    CircularQueue(const CircularQueue<T>& o) : mData(NULL) { *this = o; }

    CircularQueue<T>& operator=(const CircularQueue<T>& o) {
        if (&o != this) {
            reset(o.getTotalSpace());
            for (unsigned int i = 0; i < o.getCountOfElements(); i++) enqueue(o.mData[o.getReadIndex(i)]);
        }
        return *this;
    }
    ...
    unsigned int getRemainingSpace() const { return getTotalSpace() - getCountOfElements(); }
  ...
  protected:
    ...
    unsigned int getReadIndex(unsigned int inOffset = 0) const {
        assert(getCountOfElements() > inOffset);
        return (mReadIndex + inOffset) % mQueueSize;
    }

    unsigned int getWriteIndex(unsigned int inOffset = 0) const {
        assert(getRemainingSpace() > inOffset);
        return (mWriteIndex + inOffset) % mQueueSize;
    }
