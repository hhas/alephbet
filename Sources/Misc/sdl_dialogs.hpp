...
class table_placer : public widget_placer {
  public:
    ...
    table_placer(int columns, int space = kSpace, bool balance_widths = false)
        : widget_placer(), m_add(0), m_columns(columns), m_space(space), m_balance_widths(balance_widths) {
        m_col_flags.resize(m_columns);
        m_col_min_widths.resize(m_columns);
    }
...
class horizontal_placer : public widget_placer {
  public:
    ...
    horizontal_placer(int space = kSpace, bool balance_widths = false)
        : widget_placer(), m_space(space), m_add_flags(kDefault), m_balance_widths(balance_widths) {}
