...
#include "FileHandler.hpp" // for FileSpecifier, for w_file_chooser
#include "map.hpp"         // for entry_point, for w_levels
#include "tags.hpp"        // for Typecode, for w_file_chooser

#include <functional>
#include <set>
#include <vector>

#include "metaserver_messages.hpp" // for GameListMessage, for w_games_in_room and MetaserverPlayerInfo, for w_players_in_room
#include "network.hpp"             // for prospective_joiner_info
...
class w_label : public w_static_text {
  ...
  public:
    ...
    bool is_selectable(void) const {
        if (associated_widget)
            return associated_widget->is_selectable();
        else
            return false;
    }
...
class w_select_button : public widget {
  public:
    ...
    void set_callback(action_proc p, void* a) {
        proc = p;
        arg  = a;
    }
...
class w_select : public widget {
  public:
    ...
    // ZZZ: set selection-changed callback - this will be called (if set) at the end of selection_changed (currently,
    // anytime it "beeps") removes need to subclass this class (or its subclasses!) just to add selection-changed
    // behavior.
...
class w_percentage_slider : public w_slider {
  public:
    ...
    w_percentage_slider(int num_items, int sel) : w_slider(num_items, sel) { init_formatted_value(); }
...
class w_color_picker : public widget {
  ...
  private:
    ...
    struct update_color {
        update_color(w_percentage_slider* red, w_percentage_slider* green, w_percentage_slider* blue, uint16* i_red,
                     uint16* i_green, uint16* i_blue)
            : red_w(red), green_w(green), blue_w(blue), red(i_red), blue(i_blue), green(i_green) {}
...
class w_list : public w_list_base {
  ...
  private:
    ...
    virtual void draw_item(typename vector<T>::const_iterator i, SDL_Surface* s, int16 x, int16 y, uint16 width,
                           bool selected) const
            = 0;
...
class w_levels : public w_list<entry_point> {
  public:
    ...
    w_levels(const vector<entry_point>& items, dialog* d, uint16 inWidth, size_t inNumLines, size_t inSelectedItem,
             bool in_show_level_numbers);

    void item_selected(void);

    void draw_item(vector<entry_point>::const_iterator i, SDL_Surface* s, int16 x, int16 y, uint16 width,
                   bool selected) const;
...
class w_string_list : public w_list<string> {
  public:
    ...
    void draw_item(vector<string>::const_iterator i, SDL_Surface* s, int16 x, int16 y, uint16 width,
                   bool selected) const;
...
class w_select_popup : public w_select_button {
  public:
    ...
    void set_popup_callback(action_proc p, void* a) {
        action = p;
        arg    = a;
    }
...
class w_items_in_room : public w_list_base {
  public:
    ...
    w_items_in_room(ItemClickedCallback itemClicked, int width, int numRows)
        : w_list_base(width, numRows, 0), m_itemClicked(itemClicked) {
        num_items = 0;
        new_items();
    }
  ...
  private:
    ...
    virtual void draw_item(const tElement& item, SDL_Surface* s, int16 x, int16 y, uint16 width, bool selected) const {
        ...
        draw_text(s, w_items_in_room_get_name_of_item(item).c_str(), x, y,
                  selected ? get_theme_color(ITEM_WIDGET, ACTIVE_STATE) : get_theme_color(ITEM_WIDGET, DEFAULT_STATE),
                  font, style);
...
class w_games_in_room : public w_items_in_room<GameListMessage::GameListEntry> {
  public:
    ...
    w_games_in_room(w_items_in_room<GameListMessage::GameListEntry>::ItemClickedCallback itemClicked, int width,
                    int numRows)
        : w_items_in_room<GameListMessage::GameListEntry>(itemClicked, width, numRows),
          kGameSpacing(get_theme_space(METASERVER_GAMES, GAME_SPACING)) {
  ...
  private:
    ...
    void draw_item(const GameListMessage::GameListEntry& item, SDL_Surface* s, int16 x, int16 y, uint16 width,
                   bool selected) const;
};
...
class w_players_in_room : public w_items_in_room<MetaserverPlayerInfo> {
  ...
  private:
    ...
    void draw_item(const MetaserverPlayerInfo& item, SDL_Surface* s, int16 x, int16 y, uint16 width,
                   bool selected) const;
};
...
class w_colorful_chat : public w_list<ColoredChatEntry> {
  ...
  private:
    ...
    void draw_item(vector<ColoredChatEntry>::const_iterator i, SDL_Surface* s, int16 x, int16 y, uint16 width,
                   bool selected) const;
};
...
class ToggleWidget : public SDLWidgetWidget, public Bindable<bool> {
  public:
    ...
    ToggleWidget(w_toggle* toggle) : SDLWidgetWidget(toggle), m_toggle(toggle), m_callback(NULL) {
        m_toggle->set_selection_changed_callback(
                std::bind(&ToggleWidget::massage_callback, this, std::placeholders::_1));
    }
...
class SelectorWidget : public SDLWidgetWidget, public Bindable<int> {
  ...
  protected:
    ...
    SelectorWidget(widget* in_widget) : SDLWidgetWidget(in_widget), m_callback(NULL) {}
...
class PopupSelectorWidget : public SelectorWidget {
  public:
    ...
    PopupSelectorWidget(w_select_popup* select_popup_w)
        : SelectorWidget(select_popup_w), m_select_popup(select_popup_w) {
        select_popup_w->set_popup_callback(
                std::bind(&PopupSelectorWidget::massage_callback, this, std::placeholders::_1), NULL);
    }
  ...
  private:
    ...
    void massage_callback(void* ignored) {
        if (m_callback)
            m_callback();
    }
};
...
class SelectSelectorWidget : public SelectorWidget {
  public:
    ...
    SelectSelectorWidget(w_select* select_w) : SelectorWidget(select_w), m_select(select_w) {
        m_select->set_selection_changed_callback(
                std::bind(&SelectSelectorWidget::massage_callback, this, std::placeholders::_1));
    }
...
class ColourSelectorWidget : public SelectSelectorWidget {
  public:
    ...
    ColourSelectorWidget(w_player_color* player_color_w) : SelectSelectorWidget(player_color_w) {}
...
class ButtonWidget : public SDLWidgetWidget {
  public:
    ...
    ButtonWidget(w_button_base* button) : SDLWidgetWidget(button), m_button(button), m_callback(NULL) {
  ...
  private:
    ...
    static void bounce_callback(void* arg) { reinterpret_cast<ButtonWidget*>(arg)->push(); }
...
class EditTextWidget : public SDLWidgetWidget, public Bindable<std::string> {
  ...
  private:
    ...
    void got_submit(w_text_entry* ignored) {
        if (m_callback)
            m_callback('\r');
    }
};
...
class GameListWidget {
  public:
    ...
    GameListWidget(w_games_in_room* games_in_room) : m_games_in_room(games_in_room) {
        m_games_in_room->set_item_clicked_callback(
                std::bind(&GameListWidget::bounce_callback, this, std::placeholders::_1));
    }
...
class PlayerListWidget {
  public:
    ...
    PlayerListWidget(w_players_in_room* players_in_room) : m_players_in_room(players_in_room) {
        m_players_in_room->set_item_clicked_callback(
                std::bind(&PlayerListWidget::bounce_callback, this, std::placeholders::_1));
    }
...
class JoiningPlayerListWidget {
  public:
    ...
    JoiningPlayerListWidget(w_joining_players_in_room* joining_players_in_room)
        ...
        m_joining_players_in_room->set_item_clicked_callback(
                std::bind(&JoiningPlayerListWidget::bounce_callback, this, std::placeholders::_1));
    }
