...
void PlayerImage::setRandomFlatteringView() {
    int16 theView;
    do { theView = local_random() % 8; } while (theView >= 3 && theView <= 5);
    setView(theView);
}
...
void PlayerImage::updateLegsDrawingInfo() {
    ...
    do {
        ...
        // Find the leg action
        // We use a local here (instead of using the member directly) so we still which things to pick randomly if we
        // loop.
        ...
        uint16 theLegsLowLevelShapeIndex
                = theLegsAnimationData->low_level_shape_indexes[theLegsLowLevelShapeIndexIndex];
        ...
        mLegsSurface = get_shape_surface(theLegsLowLevelShapeIndex,
                                         BUILD_COLLECTION(theShapeDefinitions->collection, theLegsColor), &mLegsData,
                                         mLegsBrightness);
...
void PlayerImage::updateTorsoDrawingInfo() {
    ...
    do {
        ...
        // Finally, we can look up the low-level shape index
        uint16 theTorsoLowLevelShapeIndex
                = theTorsoAnimationData->low_level_shape_indexes[theTorsoLowLevelShapeIndexIndex];
        ...
        mTorsoSurface = get_shape_surface(theTorsoLowLevelShapeIndex,
                                          BUILD_COLLECTION(theShapeDefinitions->collection, theTorsoColor), &mTorsoData,
                                          mTorsoBrightness);
...
void PlayerImage::drawAt(SDL_Surface* inSurface, int16 inX, int16 inY) {
