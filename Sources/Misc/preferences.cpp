...
#include "map.hpp"
#include "shell.hpp" /* For the screen_mode structure */
...
#include "game_wad.hpp" // for set_map_file
#include "network.hpp"  // for _ethernet, etc.
#include "preferences.hpp"
#include "screen.hpp"
#include "wad.hpp"
#include "wad_prefs.hpp"

#include "InfoTree.hpp"
#include "RingGameProtocol.hpp"
...
#include "mouse.hpp"
#include "preference_dialogs.hpp"
#include "preferences_widgets_sdl.hpp"
...
static const char* sNetworkGameProtocolNames[] = { // These should match up with _network_game_protocol_ring, etc.
        "ring", "star"};

static const size_t NUMBER_OF_NETWORK_GAME_PROTOCOL_NAMES
        = sizeof(sNetworkGameProtocolNames) / sizeof(sNetworkGameProtocolNames[0]);
...
static bool ethernet_active(void) { return true; }
...
class CrosshairPref : public Bindable<int> {
  public:
    ...
    virtual int bind_export() { return (m_pref - 1); }

    virtual void bind_import(int value) { m_pref = value + 1; }
...
class ColorComponentPref : public Bindable<int> {
  public:
    ...
    virtual int bind_export() { return (m_pref >> 12); }

    virtual void bind_import(int value) { m_pref = value << 12; }
...
class OpacityPref : public Bindable<int> {
  public:
    ...
    virtual int bind_export() { return (static_cast<int>(floor(m_pref * 16))); }

    virtual void bind_import(int value) { m_pref = ((float)value / 16.0); }
...
static const char* shape_labels[3] = {"Cross", "Octagon", NULL};
...
struct update_crosshair_display {
    void operator()(dialog* d) { crosshair_binders->migrate_all_first_to_second(); }
};

class w_crosshair_slider : public w_slider {
  public:

    w_crosshair_slider(int num_items, int sel) : w_slider(num_items, sel) { init_formatted_value(); }
...
static const char* solo_profile_labels[]
        = {"Aleph One Fixes", "Classic Marathon 2", "Classic Marathon Infinity", nullptr};
    ...
    if (strlen(email_w->get_text()) == 0) {
        ...
        if (conn.Post(A1_METASERVER_SIGNUP_URL, params)) {
            if (conn.Response() == "OK") {
                ...
                strncpy(network_preferences->metaserver_login, login_w->get_text(),
                        network_preferences_data::kMetaserverLoginLength);
                strncpy(network_preferences->metaserver_password, password_w->get_text(),
                        network_preferences_data::kMetaserverLoginLength);
                write_preferences();
                d->quit(0);
            } else {
...
static void signup_dialog(void* arg) {
    ...
    w_text_entry* login_w
            = new w_text_entry(network_preferences_data::kMetaserverLoginLength, network_preferences->metaserver_login);
    login_w->set_identifier(iSIGNUP_USERNAME_W);
    table->dual_add(login_w->label("Username"), d);
    table->dual_add(login_w, d);

    w_password_entry* password_w = new w_password_entry(network_preferences_data::kMetaserverLoginLength,
                                                        network_preferences->metaserver_password);
...
static void online_dialog(void* arg) {
    ...
    if (d.run() == 0) { // Accepted
        ...
        if (strcmp(metaserver_login, network_preferences->metaserver_login)) {
            strncpy(network_preferences->metaserver_login, metaserver_login,
                    network_preferences_data::kMetaserverLoginLength - 1);
        ...
        if (!strlen(metaserver_login)) {
            ...
            if (strcmp(metaserver_password, network_preferences->metaserver_password)) {
                strncpy(network_preferences->metaserver_password, metaserver_password,
                        network_preferences_data::kMetaserverLoginLength - 1);
        ...
        if (use_custom_metaserver_colors) {
            ...
            if (primary_color.red != network_preferences->metaserver_colors[0].red
                || primary_color.green != network_preferences->metaserver_colors[0].green
                || primary_color.blue != network_preferences->metaserver_colors[0].blue) {
                network_preferences->metaserver_colors[0] = primary_color;
                changed                                   = true;
            }

            rgb_color secondary_color = secondary_w->get_selection();
            if (secondary_color.red != network_preferences->metaserver_colors[1].red
                || secondary_color.green != network_preferences->metaserver_colors[1].green
                || secondary_color.blue != network_preferences->metaserver_colors[1].blue) {
...
static const char* depth_labels[3] = {"16 Bit", "32 Bit", NULL};
#else
static const char* depth_labels[4] = {"8 Bit", "16 Bit", "32 Bit", NULL};
#endif

static const char* resolution_labels[3] = {"Low", "High", NULL};

static const char* sw_alpha_blending_labels[4] = {"Off", "Fast", "Nice", NULL};

static const char* sw_sdl_driver_labels[5] = {"Default", "None", "Direct3D", "OpenGL", NULL};


static const char* ephemera_quality_labels[] = {"Off", "Low", "Medium", "High", "Ultra", NULL};

static const char* fps_target_labels[]
        = {"30", "60 (interpolated)", "120 (interpolated)", "Unlimited (interpolated)", NULL};
static const int16_t fps_target_values[] = {30, 60, 120, 0};

static const char* gamma_labels[9]
        = {"Darkest", "Darker", "Dark", "Normal", "Light", "Really Light", "Even Lighter", "Lightest", NULL};

static const char* renderer_labels[] = {"Software", "OpenGL", NULL};

static const char* bobbing_view_labels[] = {"None", "Default", "Weapon Only", NULL};

static const char* hud_scale_labels[] = {"Normal", "Double", "Largest", NULL};

static const char* term_scale_labels[] = {"Normal", "Double", "Largest", NULL};

static const char* mouse_accel_labels[] = {"Off", "Classic", NULL};
...
static const vector<string> build_stringvector_from_cstring_array(const char** label_array) {
    ...
    for (int i = 0; label_array[i] != NULL; ++i) label_vector.push_back(std::string(label_array[i]));
...
static void software_rendering_options_dialog(void* arg) {
...
#else
    w_select* depth_w = new w_select(graphics_preferences->screen_mode.bit_depth == 8    ? 0
                                     : graphics_preferences->screen_mode.bit_depth == 16 ? 1
                                                                                         : 2,
                                     depth_labels);
#endif
    ...
    w_select* sw_alpha_blending_w
            = new w_select(graphics_preferences->software_alpha_blending, sw_alpha_blending_labels);
...
std::vector<std::string> build_resolution_labels() {
    ...
    for (std::vector<std::pair<int, int>>::const_iterator it = Screen::instance()->GetModes().begin();
         it != Screen::instance()->GetModes().end(); ++it) {
...
class w_fov_slider : public w_slider {
  public:
    ...
    w_fov_slider(int sel) : w_slider(101, sel) { init_formatted_value(); }
...
static void graphics_dialog(void* arg) {
    ...
    if (graphics_preferences->screen_mode.auto_resolution)
        ...
        size_w->set_selection(Screen::instance()->FindMode(graphics_preferences->screen_mode.width,
    ...
    w_fov_slider* fov_slider_w
            = new w_fov_slider((graphics_preferences->screen_mode.fov == 0 ? static_cast<int>(View_FOV_Normal())
                                                                           : graphics_preferences->screen_mode.fov)
                               - 30);
    fov_slider_w->set_enabled(graphics_preferences->screen_mode.fov != 0);
    override_fov_w->set_selection_changed_callback([&](w_select*) {
    ...
    if (d.run() == 0) { // Accepted
        ...
        } else if (Screen::instance()->ModeWidth(resolution - 1) != graphics_preferences->screen_mode.width
                   || Screen::instance()->ModeHeight(resolution - 1) != graphics_preferences->screen_mode.height
                   || graphics_preferences->screen_mode.auto_resolution) {
...
class w_music_slider : public w_slider {
  public:
    ...
    w_music_slider(int sel) : w_slider(41, sel) { init_formatted_value(); }
...
static void sound_dialog(void* arg) {
    ...
    w_toggle* hrtf_w = new w_toggle((OpenALManager::Get() && OpenALManager::Get()->Is_HRTF_Enabled())
    ...
    if (d.run() == 0) { // Accepted
        ...
        if (sounds3d_w->get_selection())
            flags |= _3d_sounds_flag;
        if (hrtf_w->get_selection())
            flags |= _hrtf_flag;
        ...
        if (zrd_w->get_selection())
            flags |= _lower_restart_delay;
        if (!dialog_sounds_w->get_selection())
            flags |= _mute_dialogs;
...
class w_sens_slider : public w_slider {
  public:
    ...
    w_sens_slider(int num_items, int sel) : w_slider(num_items, sel) { init_formatted_value(); }
...
class w_deadzone_slider : public w_slider {
  public:
    ...
    w_deadzone_slider(int num_items, int sel) : w_slider(num_items, sel) { init_formatted_value(); }
...
static const char* action_name[NUM_KEYS]
        = {"Move Forward", "Move Backward", "Turn Left",   "Turn Right",       "Sidestep Left", "Sidestep Right",
           "Glance Left",  "Glance Right",  "Look Up",     "Look Down",        "Recenter View", "Previous Weapon",
           "Next Weapon",  "Trigger",       "2nd Trigger", "Turn -> Sidestep", "Run/Swim",      "Move -> Look",
...
static key_binding_map default_key_bindings = {
        ...
        {11,
         {SDL_SCANCODE_F, static_cast<SDL_Scancode>(AO_SCANCODE_MOUSESCROLL_UP),
          static_cast<SDL_Scancode>(AO_SCANCODE_BASE_JOYSTICK_BUTTON + SDL_CONTROLLER_BUTTON_LEFTSHOULDER)}           },
        {12,
         {SDL_SCANCODE_R, static_cast<SDL_Scancode>(AO_SCANCODE_MOUSESCROLL_DOWN),
          static_cast<SDL_Scancode>(AO_SCANCODE_BASE_JOYSTICK_BUTTON + SDL_CONTROLLER_BUTTON_RIGHTSHOULDER)}          },
        {13,
         {SDL_SCANCODE_SPACE, static_cast<SDL_Scancode>(AO_SCANCODE_BASE_MOUSE_BUTTON + SDL_BUTTON_LEFT - 1),
          static_cast<SDL_Scancode>(AO_SCANCODE_BASE_JOYSTICK_AXIS_POSITIVE + SDL_CONTROLLER_AXIS_TRIGGERRIGHT)}      },
        {14,
         {SDL_SCANCODE_LSHIFT, static_cast<SDL_Scancode>(AO_SCANCODE_BASE_MOUSE_BUTTON + SDL_BUTTON_RIGHT - 1),
        ...
        {20,
         {SDL_SCANCODE_GRAVE, static_cast<SDL_Scancode>(AO_SCANCODE_BASE_JOYSTICK_BUTTON + SDL_CONTROLLER_BUTTON_Y)}  },
};
...
static const char* shell_action_name[NUMBER_OF_SHELL_KEYS]
        = {"Inventory Left", "Inventory Right", "Switch Player View", "Volume Up",    "Volume Down",
           "Zoom Map In",    "Zoom Map Out",    "Toggle FPS",         "Chat/Console", "Network Stats"};
...
static const char* hotkey_action_name[NUMBER_OF_HOTKEYS] = {
        "Hotkey 1", "Hotkey 2", "Hotkey 3", "Hotkey 4",  "Hotkey 5",  "Hotkey 6",
        "Hotkey 7", "Hotkey 8", "Hotkey 9", "Hotkey 10", "Hotkey 11", "Hotkey 12",
};
...
static void unset_scancode(SDL_Scancode code) {
    for (int i = 0; i < NUM_KEYS; ++i) input_preferences->key_bindings[i].erase(code);
    for (int i = 0; i < NUMBER_OF_SHELL_KEYS; ++i) input_preferences->shell_key_bindings[i].erase(code);
    for (int i = 0; i < NUMBER_OF_HOTKEYS; ++i) { input_preferences->hotkey_bindings[i].erase(code); }
}
...
static void update_mouse_feel_details(void* arg) {
    if (inside_callback)
        return;
    inside_callback = true;
    if (mouse_raw_w->get_selection() == 1 && mouse_accel_w->get_selection() == 1
        && mouse_vertical_w->get_selection() == 1 && mouse_precision_w->get_selection() == 1
        && mouse_speed_limit_w->get_selection() == 1) {
        mouse_feel_details_w->set_selection(0);
    } else if (mouse_raw_w->get_selection() == 1 && mouse_accel_w->get_selection() == 0
               && mouse_vertical_w->get_selection() == 0 && mouse_precision_w->get_selection() == 0
...
static void update_mouse_feel(void* arg) {
    if (input_preferences->raw_mouse_input == true && input_preferences->mouse_accel_type == _mouse_accel_classic
        && input_preferences->classic_vertical_aim == true && input_preferences->extra_mouse_precision == false
        && input_preferences->classic_aim_speed_limits) {
        mouse_feel_w->set_selection(0);
    } else if (input_preferences->raw_mouse_input == true && input_preferences->mouse_accel_type == _mouse_accel_none
               && input_preferences->classic_vertical_aim == false && input_preferences->extra_mouse_precision == true
               && !input_preferences->classic_aim_speed_limits) {
...
static void mouse_custom_dialog(void* arg) {
    ...
    int hSliderPosition   = (int)((hSensitivityLog - kMinSensitivityLog) * (1000.0f / kSensitivityLogRange) + 0.5f);
    ...
    int vSliderPosition   = (int)((vSensitivityLog - kMinSensitivityLog) * (1000.0f / kSensitivityLogRange) + 0.5f);
...
static void controller_details_dialog(void* arg) {
    ...
    int joySliderPosition   = (int)((joySensitivityLog - kMinSensitivityLog) * (1000.0f / kSensitivityLogRange) + 0.5f);
...
static void controls_dialog(void* arg) {
    ...
    for (int i = 0; i < NUM_KEYS; i++) {
        ...
        SDL_Scancode mcode = SDL_SCANCODE_UNKNOWN;
        SDL_Scancode jcode = SDL_SCANCODE_UNKNOWN;
        for (std::set<SDL_Scancode>::const_iterator bit = input_preferences->key_bindings[i].begin();
             bit != input_preferences->key_bindings[i].end(); ++bit) {
            SDL_Scancode code = *bit;
            if (code >= AO_SCANCODE_BASE_JOYSTICK_BUTTON
                && code < (AO_SCANCODE_BASE_JOYSTICK_BUTTON + NUM_SDL_JOYSTICK_BUTTONS)) {
                jcode = code;
            } else if (code >= AO_SCANCODE_BASE_MOUSE_BUTTON
                       && code < (AO_SCANCODE_BASE_MOUSE_BUTTON + NUM_SDL_MOUSE_BUTTONS)) {
                mcode = code;
            } else {
                kcode = code;
            }
    ...
    for (int i = 0; i < NUMBER_OF_SHELL_KEYS; i++) {
        ...
        SDL_Scancode mcode = SDL_SCANCODE_UNKNOWN;
        SDL_Scancode jcode = SDL_SCANCODE_UNKNOWN;
        for (std::set<SDL_Scancode>::const_iterator bit = input_preferences->shell_key_bindings[i].begin();
             bit != input_preferences->shell_key_bindings[i].end(); ++bit) {
            SDL_Scancode code = *bit;
            if (code >= AO_SCANCODE_BASE_JOYSTICK_BUTTON
                && code < (AO_SCANCODE_BASE_JOYSTICK_BUTTON + NUM_SDL_JOYSTICK_BUTTONS)) {
                jcode = code;
    ...
    for (int i = 0; i < NUMBER_OF_HOTKEYS; ++i) {
        ...
        SDL_Scancode mcode = SDL_SCANCODE_UNKNOWN;
        SDL_Scancode jcode = SDL_SCANCODE_UNKNOWN;
        for (std::set<SDL_Scancode>::const_iterator bit = input_preferences->hotkey_bindings[i].begin();
             bit != input_preferences->hotkey_bindings[i].end(); ++bit) {
            SDL_Scancode code = *bit;
            if (code >= AO_SCANCODE_BASE_JOYSTICK_BUTTON
                && code < (AO_SCANCODE_BASE_JOYSTICK_BUTTON + NUM_SDL_JOYSTICK_BUTTONS)) {
                jcode = code;
    ...
    for (auto it = move_keys.begin(); it != move_keys.end(); ++it) {
        if (*it < 0) {
            ...
            auto range = shell_key_w.equal_range(i);
            for (auto ik = range.first; ik != range.second; ++ik) { move_table->dual_add(ik->second, d); }
        } else {
            int i = *it;
    ...
    static const char* run_option_labels[] = {"Hold to Run", "Hold to Walk", "Tap to Toggle", nullptr};

    static const char* swim_option_labels[] = {"Hold to Swim", "Hold to Sink", nullptr};

    static const char* swim_toggle_labels[] = {"", "",
                                               "Hold to Swim", // hack, otherwise width changes
                                               nullptr};

    w_select* run_w = new w_select(input_preferences->modifiers & _inputmod_run_key_toggle         ? 2
                                   : input_preferences->modifiers & _inputmod_interchange_run_walk ? 1
                                                                                                   : 0,
                                   run_option_labels);
    ...
    w_select* swim_w
            = new w_select(input_preferences->modifiers & _inputmod_interchange_swim_sink ? 1 : 0, swim_option_labels);
    ...
    for (auto it = look_keys.begin(); it != look_keys.end(); ++it) {
        if (*it < 0) {
            ...
            auto range = shell_key_w.equal_range(i);
            for (auto ik = range.first; ik != range.second; ++ik) { look_table->dual_add(ik->second, d); }
        } else {
            int i = *it;
    ...
    for (auto it = actions_keys.begin(); it != actions_keys.end(); ++it) {
        if (*it < 0) {
            ...
            auto range = shell_key_w.equal_range(i);
            for (auto ik = range.first; ik != range.second; ++ik) { actions_table->dual_add(ik->second, d); }
        } else {
            int i = *it;
    ...
    for (auto i = 0; i < NUMBER_OF_HOTKEYS; ++i) {
        ...
        auto range = hotkey_w.equal_range(i);
        for (auto ik = range.first; ik != range.second; ++ik) { hotkey_table->dual_add(ik->second, d); }
    }
    hotkeys->add(hotkey_table, true);

    hotkeys->add(new w_spacer(), true);
    hotkeys->dual_add(new w_static_text("Hotkeys 1-9 are used to switch weapons, but can be overridden by Lua scripts"),
                      d);
    ...
    for (auto it = interface_keys.begin(); it != interface_keys.end(); ++it) {
        if (*it == -2) {
            ...
            auto range = shell_key_w.equal_range(i);
            for (auto ik = range.first; ik != range.second; ++ik) { interface_table->dual_add(ik->second, d); }
        } else {
            int i = *it;
    ...
    std::vector<std::string> game_shortcuts = {"F1",         "Decrease resolution",
    ...
    if (d.run() == 0) { // Accepted
        ...
        if (run_w->get_selection() == 2) {
            ...
            if (run_w->get_selection())
                flags |= _inputmod_interchange_run_walk;
            if (swim_w->get_selection())
        ...
        for (int i = 0; i < NUM_KEYS; i++) { input_preferences->key_bindings[i].clear(); }
        for (int i = 0; i < NUMBER_OF_SHELL_KEYS; i++) { input_preferences->shell_key_bindings[i].clear(); }

        for (int i = 0; i < NUMBER_OF_HOTKEYS; ++i) { input_preferences->hotkey_bindings[i].clear(); }
...
static const char* film_profile_labels[] = {"Aleph One 1.0", "Marathon 2", "Marathon Infinity", 0};
...
static void environment_dialog(void* arg) {
    ...
    w_env_select* physics_w = new w_env_select(environment_preferences->physics_file, "AVAILABLE PHYSICS MODELS",
                                               _typecode_physics, &d);
    table->dual_add(physics_w->label("Physics"), d);
    table->dual_add(physics_w, d);

    w_env_select* shapes_w
            = new w_env_select(environment_preferences->shapes_file, "AVAILABLE SHAPES", _typecode_shapes, &d);
    table->dual_add(shapes_w->label("Shapes"), d);
    table->dual_add(shapes_w, d);

    w_env_select* sounds_w
            = new w_env_select(environment_preferences->sounds_file, "AVAILABLE SOUNDS", _typecode_sounds, &d);
    table->dual_add(sounds_w->label("Sounds"), d);
    table->dual_add(sounds_w, d);

    w_env_select* resources_w
            = new w_env_select(environment_preferences->resources_file, "AVAILABLE FILES", _typecode_unknown, &d);
    ...
    *sound_preferences = SoundManager::Parameters();
    ...
    if (opened) {
        ...
        try {
            ...
            if (!version.length())
                logWarning("Reading older preferences of unknown version. Preferences will be upgraded to version %s "
                           "when saved. (%s)",
                           A1_DATE_VERSION, FileSpec.GetPath());
            else if (version < A1_DATE_VERSION)
                logWarning("Reading older preferences of version %s. Preferences will be upgraded to version %s when "
                           "saved. (%s)",
                           version.c_str(), A1_DATE_VERSION, FileSpec.GetPath());
            else if (version > A1_DATE_VERSION)
                logWarning("Reading newer preferences of version %s. Preferences will be downgraded to version %s when "
                           "saved. (%s)",
                           version.c_str(), A1_DATE_VERSION, FileSpec.GetPath());

            for (const InfoTree& child : root.children_named("graphics")) parse_graphics_preferences(child, version);
            for (const InfoTree& child : root.children_named("player")) parse_player_preferences(child, version);
            for (const InfoTree& child : root.children_named("input")) parse_input_preferences(child, version);
            for (const InfoTree& child : root.children_named("sound")) parse_sound_preferences(child, version);
#if !defined(DISABLE_NETWORKING)
            for (const InfoTree& child : root.children_named("network")) parse_network_preferences(child, version);
#endif
    ...
    if (defaults) {
        if (parse_error) {
            alert_user(expand_app_variables(
                               "There were default preferences-file parsing errors (see $appLogFile$ for details)")
                               .c_str(),
                       infoError);
        }
    } else if (!opened || parse_error) {
        alert_user(expand_app_variables("There were preferences-file parsing errors (see $appLogFile$ for details)")
                           .c_str(),
                   infoError);
    }
...
InfoTree graphics_preferences_tree() {
    ...
    for (int i = 0; i <= OGL_NUMBER_OF_TEXTURE_TYPES; ++i) {
        OGL_Texture_Configure& Config = (i == OGL_NUMBER_OF_TEXTURE_TYPES)
                                                ? graphics_preferences->OGL_Configure.ModelConfig
                                                : graphics_preferences->OGL_Configure.TxtrConfigList[i];
...
static const char* binding_action_name[NUM_KEYS]
        = {"forward",      "back",    "look-left", "look-right", "strafe-left", "strafe-right", "glance-left",
           "glance-right", "look-up", "look-down", "look-ahead", "prev-weapon", "next-weapon",  "trigger-1",
           "trigger-2",    "strafe",  "run",       "look",       "action",      "map",          "microphone"};
static const char* binding_shell_action_name[NUMBER_OF_SHELL_KEYS]
        = {"inventory-left", "inventory-right", "switch-player-view", "volume-up",
           "volume-down",    "map-zoom-in",     "map-zoom-out",       "fps",
           "chat",           "net-stats"};
...
static const char* binding_mouse_button_name[NUM_SDL_MOUSE_BUTTONS]
        = {"mouse-left", "mouse-middle", "mouse-right", "mouse-x1", "mouse-x2", "mouse-scroll-up", "mouse-scroll-down"};

static const char* get_binding_joystick_button_name(int offset) {
    static_assert(SDL_CONTROLLER_BUTTON_MAX <= 21 && SDL_CONTROLLER_AXIS_MAX <= 12,
    ...
    static const char* axes[]
            = {"controller-ls-right", "controller-ls-down", "controller-rs-right", "controller-rs-down",
               "controller-lt",       "controller-rt",      "controller-ls-left",  "controller-ls-up",
               "controller-rs-left",  "controller-rs-up",   "controller-lt-neg",   "controller-rt-neg"};
...
static const char* binding_scancode_name[binding_num_scancodes] = {"unknown",
                                                                   "unknown-1",
                                                                   "unknown-2",
                                                                   "unknown-3",
                                                                   "a",
...
static const char* binding_name_for_code(SDL_Scancode code) {
    ...
    if (i >= 0 && i < binding_num_scancodes)
...
InfoTree network_preferences_tree() {
    ...
    for (int i = 0; i < 2; i++) root.add_color("color", network_preferences->metaserver_colors[i], i);
...
static void default_network_preferences(network_preferences_data* preferences) {
    ...
    preferences->game_options     = _multiplayer_game | _ammo_replenishes | _weapons_replenish | _specials_replenish
                                | _burn_items_on_death | _force_unique_teams | _live_network_stats;
...
static bool validate_network_preferences(network_preferences_data* preferences) {
    ...
    // ZZZ: is this relevant anymore now with XML prefs?  if so, should validate autogather, join_by_address, and
    // join_address.
...
void load_environment_from_preferences(void) {
    ...
    if (File.Exists()) {
        ...
        if (find_wad_file_that_has_checksum(File, _typecode_scenario, strPATHS, prefs->map_checksum)) {
            set_map_file(File);
        } else {
            set_to_default_map();
        }
    ...
    if (!File.Exists()
        && !find_wad_file_that_has_checksum(File, _typecode_physics, strPATHS, prefs->physics_checksum)) {
    ...
    if (!File.Exists() && !find_file_with_modification_date(File, _typecode_shapes, strPATHS, prefs->shapes_mod_date)) {
    ...
    if (!File.Exists() && !find_file_with_modification_date(File, _typecode_sounds, strPATHS, prefs->sounds_mod_date)) {
...
void standardize_player_behavior_modifiers() { sStandardizeModifiers = true; }

void restore_custom_player_behavior_modifiers() { sStandardizeModifiers = false; }

bool is_player_behavior_standard() { return !dont_switch_to_new_weapon(); }
...
bool dont_auto_recenter() { return TEST_FLAG(input_preferences->modifiers, _inputmod_dont_auto_recenter); }
...
void parse_graphics_preferences(InfoTree root, std::string version) {
    ...
    root.read_attr_bounded<int16>("movie_export_video_quality", graphics_preferences->movie_export_video_quality, 0,
                                  100);
    root.read_attr_bounded<int16>("movie_export_audio_quality", graphics_preferences->movie_export_audio_quality, 0,
                                  100);
    ...
    for (const InfoTree& tex : root.children_named("texture")) {
        ...
        if (tex.read_indexed("index", index, OGL_NUMBER_OF_TEXTURE_TYPES + 1)) {
            OGL_Texture_Configure& Config = (index == OGL_NUMBER_OF_TEXTURE_TYPES)
                                                    ? graphics_preferences->OGL_Configure.ModelConfig
                                                    : graphics_preferences->OGL_Configure.TxtrConfigList[index];
...
SDL_Scancode translate_old_key(int code) {
    ...
    static SDL_Keycode key_lookups[] = {SDLK_UNKNOWN,
                                        SDLK_UNKNOWN,
                                        SDLK_UNKNOWN,
                                        SDLK_UNKNOWN,
                                        SDLK_UNKNOWN,
...
void parse_input_preferences(InfoTree root, std::string version) {
    ...
    root.read_attr_bounded<int16>("mouse_accel_type", input_preferences->mouse_accel_type, 0,
                                  NUMBER_OF_MOUSE_ACCEL_TYPES - 1);
    ...
    for (const InfoTree& key : root.children_named("binding")) {
        ...
        if (key.read_attr("action", action_name) && key.read_attr("pressed", pressed_name)) {
...
void parse_sound_preferences(InfoTree root, std::string version) {
    if (!version.length() || version < "20200803") {
        ...
        if (old_volume > 0) {
            sound_preferences->volume_db
                    = 10.f * std::log10(static_cast<float>(old_volume) / NUMBER_OF_SOUND_VOLUME_LEVELS);
        } else {
    ...
    if (!version.length() || version < "20200803") {
        ...
        if (old_music_volume > 0) {
            sound_preferences->music_db
                    = 10.f * std::log10(static_cast<float>(old_music_volume) / NUMBER_OF_SOUND_VOLUME_LEVELS);
        } else {
