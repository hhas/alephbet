...
#include "cseries.hpp"
#include "network_dialog_widgets_sdl.hpp"
...
#include "mouse.hpp" // (ZZZ) NUM_SDL_MOUSE_BUTTONS, SDLK_BASE_MOUSE_BUTTON
...
widget::widget()
    : active(false), dirty(false), enabled(true), font(NULL), identifier(NONE), owning_dialog(NULL), saved_min_width(0),
      saved_min_height(0), associated_label(0) {
    rect.x = 0;
    rect.y = 0;
    rect.w = 0;
    rect.h = 0;
...
widget::widget(int theme_widget)
    : active(false), dirty(false), enabled(true), font(get_theme_font(theme_widget, style)), identifier(NONE),
      owning_dialog(NULL), saved_min_width(0), saved_min_height(0), associated_label(0) {
    rect.x = 0;
    rect.y = 0;
    rect.w = 0;
    rect.h = 0;
...
void widget::associate_label(w_label* label) { associated_label = label; }
...
void w_static_text::set_text(const char* t) {
    free(text);
    text  = strdup(t);
    dirty = true;
}
...
w_static_text::~w_static_text() { free(text); }
...
void w_slider_text::draw(SDL_Surface* s) const {
    int state
            = associated_slider->enabled ? (associated_slider->active ? ACTIVE_STATE : DEFAULT_STATE) : DISABLED_STATE;
    uint16 style = 0;
    draw_text(s, text, rect.x, rect.y + font->get_ascent() + (rect.h - font->get_line_height()) / 2,
              get_theme_color(LABEL_WIDGET, state, FOREGROUND_COLOR), font, style);
}
...
w_button_base::w_button_base(const char* t, action_proc p, void* a, int _type)
    : widget(_type), text(t), proc(p), arg(a), down(false), pressed(false), type(_type) {
    rect.w = text_width(text.c_str(), font, style) + get_theme_space(_type, BUTTON_L_SPACE)
             + get_theme_space(_type, BUTTON_R_SPACE);
    button_c_default  = get_theme_image(_type, DEFAULT_STATE, BUTTON_C_IMAGE,
                                        rect.w - get_theme_image(_type, DEFAULT_STATE, BUTTON_L_IMAGE)->w
                                                - get_theme_image(_type, DEFAULT_STATE, BUTTON_R_IMAGE)->w);
    button_c_active   = get_theme_image(_type, ACTIVE_STATE, BUTTON_C_IMAGE,
                                        rect.w - get_theme_image(_type, ACTIVE_STATE, BUTTON_L_IMAGE)->w
                                                - get_theme_image(_type, ACTIVE_STATE, BUTTON_R_IMAGE)->w);
    button_c_disabled = get_theme_image(_type, DISABLED_STATE, BUTTON_C_IMAGE,
                                        rect.w - get_theme_image(_type, DISABLED_STATE, BUTTON_L_IMAGE)->w
                                                - get_theme_image(_type, DISABLED_STATE, BUTTON_R_IMAGE)->w);
    button_c_pressed  = get_theme_image(_type, PRESSED_STATE, BUTTON_C_IMAGE,
                                        rect.w - get_theme_image(_type, PRESSED_STATE, BUTTON_L_IMAGE)->w
                                                - get_theme_image(_type, PRESSED_STATE, BUTTON_R_IMAGE)->w);
...
void w_button_base::draw(SDL_Surface* s) const {
    ...
    if (use_theme_images(type)) {
        ...
        // Button image
        SDL_Rect r = {rect.x, rect.y, static_cast<Uint16>(button_l->w), static_cast<Uint16>(button_l->h)};
    ...
    draw_text(s, text.c_str(), rect.x + get_theme_space(type, BUTTON_L_SPACE),
              rect.y + get_theme_space(type, BUTTON_T_SPACE) + font->get_ascent(), get_theme_color(type, state), font,
              style);
}
...
w_hyperlink::w_hyperlink(const char* url, const char* txt)
    : w_button_base((txt ? txt : url), std::bind(&w_hyperlink::prochandler, this, std::placeholders::_1),
                    const_cast<char*>(url), HYPERLINK_WIDGET) {
...
w_tab::w_tab(const vector<string>& _labels, tab_placer* _placer)
    : widget(TAB_WIDGET), labels(_labels), placer(_placer), active_tab(1), pressed_tab(0) {
    saved_min_height = get_theme_space(TAB_WIDGET, BUTTON_HEIGHT);
    for (vector<string>::iterator it = labels.begin(); it != labels.end(); ++it) {
        int l_space = (it == labels.begin()) ? get_theme_space(TAB_WIDGET, BUTTON_L_SPACE)
                                             : get_theme_space(TAB_WIDGET, TAB_LC_SPACE);
        int r_space = (it == labels.end() - 1) ? get_theme_space(TAB_WIDGET, BUTTON_R_SPACE)
                                               : get_theme_space(TAB_WIDGET, TAB_RC_SPACE);
        ...
        for (int i = 0; i < 4; ++i) {
            int li_space = (it == labels.begin()) ? get_theme_image(TAB_WIDGET, states[i], TAB_L_IMAGE)->w
                                                  : get_theme_image(TAB_WIDGET, states[i], TAB_LC_IMAGE)->w;
            int ri_space = (it == labels.end() - 1) ? get_theme_image(TAB_WIDGET, states[i], TAB_R_IMAGE)->w
                                                    : get_theme_image(TAB_WIDGET, states[i], TAB_RC_IMAGE)->w;
...
w_tab::~w_tab() {
    for (std::vector<std::vector<SDL_Surface*>>::iterator it = images.begin(); it != images.end(); ++it) {
        for (std::vector<SDL_Surface*>::iterator it2 = it->begin(); it2 != it->end(); ++it2) { SDL_FreeSurface(*it2); }
    }
}
    ...
    for (int i = 0; i < labels.size(); ++i) {
        ...
        if (use_theme_images(TAB_WIDGET)) {
            ...
            if (use_theme_color(TAB_WIDGET, BACKGROUND_COLOR)) {
                SDL_Rect r   = {x + l_offset, rect.y + 1, l_space + c_space + r_space - l_offset - r_offset,
                                get_theme_space(TAB_WIDGET, BUTTON_HEIGHT) - 2};
                uint32 pixel = get_theme_color(TAB_WIDGET, state, BACKGROUND_COLOR);
                SDL_FillRect(s, &r, pixel);
            }
        }

        font->draw_text(s, labels[i].c_str(), labels[i].size(), x + l_space,
                        rect.y + get_theme_space(TAB_WIDGET, BUTTON_T_SPACE) + font->get_ascent(),
                        get_theme_color(TAB_WIDGET, state, FOREGROUND_COLOR), style);
...
static const char* sNoValidOptionsString
        = "(no valid options)"; // XXX should be moved outside compiled code e.g. to MML

w_select::w_select(size_t s, const char** l)
    : widget(LABEL_WIDGET), labels(l), we_own_labels(false), selection(s), selection_changed_callback(NULL),
      utf8(false) {
    num_labels = 0;
    if (labels) {
        while (labels[num_labels]) num_labels++;
...
int w_select::min_width() { return get_largest_label_width(); }
...
void w_select::set_labels(const char** inLabels) {
    ...
    num_labels = 0;
    if (labels) {
        while (inLabels[num_labels]) num_labels++;
    }
    we_own_labels = false;
    if (selection >= num_labels)
        set_selection(0);
...
void w_toggle::draw(SDL_Surface* s) const {
    ...
    if (labels == onoff_labels && use_theme_images(CHECKBOX)) {
        ...
        draw_text(s, str, rect.x, rect.y + (rect.h - saved_min_height) / 2 + get_theme_space(CHECKBOX, BUTTON_T_SPACE),
                  get_theme_color(LABEL_WIDGET, state, FOREGROUND_COLOR), font, style, utf8);
    } else {
...
void w_enabling_toggle::selection_changed() {
    w_toggle::selection_changed();

    for (DependentCollection::iterator i = dependents.begin(); i != dependents.end(); i++) update_widget_enabled(*i);
}
...
void w_color_picker::click(int, int) {
    ...
    d.set_processing_function(
            w_color_picker::update_color(red_w, green_w, blue_w, &m_color.red, &m_color.green, &m_color.blue));
...
w_text_entry::w_text_entry(size_t max_c, const char* initial_text)
    : widget(TEXT_ENTRY_WIDGET), enter_pressed_callback(NULL), value_changed_callback(NULL), max_chars(max_c),
      enable_mac_roman(false) {
    // Initialize buffer
...
w_text_entry::~w_text_entry() { delete[] buf; }
...
home:
            ...
            case SDLK_w:
                if (e.key.keysym.mod & KMOD_CTRL) {
                    ...
                    while (erase_position && buf[erase_position - 1] == ' ') --erase_position;
                    while (erase_position && buf[erase_position - 1] != ' ') --erase_position;
...
void w_text_entry::click(int x, int y) {
    ...
    if (!was_active || !num_chars || (x == 0 && y == 0) || x < 0 || y < 0 || x >= rect.w || y >= rect.h) {
        return;
    }
...
w_password_entry::w_password_entry(size_t max_chars, const char* initial_text)
    : w_text_entry(max_chars, initial_text) {}
...
static const char* get_joystick_button_key_name(int offset) {
    ...
    static const char* axes[] = {"LS Right", "LS Down", "RS Right", "RS Down", "LT",     "RT",
...
const char* GetSDLKeyName(SDL_Scancode inKey) {
...
w_key::Type w_key::event_type_for_key(SDL_Scancode key) {
    if (key >= AO_SCANCODE_BASE_MOUSE_BUTTON && key < (AO_SCANCODE_BASE_MOUSE_BUTTON + NUM_SDL_MOUSE_BUTTONS))
        return MouseButton;
    else if (key >= AO_SCANCODE_BASE_JOYSTICK_BUTTON
             && key < (AO_SCANCODE_BASE_JOYSTICK_BUTTON + NUM_SDL_JOYSTICK_BUTTONS))
...
void w_key::set_key(SDL_Scancode k) { key = k; }
...
w_slider::w_slider(int num, int s)
    : widget(LABEL_WIDGET), selection(s), num_items(num), thumb_dragging(false), readout(NULL),
      slider_changed_callback(NULL) {
    ...
    trough_width = SLIDER_WIDTH - get_theme_space(SLIDER_WIDGET, SLIDER_L_SPACE)
...
void w_slider::draw(SDL_Surface* s) const {
    if (use_theme_images(SLIDER_WIDGET)) {
        ...
        SDL_Rect r = {rect.x + slider_x, rect.y + (saved_min_height - slider_l->h) / 2,
                      static_cast<Uint16>(slider_l->w), static_cast<Uint16>(slider_l->h)};
        ...
        SDL_Rect r   = {rect.x,
                        rect.y + (saved_min_height - SLIDER_TROUGH_HEIGHT) / 2
                                + get_theme_space(SLIDER_WIDGET, SLIDER_T_SPACE),
                        SLIDER_WIDTH, SLIDER_TROUGH_HEIGHT};
...
w_list_base::w_list_base(uint16 width, size_t lines, size_t /*sel*/)
    : widget(ITEM_WIDGET), selection(0), num_items(0), shown_items(lines), thumb_dragging(false), top_item(0) {
    rect.w = width;
    rect.h = item_height() * static_cast<uint16>(shown_items) + get_theme_space(LIST_WIDGET, T_SPACE)
    ...
    min_thumb_height
            = static_cast<uint16>(thumb_t->h + thumb_tc_unscaled->h + thumb_c->h + thumb_bc_unscaled->h + thumb_b->h);
    ...
    trough_rect.h
            = rect.h - get_theme_space(LIST_WIDGET, TROUGH_T_SPACE) - get_theme_space(LIST_WIDGET, TROUGH_B_SPACE);
...
void w_list_base::draw(SDL_Surface* s) const {
    if (use_theme_images(LIST_WIDGET)) {
        ...
        draw_image(s, frame_br, x + static_cast<int16>(frame_bl->w) + static_cast<int16>(frame_b->w),
                   y + static_cast<int16>(frame_tr->h) + static_cast<int16>(frame_r->h));
        ...
        SDL_Rect thumb_rect = {rect.x + trough_rect.x, rect.y + thumb_y, trough_rect.w,
                               thumb_t->h + thumb_tc->h + thumb_c->h + thumb_bc->h + thumb_b->h};
...
void w_list_base::mouse_move(int x, int y) {
    if (thumb_dragging) {
        ...
        if ((y - get_theme_space(LIST_WIDGET, T_SPACE)) / item_height() + top_item
            < std::min(num_items, top_item + shown_items)) {
...
void w_list_base::new_items(void) {
    ...
    thumb_bc       = get_theme_image(LIST_THUMB, DEFAULT_STATE, THUMB_BC_IMAGE, 0,
                               (rem_height & 1) ? dyn_height + 1 : dyn_height);
...
void w_list_base::center_item(size_t i) { set_top_item((i > shown_items / 2) ? i - shown_items / 2 : 0); }
...
w_levels::w_levels(const vector<entry_point>& items, dialog* d, uint16 inWidth, size_t inNumLines,
                   size_t inSelectedItem, bool in_show_level_numbers)
    : w_list<entry_point>(items, inWidth, inNumLines, inSelectedItem), parent(d),
      show_level_numbers(in_show_level_numbers), offset{1} {}

void w_levels::item_selected(void) { parent->quit(0); }

void w_levels::draw_item(vector<entry_point>::const_iterator i, SDL_Surface* s, int16 x, int16 y, uint16 width,
                         bool selected) const {
    y = y + font->get_ascent();
    char str[256];
...
void w_string_list::item_selected(void) { parent->quit(0); }

void w_string_list::draw_item(vector<string>::const_iterator i, SDL_Surface* s, int16 x, int16 y, uint16 width,
                              bool selected) const {
    y = y + font->get_ascent();
    char str[256];
...
void w_file_chooser::set_file(const FileSpecifier& inFile) {
    file = inFile;
    update_filename();
}

void w_file_chooser::click(int, int) {
    if (enabled) {
...
const string w_items_in_room_get_name_of_item(GameListMessage::GameListEntry item) { return item.name(); }

const string w_items_in_room_get_name_of_item(prospective_joiner_info item) { return std::string(item.name); }

const string w_items_in_room_get_name_of_item(MetaserverPlayerInfo item) { return item.name(); }

void w_games_in_room::draw_item(const GameListMessage::GameListEntry& item, SDL_Surface* s, int16 x, int16 y,
                                uint16 width, bool selected) const {
    ...
    if (item.running()) {
        if (item.m_description.m_timeLimit
            && !(item.m_description.m_timeLimit == INT32_MAX || item.m_description.m_timeLimit == -1)) {
    ...
    if (item.running()) {
        ...
        game_settings << static_cast<uint16>(item.m_description.m_numPlayers) << "/" << item.m_description.m_maxPlayers
                      << " Players";
    }
    ...
    set_drawing_clip_rectangle(0, x, static_cast<short>(s->h), x + width);
    font->draw_styled_text(s, item.m_hostPlayerName, item.m_hostPlayerName.size(), x + width - right_text_width, y, fg,
                           game_style);
...
void w_players_in_room::draw_item(const MetaserverPlayerInfo& item, SDL_Surface* s, int16 x, int16 y, uint16 width,
                                  bool selected) const {
    set_drawing_clip_rectangle(0, x, static_cast<short>(s->h), x + width);
    ...
    if (item.target()) {
        int amount = 0x7f;
        pixel      = SDL_MapRGB(s->format, lighten(item.color()[0] >> 8, amount), lighten(item.color()[1] >> 8, amount),
        ...
        pixel = SDL_MapRGB(s->format, darken(item.color()[0] >> 8, amount), darken(item.color()[1] >> 8, amount),
    ...
    if (item.target()) {
        ...
        pixel      = SDL_MapRGB(s->format, lighten(item.team_color()[0] >> 8, amount),
                                lighten(item.team_color()[1] >> 8, amount), lighten(item.team_color()[2] >> 8, amount));
    } else {
        int amount;
        if (item.away())
            amount = 0x7f;
        ...
        pixel = SDL_MapRGB(s->format, darken(item.team_color()[0] >> 8, amount),
                           darken(item.team_color()[1] >> 8, amount), darken(item.team_color()[2] >> 8, amount));
    }
    ...
    font->draw_styled_text(s, item.name(), item.name().size(), x + kPlayerColorSwatchWidth + kSwatchGutter + 2, y + 1,
                           color, item.away() ? style : style | styleShadow);
...
void w_colorful_chat::draw_item(vector<ColoredChatEntry>::const_iterator it, SDL_Surface* s, int16 x, int16 y,
                                uint16 width, bool selected) const {
    ...
    if ((*it).type == ColoredChatEntry::ChatMessage || (*it).type == ColoredChatEntry::PrivateMessage) {
        ...
        uint32 pixel = SDL_MapRGB(s->format, (*it).color.red >> 8, (*it).color.green >> 8, (*it).color.blue >> 8);
        SDL_FillRect(s, &r, pixel);
        ...
        font->draw_styled_text(s, it->sender, it->sender.size(), x + 1, computed_y,
                               SDL_MapRGB(s->format, 0xff, 0xff, 0xff), style | styleShadow);
