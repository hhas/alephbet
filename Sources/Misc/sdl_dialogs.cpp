...
void initialize_dialogs() {
    ...
    // Default image
    default_image = SDL_CreateRGBSurface(SDL_SWSURFACE, 1, 1, 24, 0xff'0000, 0x00'ff00, 0x00'00ff, 0);
...
void shutdown_dialogs(void) { unload_theme(); }
...
static void parse_theme_image(InfoTree root, int type, int state, int max_index) {
    int index = -1;
    std::string name;
    if (!root.read_attr("file", name) || !root.read_attr_bounded("index", index, 0, max_index))
        return;
...
static void parse_theme_images(InfoTree root, int type, int state, int num_items = 1) {
    for (const InfoTree& img : root.children_named("image")) { parse_theme_image(img, type, state, num_items - 1); }
}
...
static void parse_theme_color(InfoTree root, int type, int state, int max_index) {
    ...
    if (!root.read_attr("red", red) || !root.read_attr("green", green) || !root.read_attr("blue", blue))
        return;
...
static void parse_theme_colors(InfoTree root, int type, int state, int num_items = 1) {
    for (const InfoTree& color : root.children_named("color")) { parse_theme_color(color, type, state, num_items - 1); }
}
...
static void parse_theme_fonts(InfoTree root, int type) {
    for (const InfoTree& child : root.children_named("font")) parse_theme_font(child, type);
}
...
static void parse_spacer(InfoTree root) { root.read_attr("height", dialog_theme[SPACER_WIDGET].spaces[0]); }
...
static bool parse_theme_file(FileSpecifier& theme_mml) {
    ...
    try {
        ...
        for (const InfoTree& child : root.children_named("default")) parse_default(child);
        for (const InfoTree& child : root.children_named("frame")) parse_frame(child);
        for (const InfoTree& child : root.children_named("title")) parse_title(child);
        for (const InfoTree& child : root.children_named("spacer")) parse_spacer(child);
        for (const InfoTree& child : root.children_named("button")) parse_button(child);
        for (const InfoTree& child : root.children_named("tiny_button")) parse_tiny_button(child);
        for (const InfoTree& child : root.children_named("hyperlink")) parse_hyperlink(child);
        for (const InfoTree& child : root.children_named("item")) parse_item(child);
        for (const InfoTree& child : root.children_named("label")) parse_label(child);
        for (const InfoTree& child : root.children_named("message")) parse_message(child);
        for (const InfoTree& child : root.children_named("text_entry")) parse_text_entry(child);
        for (const InfoTree& child : root.children_named("chat_entry")) parse_chat_entry(child);
        for (const InfoTree& child : root.children_named("list")) parse_list(child);
        for (const InfoTree& child : root.children_named("slider")) parse_slider(child);
        for (const InfoTree& child : root.children_named("checkbox")) parse_checkbox(child);
        for (const InfoTree& child : root.children_named("tab")) parse_tab(child);
        for (const InfoTree& child : root.children_named("metaserver")) parse_metaserver(child);
        ...
        logError("error parsing %s: %s", theme_mml.GetPath(), e.what());
    } catch (const InfoTree::unexpected_error& e) { logError("error parsing %s: %s", theme_mml.GetPath(), e.what()); }
    return success;
}
...
bool load_theme(FileSpecifier& theme) {
    ...
    for (std::map<int, theme_widget>::iterator i = dialog_theme.begin(); i != dialog_theme.end(); ++i) {
        for (std::map<int, theme_state>::iterator j = i->second.states.begin(); j != i->second.states.end(); ++j) {
            for (std::map<int, dialog_image_spec_type>::iterator k = j->second.image_specs.begin();
                 k != j->second.image_specs.end(); ++k) {
...
static void set_theme_defaults(void) {
    ...
    dialog_theme[METASERVER_GAMES].states[w_games_in_room::GAME].colors[FOREGROUND_COLOR]
            = make_color(0xff, 0xff, 0xff);
    dialog_theme[METASERVER_GAMES].states[w_games_in_room::INCOMPATIBLE_GAME].colors[FOREGROUND_COLOR]
            = make_color(0x7f, 0, 0);
    dialog_theme[METASERVER_GAMES].states[w_games_in_room::RUNNING_GAME].colors[FOREGROUND_COLOR]
            = make_color(0x7f, 0x7f, 0x7f);

    dialog_theme[METASERVER_GAMES].states[w_games_in_room::SELECTED_GAME].colors[FOREGROUND_COLOR]
            = make_color(0x0, 0x0, 0x0);
    dialog_theme[METASERVER_GAMES].states[w_games_in_room::SELECTED_GAME].colors[BACKGROUND_COLOR]
            = make_color(0xff, 0xff, 0xff);

    dialog_theme[METASERVER_GAMES].states[w_games_in_room::SELECTED_INCOMPATIBLE_GAME].colors[FOREGROUND_COLOR]
            = make_color(0x7f, 0, 0);
    dialog_theme[METASERVER_GAMES].states[w_games_in_room::SELECTED_INCOMPATIBLE_GAME].colors[BACKGROUND_COLOR]
            = make_color(0xff, 0xff, 0xff);
    dialog_theme[METASERVER_GAMES].states[w_games_in_room::SELECTED_RUNNING_GAME].colors[FOREGROUND_COLOR]
            = make_color(0x7f, 0x7f, 0x7f);
    dialog_theme[METASERVER_GAMES].states[w_games_in_room::SELECTED_RUNNING_GAME].colors[BACKGROUND_COLOR]
            = make_color(0xff, 0xff, 0xff);
}
...
int16 dialog_sound_definitions[]
        = {_snd_pattern_buffer,          _snd_pattern_buffer,       _snd_defender_hit,
           _snd_spht_door_obstructed,    _snd_major_fusion_charged, _snd_computer_interface_page,
           _snd_computer_interface_page, _snd_hummer_attack,        _snd_compiler_death};
...
void play_dialog_sound(int which) {
    if (!(sound_preferences->flags & _mute_dialogs) && dialog_sound_definitions[which] != NONE) {
        SoundManager::instance()->PlaySound(dialog_sound_definitions[which], 0, NONE);
    }
}

widget_placer::~widget_placer() {
    for (std::vector<placeable*>::iterator it = m_owned.begin(); it != m_owned.end(); it++) { delete (*it); }
}
...
int table_placer::min_height() {
    int height = 0;
    for (int row = 0; row < m_table.size(); ++row) { height += row_height(row); }
...
int table_placer::min_width() {
    ...
    if (m_balance_widths) {
        ...
        for (int col = 0; col < m_columns; ++col) { width += col_width(col); }
...
void table_placer::place(const SDL_Rect& r, placement_flags flags) {
    ...
    if (m_balance_widths) {
        if (flags & kFill) {
            ...
            for (int i = 0; i < column_widths.size(); i++) { column_widths[i] = column_width; }
        }
    } else if (flags & kFill) {
        ...
        if (columns_remaining == m_columns || columns_remaining == 0) {
            for (int i = 0; i < column_widths.size(); i++) { column_widths[i] = col_width(i) + (pool / m_columns); }
        } else {
        ...
        for (int i = 0; i < column_widths.size(); i++) { column_widths[i] = col_width(i); }
    }
...
void table_placer::visible(bool visible) {
    for (int row = 0; row < m_table.size(); ++row) {
        for (int col = 0; col < m_table[row].size(); ++col) { m_table[row][col]->visible(visible); }
    }
}
...
dialog::dialog()
    : active_widget(NULL), mouse_widget(0), active_widget_num(UNONE), done(false), cursor_was_visible(false),
      parent_dialog(NULL), processing_function(NULL), placer(0), last_redraw(0) {}
    ...
    do {
        ...
        if (i >= int(widgets.size()))
            i = 0;
              ...
              || (widgets[i]->associated_label == widgets[active_widget_num]
                  || widgets[active_widget_num]->associated_label == widgets[i]))
             && i != active_widget_num);
...
void dialog::event(SDL_Event& e) {
    ...
    switch (e.type) {
        ...
        case SDL_KEYDOWN:
            ...
            if (e.key.keysym.sym == SDLK_RETURN && ((e.key.keysym.mod & KMOD_ALT) || (e.key.keysym.mod & KMOD_GUI))) {
...
bool dialog::process_events() {
    ...
    if (SDL_WaitEventTimeout(&e, 30)) {
        event(e);
        while (!done && SDL_PollEvent(&e)) { event(e); }
    }
