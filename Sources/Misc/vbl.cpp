...
#define INCREMENT_QUEUE_COUNTER(c)     \
    {                                  \
        (c)++;                         \
        if ((c) >= MAXIMUM_QUEUE_SIZE) \
            (c) = 0;                   \
    }
...
void initialize_keyboard_controller(void) {
    ...
    //		csprintf(temporary, "NUMBER_OF_KEYS == %d, NUMBER_OF_KEY_DEFS = %d. Not Equal!", NUMBER_OF_KEYS,
...
bool get_keyboard_controller_status(void) { return input_task_active; }

int32 get_heartbeat_count(void) { return heartbeat_count; }

void sync_heartbeat_count(void) { heartbeat_count = dynamic_world->tick_count; }
...
void set_replay_speed(short speed) { replay.replay_speed = speed; }

int get_replay_speed() { return replay.replay_speed; }

bool game_is_being_replayed() { return replay.game_is_being_replayed; }

void increment_heartbeat_count(int value) { heartbeat_count += value; }
...
bool input_controller(void) {
    if (input_task_active || Movie::instance()->IsRecording()) {
        if ((heartbeat_count - dynamic_world->tick_count)
            < ((first_frame_rendered || game_is_networked) ? MAXIMUM_TIME_DIFFERENCE : 1)) {
...
bool setup_for_replay_from_file(FileSpecifier& File, uint32 map_checksum, bool prompt_to_export) {
    ...
    if (FilmFileSpec.Open(FilmFile)) {
        ...
        replay.header.game_information.cheat_flags
...
void check_recording_replaying(void) {
    ...
    if (replay.game_is_being_recorded) {
        ...
        for (player_index = 0; enough_data_to_save && player_index < dynamic_world->player_count; player_index++) {
            ...
            if (queue_size < RECORD_CHUNK_SIZE)
                enough_data_to_save = false;
        }
        ...
        if (enough_data_to_save) {
            ...
            if (success
                && freespace > (RECORD_CHUNK_SIZE * sizeof(int16) * sizeof(uint32) * dynamic_world->player_count)) {
                for (player_index = 0; player_index < dynamic_world->player_count; player_index++) {
                    save_recording_queue_chunk(player_index);
                }
            }
...
uint8* unpack_recording_header(uint8* Stream, recording_header* Objects, size_t Count) {
    ...
    for (size_t k = 0; k < Count; k++, ObjPtr++) {
        ...
        for (int m = 0; m < MAXIMUM_NUMBER_OF_PLAYERS; m++) StreamToPlayerStart(S, ObjPtr->starts[m]);
...
uint8* pack_recording_header(uint8* Stream, recording_header* Objects, size_t Count) {
    ...
    for (size_t k = 0; k < Count; k++, ObjPtr++) {
        ...
        for (size_t m = 0; m < MAXIMUM_NUMBER_OF_PLAYERS; m++) PlayerStartToStream(S, ObjPtr->starts[m]);
...
void move_replay(void) {
    ...
    if (!dst_file.WriteDialog(_typecode_film, getcstr(prompt, strPROMPTS, _save_replay_prompt),
...
static constexpr uint32_t hotkey_used = 0x8000'0000;

void encode_hotkey_sequence(int hotkey) {
    hotkey_sequence[0] = (3 << _cycle_weapons_forward_bit) | hotkey_used;

    hotkey_sequence[1] = ((hotkey / 4 + 1) << _cycle_weapons_forward_bit) | hotkey_used;

    hotkey_sequence[2] = ((hotkey % 4) << _cycle_weapons_forward_bit) | hotkey_used;
}
...
uint32 parse_keymap(void) {
    ...
    if (get_keyboard_controller_status()) {
        ...
        if (input_preferences->modifiers & _inputmod_run_key_toggle) {
            ...
            bool do_interchange = (local_player->variables.flags & _HEAD_BELOW_MEDIA_BIT)
...
bool setup_replay_from_random_resource() {
    ...
    if (demos.size()) {
        ...
        auto index                    = 0;
        if (demos.size() > 1) {
            do { index = local_random() % demos.size(); } while (index == last_played_index);
        }
...
void remove_timer_task(timer_task_proc proc) { tm_func = NULL; }

void execute_timer_tasks(uint32 time) {
    if (tm_func) {
        if (Movie::instance()->IsRecording()) {
            if (get_fps_target() == 0 || movie_export_phase++ % (get_fps_target() / 30) == 0) {
                tm_func();
            }
            return;
        }
