...
static inline void lowercase(string& s) { transform(s.begin(), s.end(), s.begin(), ::tolower); }
...
void Console::enter() {
    ...
    if (m_prev_commands.size() == 0 || m_buffer != *(m_prev_commands.end() - 1)) {
...
void Console::delete_word() {
    ...
    while (erase_position && m_buffer[erase_position - 1] == ' ') --erase_position;
    while (erase_position && m_buffer[erase_position - 1] != ' ') --erase_position;
...
void Console::up_arrow() {
    if (m_command_iter == m_prev_commands.begin())
        return;
    m_command_iter--;
...
void Console::line_home() { m_cursor_position = 0; }

void Console::line_end() { m_cursor_position = m_buffer.length(); }

void Console::activate_input(std::function<void(const std::string&)> callback, const std::string& prompt) {
    assert(!m_active);
    m_callback = callback;
    m_buffer.clear();
...
int Console::cursor_position() { return m_prompt.length() + 1 + m_cursor_position; }
...
void Console::clear_macros() { m_macros.clear(); }
...
void Console::report_kill(int16 player_index, int16 aggressor_player_index, int16 projectile_index) {
    if (!game_is_networked || !NetAllowCarnageMessages() || !m_carnage_messages_exist || projectile_index == -1)
        return;
...
void Console::clear_saves() { last_level.clear(); }
