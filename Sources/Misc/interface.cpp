...
#include "cseries.hpp" // sorry ryan, nov. 4
#include <algorithm>
#include <array>
#include <limits.h>
#include <sstream>
...
#include "interface_menus.hpp"
#include "lua_script.hpp" // PostIdle
#include "map.hpp"
#include "network.hpp"
#include "player.hpp"
...
#include "motion_sensor.hpp" // for reset_motion_sensor()
...
struct screen_data m1_display_screens[] = {
        ...
        {MAIN_MENU_BASE,                 1,                                     0                         },
        {10'000,                         NUMBER_OF_CHAPTER_HEADINGS,            CHAPTER_HEADING_DURATION  },
        {PROLOGUE_SCREEN_BASE,           NUMBER_OF_PROLOGUE_SCREENS,            PROLOGUE_DURATION         },
        {EPILOGUE_SCREEN_BASE,           NUMBER_OF_EPILOGUE_SCREENS,            EPILOGUE_DURATION         },
...
static screen_data* get_screen_data(short index);
...
void initialize_game_state(void) {
    ...
    if (shell_options.insecure_lua) {
        alert_user(
                expand_app_variables("Insecure Lua has been manually enabled. Malicious Lua scripts can use Insecure "
                                     "Lua to take over your computer. Unless you specifically trust every single Lua "
                                     "script that will be running, you should quit $appName$ IMMEDIATELY.")
                        .c_str());
    }
...
void force_game_state_change(void) { game_state.phase = 0; }
...
short get_game_state(void) { return game_state.state; }

bool current_netgame_allows_microphone() { return game_state.current_netgame_allows_microphone; }

bool suppress_background_events(void) { return game_state.suppress_background_tasks; }

short get_game_controller(void) { return game_state.user; }
...
static short get_difficulty_level(void) { return player_preferences->difficulty_level; }
...
static void synchronize_players_with_starts(const player_start_data* inStartArray, short inStartCount,
    ...
    for (; s < dynamic_world->player_count; s++) {
        ...
        if (inStartArray[s].identifier == NONE) {
            ...
            SET_PLAYER_DOESNT_AUTO_SWITCH_WEAPONS_STATUS(
                    thePlayer, player_identifier_doesnt_auto_switch_weapons(inStartArray[s].identifier));
...
static bool make_restored_game_relevant(bool inNetgame, const player_start_data* inStartArray, short inStartCount) {
    ...
    if (!success)
        clean_up_after_failed_game(inNetgame, false /*recording*/, true /*full cleanup*/);
    ...
    if (success) {
        ...
        if (userWantsMultiplayer) {
            ...
            if (success) {
                ...
                if (success && use_remote_hub)
                    return join_networked_resume_game();
            }
        }
#endif // !defined(DISABLE_NETWORKING)
        ...
        if (success) {
            ...
            if (userWantsMultiplayer) {
                ...
                success = NetStart();
                if (success) {
                    OSErr theError = NetDistributeGameDataToAllPlayers(
                            theSavedGameFlatData.get(), theSavedGameFlatDataLength, false /* do_physics? */);
                    if (theError != noErr) {
                        success = false;
                    }
                }
        ...
        if (game_state.phase <= 0) {
            switch (get_game_state()) {
                ...
                case _display_main_menu:
                    /* Start the demo.. */
                    if (!environment_preferences->auto_play_demos || !begin_game(_demo, false)) {
    ...
    if (game_state.state == _game_in_progress) {
        ...
        if (get_keyboard_controller_status()) {
            ...
            if (theUpdateResult.first
                || (last_heartbeat_fraction != -1 && last_heartbeat_fraction != heartbeat_fraction)) {
                    ...
                    switch (game_state.user) {
                        case _single_player:
                            if (PLAYER_IS_DEAD(local_player)
                                || dynamic_world->tick_count - local_player->ticks_at_last_successful_save
                                           < CLOSE_WITHOUT_WARNING_DELAY
                                || shell_options.output.size()) {
...
std::array<int, iAbout> menu_item_order = {iNewGame,
                                           iLoadGame,
                                           iGatherGame,
                                           iJoinGame,
                                           iReplaySavedFilm,
...
void process_main_menu_highlight_advance(bool reverse) {
    ...
    const auto last_index = []() {
        return std::distance(
                       std::find_if(menu_item_order.rbegin(), menu_item_order.rend(), [](int i) { return i != -1; }),
...
static void display_epilogue(void) {
    ...
    for (int i = 0; i < end_count; i++) try_and_display_chapter_screen(end_offset + i, true, true, true);
    show_cursor();
}
...
class w_authors_list : public w_string_list {
  public:
    ...
    w_authors_list(const vector<string>& items, dialog* d) : w_string_list(items, d, 0) {}
...
static void display_about_dialog() {
    ...
    about_placer->dual_add(
            new w_static_text(expand_app_variables("Aleph Bet is free software with ABSOLUTELY NO WARRANTY.").c_str()),
            d);
    ...
    about_placer->dual_add(
            new w_static_text(expand_app_variables("Scenario loaded: $scenarioName$ $scenarioVersion$").c_str()), d);
...
static void transfer_to_new_level(short level_number) {
    ...
    if (success) {
        ...
        if (!game_is_networked)
            try_and_display_chapter_screen(level_number, true, false, false);
    ...
    switch (user) {
        ...
        case _demo:
            ...
            if (success) {
                ...
                get_recording_header_data(&number_of_players, &entry.level_number, &unused1, &recording_version, starts,
                                          &game_information);
        ...
        case _single_player:
            ...
            game_information.game_options
                    = _burn_items_on_death | _ammo_replenishes | _weapons_replenish | _monsters_replenish;
    ...
    if (success) {
        if (record_game) {
            if (!get_map_file().Exists()) {
                ...
                set_recording_header_data(number_of_players, entry.level_number,
                                          (user == _network_player) ? parent_checksum : get_current_map_checksum(),
...
static void start_game(short user, bool changing_level) {
    ...
    assert((!changing_level && !get_keyboard_controller_status())
           || (changing_level && get_keyboard_controller_status()));
    if (!changing_level) {
        set_keyboard_controller_status(true);
    }
...
static void finish_game(bool return_to_main_menu) {
    ...
    assert(game_state.state == _game_in_progress || game_state.state == _switch_demo || game_state.state == _revert_game
    ...
    if (game_state.user == _network_player) {
        ...
        if (game_state.user == _replay) {
            ...
            } else if (!(dynamic_world->game_information.game_type == _game_of_kill_monsters
...
static void handle_network_game(bool gatherer) {
    ...
    if (gatherer) {
        ...
        if (successful_gather && !use_remote_hub)
            successful_gather = NetStart();
    } else {
        int theNetworkJoinResult = network_join();
        if (theNetworkJoinResult == kNetworkJoinedNewGame || theNetworkJoinResult == kNetworkJoinedResumeGame)
            successful_gather = true;
        if (theNetworkJoinResult == kNetworkJoinedResumeGame)
        ...
        if (images_picture_exists(pict_resource_number)) {
            ...
            if (game_state.state == _display_intro_screens) {
                ...
                if (get_sound_resource_from_images(pict_resource_number, SoundRsrc)) {
                    _fixed pitch = (shapes_file_is_m1() && game_state.state == _display_intro_screens)
                                           ? _m1_high_frequency
                                           : _normal_frequency;
                    SoundParameters parameters;
...
static void try_and_display_chapter_screen(short level, bool interface_table_is_valid, bool text_block,
    ...
    if (scenario_picture_exists(pict_resource_number)) {
        ...
        if (!epilogue_screen)
            SoundManager::instance()->StopAllSounds(); // don't stop the music if intro restarted for epilogue
...
void interface_fade_out(short pict_resource_number, bool fade_music) {
    assert(current_picture_clut);
    if (current_picture_clut) {
...
static bool can_interface_fade_out(void) { return (current_picture_clut == NULL) ? false : true; }

bool interface_fade_finished(void) { return fade_finished(); }
...
int movie_audio_callback(uint8* stream, int length) {
    ...
    if (movie_audio_mutex && SDL_LockMutex(movie_audio_mutex) != -1) {
        if (aframes[0]->size == length) {
            ...
            for (int i = 1; i < AUDIO_BUF_SIZE; i++) aframes[i - 1] = aframes[i];
...
void show_movie(short index) {
    ...
    if (vframe) {
        vframe->surface = SDL_CreateRGBSurface(SDL_SWSURFACE, dst_rect.w, dst_rect.h, 32, 0x00ff'0000, 0x0000'ff00,
    ...
    while (!done) {
        ...
        if (!movie_audio_player || !movie_audio_player->IsActive()) {
            movie_audio_player
                    = OpenALManager::Get()->PlayStream(movie_audio_callback, specs.channels * specs.samples * 2,
                                                       specs.freq, specs.channels == 2, AudioFormat::_32_float);
        }
        if (vframe) {
            if (!astream) {
    ...
    while (movie_audio_player->IsActive()) { sleep_for_machine_ticks(MACHINE_TICKS_PER_SECOND / 100); }

    OpenALManager::Get()->Stop();
    movie_audio_player.reset();

    if (astream) {
        for (int i = 0; i < AUDIO_BUF_SIZE; i++) { SDL_ffmpegFreeAudioFrame(aframes[i]); }
