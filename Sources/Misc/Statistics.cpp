...
class ScopedMutex {
  public:
    ...
    ScopedMutex(SDL_mutex* mutex) : mutex_(mutex) { SDL_LockMutex(mutex_); }
...
int StatsManager::Run(void* pv) {
    ...
    while (sm->run_) {
        ...
        if (entry.get()) {
            entry->parameters["platform"] = AB_DISPLAY_PLATFORM;
            ...
            // generate checksum
            uint32 checksum = 0;
            for (std::map<std::string, std::string>::const_iterator it = entry->parameters.begin();
                 it != entry->parameters.end(); ++it) {
