...
class TexQualityPref : public Bindable<int> {
  public:
    ...
    virtual void bind_import(int value) { m_pref = (value == 0) ? 0 : m_normal << (value - 1); }
...
class TimesTwoPref : public Bindable<int> {
  public:
    ...
    virtual int bind_export() { return (m_pref / 2); }

    virtual void bind_import(int value) { m_pref = value * 2; }
...
class AnisotropyPref : public Bindable<int> {
  public:
    ...
    virtual void bind_import(int value) { m_pref = (value == 0) ? 0.0 : 1 << (value - 1); }
...
void OpenGLDialog::OpenGLPrefsByRunning() {
    ...
    FarFilterPref spritesFarFilterPref(
            graphics_preferences->OGL_Configure.TxtrConfigList[OGL_Txtr_Inhabitant].FarFilter);
    ...
    Int16Pref landscapeNearFilterPref(
            graphics_preferences->OGL_Configure.TxtrConfigList[OGL_Txtr_Landscape].NearFilter);
    ...
    Int16Pref weaponNearFilterPref(
            graphics_preferences->OGL_Configure.TxtrConfigList[OGL_Txtr_WeaponsInHand].NearFilter);
...
static const char* far_filter_labels[5] = {"None", "Linear", "Bilinear", "Trilinear", NULL};

static const char* near_filter_labels[3] = {"None", "Linear", NULL};

static std::vector<std::string> ephemera_quality_labels{"Off", "Low", "Medium", "High", "Ultra"};

class w_aniso_slider : public w_slider {
  public:

    w_aniso_slider(int num_items, int sel) : w_slider(num_items, sel) { init_formatted_value(); }
...
class SdlOpenGLDialog : public OpenGLDialog {
  public:
    ...
    ~SdlOpenGLDialog() { delete m_tabs; }

    virtual bool Run() { return (m_dialog.run() == 0); }

    virtual void Stop(bool result) { m_dialog.quit(result ? 0 : -1); }
...
std::unique_ptr<OpenGLDialog> OpenGLDialog::Create(int theSelectedRenderer) {
