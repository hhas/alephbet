...
#include "player.hpp" // for mask_in_absolute_positioning_information
...
void initialize_joystick(void) {
    ...
    for (int i = 0; i < SDL_NumJoysticks(); ++i) joystick_added(i);
}

void enter_joystick(void) { joystick_active = true; }

void exit_joystick(void) { joystick_active = false; }
...
void joystick_added(int device_index) {
    if (!SDL_IsGameController(device_index)) {
        ...
        logWarning("No mapping found for controller \"%s\" (%s)", SDL_JoystickName(joystick), guidStr);
        return;
    }
...
void joystick_axis_moved(int instance_id, int axis, int value) {
    ...
    button_values[AO_SCANCODE_BASE_JOYSTICK_AXIS_POSITIVE - AO_SCANCODE_BASE_JOYSTICK_BUTTON + axis]
            = (value >= 16'384);
    button_values[AO_SCANCODE_BASE_JOYSTICK_AXIS_NEGATIVE - AO_SCANCODE_BASE_JOYSTICK_BUTTON + axis]
            = (value <= -16'384);
}
...
void joystick_buttons_become_keypresses(Uint8* ioKeyMap) {
    ...
    if (input_preferences->controller_analog) {
        ...
        for (auto it = axis_mappings.begin(); it != axis_mappings.end(); ++it) {
            ...
            if (axis >= 0) {
                buttons_to_avoid.insert(axis
                                        + (negative ? AO_SCANCODE_BASE_JOYSTICK_AXIS_NEGATIVE
...
int process_joystick_axes(int flags, int tick) {
    ...
    const fixed_angle dpitch = static_cast<fixed_angle>(angular_deltas[_flags_pitch] * FIXED_ONE)
                               * (input_preferences->controller_aim_inverted ? -1 : 1);
