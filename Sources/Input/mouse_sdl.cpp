...
void enter_mouse(short type) {
    if (type != _keyboard_or_game_pad) {
        ...
        button_mask = 0; // Disable all buttons (so a shot won't be fired if we enter the game with a mouse button down
                         // from clicking a GUI widget)
    }
}
...
static inline float MIX(float start, float end, float factor) { return (start * (1.f - factor)) + (end * factor); }
...
void mouse_idle(short type) {
    if (mouse_active) {
        ...
        float sy = angle_per_scaled_delta * (input_preferences->sens_vertical / float{FIXED_ONE})
                   * (input_preferences->classic_vertical_aim ? 0.25f : 1.f);
        switch (input_preferences->mouse_accel_type) {
            case _mouse_accel_classic:
                ...
                sy *= MIX(1.f, (1 / (input_preferences->classic_vertical_aim ? 8.f : 32.f)) * fabs(dy * sy),
                          input_preferences->mouse_accel_scale);
                break;
...
void mouse_buttons_become_keypresses(Uint8* ioKeyMap) {
    ...
    for (int i = 0; i < NUM_SDL_REAL_MOUSE_BUTTONS; i++) {
        ioKeyMap[AO_SCANCODE_BASE_MOUSE_BUTTON + i] = (buttons & SDL_BUTTON(i + 1)) ? SDL_PRESSED : SDL_RELEASED;
    }
...
void hide_cursor(void) { SDL_ShowCursor(0); }

void show_cursor(void) { SDL_ShowCursor(1); }
