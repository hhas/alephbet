...
void OGL_SetDefaults(OGL_ConfigureData& Data) {
    ...
    Data.Flags = OGL_Flag_Fader | OGL_Flag_Map | OGL_Flag_HUD | OGL_Flag_LiqSeeThru | OGL_Flag_3D_Models
                 | OGL_Flag_ZBuffer | OGL_Flag_Fog | OGL_Flag_MimicSW;
    ...
    for (int il = 0; il < 4; il++)
        for (int ie = 0; ie < 2; ie++) Data.LscpColors[il][ie] = DefaultLscpColors[il][ie];
...
inline bool StringPresent(vector<char>& String) { return (String.size() > 1); }
...
void OGL_TextureOptionsBase::Load() {
    ...
    if (Type >= 0 && Type < OGL_NUMBER_OF_TEXTURE_TYPES
        && Get_OGL_ConfigureData().TxtrConfigList[Type].FarFilter > 1 /* GL_LINEAR */) {
    ...
    if (NormalColors != FileSpecifier() && NormalColors.Exists()) {
        if (!NormalImg.LoadFromFile(NormalColors, ImageLoader_Colors,
                                    flags | (NormalIsPremultiplied ? ImageLoader_ImageIsAlreadyPremultiplied : 0),
    ...
    // load a heightmap
    if (TEST_FLAG(Get_OGL_ConfigureData().Flags, OGL_Flag_BumpMap) && OffsetMap != FileSpecifier()
        && OffsetMap.Exists()) {
        if (!OffsetImg.LoadFromFile(OffsetMap, ImageLoader_Colors,
                                    flags | (NormalIsPremultiplied ? ImageLoader_ImageIsAlreadyPremultiplied : 0),
                                    actual_width, actual_height, maxTextureSize)) {
            return;
        }
    ...
    if (!GlowImg.IsPresent()) {
        ...
        if (GlowColors != FileSpecifier() && GlowColors.Exists()) {
            if (GlowImg.LoadFromFile(GlowColors, ImageLoader_Colors,
                                     flags | (GlowIsPremultiplied ? ImageLoader_ImageIsAlreadyPremultiplied : 0),
                                     actual_width, actual_height, maxTextureSize)) {
                ...
                if (GlowMask != FileSpecifier() && GlowMask.Exists()) {
                    GlowImg.LoadFromFile(GlowMask, ImageLoader_Opacity, flags, actual_width, actual_height,
                                         maxTextureSize);
                }
            }
        }
...
int OGL_CountModelsImages(short Collection) { return OGL_CountTextures(Collection) + OGL_CountModels(Collection); }
...
OGL_FogData* OGL_GetFogData(int Type) { return GetMemberWithBounds(FogData, Type, OGL_NUMBER_OF_FOG_TYPES); }
...
void reset_mml_opengl() {
    ...
    if (OriginalFogData) {
        for (unsigned i = 0; i < OGL_NUMBER_OF_FOG_TYPES; i++) FogData[i] = OriginalFogData[i];
...
void parse_mml_opengl(const InfoTree& root) {
    ...
    if (!OriginalFogData) {
        ...
        for (unsigned i = 0; i < OGL_NUMBER_OF_FOG_TYPES; i++) OriginalFogData[i] = FogData[i];
    }
    ...
    for (const InfoTree& shader : root.children_named("shader")) { parse_mml_opengl_shader(shader); }
    ...
    for (const InfoTree& fog : root.children_named("fog")) {
        ...
        for (const InfoTree& color : fog.children_named("color")) { color.read_color(def.Color); }
    }
#endif
}
...
void SglColor3usv(const GLushort* iv) {
    GLfloat ov[3] = {sRGB_frob(iv[0] * (1.f / 65535.f)), sRGB_frob(iv[1] * (1.f / 65535.f)),
                     sRGB_frob(iv[2] * (1.f / 65535.f))};
    glColor3fv(ov);
}
...
void SglColor4usv(const GLushort* iv) {
    GLfloat ov[4] = {sRGB_frob(iv[0] * (1.f / 65535.f)), sRGB_frob(iv[1] * (1.f / 65535.f)),
                     sRGB_frob(iv[2] * (1.f / 65535.f)), iv[3] * (1.f / 65535.f)};
    glColor4fv(ov);
}
#endif
