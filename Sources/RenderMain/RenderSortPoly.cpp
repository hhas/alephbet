...
void RenderSortPolyClass::Resize(size_t NumPolygons) { polygon_index_to_sorted_node.resize(NumPolygons); }
            ...
            for (node_data* Alias = FoundNode; Alias; Alias = Alias->PS_Shared) {
                ...
                if (node->reference) {
                    *(node->reference) = node->siblings;
                    if (node->siblings)
                        (node->siblings)->reference = node->reference;
                }
            }
...
clipping_window_data* RenderSortPolyClass::build_clipping_windows(node_data* ChainBegin) {
    // LP change: growable lists
    AccumulatedLineClips.clear();
    AccumulatedEndpointClips.clear();
    endpoint_clip_data* endpoint;
    line_clip_data* line;
    short x0, x1; /* ignoring what clipping parameters we’ve gotten, this is the left and right borders of this node on
                     the screen */
    short i, j;
    ...
    for (node_data* ChainNode = ChainBegin; ChainNode; ChainNode = ChainNode->PS_Shared) {
        ...
        for (node = ChainNode; node; node = node->parent) /* examine this node and all parents! */
        {
            ...
            for (i = 0; i < node->clipping_endpoint_count; ++i) {
                ...
                {
                    if (AccumulatedEndpointClips[j] == endpoint) {
                        j = NONE;
                        break;
                    } /* found duplicate */
                ...
                if (j != NONE) /* if the endpoint was not a duplicate */
                {
                    ...
                    //					assert(AccumulatedEndpointClips.size() <= 32767);		// Originally a short
                    //value
                    if (j != Length)
                        memmove(&AccumulatedEndpointClips[j + 1], &AccumulatedEndpointClips[j],
            ...
            for (i = 0; i < node->clipping_line_count; ++i) {
                ...
                for (j = 0; j < size; ++j)
                    if (AccumulatedLineClips[j] == line)
    ...
    /* build the clipping windows */
    {
        ...
        for (i = 0; i < short(AccumulatedEndpointClips.size()); ++i) {
            ...
            switch (endpoint->flags) {
                ...
                default:
                    vassert(false,
                            csprintf(temporary, "RenderSortPoly.cpp: build_clipping_windows(): bad state: %d", state));
                    break;
            }
...
void RenderSortPolyClass::calculate_vertical_clip_data(line_clip_data** accumulated_line_clips,
                                                       size_t accumulated_line_clip_count, clipping_window_data* window,
                                                       short x0, short x1) {
    {
        short x;
        ...
        do {
            ...
            if (!highest_line || locally_highest_line->top_y < highest_line->top_y) {
                ...
                //				dprintf("%p [%d,%d] is new highest top clip line for window [%d,%d]", highest_line,
                //highest_line->x0, highest_line->x1, x0, x1);
            }
        ...
        assert(highest_line);
        //		dprintf("%p [%d,%d] is highest top clip line for window [%d,%d]", highest_line, highest_line->x0,
        //highest_line->x1, x0, x1);
        ...
        do {
            ...
            if (!highest_line || locally_highest_line->bottom_y > highest_line->bottom_y) {
                highest_line = locally_highest_line;
                //				dprintf("%p [%d,%d] is new lowest bottom clip line for window [%d,%d]", highest_line,
                //highest_line->x0, highest_line->x1, x0, x1);
            }
        ...
        assert(highest_line);
        //		dprintf("%p [%d,%d] is lowest bottom clip line for window [%d,%d]", highest_line, highest_line->x0,
        //highest_line->x1, x0, x1);
