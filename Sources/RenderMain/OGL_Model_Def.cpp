...
#include "Dim3_Loader.hpp"
#include "InfoTree.hpp"
#include "StudioLoader.hpp"
...
int OGL_SkinData::GetMaxSize() { return Get_OGL_ConfigureData().ModelConfig.MaxSize; }
...
// Src -> Dest
static void MatCopy(const GLfloat SrcMat[3][3], GLfloat DestMat[3][3]) { objlist_copy(DestMat[0], SrcMat[0], 9); }

// Sets the arg to that matrix
static void MatIdentity(GLfloat Mat[3][3]) {
    const GLfloat IMat[3][3] = {
            {1, 0, 0},
            {0, 1, 0},
            {0, 0, 1}
    };
...
static void MatMult(const GLfloat Mat1[3][3], const GLfloat Mat2[3][3], GLfloat DestMat[3][3]) {
    for (int k = 0; k < 3; k++)
        for (int l = 0; l < 3; l++) {
            GLfloat Sum = 0;
            for (int m = 0; m < 3; m++) Sum += Mat1[k][m] * Mat2[m][l];
            DestMat[k][l] = Sum;
        }
}
...
static void MatScalMult(GLfloat Mat[3][3], const GLfloat Scale) {
    for (int k = 0; k < 3; k++) {
        ...
        for (int l = 0; l < 3; l++) MatRow[l] *= Scale;
    }
}
...
static void MatVecMult(const GLfloat Mat[3][3], const GLfloat* SrcVec, GLfloat* DestVec) {
    for (int k = 0; k < 3; k++) {
        ...
        GLfloat Sum           = 0;
        for (int l = 0; l < 3; l++) Sum += MatRow[l] * SrcVec[l];
        DestVec[k] = Sum;
    }
}
...
static bool StringsEqual(const char* String1, const char* String2, int MaxStrLen) {
    ...
    for (int k = 0; k < MaxStrLen; k++, S1++, S2++) {
        ...
        // Compare!
        if (c1 == 0 && c2 == 0)
            return true; // All in both strings equal
        else if (c1 != c2)
            return false; // At least one unequal
...
void OGL_ModelData::Load() {
    ...
    if (Model.FindPositions_Neutral(false)) {
            ...
            for (int i2 = 0; i2 < 2; i2++) {
                ...
                for (int i3 = 0; i3 < 2; i3++) {
                    ...
                    if (RestOfCorners) {
                        ...
                        // Simply copy it in:
                        for (int ic = 0; ic < 3; ic++) NewBoundingBox[0][ic] = NewBoundingBox[1][ic] = Corner[ic];
                        RestOfCorners = true;
                    }
                }
            }
        ...
        for (int ic = 0; ic < 2; ic++) objlist_copy(Model.BoundingBox[ic], NewBoundingBox[ic], 3);
    } else {
        // Static model
...
int OGL_CountModels(short Collection) { return MdlList[Collection].size(); }
...
void reset_mml_opengl_model() { MdlDeleteAll(); }
...
void parse_mml_opengl_model(const InfoTree& root) {
    ...
    for (vector<ModelDataEntry>::iterator MdlIter = ML.begin(); MdlIter < ML.end(); MdlIter++) {
        ...
        if (MdlIter->Sequence != entry.Sequence)
            continue;
        if (MdlIter->SequenceMap.size() != entry.SequenceMap.size())
            continue;
