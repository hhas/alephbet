...
#else
static bool DisableClipVertex() { return false; }
#endif
...
const char* Shader::_uniform_names[NUMBER_OF_UNIFORM_LOCATIONS] = {"texture0",
                                                                   "texture1",
                                                                   "texture2",
                                                                   "texture3",
                                                                   "time",
...
const char* Shader::_shader_names[NUMBER_OF_SHADER_TYPES] = {"error",
                                                             "blur",
                                                             "bloom",
                                                             "landscape",
                                                             "landscape_bloom",
...
void Shader_MML_Parser::reset() { Shader::_shaders.clear(); }
...
void reset_mml_opengl_shader() { Shader_MML_Parser::reset(); }

void parse_mml_opengl_shader(const InfoTree& root) { Shader_MML_Parser::parse(root); }
...
void Shader::loadAll() {
    ...
    if (!_shaders.size()) {
        ...
        for (int i = 0; i < NUMBER_OF_SHADER_TYPES; ++i) { _shaders.push_back(Shader(_shader_names[i])); }
    }
}

void Shader::unloadAll() {
    for (int i = 0; i < _shaders.size(); ++i) { _shaders[i].unload(); }
}
...
Shader::Shader(const std::string& name, FileSpecifier& vert, FileSpecifier& frag, int16& passes)
    : _programObj(0), _passes(passes), _loaded(false) {
    initDefaultPrograms();
...
void Shader::setMatrix4(UniformName name, float* f) { glUniformMatrix4fvARB(getUniformLocation(name), 1, false, f); }
...
void Shader::disable() { glUseProgramObjectARB(0); }
...
int16 Shader::passes() { return _passes; }
...
void initDefaultPrograms() {
    ...
    defaultFragmentPrograms["gamma"] = ""
                                       ...
                                       "	gl_FragColor = vec4(pow(color0.r, gammaAdjust), pow(color0.g, "
                                       "gammaAdjust), pow(color0.b, gammaAdjust), 1.0);\n"
                                       "}\n";
