...
#include "OGL_Setup.hpp" // OGL_IsActive
#endif
...
const uint32* ImageDescriptor::GetMipMapPtr(int Level) const {
    int totalSize = 0;
    for (int i = 0; i < Level; i++) { totalSize += GetMipMapSize(i); }
...
bool ImageDescriptor::Minify() {
    if (MipMapCount > 1) {
        ...
        if (OGL_IsActive()) {
            ...
            gluScaleImage(GL_RGBA, Width, Height, GL_UNSIGNED_BYTE, Pixels, newWidth, newHeight, GL_UNSIGNED_BYTE,
                          newPixels);
            delete[] Pixels;
            Pixels = newPixels;
            Width  = newWidth;
...
ImageDescriptor::ImageDescriptor(const ImageDescriptor& copyFrom)
    : Width(copyFrom.Width), Height(copyFrom.Height), VScale(copyFrom.VScale), UScale(copyFrom.UScale),
      Size(copyFrom.Size), MipMapCount(copyFrom.MipMapCount), Format(copyFrom.Format),
...
ImageDescriptor::ImageDescriptor(int _Width, int _Height, uint32* _Pixels)
    : Width(_Width), Height(_Height), VScale(1.0), UScale(1.0), Pixels(_Pixels), Format(RGBA8), MipMapCount(0),
      PremultipliedAlpha(false) {
    Size = _Width * _Height * 4;
}

static inline int padfour(int x) { return (x + 3) / 4 * 4; }
...
bool ImageDescriptor::LoadMipMapFromFile(OpenedFile& file, int flags, int level, DDSURFACEDESC2& ddsd, int skip) {
    ...
    int totalSize = 0;
    for (int i = skip; i < level; i++) { totalSize += GetMipMapSize(i); }
    ...
    if (Format == RGBA8) {
        ...
        SDL_Surface* src = SDL_CreateRGBSurfaceFrom(
                &img.front(), srcWidth, srcHeight, ddsd.ddpfPixelFormat.dwRGBBitCount, pitch,
                ddsd.ddpfPixelFormat.dwRBitMask, ddsd.ddpfPixelFormat.dwGBitMask, ddsd.ddpfPixelFormat.dwBBitMask,
                (ddsd.ddpfPixelFormat.dwFlags & DDPF_ALPHAPIXELS) ? ddsd.ddpfPixelFormat.dwRGBAlphaBitMask : 0);
        SDL_SetSurfaceBlendMode(src, SDL_BLENDMODE_NONE); // disable SDL_SRCALPHA

        SDL_Surface* dst = nullptr;
        if (PlatformIsLittleEndian()) {
            dst = SDL_CreateRGBSurfaceFrom(buffer, dstWidth, dstHeight, 32, dstWidth * 4, 0x0000'00ff, 0x0000'ff00,
                                           0x00ff'0000, 0xff00'0000);
        } else {
            dst = SDL_CreateRGBSurfaceFrom(buffer, dstWidth, dstHeight, 32, dstWidth * 4, 0xff00'0000, 0x00ff'0000,
                                           0x0000'ff00, 0x0000'00ff);
        }
...
bool ImageDescriptor::LoadDDSFromFile(FileSpecifier& File, int flags, int actual_width, int actual_height,
                                      int maxSize) {
    OpenedFile dds_file;
    ...
    if ((ddsd.ddsCaps.dwCaps2 & DDSCAPS2_CUBEMAP) || (ddsd.ddsCaps.dwCaps2 & DDSCAPS2_VOLUME)) {
    ...
    if (ddsd.ddsCaps.dwCaps1 & DDSCAPS_MIPMAP) {
        ...
        if (!(OriginalMipMapCount == ExpectedMipMapCount || OriginalMipMapCount == (ExpectedMipMapCount - 1))) {
            logWarning("incomplete mipmap chain (%ix%i, %ix%i, %i mipmaps)", Width, Height, ddsd.dwWidth, ddsd.dwHeight,
                       OriginalMipMapCount);
            return false;
        }
        ...
        int skip = 0;
        if (maxSize) {
            while ((Width >> skip) > maxSize || (Height >> skip) > maxSize) { skip++; }
        }


        if (flags & ImageLoader_LoadMipMaps) {
            int totalSize = 0;
            for (int i = skip; i < OriginalMipMapCount; i++) { totalSize += GetMipMapSize(i); }

            for (int i = OriginalMipMapCount; i < MipMapCount; i++) { totalSize += GetMipMapSize(i); }

            Resize(Width, Height, totalSize);

            for (int i = 0; i < skip; i++) {
                if (!SkipMipMapFromFile(dds_file, flags, i, ddsd))
                    return false;
            }

            for (int i = skip; i < OriginalMipMapCount; i++) {
                // read in the base image
                if (!LoadMipMapFromFile(dds_file, flags, i, ddsd, skip))
                    return false;
            }
            ...
            if (!LoadMipMapFromFile(dds_file, flags, skip, ddsd, skip))
                return false;
            this->Height = max(1, this->Height >> skip);
            this->Width  = max(1, this->Width >> skip);
            if (skip && (Format == DXTC1 || Format == DXTC3 || Format == DXTC5)) {
        ...
        if ((Width > maxSize || Height > maxSize) && (Format == DXTC1 || Format == DXTC3 || Format == DXTC5))
            return false;
        ...
        if (!LoadMipMapFromFile(dds_file, flags, 0, ddsd, 0))
            return false;
    ...
    if (!(flags & ImageLoader_CanUseDXTC))
        MakeRGBA();

    if (Format == DXTC1 && (flags & ImageLoader_LoadDXTC1AsDXTC3))
        MakeDXTC3();
...
bool ImageDescriptor::MakeRGBA() {
    ...
    RGBADesc.Size = 0;
    for (int i = 0; i < MipMapCount; i++) { RGBADesc.Size += RGBADesc.GetMipMapSize(i); }

    RGBADesc.Pixels = new uint32[RGBADesc.Size / 4];

    for (int i = 0; i < MipMapCount; i++) {
        if (Format == DXTC1) {
            if (!DecompressDXTC1(RGBADesc.GetMipMapPtr(i), MAX(1, Width >> i), MAX(1, Height >> i), GetMipMapPtr(i)))
                return false;
        } else if (Format == DXTC3) {
            if (!DecompressDXTC3(RGBADesc.GetMipMapPtr(i), MAX(1, Width >> i), MAX(1, Height >> i), GetMipMapPtr(i)))
                return false;
        } else if (Format == DXTC5) {
            if (!DecompressDXTC5(RGBADesc.GetMipMapPtr(i), MAX(1, Width >> i), MAX(1, Height >> i), GetMipMapPtr(i)))
                return false;
        } else {
            return false;
        }
...
void ImageDescriptor::PremultiplyAlpha() {
    ...
    for (int i = 0; i < GetNumPixels(); i++) {
        ...
        constexpr uint32 alphaMask = PlatformIsLittleEndian() ? 0xff00'0000 : 0x0000'00ff;
