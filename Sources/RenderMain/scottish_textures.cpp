...
#include "cseries.hpp"
#include "low_level_textures.hpp"
#include "render.hpp"
...
#define MAXIMUM_PRECALCULATION_TABLE_ENTRY_SIZE \
    (MAX(sizeof(_vertical_polygon_data), sizeof(_horizontal_polygon_line_data)))
...
static void calculate_shading_table(void*& result, view_data* view, void* shading_tables, short depth,
    ...
    switch (bit_depth) {
        case 8:
            result = ((byte*)(shading_tables))
                     + MAXIMUM_SHADING_TABLE_INDEXES * sizeof(pixel8)
                               * CEILING(table_index, number_of_shading_tables - 1);
            break;
        case 16:
            result = ((byte*)(shading_tables))
                     + MAXIMUM_SHADING_TABLE_INDEXES * sizeof(pixel16)
                               * CEILING(table_index, number_of_shading_tables - 1);
            break;
        case 32:
            result = ((byte*)(shading_tables))
                     + MAXIMUM_SHADING_TABLE_INDEXES * sizeof(pixel32)
                               * CEILING(table_index, number_of_shading_tables - 1);
            break;
    }
}
...
static void _pretexture_horizontal_polygon_lines(struct polygon_definition* polygon, struct bitmap_definition* screen,
                                                 struct view_data* view, struct _horizontal_polygon_line_data* data,
                                                 short y0, short* x0_table, short* x1_table, short line_count);

template <int TEXBITS>
static void _pretexture_vertical_polygon_lines(struct polygon_definition* polygon, struct bitmap_definition* screen,
                                               struct view_data* view, struct _vertical_polygon_data* data, short x0,
                                               short* y0_table, short* y1_table, short line_count);
...
static void _prelandscape_horizontal_polygon_lines(struct polygon_definition* polygon, struct bitmap_definition* screen,
                                                   struct view_data* view, struct _horizontal_polygon_line_data* data,
                                                   short y0, short* x0_table, short* x1_table, short line_count);
...
void Rasterizer_SW_Class::texture_horizontal_polygon(polygon_definition& textured_polygon) {
    ...
    fc_assert(polygon->vertex_count >= MINIMUM_VERTICES_PER_SCREEN_POLYGON
    ...
    for (vertex = 0; vertex < polygon->vertex_count; ++vertex) {
        if (!(vertices[vertex].x >= 0 && vertices[vertex].x <= screen->width && vertices[vertex].y >= 0
              && vertices[vertex].y <= screen->height)) {
            //	dprintf("vertex #%d/#%d out of bounds:;dm %x %x;g;", vertex, polygon->vertex_count, polygon->vertices,
            //polygon->vertex_count*sizeof(point2d));
            return;
        }
    ...
    if (highest_vertex != lowest_vertex) {
        ...
        while (total_line_count > 0) {
            ...
            if (left_line_count <= 0) {
                do /* counter-clockwise vertex search */
                {
                    ...
                    if (!build_x_table(left_table + aggregate_left_line_count, vertices[left_vertex].x,
                                       vertices[left_vertex].y, vertices[vertex].x, vertices[vertex].y))
                        return;
            ...
            if (right_line_count <= 0) {
                do /* clockwise vertex search */
                {
                    ...
                    if (!build_x_table(right_table + aggregate_right_line_count, vertices[right_vertex].x,
                                       vertices[right_vertex].y, vertices[vertex].x, vertices[vertex].y))
                        return;
            ...
            fc_assert(delta);
            //			dprintf("tc=%d lc=%d rc=%d delta=%d", total_line_count, left_line_count, right_line_count,
            //delta);
        ...
        switch (polygon->transfer_mode) {
            case _textured_transfer:
                TEXBITS_DISPATCH(polygon->texture, _pretexture_horizontal_polygon_lines,
                                 (polygon, screen, view, (struct _horizontal_polygon_line_data*)precalculation_table,
        ...
        switch (bit_depth) {
                ...
                switch (polygon->transfer_mode) {
                    ...
                    case _textured_transfer:
                        TEXBITS_DISPATCH_2(polygon->texture, texture_horizontal_polygon_lines, pixel8, _sw_alpha_off,
                                           (polygon->texture, screen, view,
                                            (struct _horizontal_polygon_line_data*)precalculation_table,
                                            vertices[highest_vertex].y, left_table, right_table,
                                            aggregate_total_line_count));
                        break;
                    case _big_landscaped_transfer:
                        landscape_horizontal_polygon_lines<pixel8>(
                                polygon->texture, screen, view,
                                (struct _horizontal_polygon_line_data*)precalculation_table, vertices[highest_vertex].y,
            ...
            case 16:
                switch (polygon->transfer_mode) {
                    case _textured_transfer: {
                        ...
                        if (sw_texture && !polygon->VoidPresent && sw_texture->opac_type()) {
                            if (graphics_preferences->software_alpha_blending == _sw_alpha_fast) {
                                TEXBITS_DISPATCH_2(polygon->texture, texture_horizontal_polygon_lines, pixel16,
                                                   _sw_alpha_fast,
                                                   (polygon->texture, screen, view,
                                                    (struct _horizontal_polygon_line_data*)precalculation_table,
                                                    vertices[highest_vertex].y, left_table, right_table,
                                ...
                                TEXBITS_DISPATCH_2(polygon->texture, texture_horizontal_polygon_lines, pixel16,
                                                   ...
                                                   (polygon->texture, screen, view,
                                                    (struct _horizontal_polygon_line_data*)precalculation_table,
                                                    vertices[highest_vertex].y, left_table, right_table,
                                                    aggregate_total_line_count, sw_texture->opac_table()));
                            }
                        } else {
                            TEXBITS_DISPATCH_2(
                                    polygon->texture, texture_horizontal_polygon_lines, pixel16, _sw_alpha_off,
                                    (polygon->texture, screen, view,
                                     (struct _horizontal_polygon_line_data*)precalculation_table,
                                     vertices[highest_vertex].y, left_table, right_table, aggregate_total_line_count));
                        }
                    ...
                    case _big_landscaped_transfer:
                        landscape_horizontal_polygon_lines<pixel16>(
                                polygon->texture, screen, view,
                                (struct _horizontal_polygon_line_data*)precalculation_table, vertices[highest_vertex].y,
                                left_table, right_table, aggregate_total_line_count);
                        break;
                ...
                switch (polygon->transfer_mode) {
                    case _textured_transfer: {
                        ...
                        if (sw_texture && sw_texture->opac_type() && !polygon->VoidPresent) {
                            if (graphics_preferences->software_alpha_blending == _sw_alpha_fast) {
                                TEXBITS_DISPATCH_2(polygon->texture, texture_horizontal_polygon_lines, pixel32,
                                                   _sw_alpha_fast,
                                                   (polygon->texture, screen, view,
                                                    (struct _horizontal_polygon_line_data*)precalculation_table,
                                                    vertices[highest_vertex].y, left_table, right_table,
                            ...
                            TEXBITS_DISPATCH_2(
                                    polygon->texture, texture_horizontal_polygon_lines, pixel32, _sw_alpha_off,
                                    (polygon->texture, screen, view,
                                     (struct _horizontal_polygon_line_data*)precalculation_table,
                                     vertices[highest_vertex].y, left_table, right_table, aggregate_total_line_count));
                        }
                    ...
                    case _big_landscaped_transfer:
                        landscape_horizontal_polygon_lines<pixel32>(
                                polygon->texture, screen, view,
                                (struct _horizontal_polygon_line_data*)precalculation_table, vertices[highest_vertex].y,
                                left_table, right_table, aggregate_total_line_count);
                        break;
...
void Rasterizer_SW_Class::texture_vertical_polygon(polygon_definition& textured_polygon) {
    ...
    for (vertex = 1; vertex < polygon->vertex_count; ++vertex) {
        if (vertices[vertex].x < vertices[highest_vertex].x)
            highest_vertex = vertex;
        if (vertices[vertex].x > vertices[lowest_vertex].x)
            lowest_vertex = vertex;
    }
    ...
    for (vertex = 0; vertex < polygon->vertex_count; ++vertex) {
        if (!(vertices[vertex].x >= 0 && vertices[vertex].x <= screen->width && vertices[vertex].y >= 0
              && vertices[vertex].y <= screen->height)) {
            //			dprintf("vertex #%d/#%d out of bounds:;dm %x %x;g;", vertex, polygon->vertex_count,
            //polygon->vertices, polygon->vertex_count*sizeof(point2d));
            return;
        }
    }
    ...
    if (highest_vertex != lowest_vertex) {
        ...
        while (total_line_count > 0) {
            ...
            if (left_line_count <= 0) {
                do /* clockwise vertex search */
                {
                    ...
                    //					dprintf("left line (%d,%d) to (%d,%d) for %d points", vertices[left_vertex].x,
                    //vertices[left_vertex].y, vertices[vertex].x, vertices[vertex].y, left_line_count);
                    if (!build_y_table(left_table + aggregate_left_line_count, vertices[left_vertex].x,
                                       vertices[left_vertex].y, vertices[vertex].x, vertices[vertex].y))
                        return;
                    aggregate_left_line_count += left_line_count;
                    left_vertex                = vertex;
            ...
            if (right_line_count <= 0) {
                do /* counter-clockwise vertex search */
                {
                    ...
                    //					dprintf("right line (%d,%d) to (%d,%d) for %d points", vertices[right_vertex].x,
                    //vertices[right_vertex].y, vertices[vertex].x, vertices[vertex].y, right_line_count);
                    if (!build_y_table(right_table + aggregate_right_line_count, vertices[right_vertex].x,
                                       vertices[right_vertex].y, vertices[vertex].x, vertices[vertex].y))
                        return;
                    aggregate_right_line_count += right_line_count;
                    right_vertex                = vertex;
        ...
        if ((polygon->transfer_mode == _textured_transfer) || (polygon->transfer_mode == _static_transfer)) {
            TEXBITS_DISPATCH(polygon->texture, _pretexture_vertical_polygon_lines,
                             (polygon, screen, view, (struct _vertical_polygon_data*)precalculation_table,
                              vertices[highest_vertex].x, left_table, right_table, aggregate_total_line_count));
        } else
            VHALT_DEBUG(csprintf(temporary, "vertical_polygons dont support mode #%d", polygon->transfer_mode));
                ...
                switch (polygon->transfer_mode) {
                    case _textured_transfer:
                        if (polygon->texture->flags & _TRANSPARENT_BIT)
                            texture_vertical_polygon_lines<pixel8, _sw_alpha_off, true>(
                                    screen, view, (struct _vertical_polygon_data*)precalculation_table, left_table,
                                    right_table);
                        else
                            texture_vertical_polygon_lines<pixel8, _sw_alpha_off, false>(
                                    screen, view, (struct _vertical_polygon_data*)precalculation_table, left_table,
                                    right_table);
                        break;
                    case _static_transfer:
                        if (polygon->texture->flags & _TRANSPARENT_BIT)
                            randomize_vertical_polygon_lines<pixel8, true>(
                                    screen, view, (struct _vertical_polygon_data*)precalculation_table, left_table,
                                    right_table, polygon->transfer_data);
                        else
                            randomize_vertical_polygon_lines<pixel8, false>(
                                    screen, view, (struct _vertical_polygon_data*)precalculation_table, left_table,
                                    right_table, polygon->transfer_data);
                        break;
                ...
                switch (polygon->transfer_mode) {
                    case _textured_transfer: {
                        ...
                        if (sw_texture && !polygon->VoidPresent && sw_texture->opac_type()) {
                            if (graphics_preferences->software_alpha_blending == _sw_alpha_fast) {
                                if (polygon->texture->flags & _TRANSPARENT_BIT) {
                                    texture_vertical_polygon_lines<pixel16, _sw_alpha_fast, true>(
                                            screen, view, (struct _vertical_polygon_data*)precalculation_table,
                                            left_table, right_table);
                                } else {
                                    texture_vertical_polygon_lines<pixel16, _sw_alpha_fast, false>(
                                            screen, view, (struct _vertical_polygon_data*)precalculation_table,
                                            left_table, right_table);
                                }
                            } else if (graphics_preferences->software_alpha_blending == _sw_alpha_nice) {
                                if (polygon->texture->flags & _TRANSPARENT_BIT) {
                                    texture_vertical_polygon_lines<pixel16, _sw_alpha_nice, true>(
                                            screen, view, (struct _vertical_polygon_data*)precalculation_table,
                                            left_table, right_table, sw_texture->opac_table());
                                } else {
                                    texture_vertical_polygon_lines<pixel16, _sw_alpha_nice, false>(
                                            screen, view, (struct _vertical_polygon_data*)precalculation_table,
                                            left_table, right_table, sw_texture->opac_table());
                                }
                            }
                            ...
                            if (polygon->texture->flags & _TRANSPARENT_BIT) {
                                texture_vertical_polygon_lines<pixel16, _sw_alpha_off, true>(
                                        screen, view, (struct _vertical_polygon_data*)precalculation_table, left_table,
                                        right_table);
                            } else {
                                texture_vertical_polygon_lines<pixel16, _sw_alpha_off, false>(
                                        screen, view, (struct _vertical_polygon_data*)precalculation_table, left_table,
                                        right_table);
                            }
                        }
                    ...
                    case _static_transfer:
                        if (polygon->texture->flags & _TRANSPARENT_BIT) {
                            randomize_vertical_polygon_lines<pixel16, true>(
                                    screen, view, (struct _vertical_polygon_data*)precalculation_table, left_table,
                                    right_table, polygon->transfer_data);
                        } else {
                            randomize_vertical_polygon_lines<pixel16, false>(
                                    screen, view, (struct _vertical_polygon_data*)precalculation_table, left_table,
                                    right_table, polygon->transfer_data);
                        }
                        break;
                ...
                switch (polygon->transfer_mode) {
                    case _textured_transfer: {
                        ...
                        if (sw_texture && !polygon->VoidPresent && sw_texture->opac_type()) {
                            if (graphics_preferences->software_alpha_blending == _sw_alpha_fast) {
                                if (polygon->texture->flags & _TRANSPARENT_BIT)
                                    texture_vertical_polygon_lines<pixel32, _sw_alpha_fast, true>(
                                            screen, view, (struct _vertical_polygon_data*)precalculation_table,
                                            left_table, right_table);
                                else
                                    texture_vertical_polygon_lines<pixel32, _sw_alpha_fast, false>(
                                            screen, view, (struct _vertical_polygon_data*)precalculation_table,
                                            left_table, right_table);
                            } else if (graphics_preferences->software_alpha_blending == _sw_alpha_nice) {
                                if (polygon->texture->flags & _TRANSPARENT_BIT)
                                    texture_vertical_polygon_lines<pixel32, _sw_alpha_nice, true>(
                                            screen, view, (struct _vertical_polygon_data*)precalculation_table,
                                            left_table, right_table, sw_texture->opac_table());
                                else
                                    texture_vertical_polygon_lines<pixel32, _sw_alpha_nice, false>(
                                            screen, view, (struct _vertical_polygon_data*)precalculation_table,
                                            left_table, right_table, sw_texture->opac_table());
                            }
                        } else {
                            if (polygon->texture->flags & _TRANSPARENT_BIT)
                                texture_vertical_polygon_lines<pixel32, _sw_alpha_off, true>(
                                        screen, view, (struct _vertical_polygon_data*)precalculation_table, left_table,
                                        right_table);
                            else
                                texture_vertical_polygon_lines<pixel32, _sw_alpha_off, false>(
                                        screen, view, (struct _vertical_polygon_data*)precalculation_table, left_table,
                                        right_table);
                        }
                        break;
                    ...
                    case _static_transfer:
                        if (polygon->texture->flags & _TRANSPARENT_BIT)
                            randomize_vertical_polygon_lines<pixel32, true>(
                                    screen, view, (struct _vertical_polygon_data*)precalculation_table, left_table,
                                    right_table, polygon->transfer_data);
                        else
                            randomize_vertical_polygon_lines<pixel32, false>(
                                    screen, view, (struct _vertical_polygon_data*)precalculation_table, left_table,
                                    right_table, polygon->transfer_data);
                        break;
...
void Rasterizer_SW_Class::texture_rectangle(rectangle_definition& textured_rectangle) {
    ...
    if (rectangle->x0 < rectangle->x1 && rectangle->y0 < rectangle->y1) {
        ...
        if (rectangle->clip_left < 0)
            rectangle->clip_left = 0;
        if (rectangle->clip_right > screen->width)
            rectangle->clip_right = screen->width;
        if (rectangle->clip_top < 0)
            rectangle->clip_top = 0;
        if (rectangle->clip_bottom > screen->height)
            rectangle->clip_bottom = screen->height;

        /* subsume left and right sides of the rectangle into clipping parameters */
        if (rectangle->clip_left < rectangle->x0)
            rectangle->clip_left = rectangle->x0;
        if (rectangle->clip_right > rectangle->x1)
            rectangle->clip_right = rectangle->x1;
        if (rectangle->clip_top < rectangle->y0)
            rectangle->clip_top = rectangle->y0;
        if (rectangle->clip_bottom > rectangle->y1)
            rectangle->clip_bottom = rectangle->y1;
        ...
        if (rectangle->clip_left < rectangle->clip_right && rectangle->clip_top < rectangle->clip_bottom
            && rectangle->clip_right > 0 && rectangle->clip_left < screen->width && rectangle->clip_bottom > 0
            && rectangle->clip_top < screen->height) {
            short delta; /* scratch */
            ...
            if (texture_dx && texture_dy) {
                ...
                switch (rectangle->transfer_mode) {
                    case _textured_transfer:
                        if (!(rectangle->flags & _SHADELESS_BIT)) {
                            ...
                            calculate_shading_table(shading_table, view, rectangle->shading_tables,
                ...
                for (; screen_width; --screen_width) {
                    ...
                    if (FIXED_INTEGERAL_PART(texture_y0) < first) {
                        ...
                        fc_vassert(delta >= 0, csprintf(temporary, "[%x,%x] ∂=%x (#%d,#%d)", texture_y0, texture_y1,
                                                        texture_dy, first, last));
                ...
                switch (bit_depth) {
                    case 8:
                        switch (rectangle->transfer_mode) {
                            ...
                            case _tinted_transfer:
                                tint_vertical_polygon_lines<pixel8>(
                                        screen, view, (struct _vertical_polygon_data*)precalculation_table,
                                        scratch_table0, scratch_table1, rectangle->transfer_data);
                                break;
                    ...
                    case 16:
                        switch (rectangle->transfer_mode) {
                            ...
                            case _tinted_transfer:
                                tint_vertical_polygon_lines<pixel16>(
                                        screen, view, (struct _vertical_polygon_data*)precalculation_table,
                                        scratch_table0, scratch_table1, rectangle->transfer_data);
                                break;
                    ...
                    case 32:
                        switch (rectangle->transfer_mode) {
                            ...
                            case _tinted_transfer:
                                tint_vertical_polygon_lines<pixel32>(
                                        screen, view, (struct _vertical_polygon_data*)precalculation_table,
                                        scratch_table0, scratch_table1, rectangle->transfer_data);
                                break;
...
static void _pretexture_vertical_polygon_lines(struct polygon_definition* polygon, struct bitmap_definition* screen,
    ...
    while (--line_count >= 0) {
            ...
            then downshifted only the numerator?  too bad we can’t use BFFFO in 68k */
        {
            ...
            if (!adjusted_tx_denominator)
                adjusted_tx_denominator = 1; /* -1 will still be -1 */
            x0 = ((adjusted_tx_numerator << VERTICAL_TEXTURE_WIDTH_BITS) / adjusted_tx_denominator)
                 & (VERTICAL_TEXTURE_WIDTH - 1);
        ...
        if (world_x < 0)
            world_x = -world_x; /* it is mostly unclear what we’re supposed to do with negative x values */
        ...
        while (ty_numerator > INT16_MAX || ty_numerator < INT16_MIN) { ty_numerator >>= 1, ty_denominator >>= 1; }
        if (!ty_denominator)
            ty_denominator = 1; /* -1 will still be -1 */
        ...
        while (adjusted_world_x > INT16_MAX) {
            adjusted_world_x        >>= 1;
            adjusted_ty_denominator >>= 1;
        }
        if (!adjusted_ty_denominator)
            adjusted_ty_denominator = 1; /* -1 will still be -1 */
        ty_delta = -INTEGER_TO_FIXED(adjusted_world_x) / adjusted_ty_denominator;

        fc_vassert(ty_delta >= 0,
                   csprintf(temporary, "ty_delta=W2F(%d)/%d=%d", world_x, unadjusted_ty_denominator, ty_delta));
        ...
        if (polygon->flags & _SHADELESS_BIT) {
            ...
            calculate_shading_table(line->shading_table, view, polygon->shading_tables, (short)MIN(world_x, SHRT_MAX),
                                    polygon->ambient_shade);
            // calculate_shading_table(line->shading_table, view, polygon->shading_tables, world_x,
            // polygon->ambient_shade);
        }
...
static void _pretexture_horizontal_polygon_lines(struct polygon_definition* polygon, struct bitmap_definition* screen,
    ...
    while ((line_count -= 1) >= 0) {
        ...
        if (!screen_y)
            screen_y = 1; /* this will avoid division by zero and won't change rendering */
        ...
        if (higher_precision) {
            ...
            source_x = ((dhcosine - screen_x * hsine) / screen_y) * polygon->origin.z
                       + (polygon->origin.x << TRIG_SHIFT);
            source_dx = -(hsine * polygon->origin.z) / screen_y;
            source_y  = ((screen_x * hcosine + dhsine) / screen_y) * polygon->origin.z
                       + (polygon->origin.y << TRIG_SHIFT);
        ...
        if ((depth = hworld_to_screen / screen_y) < 0)
            depth = -depth;
        if (polygon->flags & _SHADELESS_BIT) {
            data->shading_table = polygon->shading_tables;
        } else {
            calculate_shading_table(data->shading_table, view, polygon->shading_tables, (short)MIN(depth, SHRT_MAX),
                                    polygon->ambient_shade);
        }
...
static void _prelandscape_horizontal_polygon_lines(struct polygon_definition* polygon, struct bitmap_definition* screen,
    ...
    _fixed first_horizontal_pixel
            = (view->landscape_yaw + LandOpts->Azimuth)
              << (landscape_width_bits + (LandOpts->HorizExp) + FIXED_FRACTIONAL_BITS - ANGULAR_BITS);
    _fixed horizontal_pixel_delta = (view->half_cone << (1 + landscape_width_bits + (LandOpts->HorizExp)
                                                         + FIXED_FRACTIONAL_BITS - ANGULAR_BITS))
                                    / view->standard_screen_width;
    _fixed vertical_pixel_delta = (view->half_cone << (1 + landscape_width_bits + (LandOpts->VertExp)
                                                       + FIXED_FRACTIONAL_BITS - ANGULAR_BITS))
                                  / view->standard_screen_width;
    ...
    while ((line_count -= 1) >= 0) {
        ...
        if (LandOpts->VertRepeat)
            y_txtr_offset = ((y_txtr_offset + height_repeat_shift) & height_repeat_mask) - height_repeat_shift;
