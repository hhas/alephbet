    ...
    if (bit_depth == 32) {
        for (int i = 0; i < MAXIMUM_SHADING_TABLE_INDEXES; i++) {
            ...
            if (m_opac_type == 1) {
                ...
                uint32 color
                        = ((uint32*)shading_tables)[i + MAXIMUM_SHADING_TABLE_INDEXES * (number_of_shading_tables - 1)];
        ...
        for (int i = 0; i < MAXIMUM_SHADING_TABLE_INDEXES; i++) {
            ...
            if (m_opac_type == 1) {
                ...
                uint16 color
                        = ((uint16*)shading_tables)[i + MAXIMUM_SHADING_TABLE_INDEXES * (number_of_shading_tables - 1)];
                uint32 r = (((color & fmt->Rmask) >> fmt->Rshift) << fmt->Rloss);
                uint32 g = (((color & fmt->Gmask) >> fmt->Gshift) << fmt->Gloss);
                uint32 b = (((color & fmt->Bmask) >> fmt->Bshift) << fmt->Bloss);
                if (m_opac_type == 2) {
...
void SW_Texture_Extras::Load(short collection_index) {
    for (std::vector<SW_Texture>::iterator i = texture_list[collection_index].begin();
         i != texture_list[collection_index].end(); i++) {
        i->build_opac_table();
    }
}
...
void SW_Texture_Extras::Reset() {
    // remove all textures
    for (short i = 0; i < NUMBER_OF_COLLECTIONS; i++) { texture_list[i].clear(); }
}

void reset_mml_software() { SW_Texture_Extras::instance()->Reset(); }
