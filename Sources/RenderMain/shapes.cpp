...
//get_shape_descriptors() needs to look at high-level instead of low-level shapes when fetching scenery instead of
walls/ceilings/floors
//get_shape_information() is called often, and is quite slow
//it is possible to have more than 255 low-level shapes in a collection, which means the existing shape_descriptor is
too small
...
static short find_or_add_color(struct rgb_color_value* color, struct rgb_color_value* colors, short* color_count,
                               bool update_flags);
static void _change_clut(void (*change_clut_proc)(struct color_table* color_table), struct rgb_color_value* colors,
                         short color_count);

static void build_shading_tables8(struct rgb_color_value* colors, short color_count, pixel8* shading_tables);
static void build_shading_tables16(struct rgb_color_value* colors, short color_count, pixel16* shading_tables,
                                   byte* remapping_table, bool is_opengl);
static void build_shading_tables32(struct rgb_color_value* colors, short color_count, pixel32* shading_tables,
                                   byte* remapping_table, bool is_opengl);
...
static void build_tinting_table16(struct rgb_color_value* colors, short color_count, pixel16* tint_table,
                                  struct rgb_color* tint_color);
static void build_tinting_table32(struct rgb_color_value* colors, short color_count, pixel32* tint_table,
                                  struct rgb_color* tint_color, bool is_opengl);
...
static struct high_level_shape_definition* get_high_level_shape_definition(short collection_index,
...
SDL_Surface* get_shape_surface(int shape, int inCollection, byte** outPointerToPixelData, float inIllumination,
                               bool inShrinkImage) {
    ...
    struct low_level_shape_definition* low_level_shape
            = get_low_level_shape_definition(collection_index, low_level_shape_index);
    if (!low_level_shape)
        return NULL;
    ...
    if (inIllumination >= 0) {
        ...
        extended_get_shape_bitmap_and_shading_table(BUILD_COLLECTION(collection_index, clut_index),
                                                    low_level_shape_index, &bitmap, &shading_tables_as_void,
                                                    _shading_normal);
        if (!bitmap)
            return NULL;
    ...
    if (bitmap->bytes_per_row == NONE) {
        ...
        if (collection->type == _wall_collection) {
            s = SDL_CreateRGBSurfaceFrom(bitmap->row_addresses[0], bitmap->height, bitmap->width, 8,
                                         bitmap->bytes_per_row, 0, 0, 0, 0);
        } else {
            s = SDL_CreateRGBSurfaceFrom(bitmap->row_addresses[0], bitmap->width, bitmap->height, 8,
                                         bitmap->bytes_per_row, 0, 0, 0, 0);
        }
...
static void load_high_level_shape(std::vector<uint8>& shape, SDL_RWops* p) {
    ...
    // Convert low-level shape index list
    for (int j = 0; j < num_views * d->frames_per_view; j++) { d->low_level_shape_indexes[j] = SDL_ReadBE16(p); }
}
...
static void allocate_shading_tables(short collection_index, bool strip) {
    ...
    if (strip)
        ...
        header->shading_tables.resize(get_shading_table_size(collection_index) * definition->clut_count
...
static void shutdown_shape_handler(void) { close_shapes_file(); }

static bool collection_loaded(struct collection_header* header) { return header->collection ? true : false; }
...
void unload_all_collections(void) {
    ...
    for (collection_index = 0, header = collection_headers; collection_index < MAXIMUM_COLLECTIONS;
         ++collection_index, ++header) {
...
short get_shape_descriptors(short shape_type, shape_descriptor* buffer) {
    ...
    for (collection_index = 0; collection_index < MAXIMUM_COLLECTIONS; ++collection_index) {
        ...
        if (collection && collection->type == appropriate_type) {
            for (low_level_shape_index = 0; low_level_shape_index < collection->low_level_shape_count;
                ...
                if (!low_level_shape)
                    continue;
                struct bitmap_definition* bitmap
                        = get_bitmap_definition(collection_index, low_level_shape->bitmap_index);
                if (!bitmap)
                    continue;
...
void extended_get_shape_bitmap_and_shading_table(short collection_code, short low_level_shape_index,
                                                 struct bitmap_definition** bitmap, void** shading_tables,
                                                 short shading_mode) {
    ...
    assert(!(clut_index + 1 == MAXIMUM_CLUTS_PER_COLLECTION && collection_index + 1 == MAXIMUM_COLLECTIONS
...
struct shape_information_data* extended_get_shape_information(short collection_code, short low_level_shape_index) {
    if ((GET_COLLECTION(collection_code) < 0) || (GET_COLLECTION(collection_code) >= NUMBER_OF_COLLECTIONS))
        return NULL;
    if (low_level_shape_index < 0)
        return NULL;
    short collection_index = GET_COLLECTION(collection_code);
    struct low_level_shape_definition* low_level_shape;
...
void process_collection_sounds(short collection_code, void (*process_sound)(short sound_index)) {
    ...
    for (high_level_shape_index = 0; high_level_shape_index < collection->high_level_shape_count;
         ++high_level_shape_index) {
        struct high_level_shape_definition* high_level_shape
                = get_high_level_shape_definition(collection_index, high_level_shape_index);
        if (!high_level_shape)
            return;
...
struct shape_animation_data* get_shape_animation_data(shape_descriptor shape) {
...
static short find_or_add_color(struct rgb_color_value* color, struct rgb_color_value* colors, short* color_count,
    ...
    if (*color_count >= PIXEL8_MAXIMUM_COLORS) {
        ...
        double MinDiffSq = 3 * double(65'536) * double(65'536);
        short MinIndx    = 0;
...
static void update_color_environment(bool is_opengl) {
    ...
    colors[0].red = colors[0].green = colors[0].blue = 65'535;
    ...
    for (collection_index = 0; collection_index < MAXIMUM_COLLECTIONS; ++collection_index) {
        ...
        if (collection && collection->bitmap_count) {
            struct rgb_color_value* primary_colors
            ...
            //			dprintf("primary clut %d entries;dm #%d #%d", collection->color_count, primary_colors,
            //collection->color_count*sizeof(ColorSpec));
            ...
            for (clut_index = 0; clut_index < collection->clut_count; ++clut_index) {
                ...
                if (clut_index) {
                    struct rgb_color_value* alternate_colors
                            = get_collection_colors(collection_index, clut_index) + NUMBER_OF_PRIVATE_COLORS;
                    ...
                    //					dprintf("alternate clut %d entries;dm #%d #%d", collection->color_count,
                    //alternate_colors, collection->color_count*sizeof(ColorSpec));

                    /* build a remapping table for the primary shading table which we can use to
                        calculate this alternate shading table */
                    for (color_index = 0; color_index < PIXEL8_MAXIMUM_COLORS; ++color_index)
                        shading_remapping_table[color_index] = static_cast<pixel8>(color_index);
                    for (color_index = 0; color_index < collection->color_count - NUMBER_OF_PRIVATE_COLORS;
                         ++color_index) {
                    ...
                    //					shading_remapping_table[iBLACK]= iBLACK; /* make iBLACK==>iBLACK remapping
                    //explicit */

                    switch (collection_bit_depth) {
                        case 8:
                            /* duplicate the primary shading table and remap it */
                            memcpy(alternate_shading_table, primary_shading_table,
                                   get_shading_table_size(collection_index));
                            map_bytes((unsigned char*)alternate_shading_table, shading_remapping_table,
                                      get_shading_table_size(collection_index));
                            break;

                        case 16:
                            build_shading_tables16(colors, color_count, (pixel16*)alternate_shading_table,
                                                   shading_remapping_table, is_opengl);
                            break;
                            break;

                        case 32:
                            build_shading_tables32(colors, color_count, (pixel32*)alternate_shading_table,
                                                   shading_remapping_table, is_opengl);
                            break;
                            break;
                    ...
                    switch (collection_bit_depth) {
                        case 8:
                            build_shading_tables8(colors, color_count, (unsigned char*)primary_shading_table);
                            break;
                        case 16:
                            build_shading_tables16(colors, color_count, (pixel16*)primary_shading_table, (byte*)NULL,
                                                   is_opengl);
                            break;
                        case 32:
            ...
            if (collection_index == _collection_interface && interface_bit_depth == 8
                && bit_depth != interface_bit_depth)
...
static void _change_clut(void (*change_clut_proc)(struct color_table* color_table), struct rgb_color_value* colors,
    ...
    for (i = 0; i < color_count; ++i, ++color, ++colors) { *color = *((struct rgb_color*)&colors->red); }
    for (i = color_count; i < PIXEL8_MAXIMUM_COLORS; ++i, ++color) { color->red = color->green = color->blue = 0; }
...
#else
short find_closest_color(struct rgb_color_value* color, struct rgb_color_value* colors, short color_count) {
    short i;
    ...
    // = 1 to skip the transparent color
    for (i = 1, colors += 1; i < color_count; ++i, ++colors) {
        int32 delta = (int32)ABS(colors->red - color->red) + (int32)ABS(colors->green - color->green)
                      + (int32)ABS(colors->blue - color->blue);

        if (delta < closest_delta)
            closest_index = i, closest_delta = delta;
    }
    ...
    while (get_next_color_run(colors, color_count, &start, &count)) {
        for (i = 0; i < count; ++i) {
            ...
            for (level = 0; level < number_of_shading_tables; ++level) {
                ...
                short multiplier              = (color->flags & SELF_LUMINESCENT_COLOR_FLAG)
                                                        ? ((number_of_shading_tables >> 1) + (level >> 1))
                                                        : level;
                if (!is_opengl)
                    // Find optimal pixel value for video display
                    shading_tables[PIXEL8_MAXIMUM_COLORS * level + start + i]
                            = SDL_MapRGB(fmt, ((color->red * multiplier) / (number_of_shading_tables - 1)) >> 8,
...
static void build_global_shading_table16(void) {
    if (!global_shading_table16) {
        ...
        global_shading_table16 = (pixel16*)malloc(sizeof(pixel16) * number_of_shading_tables
...
static void build_global_shading_table32(void) {
    if (!global_shading_table32) {
        ...
        global_shading_table32 = (pixel32*)malloc(sizeof(pixel32) * number_of_shading_tables
...
static bool new_color_run(struct rgb_color_value* _new, struct rgb_color_value* last) {
    if ((int32)last->red + (int32)last->green + (int32)last->blue
        < (int32)_new->red + (int32)_new->green + (int32)_new->blue) {
        return true;
    } else {
        return false;
    }
...
static short CollectionTints[NUMBER_OF_COLLECTIONS] = {
        ...
        _tint_collection_yellow,
        // Juggernaut, tick
        _tint_collection_red, _tint_collection_red,
        // Explosion effects
        _tint_collection_yellow,
        // Hunter
        _tint_collection_red,
        ...
        _tint_collection_red, _tint_collection_red, _tint_collection_red, _tint_collection_red,
        // Bob and VacBobs
        _tint_collection_yellow, _tint_collection_yellow,
        // Enforcer, Drone
        _tint_collection_red, _tint_collection_red,
        // S'pht
        _tint_collection_blue,
        // Walls
        _tint_collection_blue, _tint_collection_blue, _tint_collection_blue, _tint_collection_blue,
        _tint_collection_blue,
        // Scenery
        _tint_collection_blue, _tint_collection_blue, _tint_collection_blue, _tint_collection_blue,
        _tint_collection_blue,
...
#ifdef HAVE_OPENGL
        OGL_SetInfravisionTint(collection_index, true, Color.red / 65535.0F, Color.green / 65535.0F,
                               Color.blue / 65535.0F);
#endif
        switch (bit_depth) {
            case 8:
                build_tinting_table8(colors, color_count, (unsigned char*)tint_table, tint_colors8[tint_color].start,
                                     tint_colors8[tint_color].count);
                break;
            case 16:
...
static rgb_color m2_apply_tint(rgb_color_value base, rgb_color tint) {
    ...
    auto scale            = [base_mag](uint16 comp) { return uint16(int32(1LL * base_mag * comp) / 65'535); };
...
static void* get_collection_tint_tables(short collection_index, short tint_index) {
    ...
    tint_table = (uint8*)tint_table + get_shading_table_size(collection_index) * definition->clut_count
                 + shading_table_size * tint_index;
...
void reset_mml_infravision() {
    if (original_tint_colors16) {
        for (int i = 0; i < NUMBER_OF_TINT_COLORS; i++) tint_colors16[i] = original_tint_colors16[i];
        free(original_tint_colors16);
        original_tint_colors16 = NULL;
    }

    if (OriginalCollectionTints) {
        for (int i = 0; i < NUMBER_OF_COLLECTIONS; i++) CollectionTints[i] = OriginalCollectionTints[i];
...
void parse_mml_infravision(const InfoTree& root) {
    ...
    if (!original_tint_colors16) {
        ...
        for (int i = 0; i < NUMBER_OF_TINT_COLORS; i++) original_tint_colors16[i] = tint_colors16[i];
    }

    if (!OriginalCollectionTints) {
        OriginalCollectionTints = (short*)malloc(sizeof(short) * NUMBER_OF_COLLECTIONS);
        assert(OriginalCollectionTints);
        for (int i = 0; i < NUMBER_OF_COLLECTIONS; i++) OriginalCollectionTints[i] = CollectionTints[i];
    }
