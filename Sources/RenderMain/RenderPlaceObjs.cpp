...
struct RenderPlaceObjsClass::span_data {
    ...
    // TODO: originally used boost's small_vector with a capacity of six,
    // we may get some performance back if we do something like that
...
static void FindProjectedBoundingBox(GLfloat BoundingBox[2][3], long_point3d& TransformedPosition, GLfloat Scale,
                                     short RelativeAngle, shape_information_data& ShapeInfo, short DepthType,
                                     int& Farthest, int& ProjDistance, int& DistanceRef, int& LightDepth,
                                     GLfloat* Direction);

// Inits everything
RenderPlaceObjsClass::RenderPlaceObjsClass()
    : view(NULL), // Idiot-proofing
      RVPtr(NULL), RSPtr(NULL) {
...
render_object_data* RenderPlaceObjsClass::build_render_object(object_data* object, _fixed floor_intensity,
                                                              _fixed ceiling_intensity, float Opacity,
                                                              long_point3d* origin, long_point3d* rel_origin) {
    ...
    if (!OBJECT_IS_INVISIBLE(object) && int(RenderObjects.size()) < get_dynamic_limit(_dynamic_limit_rendered)) {
        ...
        // if (transformed_origin.x>MINIMUM_OBJECT_DISTANCE)
        {
            ...
            short ModelSequence;
            ModelPtr = OGL_GetModelData(GET_COLLECTION(data.collection_code), GET_DESCRIPTOR_SHAPE(object->shape),
                                        ModelSequence);
#endif
            ...
            if (ModelPtr) {
                ...
                if (TEST_FLAG(object->flags, _object_is_enlarged))
                    Scale = 1.25;
                else if (TEST_FLAG(object->flags, _object_is_tiny))
                    Scale = 0.5;

                FindProjectedBoundingBox(ModelPtr->Model.BoundingBox, transformed_origin, Scale,
                                         object->facing - view->yaw, model_shape_information, ModelPtr->DepthType,
...
#endif
            // Too close?
            if (Farthest < MINIMUM_OBJECT_DISTANCE)
                return NULL;
...

            {
                ...
                x0 = view->half_screen_width
                     + (int(transformed_origin.y + shape_information->world_left) * view->world_to_screen_x)
                               / DistanceRef;
                x1 = view->half_screen_width
                     + (int(transformed_origin.y + shape_information->world_right) * view->world_to_screen_x)
                               / DistanceRef;
                y0 = view->half_screen_height
                     - (view->world_to_screen_y * int(transformed_origin.z + shape_information->world_top))
                               / DistanceRef
                     + view->dtanpitch;
                y1 = view->half_screen_height
                     - (view->world_to_screen_y * int(transformed_origin.z + shape_information->world_bottom))
                               / DistanceRef
                     + view->dtanpitch;
                ...
                if (NewROPointer != OldROPointer) {
                    ...
                    for (size_t k = 0; k < Length; k++) {
                        ...
                        if (RenderObject.next_object != NULL)
                            RenderObject.next_object
                                    = (render_object_data*)(NewROPointer
                                                            + (POINTER_CAST(RenderObject.next_object) - OldROPointer));
                    }
                    for (size_t k = 0; k < SortedNodes.size(); k++) {
                        sorted_node_data& SortedNode = SortedNodes[k];
                        if (SortedNode.interior_objects != NULL)
                            SortedNode.interior_objects
                                    = (render_object_data*)(NewROPointer
                                                            + (POINTER_CAST(SortedNode.interior_objects)
                                                               - OldROPointer));
                        if (SortedNode.exterior_objects != NULL)
                            SortedNode.exterior_objects
                                    = (render_object_data*)(NewROPointer
                                                            + (POINTER_CAST(SortedNode.exterior_objects)
                                                               - OldROPointer));
                    }
                }
...

                {
                    ...
                    if (media && !OBJECT_IS_MEDIA_EFFECT(object)) {
                        render_object->rectangle.LiquidRelHeight
                                = PIN(media->height - object->location.z, SHRT_MIN, SHRT_MAX);
                        int ProjLiquidHeight
                                = view->half_screen_height
                                  - (view->world_to_screen_y * (media->height - view->origin.z)) / DistanceRef
                                  + view->dtanpitch;
                ...
                extended_get_shape_bitmap_and_shading_table(
                        data.collection_code, data.low_level_shape_index, &render_object->rectangle.texture,
                ...
                if (ModelPtr) {
                    ...
                    render_object->rectangle.MixFrac    = data.Ticks > 0 ? float(data.Phase) / float(data.Ticks) : 0;
                ...
                switch (data.transfer_mode) {
                    ...
                    case _xfer_fold_out:
                        render_object->rectangle.xc
                                = view->half_screen_width
                                  + (int(transformed_origin.y) * view->world_to_screen_x) / DistanceRef;
                }

                render_object->rectangle.ProjDistance = PIN(ProjDistance, SHRT_MIN, SHRT_MAX);
                render_object->rectangle.depth        = DistanceRef;
                instantiate_rectangle_transfer_mode(view, &render_object->rectangle, data.transfer_mode,
                                                    data.transfer_phase);

                render_object->rectangle.ambient_shade
                        = MAX(shape_information->minimum_light_intensity, floor_intensity);
                render_object->rectangle.ceiling_light
                        = MAX(shape_information->minimum_light_intensity, ceiling_intensity);

                if (view->shading_mode == _shading_infravision)
                    render_object->rectangle.flags |= _SHADELESS_BIT;
                ...
                if (object->parasitic_object != NONE) {
                    ...
                    parasitic_render_object
                            = build_render_object(get_object_data(object->parasitic_object), floor_intensity,
                                                  ceiling_intensity, Opacity, &parasitic_origin, &parasitic_rel_origin);
                    ...
                    if (parasitic_render_object) {
                        ...
                        parasitic_render_object->rectangle.ambient_shade= render_object->rectangle.ambient_shade=
                            MAX(parasitic_render_object->rectangle.ambient_shade,
                        render_object->rectangle.ambient_shade); parasitic_render_object->rectangle.ceiling_light=
                        render_object->rectangle.ceiling_light= MAX(parasitic_render_object->rectangle.ceiling_light,
                        render_object->rectangle.ceiling_light);
                        */
...
void RenderPlaceObjsClass::sort_render_object_into_tree(
    ...
    for (render_object = &RenderObjects.front(); render_object <= &RenderObjects.back(); ++render_object) {
        ...
        if (render_object->rectangle.x1 > new_render_object->rectangle.x0
            && render_object->rectangle.x0 < new_render_object->rectangle.x1
            && render_object->rectangle.y1 > new_render_object->rectangle.y0
            && render_object->rectangle.y0 < new_render_object->rectangle.y1) {
            /* update our closest and farthest matches */
            if (render_object->rectangle.depth
                > new_render_object->rectangle.depth) /* found deeper intersecting object */
            {
                ...
                if (render_object->rectangle.depth
                    < new_render_object->rectangle.depth) /* found shallower intersecting object */
                {
                    if (!shallow_render_object
                        || shallow_render_object->rectangle.depth <= render_object->rectangle.depth) {
    ...
    for (int i = 1; i < span.base_nodes.size(); ++i) desired_node = std::max(desired_node, span.base_nodes[i].node);
    ...
    if (deep_render_object) {
        ...
        //		if (shallow_render_object)
        {
            ...
            for (reference = &desired_node->exterior_objects;
                 *reference != shallow_render_object && *reference
                 && (*reference)->rectangle.depth > new_render_object->rectangle.depth;
...
auto RenderPlaceObjsClass::build_base_node_list(const render_object_data* render_object, short origin_polygon_index)
        -> span_data {
    ...
    auto scan_toward = [&](long_point2d destination, bool scanning_rightward) {
        ...
        do {
            ...
            short state = _looking_for_first_nonzero_vertex; /* really: testing first vertex state (we don’t have zero
                                                                vertices) */
            ...
            do {
                ...
                if (cross_product_k(vertex - origin.xy(), scan_vector) >= 0) {
                    ...
                    switch (state) {
                        ...
                        case _looking_clockwise_for_right_vertex: /* found the transition we were looking for */
                            ...
                            vertex_b      = vertex;
                            polygon_index = polygon->adjacent_polygon_indexes[WRAP_LOW(vertex_index,
                ...
                if (state != NONE && !vertex_index)
                    polygon_index = state = NONE; /* we can’t find a way out; give up */
            ...
            if (polygon_index != NONE) {
                ...
                if ((view->origin.z < origin.z && polygon->floor_height < origin_polygon_floor_height)
                    || (view->origin.z > origin.z && origin.z + WORLD_ONE_HALF < polygon->floor_height
                        && polygon->floor_height > origin_polygon_floor_height)) {
                    ...
                    if (!TEST_RENDER_FLAG(polygon_index, _polygon_is_visible))
                        polygon_index = NONE; /* don’t have transformed data, don’t even try! */
    ...
    auto pt_along_object_rect = [&](world_distance offset_from_origin) -> long_point2d // can be off-map
    {
        ...
        const auto v
                = (1.f * offset_from_origin / TRIG_MAGNITUDE) * long_vector2d{cosine_table[right], sine_table[right]};
        return origin.xy() + v;
    };
...
void RenderPlaceObjsClass::build_aggregate_render_object_clipping_window(render_object_data* render_object,
                                                                         const span_data& span) {
                    ...
                    if (NewCWPointer != OldCWPointer) {
                        ...
                        for (size_t k = 0; k < Length; k++) {
                            ...
                            if (ClippingWindow.next_window != NULL)
                                ClippingWindow.next_window
                                        = (clipping_window_data*)(NewCWPointer
                                                                  + (POINTER_CAST(ClippingWindow.next_window)
                                                                     - OldCWPointer));
                        }
                        for (size_t k = 0; k < SortedNodes.size(); k++) {
                            sorted_node_data& SortedNode = SortedNodes[k];
                            if (SortedNode.clipping_windows != NULL)
                                SortedNode.clipping_windows
                                        = (clipping_window_data*)(NewCWPointer
                                                                  + (POINTER_CAST(SortedNode.clipping_windows)
                                                                     - OldCWPointer));
                        }
                        for (unsigned k = 0; k < RenderObjects.size(); k++) {
                            render_object_data& RenderObject = RenderObjects[k];
                            if (RenderObject.clipping_windows != NULL)
                                RenderObject.clipping_windows
                                        = (clipping_window_data*)(NewCWPointer
                                                                  + (POINTER_CAST(RenderObject.clipping_windows)
                                                                     - OldCWPointer));
                        }
                    }
...
bool RenderPlaceObjsClass::add_object_to_sorted_nodes(object_data* object, _fixed floor_intensity,
                                                      _fixed ceiling_intensity, float Opacity) {
    const auto render_object
            = build_render_object(object, floor_intensity, ceiling_intensity, Opacity, nullptr, nullptr);
    if (!render_object)
        return false;
...
shape_information_data* RenderPlaceObjsClass::rescale_shape_information(shape_information_data* unscaled,
...
void FindProjectedBoundingBox(GLfloat BoundingBox[2][3], long_point3d& TransformedPosition, GLfloat Scale,
                              short RelativeAngle, shape_information_data& ShapeInfo, short DepthType, int& Farthest,
                              int& ProjDistance, int& DistanceRef, int& LightDepth, GLfloat* Direction) {
