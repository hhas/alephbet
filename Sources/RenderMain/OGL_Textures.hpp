...
const int BadTextureType = 32'767;
...
class TextureManager {
  ...
  public:
    ...
    short NormalBlend() {
        return (TxtrOptsPtr->NormalBlend)
               + ((NormalImage.get() && NormalImage.get()->IsPremultiplied()
                   && TxtrOptsPtr->NormalBlend < OGL_FIRST_PREMULT_ALPHA)
                          ? OGL_FIRST_PREMULT_ALPHA
                          : 0);
    }

    short GlowBlend() {
        return (TxtrOptsPtr->GlowBlend)
               + ((GlowImage.get() && GlowImage.get()->IsPremultiplied()
                   && TxtrOptsPtr->GlowBlend < OGL_FIRST_PREMULT_ALPHA)
                          ? OGL_FIRST_PREMULT_ALPHA
                          : 0);
    }
