...
void initialize_view_data(struct view_data* view, bool ignore_preferences) {
    ...
    view->half_vertical_cone
            = (angle)(NUMBER_OF_ANGLES
                              * atan(((double)view->half_screen_height * view->vertical_scale) / world_to_screen)
                              / two_pi
                      + 1.0);
...
void render_view(struct view_data* view, struct bitmap_definition* software_render_dest) {
    ...
    (dynamic_world->polygon_count/8+((dynamic_world->polygon_count%8)?1:0)*sizeof(byte))); #endif
    */
...
void check_m1_exploration(void) {
    ...
    for (int i  = (dynamic_world->tick_count % TICKS_PER_EXPLORE); i < dynamic_world->player_count;
         i     += TICKS_PER_EXPLORE) {
        ...
        we start on a vertex this can happen.  this is a destructive modification of the origin. */
    {
        ...
        for (i = 0; i < polygon->vertex_count; ++i) {
            ...
            if (vertex->x == view->origin.x && vertex->y == view->origin.y) {
                world_point2d* ccw_vertex
                        = &get_endpoint_data(polygon->endpoint_indexes[WRAP_LOW(i, polygon->vertex_count - 1)])->vertex;
                world_point2d* cw_vertex
...
static void update_render_effect(struct view_data* view) {
    ...
    if (phase > period) {
        ...
        switch (effect) {
            case _render_effect_explosion:
                shake_view_origin(view, EXPLOSION_EFFECT_RANGE
                                                - ((EXPLOSION_EFFECT_RANGE / 2) * interpolated_phase) / period);
                break;
            ...
            case _render_effect_fold_out:
                ...
                view->world_to_screen_x = view->real_world_to_screen_x
                                          + (4 * view->real_world_to_screen_x * interpolated_phase) / period;
                view->world_to_screen_y = view->real_world_to_screen_y
                                          - (view->real_world_to_screen_y * interpolated_phase) / (period + period / 4);
                break;
        }
    }
}
...
void instantiate_rectangle_transfer_mode(view_data* view, rectangle_definition* rectangle, short transfer_mode,
    ...
    switch (transfer_mode) {
        ...
        case _xfer_pulsating_static:
            rectangle->transfer_mode = _static_transfer;
            rectangle->transfer_data
                    = 0x8000
                      + ((0x6000 * sine_table[FIXED_INTEGERAL_PART(transfer_phase * NUMBER_OF_ANGLES)]) >> TRIG_SHIFT);
            break;
        ...
        case _xfer_fold_out:
            if (View_DoStaticEffect()) {
                ...
                short delta0
                        = FIXED_INTEGERAL_PART(int32(((1LL * rectangle->xc - rectangle->x0) - 1) * transfer_phase));
                short delta1
                        = FIXED_INTEGERAL_PART(int32(((1LL * rectangle->x1 - rectangle->xc) - 1) * transfer_phase));
...
void instantiate_polygon_transfer_mode(struct view_data* view, struct polygon_definition* polygon, short transfer_mode,
                                       bool horizontal) {
    ...
    switch (transfer_mode) {
        ...
        case _xfer_reverse_fast_vertical_slide:
            ...
            switch (transfer_mode) {
                case _xfer_fast_horizontal_slide:
                    transfer_phase <<= 1;
                case _xfer_horizontal_slide:
                    x0 = (transfer_phase << 2) & (WORLD_ONE - 1);
                    break;

                case _xfer_fast_vertical_slide:
                    transfer_phase <<= 1;
                case _xfer_vertical_slide:
                    y0 = (transfer_phase << 2) & (WORLD_ONE - 1);
                    break;

                case _xfer_reverse_fast_horizontal_slide:
                    transfer_phase <<= 1;
                case _xfer_reverse_horizontal_slide:
                    x0 = WORLD_ONE - (transfer_phase << 2) & (WORLD_ONE - 1);
                    break;

                case _xfer_reverse_fast_vertical_slide:
                    transfer_phase <<= 1;
                case _xfer_reverse_vertical_slide:
                    y0 = WORLD_ONE - (transfer_phase << 2) & (WORLD_ONE - 1);
                    break;
                ...
                case _xfer_wander:
                    ...
                    x0                       = (cosine_table[NORMALIZE_ANGLE(alternate_transfer_phase)]
                          ...
                          + (cosine_table[NORMALIZE_ANGLE(5 * alternate_transfer_phase)] >> 1))
                    ...
                    y0 = (sine_table[NORMALIZE_ANGLE(transfer_phase)]
                          ...
                          + (sine_table[NORMALIZE_ANGLE(3 * transfer_phase)] >> 1))
                         >> (WORLD_FRACTIONAL_BITS - TRIG_SHIFT + 2);
                    break;
            }
            if (horizontal) {
        ...
        case _xfer_fast_wobble:
            ...
            transfer_phase  = (transfer_phase >= WORLD_ONE / 32) ? (WORLD_ONE / 32 + WORLD_ONE / 64 - transfer_phase)
            ...
            if (horizontal) {
                ...
                if (transfer_mode == _xfer_pulsate) /* translate .origin perpendicular to .vector */
                {
                    ...
                    world_distance vector_magnitude
                            = isqrt(polygon->vector.i * polygon->vector.i + polygon->vector.j * polygon->vector.j);
...
static void render_viewer_sprite_layer(view_data* view, RasterizerClass* RasPtr) {
    ...
    while (get_weapon_display_information(&count, &display_data)) {
        ...
        if (ModelPtr) {
            ...
            textured_rectangle.MixFrac
                    = display_data.Ticks > 0 ? float(display_data.Phase) / float(display_data.Ticks) : 0;
        ...
        if (shape_information->flags & _X_MIRRORED_BIT)
            display_data.flip_horizontal = !display_data.flip_horizontal;
        if (shape_information->flags & _Y_MIRRORED_BIT)
            display_data.flip_vertical = !display_data.flip_vertical;

        /* calculate shape rectangle */
        position_sprite_axis(&textured_rectangle.x0, &textured_rectangle.x1, view->screen_height, view->screen_width,
                             display_data.horizontal_positioning_mode, display_data.horizontal_position,
                             display_data.flip_horizontal, shape_information->world_left,
                             shape_information->world_right);
        position_sprite_axis(&textured_rectangle.y0, &textured_rectangle.y1, view->screen_height, view->screen_height,
                             display_data.vertical_positioning_mode, display_data.vertical_position,
                             display_data.flip_vertical, -shape_information->world_top,
                             -shape_information->world_bottom);
        ...
        extended_get_shape_bitmap_and_shading_table(display_data.collection, display_data.low_level_shape_index,
                                                    &textured_rectangle.texture, &textured_rectangle.shading_tables,
                                                    view->shading_mode);
        if (!textured_rectangle.texture)
            continue;
        ...
        textured_rectangle.ambient_shade
                = get_light_intensity(get_polygon_data(view->origin_polygon_index)->floor_lightsource_index);
        textured_rectangle.ambient_shade
                = MAX(shape_information->minimum_light_intensity, textured_rectangle.ambient_shade);
        if (view->shading_mode == _shading_infravision)
            textured_rectangle.flags |= _SHADELESS_BIT;
        ...
        instantiate_rectangle_transfer_mode(view, &textured_rectangle, display_data.transfer_mode,
                                            display_data.transfer_phase);
...
static void shake_view_origin(struct view_data* view, world_distance delta) {
    ...
    new_origin.y
            += half_delta
               - ((delta
                   * sine_table[NORMALIZE_ANGLE(((view->tick_count + 5 * TICKS_PER_SECOND) & ~3) * (7 * FULL_CIRCLE))])
                  >> TRIG_SHIFT);
    new_origin.z
            += half_delta
               - ((delta
                   * sine_table[NORMALIZE_ANGLE(((view->tick_count + 7 * TICKS_PER_SECOND) & ~3) * (7 * FULL_CIRCLE))])
                  >> TRIG_SHIFT);
