...
void RenderRasterize_Shader::setupGL(Rasterizer_Shader_Class& Rasterizer) {
    ...
    if (TEST_FLAG(Get_OGL_ConfigureData().Flags, OGL_Flag_Blur)) {
        if (s_blur && s_bloom) {
            blur.reset(new Blur(
                    640., 640. * graphics_preferences->screen_mode.height / graphics_preferences->screen_mode.width,
                    s_blur, s_bloom));
        }
    }
...
void RenderRasterize_Shader::render_tree() {
    ...
    Shader* landscape_shaders[] = {Shader::get(Shader::S_Landscape),
    ...
    Shader* fog_mode_shaders[] = {Shader::get(Shader::S_Bump),
    ...
    if (current_player->infravision_duration == 0 && TEST_FLAG(Get_OGL_ConfigureData().Flags, OGL_Flag_Blur)
        && blur.get()) {
        blur->begin();
...
void RenderRasterize_Shader::store_endpoint(endpoint_data* endpoint, long_vector2d& p) {
    p.i = endpoint->vertex.x;
    p.j = endpoint->vertex.y;
}

std::unique_ptr<TextureManager> RenderRasterize_Shader::setupSpriteTexture(const rectangle_definition& rect, short type,
    ...
    if (current_player->infravision_duration) {
        ...
        extended_get_shape_bitmap_and_shading_table(GET_DESCRIPTOR_COLLECTION(TMgr->ShapeDesc), TMgr->LowLevelShape,
...
std::unique_ptr<TextureManager> RenderRasterize_Shader::setupWallTexture(const shape_descriptor& Texture,
                                                                         short transferMode, float pulsate,
    ...
    if (TMgr->TextureType == OGL_Txtr_Landscape && opts) {
        if (opts->SphereMap) {
            ...
            double VertScale
                    = (AdjustedVertExp >= 0) ? double(1 << AdjustedVertExp) : 1 / double(1 << (-AdjustedVertExp));
...
bool setupGlow(struct view_data* view, std::unique_ptr<TextureManager>& TMgr, float wobble, float intensity,
               float flare, float selfLuminosity, float offset, RenderStep renderStep) {
...
void RenderRasterize_Shader::render_node_floor_or_ceiling(clipping_window_data* window, polygon_data* polygon,
                                                          horizontal_surface_data* surface, bool void_present,
...
void RenderRasterize_Shader::render_node_side(clipping_window_data* window, vertical_surface_data* surface,
...
bool RenderModel(rectangle_definition& RenderRectangle, short Collection, short CLUT, float flare, float selfLuminosity,
                 RenderStep renderStep) {
    ...
    if (RenderRectangle.transfer_mode == _static_transfer) {
        ...
        s->enable();
        s->setFloat(Shader::U_TransferFadeOut,
                    ((float)((uint16)RenderRectangle.transfer_data)) / (float)((int)FIXED_ONE));
    } else if (current_player->infravision_duration) {
        color[0] = color[1] = color[2] = 1;
...
void RenderRasterize_Shader::render_node_object(render_object_data* object, bool other_side_of_media,
                                                RenderStep renderStep) {
...
void RenderRasterize_Shader::_render_node_object_helper(render_object_data* object, RenderStep renderStep) {
    ...
    GLfloat vertex_array[12] = {0, rect.WorldLeft * rect.HorizScale * rect.Scale,  rect.WorldTop * rect.Scale,
    ...
    GLfloat texcoord_array[8] = {texCoords[0][0], texCoords[1][0], texCoords[0][0], texCoords[1][1],
...
extern void position_sprite_axis(short* x0, short* x1, short scale_width, short screen_width, short positioning_mode,
...
void RenderRasterize_Shader::render_viewer_sprite_layer(RenderStep renderStep) {
    ...
    while (get_weapon_display_information(&count, &display_data)) {
        ...
        position_sprite_axis(&rect.x0, &rect.x1, view->screen_height, view->screen_width,
        ...
        position_sprite_axis(&rect.y0, &rect.y1, view->screen_height, view->screen_height,
        ...
        extended_get_shape_bitmap_and_shading_table(display_data.collection, display_data.low_level_shape_index,
                                                    &rect.texture, &rect.shading_tables, view->shading_mode);
        if (!rect.texture)
            continue;
