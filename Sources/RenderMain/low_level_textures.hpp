...
inline pixel32 average(pixel32 fg, pixel32 bg) {
    ...
    return (((((fg) ^ (bg)) & 0xfffe'fefeL) >> 1) + ((fg) & (bg)));
}
...
inline T alpha_blend(T fg, T bg, pixel8 alpha, pixel32 rmask, pixel32 bmask, pixel32 gmask) {
    return ((rmask & ((bg & rmask) + ((int)(1LL * ((int)(fg & rmask) - (int)(bg & rmask)) * alpha) >> 8)))
...
void inline write_pixel(T* dst, pixel8 pixel, T* shading_table, uint8* opacity_table, pixel32 rmask, pixel32 gmask,
                        pixel32 bmask) {
...
void texture_horizontal_polygon_lines(struct bitmap_definition* texture, struct bitmap_definition* screen,
                                      struct view_data* view, struct _horizontal_polygon_line_data* data, short y0,
                                      short* x0_table, short* x1_table, short line_count, uint8* opacity_table = 0) {
    (void)(view);
    ...
    while ((line_count -= 1) >= 0) {
        ...
        while ((count -= 1) >= 0) {
            write_pixel<T, sw_alpha_blend, false>(write++,
                                                  base_address[((source_y >> (HORIZONTAL_HEIGHT_DOWNSHIFT - TEXBITS))
                                                                & (((1 << TEXBITS) - 1) << TEXBITS))
                                                               + (source_x >> HORIZONTAL_WIDTH_DOWNSHIFT)],
                                                  shading_table, opacity_table, rmask, gmask, bmask);
...
template <typename T>
void landscape_horizontal_polygon_lines(struct bitmap_definition* texture, struct bitmap_definition* screen,
                                        struct view_data* view, struct _horizontal_polygon_line_data* data, short y0,
...
void texture_vertical_polygon_lines(struct bitmap_definition* screen, struct view_data* view,
    ...
    while (line_count > 0) {
        if (line_count < 4 || (x & 3) || aborted) {
            ...
            read          = line->texture;
            write         = (T*)(screen->row_addresses[0] + bytes_per_row * y0)
                    + x; // invalid but unread if y0 == screen->height

            for (count = y1 - y0; count > 0; --count) {
                write_pixel<T, sw_alpha_blend, check_transparent>(write, read[texture_y >> downshift], shading_table,
                                                                  opacity_table, rmask, gmask, bmask);
            ...
            /* sync */
            {
                ...
                for (count = ymax - y0, temp_write = (T*)screen->row_addresses[y0] + x; count > 0; --count) {
                    copy_check_transparent<T, check_transparent>(temp_write, read0[texture_y0 >> downshift],
                                                                 shading_table0);
                ...
                for (count = ymax - y1, temp_write = (T*)screen->row_addresses[y1] + x; count > 0; --count) {
                    copy_check_transparent<T, check_transparent>(temp_write + 1, read1[texture_y1 >> downshift],
                                                                 shading_table1);
                    temp_write  = (T*)((byte*)temp_write + bytes_per_row);
                ...
                for (count = ymax - y2, temp_write = (T*)screen->row_addresses[y2] + x; count > 0; --count) {
                    copy_check_transparent<T, check_transparent>(temp_write + 2, read2[texture_y2 >> downshift],
                                                                 shading_table2);
                    temp_write  = (T*)((byte*)temp_write + bytes_per_row);
                ...
                for (count = ymax - y3, temp_write = (T*)screen->row_addresses[y3] + x; count > 0; --count) {
                    copy_check_transparent<T, check_transparent>(temp_write + 3, read3[texture_y3 >> downshift],
                                                                 shading_table3);
                    temp_write  = (T*)((byte*)temp_write + bytes_per_row);
            ...
            /* parallel map (x4) */
            {
                ...
                for (; count > 0; --count) {
                    write_pixel<T, sw_alpha_blend, check_transparent>(
                            write, read0[texture_y0 >> downshift], shading_table0, opacity_table, rmask, gmask, bmask);
                    texture_y0 += texture_dy0;

                    write_pixel<T, sw_alpha_blend, check_transparent>(write + 1, read1[texture_y1 >> downshift],
                                                                      shading_table1, opacity_table, rmask, gmask,
                                                                      bmask);
                    texture_y1 += texture_dy1;

                    write_pixel<T, sw_alpha_blend, check_transparent>(write + 2, read2[texture_y2 >> downshift],
                                                                      shading_table2, opacity_table, rmask, gmask,
                                                                      bmask);
                    texture_y2 += texture_dy2;

                    write_pixel<T, sw_alpha_blend, check_transparent>(write + 3, read3[texture_y3 >> downshift],
                                                                      shading_table3, opacity_table, rmask, gmask,
                                                                      bmask);
                    texture_y3 += texture_dy3;
...
template <typename T>
void tint_vertical_polygon_lines(struct bitmap_definition* screen, struct view_data* view,
                                 struct _vertical_polygon_data* data, short* y0_table, short* y1_table,
...
template <typename T, bool check_transparent>
void randomize_vertical_polygon_lines(struct bitmap_definition* screen, struct view_data* view,
                                      struct _vertical_polygon_data* data, short* y0_table, short* y1_table,
                                      uint16 transfer_data) {
    struct _vertical_polygon_line_data* line = (struct _vertical_polygon_line_data*)(data + 1);
    short bytes_per_row                      = screen->bytes_per_row;
    ...
    while ((line_count -= 1) >= 0) {
        ...
        while ((count -= 1) >= 0) {
            if (!check_transparent || read[texture_y >> (data->downshift)]) {
                if (seed >= drop_less_than)
                    *write = randomize_vertical_polygon_lines_write<T>(seed);
                if (seed & 1)
                    seed = (seed >> 1) ^ 0xb400;
                else
                    seed = seed >> 1;
