...
OGL_TexturesStats gGLTxStats = {0, 0, 0, 500'000, 0, 0, 0};
...
void TextureState::FrameTick() {
    ...
    if (used) {
        ...
        switch (TextureType) {
            case OGL_Txtr_Wall:
                if (unusedFrames > 300)
                    Reset(); // at least 10 seconds till wall textures are released
                break;
            ...
            case OGL_Txtr_Inhabitant:
                if (unusedFrames > 450)
                    Reset(); // release unused sprites in 15 seconds
                break;
            case OGL_Txtr_WeaponsInHand:
            case OGL_Txtr_HUD:
                if (unusedFrames > 600)
                    Reset(); // release weapons in hand in 20 seconds
                break;
        }
    }
}
...
void OGL_StartTextures() {
    ...
    for (int it = 0; it < OGL_NUMBER_OF_TEXTURE_TYPES; it++)
        for (int ic = 0; ic < MAXIMUM_COLLECTIONS; ic++) {
            ...
            TextureStateSets[it][ic]
                    = (CollectionPresent && NumberOfBitmaps > 0) ? (new CollBitmapTextureState[NumberOfBitmaps]) : 0;
        }

    // Initialize the texture-type info
    const int NUMBER_OF_NEAR_FILTERS                      = 2;
    const GLenum NearFilterList[NUMBER_OF_NEAR_FILTERS]   = {GL_NEAREST, GL_LINEAR};
    const int NUMBER_OF_FAR_FILTERS                       = 6;
    const GLenum FarFilterList[NUMBER_OF_FAR_FILTERS]     = {GL_NEAREST,
                                                             GL_LINEAR,
    ...
    const GLenum ColorFormatList[NUMBER_OF_COLOR_FORMATS] = {GL_RGBA8, GL_RGBA4, GL_RGBA2};
...
void OGL_FrameTickTextures() {
    ...
    for (i = sgActiveTextureStates.begin(); i != sgActiveTextureStates.end(); i++) { (*i)->FrameTick(); }
}
...
short ModifyCLUT(short TransferMode, short CLUT) {
    ...
    if (TransferMode == _static_transfer)
        CTable = SILHOUETTE_BITMAP_CLUTSPECIFIC + CLUT;
    else if (TransferMode == _tinted_transfer)
        CTable = SILHOUETTE_BITMAP_CLUTSPECIFIC + CLUT;
    else if (InfravisionActive)
        CTable = INFRAVISION_BITMAP_CLUTSPECIFIC + CLUT;
    else
        CTable = CLUT;
...
bool TextureManager::LoadSubstituteTexture() {
    ...
    switch (TextureType) {
        case OGL_Txtr_Wall:
            ...
            TxtrWidth  = Width;
            if (!npotTextures) {
                if (TxtrWidth != NextPowerOfTwo(TxtrWidth))
                    return false;
                if (TxtrHeight != NextPowerOfTwo(TxtrHeight))
                    return false;
            }
        ...
        case OGL_Txtr_Landscape:
            ...
            TxtrWidth  = Width;
            TxtrHeight = (Landscape_AspRatExp >= 0) ? (TxtrWidth >> Landscape_AspRatExp)
            ...
            U_Offset = -1.0 + ((TxtrHeight - NormalImg.GetHeight()) / 2.0 / TxtrHeight)
                       + (1.0 - NormalImg.GetUScale()) / 2.0;
...
void TextureManager::FindColorTables() {
    ...
    if (IsSilhouetteTable(CTable)) {
        ...
        for (int k = 1; k < MAXIMUM_SHADING_TABLE_INDEXES; k++) NormalColorTable[k] = 0xffff'ffff;
        return;
    }
    ...
    if (IsInfravisionTable(CTable) || !IsShadeless)
        OrigColorTable += NumSrcBytes * (number_of_shading_tables - 1) * MAXIMUM_SHADING_TABLE_INDEXES;
    ...
    if (!IsShadeless && (TextureType != OGL_Txtr_Landscape)) {
        ...
        for (int k = 1; k < MAXIMUM_SHADING_TABLE_INDEXES; k++) {
            ...
            for (int q = 0; q < 3; q++) GlowEntry[q] = NormalEntry[q];
...
void TextureManager::PremultiplyColorTables() {
    uint32 alphaMask = PlatformIsLittleEndian() ? 0xff00'0000 : 0x0000'00ff;
...
uint32* TextureManager::GetOGLTexture(uint32* ColorTable) {
    ...
    uint32 rgb_mask = PlatformIsLittleEndian() ? 0x00ff'ffff : 0xffff'ff00;
    ...
    for (short h = OGLHeightOffset; h < OGLHeightFinish; h++) {
        ...
        for (short w = 0; w < OGLWidthOffset; w++) *(OGLStrip++) = ColorTable[*OrigStrip] & rgb_mask;

        for (short w = OGLWidthOffset; w < OGLWidthFinish; w++) *(OGLStrip++) = ColorTable[*(OrigStrip++)];

        // smear last pixel to right edge
        for (short w = OGLWidthFinish; w < TxtrWidth; w++) *(OGLStrip++) = ColorTable[*(OrigStrip - 1)] & rgb_mask;
    }
    ...
    for (short h = 0; h < OGLHeightOffset; h++) {
        ...
        for (short w = 0; w < TxtrWidth; w++) *(OGLStrip++) = *(SrcStrip++) & rgb_mask;
    }
...
uint32* TextureManager::GetFakeLandscape() const {
    ...
    for (int h = 0; h < TxtrHeight / 2; h++)
        for (int w = 0; w < TxtrWidth; w++) *(BufPtr++) = TxtrLandColor;
    for (int h = 0; h < TxtrHeight / 2; h++)
        for (int w = 0; w < TxtrWidth; w++) *(BufPtr++) = TxtrSkyColor;
...
uint32* TextureManager::Shrink(uint32* Buffer) {
    ...
    gluScaleImage(GL_RGBA, TxtrWidth, TxtrHeight, GL_UNSIGNED_BYTE, Buffer, LoadedWidth, LoadedHeight, GL_UNSIGNED_BYTE,
                  NewBuffer);
...
void TextureManager::PlaceTexture(const ImageDescriptor* Image, bool normal_map) {
    ...
    bool load_as_sRGB = (Wanting_sRGB && !normal_map && Collection != _collection_interface
    ...
    if (Image->GetFormat() == ImageDescriptor::RGBA8) {
        switch (TxtrTypeInfo.FarFilter) {
            case GL_NEAREST:
            case GL_LINEAR:
                glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, Image->GetWidth(), Image->GetHeight(), 0, GL_RGBA,
            ...
            case GL_LINEAR_MIPMAP_LINEAR:
                if (Image->GetMipMapCount() > 1) {
                    ...
                    for (i = 0; i < Image->GetMipMapCount(); i++) {
                        glTexImage2D(GL_TEXTURE_2D, i, internalFormat, max(1, Image->GetWidth() >> i),
                                     max(1, Image->GetHeight() >> i), 0, GL_RGBA, GL_UNSIGNED_BYTE,
                    ...
                    if (useSGISMipmaps) {
                        ...
                        glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, Image->GetWidth(), Image->GetHeight(), 0,
                                     GL_RGBA, GL_UNSIGNED_BYTE, Image->GetBuffer());
                    } else
#endif
                    {
                        gluBuild2DMipmaps(GL_TEXTURE_2D, internalFormat, Image->GetWidth(), Image->GetHeight(), GL_RGBA,
                                          GL_UNSIGNED_BYTE, Image->GetBuffer());
                    }
                    mipmapsLoaded = true;
                }
    ...
    } else if (Image->GetFormat() == ImageDescriptor::DXTC1 || Image->GetFormat() == ImageDescriptor::DXTC3
        ...
        switch (TxtrTypeInfo.FarFilter) {
            case GL_NEAREST:
            case GL_LINEAR:
                glCompressedTexImage2DARB(GL_TEXTURE_2D, 0, internalFormat, Image->GetWidth(), Image->GetHeight(), 0,
                                          Image->GetMipMapSize(0), Image->GetBuffer());
                break;
            case GL_NEAREST_MIPMAP_NEAREST:
            case GL_LINEAR_MIPMAP_NEAREST:
            case GL_NEAREST_MIPMAP_LINEAR:
            case GL_LINEAR_MIPMAP_LINEAR:
                if (Image->GetMipMapCount() > 1) {
                    ...
                    int i = 0;
                    for (i = 0; i < Image->GetMipMapCount(); i++) {
                        glCompressedTexImage2DARB(GL_TEXTURE_2D, i, internalFormat, max(1, Image->GetWidth() >> i),
                                                  max(1, Image->GetHeight() >> i), 0, Image->GetMipMapSize(i),
                                                  Image->GetMipMapPtr(i));
                    }
                    mipmapsLoaded = true;
                } else {
...
#endif
                    glCompressedTexImage2DARB(GL_TEXTURE_2D, 0, internalFormat, Image->GetWidth(), Image->GetHeight(),
                                              0, Image->GetMipMapSize(0), Image->GetBuffer());
                }
                break;
    ...
    if ((TxtrTypeInfo.FarFilter == GL_NEAREST_MIPMAP_NEAREST || TxtrTypeInfo.FarFilter == GL_LINEAR_MIPMAP_NEAREST
         || TxtrTypeInfo.FarFilter == GL_NEAREST_MIPMAP_LINEAR || TxtrTypeInfo.FarFilter == GL_LINEAR_MIPMAP_LINEAR)
        && !mipmapsLoaded) {
            ...
            // enable anisotropic filtering
            {
                ...
                if (anisoLevel > 0.0) {
                    ...
                    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT,
                                    1.0F + ((anisoLevel - 1.0F) / 15.0F) * (max_aniso - 1.0F));
                }
            }
#endif
            break;
...
void TextureManager::RenderNormal() {
    ...
    if (gGLTxStats.minBind > time)
        gGLTxStats.minBind = time;
    if (gGLTxStats.maxBind < time)
        gGLTxStats.maxBind = time;
    if (time > 2)
        gGLTxStats.longNormalSetups++;
}
    ...
    for (int it = 0; it < OGL_NUMBER_OF_TEXTURE_TYPES; it++)
        for (int ic = 0; ic < MAXIMUM_COLLECTIONS; ic++) {
            ...
            for (int ib = 0; ib < NumberOfBitmaps; ib++) {
                ...
                for (int ist = 0; ist < NUMBER_OF_OPENGL_BITMAP_SETS; ist++) TSSet[ist].Reset();
            }
        }
...
void LoadModelSkin(ImageDescriptor& SkinImage, short Collection, short CLUT) {
    ...
    if (Image.get()->GetFormat() == ImageDescriptor::RGBA8) {
        switch (TxtrTypeInfo.FarFilter) {
            case GL_NEAREST:
            case GL_LINEAR:
                glTexImage2D(GL_TEXTURE_2D, 0, TxtrTypeInfo.ColorFormat, LoadedWidth, LoadedHeight, 0, GL_RGBA,
                             GL_UNSIGNED_BYTE, Image.get()->GetBuffer());
                break;
            case GL_NEAREST_MIPMAP_NEAREST:
            case GL_LINEAR_MIPMAP_NEAREST:
            case GL_NEAREST_MIPMAP_LINEAR:
            case GL_LINEAR_MIPMAP_LINEAR:
                if (Image.get()->GetMipMapCount() > 1) {
                    ...
                    for (i = 0; i < Image.get()->GetMipMapCount(); i++) {
                        glTexImage2D(GL_TEXTURE_2D, i, internalFormat, max(1, Image.get()->GetWidth() >> i),
                                     max(1, Image.get()->GetHeight() >> i), 0, GL_RGBA, GL_UNSIGNED_BYTE,
                                     Image.get()->GetMipMapPtr(i));
                    }
                    mipmapsLoaded = true;
                } else {
#ifdef GL_SGIS_generate_mipmap
                    if (useSGISMipmaps) {
                        glTexParameteri(GL_TEXTURE_2D, GL_GENERATE_MIPMAP_SGIS, GL_TRUE);
                        glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, Image.get()->GetWidth(),
                                     Image.get()->GetHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE, Image.get()->GetBuffer());
                    } else
#endif
                    {
                        gluBuild2DMipmaps(GL_TEXTURE_2D, TxtrTypeInfo.ColorFormat, LoadedWidth, LoadedHeight, GL_RGBA,
                                          GL_UNSIGNED_BYTE, Image.get()->GetBuffer());
                    }
                    mipmapsLoaded = true;
                }
    ...
    } else if (Image.get()->GetFormat() == ImageDescriptor::DXTC1 || Image.get()->GetFormat() == ImageDescriptor::DXTC3
        ...
        switch (TxtrTypeInfo.FarFilter) {
            case GL_NEAREST:
            case GL_LINEAR:
                glCompressedTexImage2DARB(GL_TEXTURE_2D, 0, internalFormat, Image.get()->GetWidth(),
                                          Image.get()->GetHeight(), 0, Image.get()->GetMipMapSize(0),
                                          Image.get()->GetBuffer());
                break;
            case GL_NEAREST_MIPMAP_NEAREST:
            case GL_LINEAR_MIPMAP_NEAREST:
                ...
                if (Image.get()->GetMipMapCount() > 1) {
                    ...
                    for (i = 0; i < Image.get()->GetMipMapCount(); i++) {
                        glCompressedTexImage2DARB(GL_TEXTURE_2D, i, internalFormat,
                                                  max(1, Image.get()->GetWidth() >> i),
                                                  max(1, Image.get()->GetHeight() >> i), 0,
                                                  Image.get()->GetMipMapSize(i), Image.get()->GetMipMapPtr(i));
                    }
                    mipmapsLoaded = true;
                } else {
#ifdef GL_SGIS_generate_mipmap
...
void FindSilhouetteVersionRGBA(int NumPixels, uint32* Pixels) {
    for (int i = 0; i < NumPixels; i++) { Pixels[i] |= PlatformIsLittleEndian() ? 0x00ff'ffff : 0xffff'ff00; }
}
...
void FindSilhouetteVersion(ImageDescriptorManager& imageManager) {
    if (imageManager.get()->GetFormat() == ImageDescriptor::RGBA8) {
        ...
        FindSilhouetteVersionDXTC1(imageManager.edit()->GetBufferSize(),
                                   (unsigned char*)imageManager.edit()->GetBuffer());
    } else if (imageManager.get()->GetFormat() == ImageDescriptor::DXTC3
               || imageManager.get()->GetFormat() == ImageDescriptor::DXTC5) {
        FindSilhouetteVersionDXTC35(imageManager.edit()->GetBufferSize(),
                                    (unsigned char*)imageManager.edit()->GetBuffer());
    }
...
void SetPixelOpacities(OGL_TextureOptions& Options, ImageDescriptorManager& imageManager) {
    if (Options.OpacityType != OGL_OpacType_Avg && Options.OpacityType != OGL_OpacType_Max
        && Options.OpacityScale == 1.0 && Options.OpacityShift == 0.0)
        return;
    ...
    if (imageManager.get()->GetFormat() == ImageDescriptor::RGBA8) {
        ...
        if (Options.OpacityType == OGL_OpacType_Avg || Options.OpacityType == OGL_OpacType_Max) {
            if (imageManager.edit()->MakeRGBA()) {
                SetPixelOpacitiesRGBA(Options, imageManager.edit()->GetBufferSize() / 4,
                                      imageManager.edit()->GetBuffer());
            } else if (Options.OpacityScale == 1.0 && Options.OpacityShift == 0.0) {
                return;
            } else {
                SetPixelOpacitiesDXTC3(Options, imageManager.edit()->GetBufferSize() / 4,
                                       (unsigned char*)imageManager.edit()->GetBuffer());
            }
        } else {
            // if it's just scale/shift, we can do without decompressing
            SetPixelOpacitiesDXTC3(Options, imageManager.edit()->GetBufferSize(),
                                   (unsigned char*)imageManager.edit()->GetBuffer());
        }
        ...
        if (Options.OpacityType == OGL_OpacType_Avg || Options.OpacityType == OGL_OpacType_Max) {
            if (imageManager.edit()->MakeRGBA()) {
                ...
                SetPixelOpacitiesDXTC5(Options, imageManager.edit()->GetBufferSize() / 4,
                                       (unsigned char*)imageManager.edit()->GetBuffer());
            }
        } else {
            SetPixelOpacitiesDXTC5(Options, imageManager.edit()->GetBufferSize(),
                                   (unsigned char*)imageManager.edit()->GetBuffer());
        }
    } else {
