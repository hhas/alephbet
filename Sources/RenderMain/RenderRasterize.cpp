...
RenderRasterizerClass::RenderRasterizerClass()
    : view(NULL), // Idiot-proofing
      RSPtr(NULL), RasPtr(NULL) {}

/* ---------- rendering the tree */

void RenderRasterizerClass::render_tree() { render_tree(kDiffuse); }
...
void RenderRasterizerClass::render_tree(RenderStep renderStep) {
    ...
    bool SeeThruLiquids = get_screen_mode()->acceleration != _no_acceleration
    ...
    for (node = SortedNodes.begin(); node != SortedNodes.end(); ++node) render_node(&*node, SeeThruLiquids, renderStep);
}

void RenderRasterizerClass::render_node(sorted_node_data* node, bool SeeThruLiquids, RenderStep renderStep) {
        ...
        if (ceiling_surface.height > floor_surface.height) {
            ...
            for (i = 0; i < polygon->vertex_count; ++i) {
                ...
                if (side_index != NONE && TEST_RENDER_FLAG(side_index, _side_is_visible)) {
                    ...
                    store_endpoint(
                            get_endpoint_data(polygon->endpoint_indexes[WRAP_HIGH(i, polygon->vertex_count - 1)]),
                            surface.p1);
...
void RenderRasterizerClass::store_endpoint(endpoint_data* endpoint, long_vector2d& p) {
...
void RenderRasterizerClass::render_node_floor_or_ceiling(clipping_window_data* window, polygon_data* polygon,
    ...
    if (Texture != UNONE) {
        ...
        if (vertex_count) {
            ...
            for (i = 0; i < vertex_count; ++i) {
                ...
                // vassert(screen->y>=0&&screen->y<=view->screen_height, csprintf(temporary, "horizontal: flags==%x,
                // window @ %p", world->flags, window));
            }
            ...
            get_shape_bitmap_and_shading_table(Texture, &textured_polygon.texture, &textured_polygon.shading_tables,
                                               view->shading_mode);
            // Bug out if bitmap is nonexistent
            if (!textured_polygon.texture)
                return;
            ...
            if (view->shading_mode == _shading_infravision)
                textured_polygon.flags |= _SHADELESS_BIT;
...
// LP change: added "void present on other side" flag
void RenderRasterizerClass::render_node_side(clipping_window_data* window, vertical_surface_data* surface,
    ...
    if (h > surface->h0 && Texture != UNONE) {
        ...
        vertex_count   = 2;
        posts[0].x     = surface->p0.i;
        posts[0].y     = surface->p0.j;
        posts[0].flags = 0;
        posts[1].x     = surface->p1.i;
        posts[1].y     = surface->p1.j;
        posts[1].flags = 0;
        ...
        if (vertex_count) {
            ...
            if (vertex_count) {
                ...
                for (i = 0; i < vertex_count; ++i) {
                    ...
                    switch (world->flags & (_clip_left | _clip_right)) {
                        case 0:
                            // LP change: making it long-distance friendly
                            {
                                int32 screen_x = view->half_screen_width
                                                 + (world->x ? (world->y * view->world_to_screen_x) / world->x : 0);
                                screen->x = PIN(screen_x, 0, view->screen_width);
                            }
                            break;
                        case _clip_left:
                    ...
                    switch (world->flags & (_clip_up | _clip_down)) {
                        case 0:
                            // LP change: making it long-distance friendly
                            {
                                int32 screen_y = view->half_screen_height
                                                 - (world->x ? (world->z * view->world_to_screen_y) / world->x : 0)
                                                 + view->dtanpitch;
                                screen->y = PIN(screen_y, 0, view->screen_height);
                            }
                            break;
                    ...
                    // vassert(screen->y>=0&&screen->y<=view->screen_height, csprintf(temporary, "#%d!in[#0,#%d]:
                    // flags==%x, wind@%p #%d w@%p s@%p", screen->y, view->screen_height, world->flags, window,
                    // vertex_count, world, screen));
                }
                ...
                textured_polygon.ShapeDesc = Texture;
                textured_polygon.ambient_shade
                        = get_light_intensity(surface->lightsource_index) + surface->ambient_delta;
...
void RenderRasterizerClass::render_node_object(render_object_data* object, bool other_side_of_media,
                                               RenderStep renderStep) {
...
short RenderRasterizerClass::xy_clip_horizontal_polygon(flagged_world_point2d* vertices, short vertex_count,
                                                        long_vector2d* line, uint16 flag) {
#ifdef QUICKDRAW_DEBUG
    debug_flagged_points(vertices, vertex_count);
    debug_vector(line);
#endif
    //	dprintf("clipping %p (#%d vertices) to vector %x,%x (slope==%x)", vertices, vertex_count, line->i, line->j,
    //slope);
    ...
    if (vertex_count) {
        ...
        bool clipped_exit_vertex     = true,
             clipped_entrance_vertex = true; /* will be false if these points lie directly on a vertex */

        do {
            // LP change:
            CROSSPROD_TYPE cross_product = CROSSPROD_TYPE(line->i) * vertices[vertex_index].y
                                           - CROSSPROD_TYPE(line->j) * vertices[vertex_index].x;
        ...
        if (exit_vertex != NONE) /* we’ve got clipping to do */
        {
            ...
            //			dprintf("entrance_vertex==#%d (%s), exit_vertex==#%d (%s)", entrance_vertex,
            //clipped_entrance_vertex ? "clipped" : "unclipped", exit_vertex, clipped_exit_vertex ? "clipped" :
            //"unclipped");

            /* clip the entrance to the clipped area */
            if (clipped_entrance_vertex) {
                xy_clip_flagged_world_points(vertices + WRAP_LOW(entrance_vertex, vertex_count - 1),
                                             vertices + entrance_vertex, &new_entrance_point, line);
            } else {
            ...
            if (vertex_delta < 0) {
                if (vertex_delta != -2)
                    memmove(vertices + entrance_vertex + 2, vertices + exit_vertex,
                            (vertex_count - exit_vertex) * sizeof(flagged_world_point2d));
...
// LP change: make it better able to do long-distance views
void RenderRasterizerClass::xy_clip_flagged_world_points(flagged_world_point2d* p0, flagged_world_point2d* p1,
    ...
    while (numerator <= (int32)0x3fff'ffff && numerator >= (int32)0xc000'0000 && shift_count--) numerator <<= 1;
    if (shift_count > 0)
        denominator >>= shift_count;
    t = numerator;
    if (denominator)
        t /= denominator;
...
// LP change: make it better able to do long-distance views
short RenderRasterizerClass::z_clip_horizontal_polygon(flagged_world_point2d* vertices, short vertex_count,
    ...
    if (vertex_count) {
        ...
        if (exit_vertex != NONE) /* we’ve got clipping to do */
        {
            ...
            /* clip the entrance to the clipped area */
            if (clipped_entrance_vertex) {
                z_clip_flagged_world_points(vertices + WRAP_LOW(entrance_vertex, vertex_count - 1),
                                            vertices + entrance_vertex, height, &new_entrance_point, line);
            } else {
                new_entrance_point = vertices[entrance_vertex];
            }
            new_entrance_point.flags |= flag;
...
// LP change: make it better able to do long-distance views
void RenderRasterizerClass::z_clip_flagged_world_points(flagged_world_point2d* p0, flagged_world_point2d* p1,
                                                        world_distance height, flagged_world_point2d* clipped,
                                                        long_vector2d* line) {
    bool swap                       = (p1->y > p0->y) ? false : ((p0->y == p1->y) ? (p1->x < p0->x) : true);
    flagged_world_point2d* local_p0 = swap ? p1 : p0;
    flagged_world_point2d* local_p1 = swap ? p0 : p1;
...
// LP change: make it better able to do long-distance views
short RenderRasterizerClass::xy_clip_line(flagged_world_point2d* posts, short vertex_count, long_vector2d* line,
                                          uint16 flag) {
#ifdef QUICKDRAW_DEBUG
...
// LP change: make it better able to do long-distance views
short RenderRasterizerClass::xz_clip_vertical_polygon(flagged_world_point3d* vertices, short vertex_count,
                                                      long_vector2d* line, /* i==x, j==z */
                                                      uint16 flag) {
#ifdef QUICKDRAW_DEBUG
...
//	debug_vector(line);
    ...
    if (vertex_count) {
        ...
        do {
            CROSSPROD_TYPE cross_product = CROSSPROD_TYPE(line->i) * vertices[vertex_index].z
                                           - CROSSPROD_TYPE(line->j) * vertices[vertex_index].x;
        ...
        if (exit_vertex != NONE) /* we’ve got clipping to do */
        {
            ...
            /* clip the entrance to the clipped area */
            if (clipped_entrance_vertex) {
                xz_clip_flagged_world_points(vertices + WRAP_LOW(entrance_vertex, vertex_count - 1),
                                             vertices + entrance_vertex, &new_entrance_point, line);
            } else {
                new_entrance_point = vertices[entrance_vertex];
            }
            ...
            if (vertex_delta < 0) {
                if (vertex_delta != -2)
                    memmove(vertices + entrance_vertex + 2, vertices + exit_vertex,
                            (vertex_count - exit_vertex) * sizeof(flagged_world_point3d));
                vertex_delta = vertex_count + vertex_delta;
            } else {
                /* move down by exit_vertex, add new vertices to end */
                assert(vertex_delta);
...
// LP change: make it better able to do long-distance views
void RenderRasterizerClass::xz_clip_flagged_world_points(flagged_world_point3d* p0, flagged_world_point3d* p1,
