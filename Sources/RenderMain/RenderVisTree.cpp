...
inline void INITIALIZE_NODE(node_data* node, short node_polygon_index, uint16 node_flags, node_data* node_parent,
                            node_data** node_reference) {
...
// Inits everything
RenderVisTreeClass::RenderVisTreeClass() : view(NULL), mark_as_explored(false), add_to_automap(true) {
    ...
    while (polygon_queue_size) {
        ...
        for (vertex_index = 0; vertex_index < polygon->vertex_count; ++vertex_index) {
            ...
            if (!TEST_RENDER_FLAG(endpoint_index, _endpoint_has_been_visited)) {
                ...
                transform_overflow_point2d(&endpoint->transformed, (world_point2d*)&view->origin, view->yaw,
                                           &endpoint->flags);
                ...
                if ((view->right_edge.i * _vector.j - view->right_edge.j * _vector.i) <= 0
                    ...
                    cast_render_ray(&_vector, ENDPOINT_IS_TRANSPARENT(endpoint) ? NONE : endpoint_index, &Nodes.front(),
                                    _no_bias);
                }
...
void RenderVisTreeClass::cast_render_ray(long_vector2d* _vector, // world_vector2d *vector,
                                         short endpoint_index, node_data* parent,
                                         short bias) /* _clockwise or _counterclockwise for walking endpoints */
{
    ...
    do {
        ...
        uint16 clip_flags = next_polygon_along_line(&polygon_index, (world_point2d*)&view->origin, _vector,
                                                    &clipping_endpoint_index, &clipping_line_index, bias);
...
void RenderVisTreeClass::initialize_polygon_queue() { polygon_queue_size = 0; }

// LP change: make it better able to do long-distance views
uint16 RenderVisTreeClass::next_polygon_along_line(
        short* polygon_index, world_point2d* origin, /* not necessairly in polygon_index */
    ...
    do {
        ...
        CROSSPROD_TYPE cross_product = CROSSPROD_TYPE(int32(vertex->x) - int32(origin->x)) * _vector->j
                                       - CROSSPROD_TYPE(int32(vertex->y) - int32(origin->y)) * _vector->i;

        //		dprintf("p#%d, e#%d:#%d, SGN(cp)=#%d, state=#%d", *polygon_index, vertex_index,
        //polygon->endpoint_indexes[vertex_index], SGN(cross_product), state);
        if (cross_product < 0) {
            switch (state) {
            ...
            if (state != _looking_for_first_nonzero_vertex) {
                ...
                switch (state) {
                    ...
                    case _looking_counterclockwise_for_left_vertex:
                        ...
                        clip_flags         |= decide_where_vertex_leads(&next_polygon_index, &crossed_line_index,
                                                                        &crossed_side_index, vertex_index, origin, _vector,
                                                                        clip_flags, bias);
...
// LP change: make it better able to do long-distance views
uint16 RenderVisTreeClass::decide_where_vertex_leads(short* polygon_index, short* line_index, short* side_index,
                                                     short endpoint_index_in_polygon_list, world_point2d* origin,
                                                     long_vector2d* _vector, // world_vector2d *vector,
                                                     uint16 clip_flags, short bias) {
    polygon_data* polygon = get_polygon_data(*polygon_index);
        ...
        *polygon_index = polygon->adjacent_polygon_indexes[index];
        ...
        if (*polygon_index != NONE && LINE_IS_TRANSPARENT(line)) {
            ...
            for (index = 0; index < polygon->vertex_count && polygon->endpoint_indexes[index] != endpoint_index;
                 ++index);
            vassert(index != polygon->vertex_count,
                    csprintf(temporary, "endpoint #%d not in polygon #%d", endpoint_index, *polygon_index));
            ...
            cross_product = CROSSPROD_TYPE(int32(vertex->x) - int32(origin->x)) * _vector->j
                            - CROSSPROD_TYPE(int32(vertex->y) - int32(origin->y)) * _vector->i;

            if ((bias == _clockwise_bias && cross_product >= 0)
                || (bias == _counterclockwise_bias && cross_product <= 0)) {
                /* we’re leaving this endpoint, set clip flag in case it’s solid */
                clip_flags |= (bias == _clockwise_bias) ? _clip_left : _clip_right;
            }
        }

        //		dprintf("left endpoint #%d via line #%d to polygon #%d (bias==#%d)", endpoint_index, *line_index,
        //*polygon_index, bias);
    }
    ...
    if (p0.x > 0 && p1.x > 0) {
        ...
        if (data->x1 > data->x0) {
            ...
            if (clip_flags & _clip_down) {
                ...
                /* calculate and clip y0 and y1 (screen y-coordinates of each side of the line) */
                y0 = (p0.x > 0) ? (view->half_screen_height - transformed_z / p0.x + view->dtanpitch)
                                : view->screen_height;
                y1 = (p1.x > 0) ? (view->half_screen_height - transformed_z / p1.x + view->dtanpitch)
                                : view->screen_height;
...
void RenderVisTreeClass::ResetEndpointClips(void) {
    ...
    endpoint_clip_data Dummy;
    Dummy.flags = 0; // Fake initialization to shut up CW
    for (int k = 0; k < NUMBER_OF_INITIAL_ENDPOINT_CLIPS; k++) EndpointClips.push_back(Dummy);
}
...
void RenderVisTreeClass::ResetLineClips(void) {
    ...
    line_clip_data Dummy;
    Dummy.flags = 0; // Fake initialization to shut up CW
    for (int k = 0; k < NUMBER_OF_INITIAL_LINE_CLIPS; k++) LineClips.push_back(Dummy);
}
