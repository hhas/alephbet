...
#include "fades.hpp"
#include "game_wad.hpp"    /* yuck... */
#include "game_window.hpp" /* for draw_interface() */
#include "images.hpp"
...
#include "shell.hpp"
#include "tags.hpp" /* for scenario file type.. */
#include "vbl.hpp"
#include "weapons.hpp"
...
#include "mytm.hpp" // mytm_initialize(), for platform-specific shell_*.h
...
static int char_is_not_filesafe(int c) { return (c != ' ' && !std::isalnum(c)); }

void initialize_application(void) {
#if defined(__WIN32__)
    if (LoadLibraryW(L"exchndl.dll"))
        shell_options.debug = true;
    SDL_setenv("SDL_AUDIODRIVER", "directsound", 0);
#endif

    // Initialize SDL
    int retval = SDL_Init(SDL_INIT_VIDEO | (shell_options.nosound ? 0 : SDL_INIT_AUDIO)
...
#ifdef HAVE_STEAM
    if (!STEAMSHIM_init()) {
        alert_user("You must launch the Steam version of Classic Marathon using the Classic Marathon Launcher.",
                   fatalError);
        exit(1);
    }
#endif
...
const int32 AllPlayableLevels = _single_player_entry_point | _multiplayer_carnage_entry_point
                                | _multiplayer_cooperative_entry_point | _kill_the_man_with_the_ball_entry_point
                                | _king_of_hill_entry_point | _rugby_entry_point | _capture_the_flag_entry_point;
...
void main_event_loop(void) {
    ...
    while ((game_state = get_game_state()) != _quit_game) {
        ...
        switch (game_state) {
            ...
            case _change_level:
                if ((get_fps_target() == 0 && get_keyboard_controller_status()) || Console::instance()->input_active()
                    || cur_time - last_event_poll >= TICKS_BETWEEN_EVENT_POLL) {
                    poll_event      = true;
...
static void handle_game_key(const SDL_Event& event) {
    ...
    if (Console::instance()->input_active()) {
        ...
        if (sc == SDL_SCANCODE_ESCAPE || sc == AO_SCANCODE_JOYSTICK_ESCAPE) // (ZZZ) Quit gesture (now safer)
        {
            ...
            if (game_is_networked) {
#if !defined(DISABLE_NETWORKING)
                Console::instance()->activate_input(InGameChatCallbacks::SendChatMessage,
        ...
        case SDL_WINDOWEVENT:
            switch (event.window.event) {
                case SDL_WINDOWEVENT_FOCUS_LOST:
                    if (get_game_state() == _game_in_progress && get_keyboard_controller_status()
                        && !Movie::instance()->IsRecording() && shell_options.replay_directory.empty()) {
                        //					darken_world_window();
...
//		aoIMG_SavePNG(file.GetPath(), MainScreenSurface(), IMG_COMPRESS_DEFAULT, textp, texts.size());
    ...
    SDL_Surface* t = SDL_CreateRGBSurface(SDL_SWSURFACE, video_w, video_h, 24,
                                          ...
                                          0x0000'00ff, 0x0000'ff00, 0x00ff'0000, 0);
#else
...
bool expand_symbolic_paths_helper(char* dest, const char* src, int maxlen, const char* symbol,
...
bool contract_symbolic_paths_helper(char* dest, const char* src, int maxlen, const char* symbol,
                                    DirectorySpecifier& dir) {
...
char* contract_symbolic_paths(char* dest, const char* src, int maxlen) {
    bool contracted =
            ...
            || // default first in case user installed his game in his local data dir
