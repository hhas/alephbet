...
void reset_intermediate_action_queues() {
    ...
    for (size_t i = 0; i < MAXIMUM_NUMBER_OF_PLAYERS; i++) sMostRecentFlagsForPlayer[i] = 0;
...
void set_prediction_wanted(bool inPrediction) { sPredictionWanted = inPrediction; }
            ...
            if (sSavedPlayerData[i].monster_index != NONE) {
                ...
                if (sSavedPlayerMonsterData[i].object_index != NONE) {
                    ...
                    if (sSavedPlayerObjectData[i].parasitic_object != NONE)
                        sSavedPlayerParasiticObjectData[i]
                                = *get_object_data(sSavedPlayerObjectData[i].parasitic_object);
                }
            }
        }
...
#if COMPARE_MEMORY
// ZZZ: I wrote this function to help catch incomplete state save/restore operations on entering and exiting predictive
// mode It's not currently in use anywhere, but may prove useful sometime?  so I'm including it in my submission.
static void compare_memory(const char* inChunk1, const char* inChunk2, size_t inSize, size_t inIgnoreStart,
                           size_t inIgnoreEnd, const char* inDescription, int inDescriptionNumber) {
        ...
        if (inChunk1[i] != inChunk2[i]) {
            ...
            if (trackingDifferences) {
                if (theDifferenceStart < inIgnoreStart || i >= inIgnoreEnd)
                    logWarning4("%s %d: differences in bytes [%d,%d)", inDescription, inDescriptionNumber,
                                theDifferenceStart, i);
    ...
    if (trackingDifferences) {
        if (theDifferenceStart < inIgnoreStart || inSize >= inIgnoreEnd)
            logWarning4("%s %d: differences in bytes [%d,%d)", inDescription, inDescriptionNumber, theDifferenceStart,
                        inSize);
    }
}
#endif
...
static void exit_predictive_mode() {
    if (sPredictedTicks > 0) {
        for (short i = 0; i < dynamic_world->player_count; i++) {
            ...
            if (sSavedPlayerData[i].monster_index != NONE) {
                assert(get_monster_data(sSavedPlayerData[i].monster_index)->object_index
                       == sSavedPlayerMonsterData[i].object_index);

                *get_monster_data(sSavedPlayerData[i].monster_index) = sSavedPlayerMonsterData[i];

                if (sSavedPlayerMonsterData[i].object_index != NONE) {
                    assert(get_object_data(sSavedPlayerMonsterData[i].object_index)->parasitic_object
                           == sSavedPlayerObjectData[i].parasitic_object);
                    ...
                    *get_object_data(sSavedPlayerMonsterData[i].object_index) = sSavedPlayerObjectData[i];
                    ...
                    deferred_add_object_to_polygon_object_list(sSavedPlayerMonsterData[i].object_index,
                                                               sSavedPlayerObjectNextObject[i]);
        ...
        if (sSavedTickCount != dynamic_world->tick_count)
            logWarning("saved tick count %d != dynamic_world->tick_count %d", sSavedTickCount,
...
static bool overlay_queue_with_queue_into_queue(ActionQueues* inBaseQueues, ActionQueues* inOverlayQueues,
                                                ActionQueues* inOutputQueues) {
    ...
    if (theUpdateResult == kUpdateNormalCompletion && sPredictionWanted) {
        ...
        for (; sPredictedTicks < NetGetUnconfirmedActionFlagsCount(); sPredictedTicks++) {
            ...
            for (short thePlayerIndex = 0; thePlayerIndex < dynamic_world->player_count; thePlayerIndex++) {
                uint32 theFlags = (thePlayerIndex == local_player_index) ? NetGetUnconfirmedActionFlag(sPredictedTicks)
                                                                         : sMostRecentFlagsForPlayer[thePlayerIndex];
...
#if !defined(DISABLE_NETWORKING)
    if (dynamic_world->player_count > 1 && !restoring_saved)
        initialize_net_game();
#endif // !defined(DISABLE_NETWORKING)
        ...
        case _polygon_is_visible_monster_trigger:
            if (player) {
                activate_nearby_monsters(player->monster_index, player->monster_index,
                                         _pass_solid_lines | _activate_deaf_monsters | _use_activation_biases
                                                 | _activation_cannot_be_avoided);
            }
            break;
            ...
            if (player) {
                activate_nearby_monsters(player->monster_index, player->monster_index,
                                         _pass_solid_lines | _activate_deaf_monsters | _activate_invisible_monsters
                                                 | _use_activation_biases | _activation_cannot_be_avoided);
            }
            break;
...
short calculate_classic_level_completion_state(void) {
    ...
    if (static_world->mission_flags & _mission_retrieval) {
        if (unretrieved_items_on_map())
            completion_state = _level_unfinished;
    }

    /* if there are any untoggled repair switches on this level then we’re not there */
    if ((static_world->mission_flags & _mission_repair) || (static_world->mission_flags & _mission_repair_m1)) {
        /* M1 only required last repair switch to be toggled */
        bool only_last_switch
                = (film_profile.m1_buggy_repair_goal && (static_world->mission_flags & _mission_repair_m1));
        if (untoggled_repair_switches_on_level(only_last_switch))
            completion_state = _level_unfinished;
    }
...
void cause_polygon_damage(short polygon_index, short monster_index) {
    ...
    if ((polygon_type == _polygon_is_minor_ouch && !(dynamic_world->tick_count & MINOR_OUCH_FREQUENCY)
