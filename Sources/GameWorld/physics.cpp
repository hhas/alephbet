...
#define FLAGS_WHICH_PREVENT_RECENTERING \
    (_turning | _looking | _sidestepping | _looking_vertically | _look_dont_turn | _sidestep_dont_turn)
...
/*static*/ void instantiate_physics_variables(struct physics_constants* constants, struct physics_variables* variables,
                                              short player_index, bool first_time, bool take_action);
static void physics_update(struct physics_constants* constants, struct physics_variables* variables,
                           struct player_data* player, uint32 action_flags);
...
void initialize_player_physics_variables(short player_index) {
    ...
    variables->old_flags = variables->flags
            = 0; /* not recentering, not above ground, not below ground (i.e., on floor) */
        ...
        if (saved_point_iterations == 1) {
            ...
            *facing                         = object->facing;
            ...
            if (p.x != q->x || p.y != q->y || p.z != q->z || *facing != object->facing && !saved_divergence_warning) {
                dprintf("divergence @ tick %d: (%d,%d,%d,%d)!=(%d,%d,%d,%d)", saved_point_count, q->x, q->y, q->z,
                        *facing, p.x, p.y, p.z, object->facing);
...
void accelerate_player(short monster_index, world_distance vertical_velocity, angle direction,
    ...
    variables->external_velocity.k
            = PIN(variables->external_velocity.k, -constants->terminal_velocity, constants->terminal_velocity);

    if (get_monster_definition_external(_monster_marine)->flags & _monster_can_grenade_climb) {
        variables->external_velocity.i
                = (cosine_table[direction] * velocity) >> (TRIG_SHIFT + WORLD_FRACTIONAL_BITS - FIXED_FRACTIONAL_BITS);
        variables->external_velocity.j
                = (sine_table[direction] * velocity) >> (TRIG_SHIFT + WORLD_FRACTIONAL_BITS - FIXED_FRACTIONAL_BITS);
    } else {
        variables->external_velocity.i
                += (cosine_table[direction] * velocity) >> (TRIG_SHIFT + WORLD_FRACTIONAL_BITS - FIXED_FRACTIONAL_BITS);
        variables->external_velocity.j
...
_fixed get_player_forward_velocity_scale(short player_index) {
    ...
    return INTEGER_TO_FIXED(((dx * cosine_table[FIXED_INTEGERAL_PART(variables->direction)]
                              + dy * sine_table[FIXED_INTEGERAL_PART(variables->direction)])
                             >> TRIG_SHIFT))
           / constants->maximum_forward_velocity;
}

fixed_yaw_pitch virtual_aim_delta() { return vir_aim_delta; }

void resync_virtual_aim() { vir_aim_delta = {0, 0}; }
...
uint32 process_aim_input(uint32 action_flags, fixed_yaw_pitch delta) {
    ...
    auto round = [classic_precision](fixed_angle theta) -> angle {
        return classic_precision ? SGN(theta) * std::max<angle>(1, std::abs(theta / FIXED_ONE))
    ...
    if (!(action_flags & _override_absolute_pitch) && !explicitly_recentering) {
        ...
        if (payload || local_phys.vertical_angular_velocity || dont_auto_recenter())
            action_flags
                    = SET_ABSOLUTE_PITCH(action_flags, encode(payload, ABSOLUTE_PITCH_BITS)) | _absolute_pitch_mode;
...
/*static*/ struct physics_constants* get_physics_constants_for_model(short physics_model, uint32 action_flags) {
    ...
    switch (physics_model) {
        case _editor_model:
        case _earth_gravity_model:
            constants = physics_models + ((action_flags & _run_dont_walk) ? _model_game_running : _model_game_walking);
            break;
...
/*static*/ void instantiate_physics_variables(struct physics_constants* constants, struct physics_variables* variables,
    ...
    if (take_action && !first_time && player->last_supporting_polygon_index != player->supporting_polygon_index)
        changed_polygon(player->last_supporting_polygon_index, player->supporting_polygon_index, player_index);
    player->last_supporting_polygon_index = first_time ? NONE : player->supporting_polygon_index;
    clipped = keep_line_segment_out_of_walls(legs->polygon, &legs->location, &new_location, WORLD_ONE / 3,
                                             FIXED_TO_WORLD(variables->actual_height), &adjusted_floor_height,
                                             &adjusted_ceiling_height, &player->supporting_polygon_index);
    if (PLAYER_IS_DEAD(player))
        new_location.z -= FIXED_TO_WORLD(DROP_DEAD_HEIGHT);
    ...
    if (translate_map_object(monster->object_index, &new_location, NONE)) {
        if (old_polygon_index == legs->polygon)
            clipped = true; /* oops; trans_map_obj destructively changed our position */
        if (take_action)
    ...
    step_height = (constants->step_amplitude
                   * sine_table[variables->step_phase >> (FIXED_FRACTIONAL_BITS - ANGULAR_BITS + 1)])
                  >> TRIG_SHIFT;
    step_height = (step_height * variables->step_amplitude) >> FIXED_FRACTIONAL_BITS;

    player->camera_location = new_location;
    if (PLAYER_IS_DEAD(player) && new_location.z < adjusted_floor_height)
        new_location.z = adjusted_floor_height;
...
static void physics_update(struct physics_constants* constants, struct physics_variables* variables,
    ...
    if (PLAYER_IS_DEAD(player)) /* dead players immediately loose all bodily control */
    {
        ...
        dot_product = ((((variables->velocity * cosine) >> TRIG_SHIFT) + variables->external_velocity.i) * cosine
                       + (((variables->velocity * sine) >> TRIG_SHIFT) + variables->external_velocity.j) * sine)
                      >> TRIG_SHIFT;

        if (dot_product > 0 && dot_product < (constants->maximum_forward_velocity >> 4))
            dot_product = 0;
    ...
    if ((action_flags & _turning) && (action_flags & _sidestep_dont_turn) && !(action_flags & _absolute_yaw_mode)) {
        if (action_flags & _turning_left)
            action_flags |= _sidestepping_left;
        if (action_flags & _turning_right)
    ...
    if ((action_flags & _moving) && (action_flags & _look_dont_turn) && !(action_flags & _absolute_position_mode)) {
        if (action_flags & _moving_forward)
            action_flags |= _looking_up;
        if (action_flags & _moving_backward)
            action_flags |= _looking_down;
    ...
    if (action_flags & _absolute_yaw_mode) {
        variables->angular_velocity = (GET_ABSOLUTE_YAW(action_flags) - MAXIMUM_ABSOLUTE_YAW / 2)
                                      << (FIXED_FRACTIONAL_BITS); // !!!!!!!!!!
    } else {
        if (variables->angular_velocity < -constants->maximum_angular_velocity
            || variables->angular_velocity > constants->maximum_angular_velocity)
            action_flags &= ~_turning;
        switch (action_flags & _turning) {
            case _turning_left:
                delta = variables->angular_velocity > 0
                                ? constants->angular_acceleration + constants->angular_deceleration
                                : constants->angular_acceleration;
                variables->angular_velocity
                        = FLOOR(variables->angular_velocity - delta, -constants->maximum_angular_velocity);
                break;
            case _turning_right:
                delta = variables->angular_velocity < 0
                                ? constants->angular_acceleration + constants->angular_deceleration
                                : constants->angular_acceleration;
                variables->angular_velocity
                        = CEILING(variables->angular_velocity + delta, constants->maximum_angular_velocity);
                break;

            default: /* slow down */
                variables->angular_velocity
                        = (variables->angular_velocity >= 0)
        ...
        switch (action_flags & _looking) {
            case _looking_left:
                variables->head_direction = FLOOR(variables->head_direction - constants->fast_angular_velocity,
                                                  -constants->fast_angular_maximum);
                break;
            case _looking_right:
                variables->head_direction = CEILING(variables->head_direction + constants->fast_angular_velocity,
                                                    constants->fast_angular_maximum);
                break;
    ...
    if (action_flags & _absolute_pitch_mode) {
        variables->vertical_angular_velocity = (GET_ABSOLUTE_PITCH(action_flags) - MAXIMUM_ABSOLUTE_PITCH / 2)
                                               << (FIXED_FRACTIONAL_BITS);
    } else {
        /* if the user touched the recenter key, set the recenter flag and override all up/down
            keypresses with our own */
        if (action_flags & _looking_center)
            variables->flags |= _RECENTERING_BIT;
        ...
        if (!(action_flags & FLAGS_WHICH_PREVENT_RECENTERING)) /* can’t recenter if any of these are true */
        {
            if (((action_flags & _moving_forward) && (variables->velocity == constants->maximum_forward_velocity))
                || ((action_flags & _moving_backward)
                    && (variables->velocity == -constants->maximum_backward_velocity))) {
        ...
        switch (action_flags & _looking_vertically) {
            case _looking_down:
                delta = variables->vertical_angular_velocity > 0
                                ? constants->angular_acceleration + constants->angular_deceleration
                                : constants->angular_acceleration;
                variables->vertical_angular_velocity
                        = FLOOR(variables->vertical_angular_velocity - delta,
                                PLAYER_IS_DEAD(player) ? -(constants->maximum_angular_velocity >> 3)
                                                       : -constants->maximum_angular_velocity);
                break;
            case _looking_up:
                delta = variables->vertical_angular_velocity < 0
                                ? constants->angular_acceleration + constants->angular_deceleration
                                : constants->angular_acceleration;
                variables->vertical_angular_velocity
                        = CEILING(variables->vertical_angular_velocity + delta,
                                  PLAYER_IS_DEAD(player) ? (constants->maximum_angular_velocity >> 3)
                                                         : constants->maximum_angular_velocity);
                break;
    ...
    if (delta_z <= 0 || (variables->flags & _HEAD_BELOW_MEDIA_BIT)) {
        if (action_flags & _absolute_position_mode) {
            ...
            if (encoded_delta < 0) {
                variables->velocity
                        = (encoded_delta * constants->maximum_backward_velocity) >> (ABSOLUTE_POSITION_BITS - 1);
            } else {
                variables->velocity
                        = (encoded_delta * constants->maximum_forward_velocity) >> (ABSOLUTE_POSITION_BITS - 1);
            }
        } else {
            /* handle moving forward or backward; if we’ve exceeded our maximum velocity lock out user actions
                until we return to a legal range */
            if (variables->velocity < -constants->maximum_backward_velocity
                || variables->velocity > constants->maximum_forward_velocity)
                action_flags &= ~_moving;
            switch (action_flags & _moving) {
                case _moving_forward:
                    delta               = variables->velocity < 0 ? constants->deceleration + constants->acceleration
        ...
        switch (action_flags & _sidestepping) {
            case _sidestepping_left:
                delta = variables->perpendicular_velocity > 0 ? constants->acceleration + constants->deceleration
                                                              : constants->acceleration;
                variables->perpendicular_velocity
                        = FLOOR(variables->perpendicular_velocity - delta, -constants->maximum_perpendicular_velocity);
                break;
            case _sidestepping_right:
                delta = variables->perpendicular_velocity < 0 ? constants->acceleration + constants->deceleration
                                                              : constants->acceleration;
                variables->perpendicular_velocity
                        = CEILING(variables->perpendicular_velocity + delta, constants->maximum_perpendicular_velocity);
                break;
    ...
    if (delta_z < 0) {
        variables->external_velocity.k = CEILING(variables->external_velocity.k + constants->climbing_acceleration,
    ...
    if (delta_z > 0) {
        ...
        if (static_world->environment_flags & _environment_low_gravity)
            gravity >>= 1;
        if (variables->flags & _FEET_BELOW_MEDIA_BIT)
            gravity >>= 1, terminal_velocity >>= 1;

        variables->external_velocity.k = FLOOR(variables->external_velocity.k - gravity, -terminal_velocity);
    }

    if ((action_flags & _swim) && (variables->flags & _HEAD_BELOW_MEDIA_BIT)
    ...
    if ((variables->flags & _RECENTERING_BIT) && !(action_flags & _absolute_pitch_mode)) {
        if ((variables->elevation <= 0 && (action_flags & _looking_down))
            || (variables->elevation >= 0 && (action_flags & _looking_up))) {
    ...
    variables->old_flags = variables->flags;
    if (new_position.z < variables->floor_height)
        variables->flags |= _BELOW_GROUND_BIT;
    else
        variables->flags &= (uint16)~_BELOW_GROUND_BIT;
    if (new_position.z > variables->floor_height)
    ...
    if (variables->external_velocity.k > 0 && (variables->old_flags & _BELOW_GROUND_BIT)
        && !(variables->flags & _BELOW_GROUND_BIT)) {
    ...
    if (variables->external_velocity.k > 0 && new_position.z + variables->actual_height >= variables->ceiling_height) {
        variables->external_velocity.k /= -COEFFICIENT_OF_ABSORBTION,
                ...
                = variables->ceiling_height
                  - variables
                            ->actual_height; // &&variables->position.z+variables->actual_height<variables->ceiling_height
    }
    if (variables->external_velocity.k < 0 && !(variables->old_flags & _BELOW_GROUND_BIT)
        && !(variables->flags & _ABOVE_GROUND_BIT)) {
        variables->external_velocity.k /= -COEFFICIENT_OF_ABSORBTION;
    ...
    variables->external_angular_velocity
            = (variables->external_angular_velocity >= 0)
    ...
    if (constants->maximum_forward_velocity)
        variables->step_amplitude
                = (MAX(std::abs(variables->velocity), std::abs(variables->perpendicular_velocity)) * FIXED_ONE)
                  / constants->maximum_forward_velocity;
        ...
        if (variables->velocity || variables->perpendicular_velocity) {
            ...
            if (variables->step_phase) {
                if (variables->step_phase > FIXED_ONE_HALF) {
                    ...
                    if ((variables->step_phase -= constants->step_delta) < 0)
                        variables->step_phase = 0;
                }
            }
        }
    ...
    if (delta_z >= (PLAYER_IS_DEAD(player) ? (AIRBORNE_HEIGHT + DROP_DEAD_HEIGHT) : AIRBORNE_HEIGHT)) {
        ...
        if (variables->angular_velocity || variables->velocity || variables->perpendicular_velocity) {
            ...
            variables->action = (variables->external_velocity.i || variables->external_velocity.j
                                 || variables->external_velocity.k)
                                        ? _player_sliding
                                        : _player_stationary;
        }
...
void init_physics_constants() { memcpy(physics_models, original_physics_models, sizeof(physics_models)); }
