...
class ObjectDataPool {
  public:
    ...
    ObjectDataPool() : pool_{get_dynamic_limit(_dynamic_limit_ephemera)}, first_unused_{NONE} {}
...
void allocate_ephemera_storage(int max_ephemera) { ephemera_pool.resize(max_ephemera); }
...
object_data* get_ephemera_data(int16_t ephemera_index) { return &ephemera_pool.get(ephemera_index); }
...
void remove_ephemera_from_polygon(int16_t ephemera_index) {
    ...
    while (*p != ephemera_index) { p = &ephemera_pool.get(*p).next_object; }
