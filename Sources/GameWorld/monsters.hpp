...
#define SET_MONSTER_NEEDS_PATH_STATUS(m, v) \
    ((void)((v) ? ((m)->flags |= (uint16)0x4000) : ((m)->flags &= (uint16)~0x4000)))
...
#define SET_MONSTER_BERSERK_STATUS(m, v) \
    ((void)((v) ? ((m)->flags |= (uint16)0x0400) : ((m)->flags &= (uint16)~0x0400)))
...
#define MONSTER_IS_DYING(m)                                                         \
    ((m)->action == _monster_is_dying_hard || (m)->action == _monster_is_dying_soft \
     || (m)->action == _monster_is_dying_flaming)
...
struct monster_data /* 64 bytes */
{
    ...
    uint16 flags;   /* [slot_used.1] [need_path.1] [recovering_from_hit.1] [active.1] [idle.1] [berserk.1]
                       [target_damage.1] [unused.6] [never_activated.1] [demoted.1] [promoted.1] */
    ...
    short changes_until_lock_lost; /* number of times more the target can change polygons until _losing_lock becomes
                                      _lost_lock */
...
void damage_monster(short monster_index, short aggressor_index, short aggressor_type, world_point3d* epicenter,
                    struct damage_definition* damage, short projectile_index);
