    ...
    if (lowest_cost_node_index != NONE) {
        ...
        for (i = 0; i < polygon->vertex_count; ++i) {
            ...
            if (destination_polygon_index != NONE
                ...
                int32 cost
                        = cost_proc
                                  ? cost_proc(node->polygon_index, polygon->line_indexes[i], destination_polygon_index,
                                              (flood_mode == _flagged_breadth_first) ? &new_user_flags : caller_data)
                                  : polygon->area;

                /* polygons with zero or negative costs are not added to the node list */
                if (cost > 0)
                    add_node(lowest_cost_node_index, destination_polygon_index, node->depth + 1, lowest_cost + cost,
                             new_user_flags);
            }
        }
        ...
        if (flood_mode == _flagged_breadth_first)
            *((int32*)caller_data) = node->user_flags;
    } else {
        polygon_index = NONE;
    }
...
static void add_node(short parent_node_index, short polygon_index, short depth, int32 cost, int32 user_flags) {
    if (node_count < MAXIMUM_FLOOD_NODES) {
        ...
        if (node) {
            ...
            //			dprintf("added polygon #%d to node #%d (nodes=%p,visited=%p)", polygon_index, node_index, nodes,
            //visited_polygons);
        }
    }
}
