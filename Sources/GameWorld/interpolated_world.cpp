...
void init_interpolated_world() {
    ...
    previous_tick_objects.assign(current_tick_objects.begin(), current_tick_objects.end());
    ...
    previous_tick_polygons.assign(current_tick_polygons.begin(), current_tick_polygons.end());

    current_tick_sides.resize(MAXIMUM_SIDES_PER_MAP);
    for (auto i = 0; i < MAXIMUM_SIDES_PER_MAP; ++i) { current_tick_sides[i].y0 = map_sides[i].primary_texture.y0; }
    previous_tick_sides.assign(current_tick_sides.begin(), current_tick_sides.end());
    ...
    previous_tick_lines.assign(current_tick_lines.begin(), current_tick_lines.end());
    ...
    previous_tick_ephemera.assign(current_tick_ephemera.begin(), current_tick_ephemera.end());

    current_tick_polygon_ephemera.resize(dynamic_world->polygon_count);
    for (auto i = 0; i < dynamic_world->polygon_count; ++i) { current_tick_polygon_ephemera[i] = polygon_ephemera[i]; }
    ...
    while (get_weapon_display_information(&count, &data)) { current_tick_weapon_display.push_back(data); }
    previous_tick_weapon_display.assign(current_tick_weapon_display.begin(), current_tick_weapon_display.end());

    contrail_tracking.resize(MAXIMUM_OBJECTS_PER_MAP);
    for (auto i = 0; i < contrail_tracking.size(); ++i) { contrail_tracking[i].projectile_index = NONE; }
...
void enter_interpolated_world() {
    ...
    if (current_tick_sides.size() != MAXIMUM_SIDES_PER_MAP) {
        for (auto i = current_tick_sides.size(); i < MAXIMUM_SIDES_PER_MAP; ++i) {
...
void exit_interpolated_world() {
    ...
    for (auto i = 0; i < MAXIMUM_SIDES_PER_MAP; ++i) { map_sides[i].primary_texture.y0 = current_tick_sides[i].y0; }
...
static int16_t lerp(int16_t a, int16_t b, float t) { return static_cast<int16_t>(std::round(a + (b - a) * t)); }

static _fixed lerp(_fixed a, _fixed b, float t) { return static_cast<_fixed>(std::round(a + (b - a) * t)); }
...
static bool should_interpolate(world_point3d& prev, world_point3d& next,
    ...
    return world_is_interpolated
           && guess_distance2d(reinterpret_cast<world_point2d*>(&prev), reinterpret_cast<world_point2d*>(&next))
                      <= speed_limit;
}
...
void update_interpolated_world(float heartbeat_fraction) {
    ...
    for (auto i = 0; i < dynamic_world->polygon_count; ++i) {
        ...
        for (auto j = 0; j < polygon.vertex_count; ++j) {
            ...
            if (side_index != NONE && current_tick_sides[side_index].y0 != previous_tick_sides[side_index].y0) {
                map_sides[side_index].primary_texture.y0 = lerp(previous_tick_sides[side_index].y0,
    ...
    for (auto i = 0; i < MAXIMUM_LINES_PER_MAP; ++i) {
        ...
        if ((line->clockwise_polygon_owner == NONE
             || !TEST_RENDER_FLAG(line->clockwise_polygon_owner, _polygon_is_visible))
            && (line->counterclockwise_polygon_owner == NONE
                || !TEST_RENDER_FLAG(line->counterclockwise_polygon_owner, _polygon_is_visible))) {
            continue;
        }
        ...
        if (prev.highest_adjacent_floor != next.highest_adjacent_floor) {
            line->highest_adjacent_floor
                    = lerp(prev.highest_adjacent_floor, next.highest_adjacent_floor, heartbeat_fraction);
        }

        if (prev.lowest_adjacent_ceiling != next.lowest_adjacent_ceiling) {
            line->lowest_adjacent_ceiling
                    = lerp(prev.lowest_adjacent_ceiling, next.lowest_adjacent_ceiling, heartbeat_fraction);
        }
    }
    ...
    for (auto i = 0; i < MAXIMUM_OBJECTS_PER_MAP; ++i) {
        ...
        if (object->polygon != next->polygon) {
            auto polygon_index
                    = find_new_object_polygon(reinterpret_cast<world_point2d*>(&object->location),
                                              reinterpret_cast<world_point2d*>(&next->location), object->polygon);
    ...
    for (auto i = 0; i < get_dynamic_limit(_dynamic_limit_ephemera); ++i) {
        ...
        auto ephemera        = get_ephemera_data(i);
        ephemera->location.x = lerp(prev->location.x, next->location.x, heartbeat_fraction);

        ephemera->location.y = lerp(prev->location.y, next->location.y, heartbeat_fraction);

        ephemera->location.z = lerp(prev->location.z, next->location.z, heartbeat_fraction);

        if (ephemera->polygon != next->polygon) {
            auto polygon_index
                    = find_new_object_polygon(reinterpret_cast<world_point2d*>(&ephemera->location),
                                              reinterpret_cast<world_point2d*>(&next->location), ephemera->polygon);
...
void interpolate_world_view(float heartbeat_fraction) {
    ...
    if (!world_is_interpolated || heartbeat_fraction > 1.f || prev->origin_polygon_index == NONE
    ...
    view->maximum_depth_intensity
            = lerp(prev->maximum_depth_intensity, next->maximum_depth_intensity, heartbeat_fraction);
    ...
    if (prev->origin_polygon_index != next->origin_polygon_index) {
        auto polygon_index
                = find_new_object_polygon(reinterpret_cast<world_point2d*>(&prev->origin),
...
float get_heartbeat_fraction() {
    ...
    if (get_fps_target() == 30) {
        ...
        auto fraction = static_cast<float>((machine_tick_count() - start_machine_tick) * TICKS_PER_SECOND + 1)
...
static void interpolate_weapon_display_information(short index, weapon_display_information* data) {
    ...
    data->vertical_position = lerp(prev->vertical_position, next->vertical_position, heartbeat_fraction);

    data->horizontal_position = lerp(prev->horizontal_position, next->horizontal_position, heartbeat_fraction);
}
