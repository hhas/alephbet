...
static short Environments[NUMBER_OF_ENVIRONMENTS][NUMBER_OF_ENV_COLLECTIONS] = {
        ...
        {_collection_walls4, _collection_scenery4, NONE, NONE, NONE, NONE,
         NONE                                                                  }, // Jjaro (originally to be Pathways or Marathon)
...
short* get_map_indexes(const short index, const short count) {
    assert(map_indexes);
    short* map_index = GetMemberWithBounds(map_indexes, static_cast<unsigned short>(index),
                                           static_cast<unsigned short>(dynamic_world->map_index_count) - count + 1);
...
ambient_sound_image_data* get_ambient_sound_image_data(const short ambient_sound_image_index) {
    return GetMemberWithBounds(ambient_sound_images, ambient_sound_image_index,
                               dynamic_world->ambient_sound_image_count);
}
...
bool valid_point2d(world_point2d* p) { return world_point_to_polygon_index(p) == NONE ? false : true; }
...
/* can be NONE if there is no direct route between the two points or the child point is not in any
    polygon */
short find_new_object_polygon(world_point2d* parent_location, world_point2d* child_location,
...
short attach_parasitic_object(short host_index, shape_descriptor shape, angle facing) {
    ...
    for (host_object = get_object_data(host_index); host_object->parasitic_object != NONE;
...
void perform_deferred_polygon_object_list_manipulations() {
    ...
    while (!sDeferredObjectListInsertions.empty()) {
        ...
        for (DeferredObjectListInsertionList::iterator i = sDeferredObjectListInsertions.begin();
             i != sDeferredObjectListInsertions.end();) {
...
bool translate_map_object(short object_index, world_point3d* new_location, short new_polygon_index) {
    ...
    if (new_polygon_index == NONE) {
        ...
        do {
            line_index = find_line_crossed_leaving_polygon(new_polygon_index, (world_point2d*)&object->location,
...
void get_object_shape_and_transfer_mode(world_point3d* camera_location, object_data* object,
    ...
    theta = arctangent(int32(object->location.x) - int32(camera_location->x),
                       int32(object->location.y) - int32(camera_location->y))
            - object->facing;
    switch (animation->number_of_views) {
        ...
        case _animated4:
            switch (FACING4(theta)) {
                case 0:
                    view = 3;
                    break; /* 90° (facing left) */
                case 1:
                    view = 0;
                    break; /* 0° (facing forward) */
                case 2:
                    view = 1;
                    break; /* -90° (facing right) */
                case 3:
                    view = 2;
                    break; /* ±180° (facing away) */
                default:
                    data->collection_code = NONE; // Deliberate bad value
                    return;
            }
        ...
        case _animated8:
            switch (FACING8(theta)) {
                case 0:
                    view = 3;
                    break; /* 135° (facing left) */
                case 1:
                    view = 2;
                    break; /* 90° (facing left) */
                case 2:
                    view = 1;
                    break; /* 45° (facing left) */
                case 3:
                    view = 0;
                    break; /* 0° (facing forward) */
                case 4:
                    view = 7;
                    break; /* -45° (facing right) */
                case 5:
                    view = 6;
                    break; /* -90° (facing right) */
                case 6:
                    view = 5;
                    break; /* -135° (facing right) */
                case 7:
                    view = 4;
                    break; /* ±180° (facing away) */
                default:
    ...
    if (animation->transfer_mode == _xfer_normal && object->transfer_mode != NONE) {
        ...
        data->transfer_phase
                = object->transfer_period ? INTEGER_TO_FIXED(object->transfer_phase) / object->transfer_period : 0;

        //		if (object->transfer_mode==_xfer_fold_out) dprintf("#%d/#%d==%x", object->transfer_phase,
        //object->transfer_period, data->transfer_phase);
    } else {
        data->transfer_mode  = animation->transfer_mode;
        data->transfer_phase = (animation->transfer_mode != _xfer_normal && animation->transfer_mode_period)
                                       ? INTEGER_TO_FIXED(object->transfer_phase) / animation->transfer_mode_period
                                       : 0;
    }
}
...
void animate_object(short object_index) { animate_object(get_object_data(object_index), object_index); }

/* no longer called by RENDER.C; must be called by monster, projectile or effect controller;
    now assumes ∂t==1 tick */
void animate_object(object_data* object, int16_t sound_id) {
    struct shape_animation_data* animation;
    ...
    if (!OBJECT_IS_INVISIBLE(object)) /* invisible objects don’t have valid .shape fields */
    {
        ...
        if (animation->frames_per_view >= 1 && animation->number_of_views != _unanimated) {
            ...
            if (sound_id != NONE && !frame && (!phase || phase >= animation->ticks_per_frame))
                play_object_sound(sound_id, animation->first_frame_sound);
            ...
            if ((phase += 1) >= animation->ticks_per_frame) {
                ...
                if (!film_profile.keyframe_fix) {
                    ...
                    if (frame == animation->key_frame) {
                        ...
                        if (animation->key_frame_sound != NONE && sound_id != NONE)
                            play_object_sound(sound_id, animation->key_frame_sound);
                    }
                    if (frame >= animation->frames_per_view) {
                        frame           = animation->loop_frame;
                        animation_type |= _obj_last_frame_animated;
                        if (animation->last_frame_sound != NONE && sound_id != NONE)
                            play_object_sound(sound_id, animation->last_frame_sound);
                    }
                } else {
        ...
        {
            short period = (animation->transfer_mode == _xfer_normal && object->transfer_mode != NONE)
                                   ? object->transfer_period
                                   : animation->transfer_mode_period;
    ...
    if (object->parasitic_object != NONE)
        animate_object(object->parasitic_object);
}
...
short world_point_to_polygon_index(world_point2d* location) {
    ...
    for (polygon_index = 0, polygon = map_polygons; polygon_index < dynamic_world->polygon_count;
         ++polygon_index, ++polygon) {
        if (!POLYGON_IS_DETACHED(polygon)) {
            if (point_in_polygon(polygon_index, location))
                break;
        }
    }
...
static short find_flooding_polygon_helper(short parent, short polygon_index) {
    ...
    for (auto i = 0; i < polygon->vertex_count; ++i) {
        ...
        if (adjacent_index != NONE && adjacent_index != parent) {
            ...
            if (adjacent->type == _polygon_is_major_ouch || adjacent->type == _polygon_is_minor_ouch) {
...
short find_flooding_polygon(short polygon_index) { return find_flooding_polygon_helper(NONE, polygon_index); }
...
bool line_is_landscaped(short polygon_index, short line_index, world_distance z) {
    ...
    if (side_index != NONE) {
        ...
        switch (side->type) {
            ...
            case _high_side:
                landscaped = z > line->lowest_adjacent_ceiling ? side->primary_transfer_mode == _xfer_landscape
                                                               : side->transparent_transfer_mode == _xfer_landscape;
                break;
            case _low_side:
                landscaped = z < line->highest_adjacent_floor ? side->primary_transfer_mode == _xfer_landscape
                                                              : side->transparent_transfer_mode == _xfer_landscape;
                break;
...
short find_line_crossed_leaving_polygon(short polygon_index,
                                        ...
                                        world_point2d* p1) /* destination (not necessairly in polygon_index) */
{
    ...
    for (i = 0; i < polygon->vertex_count; ++i) {
        ...
        world_point2d* e1
                = &get_endpoint_data(polygon->endpoint_indexes[i == polygon->vertex_count - 1 ? 0 : i + 1])->vertex;
...
_fixed find_line_intersection(world_point2d* e0, world_point2d* e1, world_point3d* p0, world_point3d* p1,
...
_fixed find_floor_or_ceiling_intersection(world_distance h, world_point3d* p0, world_point3d* p1,
                                          world_point3d* intersection) {
    _fixed t;
...
bool keep_line_segment_out_of_walls(
        ...
        world_point3d* p0, world_point3d* p1,
        ...
        world_distance* adjusted_floor_height, world_distance* adjusted_ceiling_height,
        short* supporting_polygon_index) {
    struct polygon_data* polygon = get_polygon_data(polygon_index);
    short* indexes               = get_map_indexes(polygon->first_exclusion_zone_index,
                                                   polygon->line_exclusion_zone_count + polygon->point_exclusion_zone_count);
    ...
    //	if (polygon_index==23) dprintf("#%d lines, #%d endpoints at %p", polygon->line_exclusion_zone_count,
    //polygon->point_exclusion_zone_count, indexes);
    ...
    *adjusted_ceiling_height  = polygon->ceiling_height;
    do {
        for (i = 0; i < polygon->line_exclusion_zone_count && state != _aborted; ++i) {
            ...
            short side_index       = signed_line_index < 0 ? line->counterclockwise_polygon_side_index
            ...
            if (side_index != NONE) {
                ...
                if ((p1->x - zone->e0.x) * (zone->e1.y - zone->e0.y) - (p1->y - zone->e0.y) * (zone->e1.x - zone->e0.x)
                            > 0
                    && (p1->x - zone->e2.x) * (zone->e0.y - zone->e2.y)
                                       - (p1->y - zone->e2.y) * (zone->e0.x - zone->e2.x)
                               > 0
                    && (p1->x - zone->e1.x) * (zone->e3.y - zone->e1.y)
                                       - (p1->y - zone->e1.y) * (zone->e3.x - zone->e1.x)
                               > 0) {
                    short adjacent_polygon_index = signed_line_index < 0 ? line->clockwise_polygon_owner
                                                                         : line->counterclockwise_polygon_owner;
                    struct polygon_data* adjacent_polygon
                            = adjacent_polygon_index == NONE ? NULL : get_polygon_data(adjacent_polygon_index);
                    world_distance lowest_ceiling;
                    world_distance highest_floor;

                    if (adjacent_polygon) {
                        lowest_ceiling = adjacent_polygon->ceiling_height < polygon->ceiling_height
                                                 ? adjacent_polygon->ceiling_height
                                                 : polygon->ceiling_height;
                        highest_floor  = adjacent_polygon->floor_height > polygon->floor_height
                                                 ? adjacent_polygon->floor_height
                    ...
                    if (LINE_IS_SOLID(line) || adjacent_polygon == NULL
                        ...
                        //	if (unsigned_line_index==104) dprintf("inside solid line #%d (%p) in polygon #%d",
                        //unsigned_line_index, line, polygon_index);
    ...
    if (state != _aborted) {
        for (i = 0; i < polygon->point_exclusion_zone_count; ++i) {
            ...
            //				case 34:
            //				case 35:
            //					dprintf("endpoint#%d is %d away", indexes[polygon->line_exclusion_zone_count+i],
            //distance_squared);
            //			}
            ...
            if (distance_squared < MINIMUM_SEPARATION_FROM_WALL * MINIMUM_SEPARATION_FROM_WALL) {
                if (endpoint->highest_adjacent_floor_height - p1->z > maximum_delta_height
                    || endpoint->lowest_adjacent_ceiling_height - p1->z < height || ENDPOINT_IS_SOLID(endpoint)) {
                    closest_point_on_circle(&endpoint->vertex, MINIMUM_SEPARATION_FROM_WALL, (world_point2d*)p1,
                                            (world_point2d*)p1);
                    clipped = true;
                } else {
...
void ray_to_line_segment(world_point2d* p0, world_point2d* p1, angle theta, world_distance d) {
    ...
    if (x < INT16_MIN)
        x = INT16_MIN, y = (int32)p0->y + (dy * (INT16_MIN - p0->x)) / dx;
    if (x > INT16_MAX)
        x = INT16_MAX, y = (int32)p0->y + (dy * (INT16_MAX - p0->x)) / dx;
    if (y < INT16_MIN)
        y = INT16_MIN, x = (int32)p0->x + (dx * (INT16_MIN - p0->y)) / dy;
    if (y > INT16_MAX)
        y = INT16_MAX, x = (int32)p0->x + (dx * (INT16_MAX - p0->y)) / dy;
...
struct map_annotation* get_next_map_annotation(short* count) {
    ...
    if (*count < dynamic_world->default_annotation_count)
        annotation = map_annotations + (*count)++;
...
void recalculate_map_counts(void) {
    ...
    for (count = MAXIMUM_OBJECTS_PER_MAP, object = objects + MAXIMUM_OBJECTS_PER_MAP - 1;
         count > 0 && (!SLOT_IS_USED(object)); --count, --object);
    ...
    for (count = MAXIMUM_MONSTERS_PER_MAP, monster = monsters + MAXIMUM_MONSTERS_PER_MAP - 1;
         count > 0 && (!SLOT_IS_USED(monster)); --count, --monster);
    ...
    for (count = MAXIMUM_PROJECTILES_PER_MAP, projectile = projectiles + MAXIMUM_PROJECTILES_PER_MAP - 1;
         count > 0 && (!SLOT_IS_USED(projectile)); --count, --projectile);
    ...
    for (count = MAXIMUM_EFFECTS_PER_MAP, effect = effects + MAXIMUM_EFFECTS_PER_MAP - 1;
         count > 0 && (!SLOT_IS_USED(effect)); --count, --effect);
    dynamic_world->effect_count = static_cast<int16>(count);

    for (count = MAXIMUM_LIGHTS_PER_MAP, light = lights + MAXIMUM_LIGHTS_PER_MAP - 1;
         count > 0 && (!SLOT_IS_USED(light)); --count, --light);
    dynamic_world->light_count = static_cast<int16>(count);
}

bool change_polygon_height(short polygon_index, world_distance new_floor_height, world_distance new_ceiling_height,
            ...
            if (OBJECT_IS_VISIBLE(object)) {
                switch (GET_OBJECT_OWNER(object)) {
                    case _object_is_monster:
                        adjust_monster_for_polygon_height_change(object->permutation, polygon_index, new_floor_height,
                                                                 new_ceiling_height);
                        break;
...
bool line_is_obstructed(short polygon_index1, world_point2d* p1, short polygon_index2, world_point2d* p2) {
    ...
    do {
        ...
        if (film_profile.line_is_obstructed_fix) {
            ...
            line_index = _find_line_crossed_leaving_polygon(polygon_index, (world_point2d*)p1, (world_point2d*)p2,
                                                            &last_line);
        }

        if (line_index != NONE) {
            if (last_line && polygon_index == polygon_index2)
                break;
    ...
    if (garbage_objects_in_polygon > MAXIMUM_GARBAGE_OBJECTS_PER_POLYGON) {
        ...
        if (dynamic_world->garbage_object_count >= MAXIMUM_GARBAGE_OBJECTS_PER_MAP) {
            ...
            for (object_index = garbage_object_index, object = garbage_object;
                 ...
                 object_index = (object_index == MAXIMUM_OBJECTS_PER_MAP - 1) ? 0 : (object_index + 1),
...
/* find an (x,y) and polygon_index for a random point on the given circle, at the same height
    as the center point */
void random_point_on_circle(world_point3d* center, short center_polygon_index, world_distance radius,
                            world_point3d* random_point, short* random_polygon_index) {
    ...
    *random_polygon_index
            = find_new_object_polygon((world_point2d*)center, (world_point2d*)random_point, center_polygon_index);
    if (*random_polygon_index != NONE) {
        ...
        if (center_polygon->floor_height != random_polygon->floor_height)
            *random_polygon_index = NONE;
    }
}
...
/* returns the line_index of the line we intersected to leave this polygon, or NONE if destination
    is in the given polygon */
short _find_line_crossed_leaving_polygon(short polygon_index,
                                         world_point2d* p0, /* origin (not necessairly in polygon_index) */
                                         world_point2d* p1, /* destination (not necessairly in polygon_index) */
...
static short _new_map_object(shape_descriptor shape, angle facing) {
    ...
    if (object_index == MAXIMUM_OBJECTS_PER_MAP)
        object_index = NONE;
...
world_location3d* get_object_sound_location(short object_index) {
    ...
    switch (GET_OBJECT_OWNER(object)) {
        ...
        case _object_is_effect: {
            ...
            if (object_owner_index != NONE && object_owner && SLOT_IS_USED(object_owner))
                return get_object_sound_location(object_owner_index);
        }
            [[fallthrough]];
        default:
...
uint16 _sound_obstructed_proc(world_location3d* source) {
    ...
    if (listener) {
        if (line_is_obstructed(source->polygon_index, (world_point2d*)&source->point, listener->polygon_index,
                               (world_point2d*)&listener->point)) {
...
void _sound_add_ambient_sources_proc(void* data, add_ambient_sound_source_proc_ptr add_one_ambient_sound_source) {
    ...
    if (listener) {
        struct polygon_data* listener_polygon = get_polygon_data(listener->polygon_index);
        struct media_data* media = listener_polygon->media_index != NONE ? get_media_data(listener_polygon->media_index)
                                                                         : (struct media_data*)NULL;
        ...
        if (media && listener->point.z < media->height) {
            ...
            add_one_ambient_sound_source((struct ambient_sound_data*)data, (world_location3d*)NULL, listener,
                                         get_media_sound(listener_polygon->media_index, _media_snd_ambient_under),
                                         MAXIMUM_SOUND_VOLUME);
            under_media = true;
        } else {
            ...
            if (listener_polygon->ambient_sound_image_index != NONE) {
                struct ambient_sound_image_data* image
                        = get_ambient_sound_image_data(listener_polygon->ambient_sound_image_index);

                // LP change: returning NULL means this is invalid; do some editing if necessary
                if (image)
                    add_one_ambient_sound_source((struct ambient_sound_data*)data, (world_location3d*)NULL, listener,
                                                 image->sound_index, image->volume);
                else
            ...
            if (media && (media->height >= listener_polygon->floor_height || !MEDIA_SOUND_OBSTRUCTED_BY_FLOOR(media))) {
                ...
                add_one_ambient_sound_source((struct ambient_sound_data*)data, &source, listener,
                                             get_media_sound(listener_polygon->media_index, _media_snd_ambient_over),
                                             MAXIMUM_SOUND_VOLUME);
            }
        }
        ...
        if (listener_polygon->type == _polygon_is_platform) {
            ...
            if (PLATFORM_IS_ACTIVE(platform) && PLATFORM_IS_MOVING(platform)) {
                ...
                add_one_ambient_sound_source((struct ambient_sound_data*)data, &source, listener,
                                             get_platform_moving_sound(listener_polygon->permutation),
                                             MAXIMUM_SOUND_VOLUME);
            }
        }
        ...
        if (indexes) {
            while ((index = *indexes++) != NONE && index < MAXIMUM_SAVED_OBJECTS) {
                ...
                struct media_data* media     = polygon->media_index != NONE ? get_media_data(polygon->media_index)
                                                                            : (struct media_data*)NULL;
                ...
                if (object->flags & _map_object_is_platform_sound) {
                    if (polygon->type == _polygon_is_platform
                        && PLATFORM_IS_MOVING(get_platform_data(polygon->permutation))) {
                ...
                if (active
                    && (!under_media
                        || (media && source.point.z < media->height
                            && polygon->media_index == listener_polygon->media_index))) {
                    add_one_ambient_sound_source((struct ambient_sound_data*)data, &source, listener, sound_type,
                                                 sound_volume);
                }
            }
        }
    ...
    if (polygon->random_sound_image_index != NONE) {
        ...
        if (image) {
            ...
            if (!image->phase) {
                ...
                if (image->delta_volume)
                    volume += local_random() % image->delta_volume;
                if (image->delta_direction)
                    direction = NORMALIZE_ANGLE(direction + local_random() % image->delta_direction);
                if (image->delta_pitch)
                    pitch += local_random() % image->delta_pitch;

                SoundManager::instance()->DirectPlaySound(
                        SoundManager::instance()->RandomSoundIndexToSoundIndex(image->sound_index),
            ...
            if ((image->phase -= 1) < 0) {
                ...
                if (image->delta_period)
                    image->phase += local_random() % image->delta_period;
            }
        } else
...
void reset_mml_texture_loading() {
    ...
    if (OriginalEnvironments) {
        for (int i = 0; i < NUMBER_OF_ENVIRONMENTS; i++) {
            for (int j = 0; j < NUMBER_OF_ENV_COLLECTIONS; j++) Environments[i][j] = OriginalEnvironments[i][j];
...
void parse_mml_texture_loading(const InfoTree& root) {
    ...
    if (!OriginalEnvironments) {
        ...
        for (int i = 0; i < NUMBER_OF_ENVIRONMENTS; i++) {
            ...
            for (int j = 0; j < NUMBER_OF_ENV_COLLECTIONS; j++) OriginalEnvironments[i][j] = Environments[i][j];
        }
    }
