        ...
        if (definition->flags & _sound_only) {
            ...
            for (effect_index = 0, effect = effects; effect_index < MAXIMUM_EFFECTS_PER_MAP; ++effect_index, ++effect) {
                if (SLOT_IS_FREE(effect)) {
                    short object_index = new_map_object3d(
                            origin, polygon_index, BUILD_DESCRIPTOR(definition->collection, definition->shape), facing);
        ...
        if (SLOT_IS_USED(effect)) {
            ...
            if (effect->delay) {
                ...
                if (((GET_OBJECT_ANIMATION_FLAGS(object) & _obj_last_frame_animated)
                    ...
                    || ((GET_OBJECT_ANIMATION_FLAGS(object) & _obj_transfer_mode_finished)
                        && (definition->flags & _end_when_transfer_animation_loops))) {
...
void teleport_object_out(short object_index) {
    ...
    if (!OBJECT_IS_INVISIBLE(object)) {
        short effect_index
                = new_effect(&object->location, object->polygon, _effect_teleport_object_out, object->facing);
...
void init_effect_definitions() { memcpy(effect_definitions, original_effect_definitions, sizeof(effect_definitions)); }
