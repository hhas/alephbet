...
static short adjust_projectile_type(world_point3d* origin, short polygon_index, short type, short owner_index,
                                    short owner_type, short intended_target_index, _fixed damage_scale);

static void update_guided_projectile(short projectile_index);

/*static*/ projectile_definition* get_projectile_definition(short type);

/* ---------- code */

projectile_data* get_projectile_data(const short projectile_index) {
    struct projectile_data* projectile
            = GetMemberWithBounds(projectiles, projectile_index, MAXIMUM_PROJECTILES_PER_MAP);

    vassert(projectile, csprintf(temporary, "projectile index #%d is out of range", projectile_index));
    vassert(SLOT_IS_USED(projectile),
            csprintf(temporary, "projectile index #%d (%p) is unused", projectile_index, (void*)projectile));
...
bool preflight_projectile(world_point3d* origin, short origin_polygon_index, world_point3d* destination,
                          angle delta_theta, short type, short owner, short owner_type, short* obstruction_index) {
    ...
    if (origin_polygon_index != NONE) {
        ...
        if (elevation < MAXIMUM_PROJECTILE_ELEVATION || elevation > FULL_CIRCLE - MAXIMUM_PROJECTILE_ELEVATION) {
            ...
            if (origin->z > origin_polygon->floor_height && origin->z < origin_polygon->ceiling_height
                && (origin_polygon->media_index == NONE || definition->flags & (_penetrates_media)
                    || (media ? origin->z > media->height : true))) {
                /* make sure it hits something */
                uint16 flags = translate_projectile(type, origin, origin_polygon_index, destination, (short*)NULL,
                                                    owner, obstruction_index, 0, true, NONE);

                *obstruction_index
                        = (flags & _projectile_hit_monster) ? get_object_data(*obstruction_index)->permutation : NONE;
...
void detonate_projectile(world_point3d* origin, short polygon_index, short type, short owner_index, short owner_type,
                         _fixed damage_scale) {
    struct projectile_definition* definition = get_projectile_definition(type);
    struct damage_definition* damage         = &definition->damage;

    damage->scale = damage_scale;
    damage_monsters_in_radius(NONE, owner_index, owner_type, origin, polygon_index, definition->area_of_effect, damage,
                              NONE);
    if (definition->detonation_effect != NONE)
        new_effect(origin, polygon_index, definition->detonation_effect, 0);
...
short new_projectile(world_point3d* origin, short polygon_index, world_point3d* _vector,
                     ...
                     short type, short owner_index, short owner_type, short intended_target_index, /* can be NONE */
                     _fixed damage_scale) {
    struct projectile_definition* definition;
    struct projectile_data* projectile;
    short projectile_index;

    type       = adjust_projectile_type(origin, polygon_index, type, owner_index, owner_type, intended_target_index,
                                        damage_scale);
    ...
    for (projectile_index = 0, projectile = projectiles; projectile_index < MAXIMUM_PROJECTILES_PER_MAP;
        ...
        if (SLOT_IS_FREE(projectile)) {
            ...
            if (delta_theta) {
                if (!(definition->flags & _no_horizontal_error))
                    facing = normalize_angle(facing + global_random() % (2 * delta_theta) - delta_theta);
                if (!(definition->flags & _no_vertical_error))
                    elevation
                            = (definition->flags & _positive_vertical_error)
                                      ? normalize_angle(elevation + global_random() % delta_theta)
                                      : normalize_angle(elevation + global_random() % (2 * delta_theta) - delta_theta);
            }

            object_index = new_map_object3d(
                    origin, polygon_index,
                    definition->collection == NONE ? NONE : BUILD_DESCRIPTOR(definition->collection, definition->shape),
                    facing);
    ...
    if (projectile_index == MAXIMUM_PROJECTILES_PER_MAP)
        projectile_index = NONE;
    ...
    for (projectile_index = 0, projectile = projectiles; projectile_index < MAXIMUM_PROJECTILES_PER_MAP;
        ...
        if (SLOT_IS_USED(projectile)) {
            ...
            //			if (!OBJECT_IS_INVISIBLE(object))
            {
                ...
                if ((definition->flags & _stop_when_animation_loops)
                    && (GET_OBJECT_ANIMATION_FLAGS(object) & _obj_last_frame_animated)) {
                    ...
                    if ((definition->flags & _guided) && projectile->target_index != NONE
                        && (dynamic_world->tick_count & 1))
                        update_guided_projectile(projectile_index);

                    if (PROJECTILE_HAS_CROSSED_MEDIA_BOUNDARY(projectile))
                        adjusted_definition_flags = _penetrates_media;
                    ...
                    if ((definition->flags & _affected_by_half_gravity) && (dynamic_world->tick_count & 1))
                        projectile->gravity -= GRAVITATIONAL_ACCELERATION;
                    if (definition->flags & _affected_by_gravity)
                        projectile->gravity -= GRAVITATIONAL_ACCELERATION;
                    if (definition->flags & _doubly_affected_by_gravity)
                        projectile->gravity -= 2 * GRAVITATIONAL_ACCELERATION;
                    if (film_profile.m1_low_gravity_projectiles
                    ...
                    if (definition->flags & _vertical_wander)
                        new_location.z += (global_random() & 1) ? WANDER_MAGNITUDE : -WANDER_MAGNITUDE;
                    if (definition->flags & _horizontal_wander)
                        translate_point3d(&new_location, (global_random() & 1) ? WANDER_MAGNITUDE : -WANDER_MAGNITUDE,
                                          NORMALIZE_ANGLE(object->facing + QUARTER_CIRCLE), 0);
                    if (film_profile.infinity_smg) {
                        definition->flags ^= adjusted_definition_flags;
                    }
                    flags = translate_projectile(projectile->type, &old_location, object->polygon, &new_location,
                                                 &new_polygon_index, projectile->owner_index, &obstruction_index,
                                                 &line_index, false, projectile_index);
                    if (film_profile.infinity_smg) {
                        definition->flags ^= adjusted_definition_flags;
                    }
                    ...
                    if (flags & _projectile_hit) {
                        if ((flags & _projectile_hit_floor) && (definition->flags & _rebounds_from_floor)
                            ...
                            short monster_obstruction_index    = (flags & _projectile_hit_monster)
                                                                         ? get_object_data(obstruction_index)->permutation
                                                                         : NONE;
                            bool destroy_persistent_projectile = false;

                            if (flags & _projectile_hit_scenery)
                                damage_scenery(obstruction_index);
                                ...
                                if (definition->flags & _becomes_item_on_detonation) {
                                    if (monster_obstruction_index == NONE) {
                                        ...
                                        if (MONSTER_IS_PLAYER(get_monster_data(monster_obstruction_index))) {
                                            short player_obstruction_index
                                                    = monster_index_to_player_index(monster_obstruction_index);
                                            destroy_persistent_projectile = try_and_add_player_item(
                                                    player_obstruction_index, projectile->permutation);
                                        }
                                    }
                                } else {
                                    if (definition->area_of_effect) {
                                        damage_monsters_in_radius(monster_obstruction_index, projectile->owner_index,
                                                                  projectile->owner_type, &old_location,
                                                                  object->polygon, definition->area_of_effect, damage,
                                                                  projectile_index);
                                    } else {
                                        if (monster_obstruction_index != NONE)
                                            damage_monster(monster_obstruction_index, projectile->owner_index,
                                                           projectile->owner_type, &old_location, damage,
                                                           projectile_index);
                                    }
                                }
                            }
                            ...
                            if ((definition->flags & _persistent) && !destroy_persistent_projectile) {
                                ...
                                if (monster_obstruction_index != NONE) {
                                    ...
                                    if (definition->flags & _melee_projectile) {
                                        short new_detonation_effect
                                                = get_monster_melee_impact_effect(monster_obstruction_index);
                                        if (new_detonation_effect != NONE)
                                            detonation_effect = new_detonation_effect;
                                    }
                                }
                                if (flags & _projectile_hit_media) {
                                    get_media_detonation_effect(get_polygon_data(obstruction_index)->media_index,
                                                                definition->media_detonation_effect,
                                                                &detonation_effect);
                                    ...
                                    if (film_profile.a1_smg) {
                                        ...
                                        if (will_go_through) {
                                            ...
                                            } else if (projectile->elevation < HALF_CIRCLE)
                                                new_location.z++;
                                            else if (projectile->elevation > HALF_CIRCLE)
                                                new_location.z--;
                                        }
                                ...
                                if (film_profile.a1_smg) {
                                    ...
                                    // so be careful.
                                    if (flags & _projectile_hit_landscape && !(flags & _projectile_hit_media))
                                        detonation_effect = NONE;
                                } else if (!film_profile.m1_landscape_effects
                                           || !(static_world->environment_flags & _environment_m1_weapons)) {
                                    if (flags & _projectile_hit_landscape)
                                        detonation_effect = NONE;
                                }

                                if (detonation_effect != NONE)
                                    new_effect(&new_location, new_polygon_index, detonation_effect, object->facing);
                                L_Call_Projectile_Detonated(projectile->type, projectile->owner_index,
                                                            new_polygon_index, new_location, flags, obstruction_index,
                                                            line_index);

                                if (!film_profile.infinity_smg
                                    || (!(definition->flags & _penetrates_media_boundary)
                                        || !(flags & _projectile_hit_media))) {
                                    if ((definition->flags & _persistent_and_virulent) && !destroy_persistent_projectile
                                        && monster_obstruction_index != NONE) {
                                        ...
                                        if (film_profile.prevent_dead_projectile_owners) {
                                            ...
                                            reassign_projectile
                                                    = MONSTER_IS_PLAYER(monster) || !MONSTER_IS_DYING(monster);
                                        }
                                        if (reassign_projectile)
                                            projectile->owner_index
                                                    = monster_obstruction_index; /* keep going, but don’t hit this
                                                                                    target again */
                                    }
                                    // LP addition: don't remove a projectile that will hit media and continue (PMB
                                    // flag)
                    ...
                    // else
                    {
                        ...
                        if ((projectile->ticks_since_last_contrail += 1) >= definition->ticks_between_contrails) {
                            if (definition->maximum_contrails == NONE
                                || projectile->contrail_count < definition->maximum_contrails) {
                                projectile->contrail_count            += 1;
                                projectile->ticks_since_last_contrail  = 0;
                                if (definition->contrail_effect != NONE) {
                                    auto effect_index = new_effect(&old_location, old_polygon_index,
                                                                   definition->contrail_effect, object->facing);
                                    if (effect_index != NONE && definition->ticks_between_contrails <= 1) {
                                        track_contrail_interpolation(projectile->object_index,
                                                                     get_effect_data(effect_index)->object_index);
                                    }
                                }
                            }
                        }
...
void orphan_projectiles(short monster_index) {
    ...
    for (projectile_index = 0, projectile = projectiles; projectile_index < MAXIMUM_PROJECTILES_PER_MAP;
         ++projectile_index, ++projectile) {
        if (projectile->owner_index == monster_index)
            projectile->owner_index = NONE;
        if (projectile->target_index == monster_index)
...
void mark_projectile_collections(short projectile_type, bool loading) {
    if (projectile_type != NONE) {
        struct projectile_definition* definition = get_projectile_definition(projectile_type);
...
void drop_the_ball(world_point3d* origin, short polygon_index, short owner_index, short owner_type, short item_type) {
    struct world_point3d _vector;
    short projectile_index;

    _vector.x = _vector.y = _vector.z = 0;
    projectile_index = new_projectile(origin, polygon_index, &_vector, 0, _projectile_ball, owner_index, owner_type,
                                      NONE, FIXED_ONE);
    ...
    if (media_index != NONE) {
        ...
        if (media) {
            if (media->height > origin->z) {
                if (definition->media_projectile_promotion != NONE)
                    type = definition->media_projectile_promotion;
            }
        }
    }
...
static void update_guided_projectile(short projectile_index) {
    ...
    switch (target_object->transfer_mode) {
        ...
        case _xfer_subtle_invisibility:
            ...
            if (dynamic_world->game_information.difficulty_level != _total_carnage_level)
                break;
        default: {
            ...
            short delta_yaw
                    = MAXIMUM_GUIDED_DELTA_YAW + _normal_level - dynamic_world->game_information.difficulty_level;
            short delta_pitch
                    = MAXIMUM_GUIDED_DELTA_PITCH + _normal_level - dynamic_world->game_information.difficulty_level;
            ...
            if (GUESS_HYPOTENUSE(dx, dy) * sine_table[projectile->elevation] - dz * cosine_table[projectile->elevation]
                > 0) {
                // turn down
...
uint16 translate_projectile(short type, world_point3d* old_location, short old_polygon_index,
                            world_point3d* new_location, short* new_polygon_index, short owner_index,
                            short* obstruction_index, short* last_line_index, bool preflight, short projectile_index) {
    struct projectile_definition* definition = get_projectile_definition(type);
    ...
    *obstruction_index = NONE;
    ...
    do {
        ...
        media_height      = (old_polygon->media_index == NONE || definition->flags & _penetrates_media || !media)
                                    ? INT16_MIN
                                    : media->height;
        ...
        if (!(definition->flags & _passes_through_objects)) {
            possible_intersecting_monsters(&IntersectedObjects, GLOBAL_INTERSECTING_MONSTER_BUFFER_SIZE,
                                           old_polygon_index, true);
            intersected_object_count = IntersectedObjects.size();
        }

        line_index = find_line_crossed_leaving_polygon(old_polygon_index, (world_point2d*)old_location,
                                                       (world_point2d*)new_location);
        if (line_index != NONE) {
            ...
            if ((!LINE_IS_SOLID(line) || LINE_HAS_TRANSPARENT_SIDE(line)) && (adjacent_polygon_index != NONE)) {
                ...
                if ((traveled_underneath || intersection.z > media_height)
                    && intersection.z > old_polygon->floor_height) {
                    // If PMB was set, check to see if the projectile hit the media surface.
                    if ((!traveled_underneath || intersection.z < media_height)
                        && intersection.z < old_polygon->ceiling_height) {
                        if (intersection.z > adjacent_polygon->floor_height
                            && intersection.z < adjacent_polygon->ceiling_height) {
                            if (!LINE_HAS_TRANSPARENT_SIDE(line) || line->is_decorative()
                                || (preflight
                                    && (definition->flags
                                        & (_usually_pass_transparent_side | _sometimes_pass_transparent_side)))
                                ...
                                if (new_polygon_index)
                                    *new_polygon_index = adjacent_polygon_index;
                            ...
                            if (!preflight && (definition->flags & _can_toggle_control_panels))
                                try_and_toggle_control_panel(old_polygon_index, line_index, projectile_index);
                            contact = _hit_wall;
                        }
                    } else {
                        /* hit ceiling of old polygon */
                        *obstruction_index = old_polygon_index;
                        if (adjacent_polygon->ceiling_transfer_mode == _xfer_landscape)
                            flags |= _projectile_hit_landscape;
                        // LP change: if PMB was set, check to see if hit media
                        contact = (!traveled_underneath || old_polygon->ceiling_height < media_height) ? _hit_ceiling
                                                                                                       : _hit_media;
                    }
                } else {
                    /* hit floor or media of old polygon */
                    *obstruction_index = old_polygon_index;
                    if (adjacent_polygon->floor_transfer_mode == _xfer_landscape)
                        flags |= _projectile_hit_landscape;
                    // LP change: suppress media-hit test only if PMB was set and the projectile was underneath the
                    // media
                    contact = (traveled_underneath || old_polygon->floor_height > media_height) ? _hit_floor
                                                                                                : _hit_media;
                }
            } else {
                ...
                if (!preflight && (definition->flags & _can_toggle_control_panels))
                    try_and_toggle_control_panel(old_polygon_index, line_index, projectile_index);
                if (line_is_landscaped(old_polygon_index, line_index, intersection.z))
                    flags |= _projectile_hit_landscape;
                contact = _hit_wall;
            }
        } else {
            ...
            // Here, test for whether the projectile is above the floor or the media surface;
            // ignore the latter test if PMB is set.
            if ((traveled_underneath || new_location->z > media_height)
                && new_location->z > old_polygon->floor_height) {
                // If PMB was set, check to see if the projectile hit the media surface.
                if ((!traveled_underneath || new_location->z < media_height)
                    && new_location->z < old_polygon->ceiling_height) {
                    ...
                    *obstruction_index = old_polygon_index;
                    if (old_polygon->ceiling_transfer_mode == _xfer_landscape)
                        flags |= _projectile_hit_landscape;
                    // LP change: if PMB was set, check to see if hit media
                    contact = (!traveled_underneath || old_polygon->ceiling_height < media_height) ? _hit_ceiling
                                                                                                   : _hit_media;
                ...
                *obstruction_index = old_polygon_index;
                if (old_polygon->floor_transfer_mode == _xfer_landscape)
                    flags |= _projectile_hit_landscape;
    ...
    if (contact != _hit_nothing) {
        switch (contact) {
            case _hit_media:
                ...
                break;
            case _hit_floor:
                find_floor_or_ceiling_intersection(old_polygon->floor_height, old_location, new_location,
                                                   &intersection);
                break;
            case _hit_ceiling:
                find_floor_or_ceiling_intersection(old_polygon->ceiling_height, old_location, new_location,
                                                   &intersection);
                break;
        }
    ...
    if (!(definition->flags & _passes_through_objects)) {
        ...
        for (size_t i = 0; i < intersected_object_count; ++i) {
            ...
            int32 separation           = point_to_line_segment_distance_squared(
                    (world_point2d*)&object->location, (world_point2d*)old_location, (world_point2d*)new_location);
            ...
            if (object->permutation != owner_index) /* don’t hit ourselves */
            {
                ...
                switch (GET_OBJECT_OWNER(object)) {
                    ...
                    case _object_is_scenery:
                        get_scenery_dimensions(object->permutation, &radius, &height);
                        break;
                    default:
                        assert(false);
                        break;
                ...
                if (separation < radius_squared) /* if we’re within radius^2 we passed through this monster */
                {
                    world_distance distance
                            = distance2d((world_point2d*)old_location, (world_point2d*)&object->location);
                    world_distance projectile_z
                            = distance_traveled
                                      ? old_location->z
                                                + (distance * (new_location->z - old_location->z)) / distance_traveled
                                      : old_location->z;

                    if ((height > 0 && projectile_z >= object->location.z
                         && projectile_z <= object->location.z + height)
                        || (height < 0 && projectile_z >= object->location.z + height
                            && projectile_z <= object->location.z)) {
                        if (best_intersection_object == NONE || distance < best_intersection_distance) {
                            ...
                            switch (GET_OBJECT_OWNER(object)) {
                                case _object_is_monster:
                                    contact = _hit_monster;
                                    break;
                                case _object_is_scenery:
                                    contact = _hit_scenery;
                                    break;
                                default:
                                    assert(false);
                                    break;
                    ...
                    if (GET_OBJECT_OWNER(object) == _object_is_monster
                        && separation
                                   < 12 * radius_squared) /* if we’re within (x*radius)^2 we passed near this monster */
                    {
            ...
            *obstruction_index = best_intersection_object;
            ...
            if (distance_traveled) {
                ...
                new_location->x = old_location->x
                                  + (actual_distance_to_hit * (new_location->x - old_location->x)) / distance_traveled;
                new_location->y = old_location->y
                                  + (actual_distance_to_hit * (new_location->y - old_location->y)) / distance_traveled;
                new_location->z = old_location->z
                                  + (actual_distance_to_hit * (new_location->z - old_location->z)) / distance_traveled;

                if (new_polygon_index)
                    *new_polygon_index = find_new_object_polygon((world_point2d*)&object->location,
    ...
    switch (contact) {
        case _hit_monster:
            flags |= _projectile_hit | _projectile_hit_monster;
            break;
        case _hit_floor:
            flags |= _projectile_hit | _projectile_hit_floor;
            break;
        case _hit_media:
            flags |= _projectile_hit | _projectile_hit_media;
            break;
        case _hit_scenery:
            flags |= _projectile_hit | _projectile_hit_scenery;
            break;
        case _hit_nothing:
            break;
        default:
            flags |= _projectile_hit;
            break;
    }
