...
//keep_line_segment_out_of_walls() can’t use precalculated height information and should do weird things next to
elevators and doors
*/
...
static int32 intersecting_flood_proc(short source_polygon_index, short line_index, short destination_polygon_index,
                                     void* data);
...
short new_side(short polygon_index, short line_index) {
    ...
    assert((line->clockwise_polygon_owner == polygon_index && line->clockwise_polygon_side_index == NONE)
           || (line->counterclockwise_polygon_owner == polygon_index
    ...
    for (line_index = 0, line = map_lines; line_index < dynamic_world->line_count; ++line_index, ++line) {
        ...
        if (line->endpoint_indexes[0] == endpoint_index || line->endpoint_indexes[1] == endpoint_index) {
            ...
            if (polygon_index != NONE) {
                ...
                if (lowest_adjacent_ceiling_height > polygon->ceiling_height)
                    lowest_adjacent_ceiling_height = polygon->ceiling_height;
            }
...
void recalculate_redundant_line_data(short line_index) {
    ...
    /* find highest adjacent floor and lowest adjacent ceiling */
    {
        ...
        polygon1 = (line->clockwise_polygon_owner == NONE) ? (struct polygon_data*)NULL
                                                           : get_polygon_data(line->clockwise_polygon_owner);
        polygon2 = (line->counterclockwise_polygon_owner == NONE)
                           ? (struct polygon_data*)NULL
                           : get_polygon_data(line->counterclockwise_polygon_owner);

        if ((polygon1 && polygon1->type == _polygon_is_platform)
            || (polygon2 && polygon2->type == _polygon_is_platform))
            variable_elevation = true;
        ...
        if (polygon1 && polygon2) {
            ...
            if (polygon1->floor_height != polygon2->floor_height)
                elevation = true;
        } else {
            elevation = true;
...
void recalculate_redundant_side_data(short side_index, short line_index) {
    ...
    //	if (line_index==98) dprintf("line sides: %d,%d side_index==%d", line->clockwise_polygon_side_index,
    //line->counterclockwise_polygon_side_index, side_index);
...
void guess_side_lightsource_indexes(short side_index) {
    struct side_data* side = get_side_data(side_index);
    if (side->line_index < 0 || side->line_index >= dynamic_world->line_count || side->polygon_index < 0
    ...
    switch (side->type) {
        case _full_side:
            ...
            break;
        case _split_side:
            side->secondary_lightsource_index
                    = (line->lowest_adjacent_ceiling - line->highest_adjacent_floor > CONTINUOUS_SPLIT_SIDE_HEIGHT)
                              ? floor_index
                              : ceiling_index;
...
void set_map_index_buffer_size(int32 length) { map_index_buffer_count = length / sizeof(short); }
...
void precalculate_map_indexes(void) {
    ...
    for (; polygon_index < dynamic_world->polygon_count; ++polygon, ++polygon_index) {
        if (!POLYGON_IS_DETACHED(polygon)) /* we’ll handle detached polygons during the second pass */
        {
            ...
            //			if (polygon_index==155) dprintf("polygon index #%d has %d neighbors:;dm %x %x;", polygon_index,
            //polygon_count, polygon_indexes, sizeof(short)*polygon_count);

            size_t polygon_count = PolygonIndices.size();

            for (size_t i = 0; i < polygon_count; ++i) { add_map_index(PolygonIndices[i], &polygon->neighbor_count); }
        }
    }
...
#pragma unused (line_index,destination_polygon_index)
    ...
    if
((polygon->floor_height<=original_polygon->ceiling_height)&&(polygon->ceiling_height>=original_polygon->floor_height))
    {
    ...
    short line_index)
{
     ...
     (line->highest_adjacent_floor>original_polygon->floor_height))
    {
        ...
        if (i==data->line_count && data->line_count<MAXIMUM_INTERSECTING_INDEXES)
        {
            ...
            data->line_indexes[data->line_count++]= clockwise ? polygon->line_indexes[i] :
(-polygon->line_indexes[i]-1);
//			if (data->original_polygon_index==23) dprintf("found line %d (%s)", polygon->line_indexes[i], clockwise ?
"clockwise" : "counterclockwise"); keep_searching= true; break;
        }
    }
    ...
    return keep_searching;
            ...
            if (j==data->endpoint_count && data->endpoint_count<MAXIMUM_INTERSECTING_INDEXES)
            {
                ...
                // check and see if this endpoint is close enough to any line in our original polygon to care about; if
it is, add it to our list for (j=0;j<original_polygon->vertex_count;++j)
                {
                    ...
                    {
                        if (DoIncorrectCountVWarn)
                            vwarn(data->endpoint_count!=MAXIMUM_INTERSECTING_INDEXES-1, csprintf(temporary, "incomplete
endpoint list for polygon#%d", data->original_polygon_index)); data->endpoint_indexes[data->endpoint_count++]=
polygon->endpoint_indexes[i];
//						if (data->original_polygon_index==23) dprintf("found endpoint %d",
data->endpoint_indexes[data->endpoint_count-1]);
//						switch (data->endpoint_indexes[data->endpoint_count-1])
//						{
//							case 34:
//							case 35:
//								dprintf("found endpoint#%d from polygon#%d",
data->endpoint_indexes[data->endpoint_count-1], data->original_polygon_index);
//						}
                        break;
                    }
                }
...
static int32 intersecting_flood_proc(short source_polygon_index, short line_index, short destination_polygon_index,
    ...
    /* we only care about this polygon if it intersects us in z */
    if ((polygon->floor_height <= original_polygon->ceiling_height)
        && (polygon->ceiling_height >= original_polygon->floor_height)) {
        ...
        for (i = 0; i < polygon->vertex_count; ++i) {
            ...
            for (j = 0; j < LineIndices.size(); ++j) {
                if (LineIndices[j] == polygon->line_indexes[i] || -LineIndices[j] - 1 == polygon->line_indexes[i]) {
                    keep_searching = true;
            ...
            if (j == LineIndices.size()) {
                ...
                if (LINE_IS_SOLID(line) || line_has_variable_height(line_index)
                    || line->lowest_adjacent_ceiling < original_polygon->ceiling_height
                    || line->highest_adjacent_floor > original_polygon->floor_height) {
                    world_point2d* a = &(get_endpoint_data(line->endpoint_indexes[0])->vertex);
                    world_point2d* b = &(get_endpoint_data(line->endpoint_indexes[1])->vertex);
                    ...
                    for (j = 0; j < original_polygon->vertex_count; ++j) {
                        ...
                        if (point_to_line_segment_distance_squared(p, a, b) < data->minimum_separation_squared) {
                            bool clockwise = !!((((b->x - a->x) * (data->center.y - b->y))
                                                 - ((b->y - a->y) * (data->center.x - b->x)))
                                                > 0);

                            LineIndices.push_back(clockwise ? polygon->line_indexes[i]
                                                            : (-polygon->line_indexes[i] - 1));
                            keep_searching = true;
                            break;
                        }
...
//				if (data->original_polygon_index==23&&line_index==104) dprintf("line#%d @ %p", line_index, line);
                    ...
                    line->highest_adjacent_floor>original_polygon->floor_height)
                {
                    ...
                    // check and see if this line is close enough to any point in our original polygon to care about; if
it is, add it to our list for (j=0;j<original_polygon->vertex_count;++j)
                    {
                        world_point2d *p= &get_endpoint_data(original_polygon->endpoint_indexes[j])->vertex;
...
true : false;
                            ...
                            if (DoIncorrectCountVWarn)
                                vwarn(data->line_count!=MAXIMUM_INTERSECTING_INDEXES-1, csprintf(temporary, "incomplete
line list for polygon#%d", data->original_polygon_index)); data->line_indexes[data->line_count++]= clockwise ?
polygon->line_indexes[i] : (-polygon->line_indexes[i]-1);
//							if (data->original_polygon_index==23) dprintf("found line %d (%s)",
polygon->line_indexes[i], clockwise ? "clockwise" : "counterclockwise"); keep_searching= true; break;
                        }
                    }
                }
            }
...
//						}
    ...
    if (keep_searching)
    {
        ...
        if (j==data->polygon_count && data->polygon_count<MAXIMUM_INTERSECTING_INDEXES)
        {
            ...
            if (DoIncorrectCountVWarn)
                vwarn(data->polygon_count!=MAXIMUM_INTERSECTING_INDEXES-1, csprintf(temporary, "incomplete neighbor list
for polygon#%d", data->original_polygon_index)); data->polygon_indexes[data->polygon_count++]= source_polygon_index;

            // if this polygon has a detached twin, add it too
            if (detached_twin_index!=NONE && data->polygon_count<MAXIMUM_INTERSECTING_INDEXES)
            {
                if (DoIncorrectCountVWarn)
                    vwarn(data->polygon_count!=MAXIMUM_INTERSECTING_INDEXES-1, csprintf(temporary, "incomplete neighbor
list for polygon#%d", data->original_polygon_index)); data->polygon_indexes[data->polygon_count++]= detached_twin_index;
            }
        }
    }
...
static void precalculate_polygon_sound_sources(void) {
    ...
    for (polygon_index = 0, polygon = map_polygons; polygon_index < dynamic_world->polygon_count;
        ...
        for (object_index = 0, object = saved_objects; object_index < dynamic_world->initial_objects_count;
             ++object, ++object_index) {
            if (object->type == _saved_sound_source) {
                ...
                if (close)
                    add_map_index(object_index, &sound_sources);
            }
        }
...
uint8* unpack_map_object(uint8* Stream, map_object* Objects, size_t Count, int version) {
    ...
    for (size_t k = 0; k < Count; k++, ObjPtr++) {
        ...
        StreamToValue(S, ObjPtr->location.x);
        StreamToValue(S, ObjPtr->location.y);
        if (version == MARATHON_ONE_DATA_VERSION && film_profile.m1_object_unused) {
            ObjPtr->location.z  = 0;
            ObjPtr->flags       = 0;
