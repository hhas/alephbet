...
enum { /* entry point types- this is per map level (int32). */
    ...
    _kill_the_man_with_the_ball_entry_point
            = 0x08, // was _capture_the_flag_entry_point, even though Bungie used it for KTMWTB
...
inline int16 player_identifier_value(int16 identifier) { return identifier & player_start_identifier_mask; }
...
#define SET_OBJECT_ANIMATION_FLAGS(o, n) \
    {                                    \
        (o)->flags &= (uint16)~0x3c00;   \
        (o)->flags |= (n);               \
    }
...
#define SET_LINE_TRANSPARENCY(l, v) \
    ((v) ? ((l)->flags |= (uint16)TRANSPARENT_LINE_BIT) : ((l)->flags &= (uint16)~TRANSPARENT_LINE_BIT))
#define LINE_IS_TRANSPARENT(l) ((l)->flags & TRANSPARENT_LINE_BIT)

#define SET_LINE_LANDSCAPE_STATUS(l, v) \
    ((v) ? ((l)->flags |= (uint16)LANDSCAPE_LINE_BIT) : ((l)->flags &= (uint16)~LANDSCAPE_LINE_BIT))
#define LINE_IS_LANDSCAPED(l) ((l)->flags & LANDSCAPE_LINE_BIT)

#define SET_LINE_ELEVATION(l, v) \
    ((v) ? ((l)->flags |= (uint16)ELEVATION_LINE_BIT) : ((l)->flags &= (uint16)~ELEVATION_LINE_BIT))
#define LINE_IS_ELEVATION(l) ((l)->flags & ELEVATION_LINE_BIT)

#define SET_LINE_VARIABLE_ELEVATION(l, v) \
    ((v) ? ((l)->flags |= (uint16)VARIABLE_ELEVATION_LINE_BIT) : ((l)->flags &= (uint16)~VARIABLE_ELEVATION_LINE_BIT))
#define LINE_IS_VARIABLE_ELEVATION(l) ((l)->flags & VARIABLE_ELEVATION_LINE_BIT)

#define SET_LINE_HAS_TRANSPARENT_SIDE(l, v)                      \
    ((v) ? ((l)->flags |= (uint16)LINE_HAS_TRANSPARENT_SIDE_BIT) \
         : ((l)->flags &= (uint16)~LINE_HAS_TRANSPARENT_SIDE_BIT))
...
struct line_data /* 32 bytes */
{
    ...
    bool is_decorative() const { return flags & LINE_IS_DECORATIVE_BIT; }

    void set_decorative(bool b) {
        if (b)
            flags |= LINE_IS_DECORATIVE_BIT;
        else
            flags &= ~LINE_IS_DECORATIVE_BIT;
    }
};
...
#define SET_SIDE_CONTROL_PANEL(s, t) \
    ((void)((t) ? (s->flags |= (uint16)_side_is_control_panel) : (s->flags &= (uint16)~_side_is_control_panel)))

#define GET_CONTROL_PANEL_STATUS(s) (((s)->flags & _control_panel_status) != 0)
#define SET_CONTROL_PANEL_STATUS(s, t) \
    ((t) ? (s->flags |= (uint16)_control_panel_status) : (s->flags &= (uint16)~_control_panel_status))
...
#define SET_SIDE_IS_REPAIR_SWITCH(s, t) \
    ((t) ? (s->flags |= (uint16)_side_is_repair_switch) : (s->flags &= (uint16)~_side_is_repair_switch))

/* Flags used by Vulcan */
#define SIDE_IS_DIRTY(s) ((s)->flags & _editor_dirty_bit)
#define SET_SIDE_IS_DIRTY(s, t) \
    ((t) ? (s->flags |= (uint16)_editor_dirty_bit) : (s->flags &= (uint16)~_editor_dirty_bit))
...
enum /* side types (largely redundant; most of this could be guessed for examining adjacent polygons) */
{
    ...
    _composite_side, /* primary texture is mapped floor-to-ceiling, secondary texture is mapped into it (i.e., control
                        panel) */
...
#define SET_POLYGON_DETACHED_STATE(p, v) \
    ((v) ? ((p)->flags |= POLYGON_IS_DETACHED_BIT) : ((p)->flags &= ~POLYGON_IS_DETACHED_BIT))
...
enum /* game options.. */
{
    ...
    _motion_sensor_does_not_work    = 0x0'0020, /* Motion sensor works */
...
bool keep_line_segment_out_of_walls(short polygon_index, world_point3d* p0, world_point3d* p1,
                                    world_distance maximum_delta_height, world_distance height,
...
short new_map_polygon(short* line_indexes, short line_count, short floor_height, short ceiling_height,
                      short floor_texture, short ceiling_texture, short lightsource_index);
...
short get_random_player_starting_location_and_facing(short max_player_index, short team,
...
short new_device(world_point2d* location, short initial_polygon_index, short type, short extra_data, bool active);
...
short get_player_starting_location_and_facing(short team, short index, struct object_location* location);
...
bool new_game(short number_of_players, bool network, struct game_data* game_information,
              struct player_start_data* player_start_information, struct entry_point* entry_point);
