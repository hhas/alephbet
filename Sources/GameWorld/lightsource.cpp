...
// LP: "static" removed
static struct lighting_function_specification* get_lighting_function_specification(struct static_light_data* data,
                                                                                   short state);

static _fixed lighting_function_dispatch(short function_index, _fixed initial_intensity, _fixed final_intensity,
...
struct light_definition light_definitions[NUMBER_OF_LIGHT_TYPES] = {
        ...
        {NONE,
         ...
         {
                 _normal_light, // type
                 FLAG(_light_is_initially_active) | FLAG(_light_has_slaved_intensities),
                 0, // flags, phase
    ...
    for (light_index = 0, light = lights; light_index < short(MAXIMUM_LIGHTS_PER_MAP); ++light_index, ++light) {
        if (SLOT_IS_FREE(light)) {
            ...
            change_light_state(light_index,
                               LIGHT_IS_INITIALLY_ACTIVE(data) ? _light_secondary_active : _light_secondary_inactive);
            ...
            light->intensity = lighting_function_dispatch(
                    get_lighting_function_specification(&light->static_data, light->state)->function,
...
void change_light_state(size_t light_index, short new_state) {
    ...
    struct lighting_function_specification* function
            = get_lighting_function_specification(&light->static_data, new_state);
...
static struct lighting_function_specification* get_lighting_function_specification(struct static_light_data* data,
    ...
    switch (state) {
        ...
        case _light_secondary_inactive:
            ...
            break;
        default:
            vhalt(csprintf(temporary, "what is light state #%d?", state));
    }
...
typedef _fixed (*lighting_function)(_fixed initial_intensity, _fixed final_intensity, short phase, short period);

static lighting_function lighting_functions[NUMBER_OF_LIGHTING_FUNCTIONS] = {
        constant_lighting_proc, linear_lighting_proc, smooth_lighting_proc,
        flicker_lighting_proc,  random_lighting_proc, fluorescent_lighting_proc,
};
...
static _fixed smooth_lighting_proc(_fixed initial_intensity, _fixed final_intensity, short phase, short period) {
    return initial_intensity
           + (((final_intensity - initial_intensity)
               * (cosine_table[(phase * HALF_CIRCLE) / period + HALF_CIRCLE] + TRIG_MAGNITUDE))
              >> (TRIG_SHIFT + 1));
}
...
static_light_data old_light_definitions[NUMBER_OF_OLD_LIGHTS] = {
        ...
        {_normal_light,
         FLAG(_light_is_initially_active) | FLAG(_light_has_slaved_intensities),
         0, {_constant_lighting_function, TICKS_PER_SECOND, 0, FIXED_ONE, 0},
        ...
        {_normal_light,
         FLAG(_light_is_initially_active) | FLAG(_light_has_slaved_intensities),
         0, {_smooth_lighting_function, 2 * TICKS_PER_SECOND, 0, FIXED_ONE, 0},
        ...
        {_normal_light,
         FLAG(_light_is_initially_active) | FLAG(_light_has_slaved_intensities),
         0, {_random_lighting_function, 2, 1, FIXED_ONE, 0},
