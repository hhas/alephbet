...
enum /* static platform flags */
{
    ...
    _platform_is_initially_extended,        /* high for floor platforms, low for ceiling platforms, closed for two-way
                                               platforms */
    ...
    _platform_activates_adjacent_platforms_when_deactivating, /* when deactivating, this platform activates adjacent
                                                                 platforms */
    ...
    _platform_is_player_controllable,  /* i.e., door: players can use action key to change the state and/or direction of
                                          this platform */
    _platform_is_monster_controllable, /* i.e., door: monsters can expect to be able to move this platform even if
                                          inactive */
    ...
    _platform_uses_native_polygon_heights, /* complicated interpretation; uses native polygon heights during automatic
                                              min,max calculation */
...
#define PLATFORM_REVERSES_DIRECTION_WHEN_OBSTRUCTED(p) \
    TEST_FLAG32((p)->static_flags, _platform_reverses_direction_when_obstructed)
#define PLATFORM_ACTIVATES_ONLY_ONCE(p) TEST_FLAG32((p)->static_flags, _platform_activates_only_once)
#define PLATFORM_ACTIVATES_ADJACENT_PLATFORMS_WHEN_DEACTIVATING(p) \
    TEST_FLAG32((p)->static_flags, _platform_activates_adjacent_platforms_when_deactivating)
...
#define PLATFORM_CANNOT_BE_EXTERNALLY_DEACTIVATED(p) \
    TEST_FLAG32((p)->static_flags, _platform_cannot_be_externally_deactivated)
...
#define PLATFORM_ACTIVATES_ADJACENT_PLATFORMS_WHEN_ACTIVATING(p) \
    TEST_FLAG32((p)->static_flags, _platform_activates_adjacent_platforms_when_activating)
#define PLATFORM_DEACTIVATES_ADJACENT_PLATFORMS_WHEN_ACTIVATING(p) \
    TEST_FLAG32((p)->static_flags, _platform_deactivates_adjacent_platforms_when_activating)
#define PLATFORM_DEACTIVATES_ADJACENT_PLATFORMS_WHEN_DEACTIVATING(p) \
    TEST_FLAG32((p)->static_flags, _platform_deactivates_adjacent_platforms_when_deactivating)
#define PLATFORM_CONTRACTS_SLOWER(p) TEST_FLAG32((p)->static_flags, _platform_contracts_slower)
#define PLATFORM_ACTIVATES_ADJACENT_PLATFORMS_AT_EACH_LEVEL(p) \
    TEST_FLAG32((p)->static_flags, _platform_activates_adjacent_platforms_at_each_level)
...
#define SET_PLATFORM_EXTENDS_FLOOR_TO_CEILING(p, v) \
    SET_FLAG32((p)->static_flags, _platform_extends_floor_to_ceiling, (v))
...
#define SET_PLATFORM_REVERSES_DIRECTION_WHEN_OBSTRUCTED(p, v) \
    SET_FLAG32((p)->static_flags, _platform_reverses_direction_when_obstructed, (v))
#define SET_PLATFORM_ACTIVATES_ONLY_ONCE(p, v) SET_FLAG32((p)->static_flags, _platform_activates_only_once, (v))
#define SET_PLATFORM_ACTIVATES_ADJACENT_PLATFORMS_WHEN_DEACTIVATING(p, v) \
    SET_FLAG32((p)->static_flags, _platform_activates_adjacent_platforms_when_deactivating, (v))
...
#define SET_PLATFORM_DEACTIVATES_AT_EACH_LEVEL(p, v) \
    SET_FLAG32((p)->static_flags, _platform_deactivates_at_each_level, (v))
#define SET_PLATFORM_DEACTIVATES_AT_INITIAL_LEVEL(p, v) \
    SET_FLAG32((p)->static_flags, _platform_deactivates_at_initial_level, (v))
#define SET_PLATFORM_DOES_NOT_ACTIVATE_PARENT(p, v) \
    SET_FLAG32((p)->static_flags, _platform_does_not_activate_parent, (v))
...
#define SET_PLATFORM_CANNOT_BE_EXTERNALLY_DEACTIVATED(p, v) \
    SET_FLAG32((p)->static_flags, _platform_cannot_be_externally_deactivated, (v))
#define SET_PLATFORM_USES_NATIVE_POLYGON_HEIGHTS(p, v) \
    SET_FLAG32((p)->static_flags, _platform_uses_native_polygon_heights, (v))
#define SET_PLATFORM_DELAYS_BEFORE_ACTIVATION(p, v) \
    SET_FLAG32((p)->static_flags, _platform_delays_before_activation, (v))
#define SET_PLATFORM_ACTIVATES_ADJACENT_PLATFORMS_WHEN_ACTIVATING(p, v) \
    SET_FLAG32((p)->static_flags, _platform_activates_adjacent_platforms_when_activating, (v))
#define SET_PLATFORM_DEACTIVATES_ADJACENT_PLATFORMS_WHEN_ACTIVATING(p, v) \
    SET_FLAG32((p)->static_flags, _platform_deactivates_adjacent_platforms_when_activating, (v))
#define SET_PLATFORM_DEACTIVATES_ADJACENT_PLATFORMS_WHEN_DEACTIVATING(p, v) \
    SET_FLAG32((p)->static_flags, _platform_deactivates_adjacent_platforms_when_deactivating, (v))
#define SET_PLATFORM_CONTRACTS_SLOWER(p, v) SET_FLAG32((p)->static_flags, _platform_contracts_slower, (v))
#define SET_PLATFORM_ACTIVATES_ADJACENT_PLATFORMS_AT_EACH_LEVEL(p, v) \
    SET_FLAG32((p)->static_flags, _platform_activates_adjacent_platforms_at_each_level, (v))
...
#define PLATFORM_WAS_JUST_ACTIVATED_OR_DEACTIVATED(p) \
    TEST_FLAG16((p)->dynamic_flags, _platform_was_just_activated_or_deactivated)
...
#define CLEAR_PLATFORM_POSITIONING_FLAGS(p)                               \
    SET_FLAG16((p)->dynamic_flags, _platform_is_fully_contracted, false), \
            SET_FLAG16((p)->dynamic_flags, _platform_is_fully_extended, false)
#define SET_PLATFORM_WAS_JUST_ACTIVATED_OR_DEACTIVATED(p) \
    SET_FLAG16((p)->dynamic_flags, _platform_was_just_activated_or_deactivated, true)
#define CLEAR_PLATFORM_WAS_JUST_ACTIVATED_OR_DEACTIVATED(p) \
    SET_FLAG16((p)->dynamic_flags, _platform_was_just_activated_or_deactivated, false)
...
void adjust_platform_sides(platform_data* platform, world_distance old_ceiling_height,
...
short monster_can_enter_platform(short platform_index, short source_polygon_index, world_distance height,
                                 world_distance minimum_ledge_delta, world_distance maximum_ledge_delta);
short monster_can_leave_platform(short platform_index, short destination_polygon_index, world_distance height,
                                 world_distance minimum_ledge_delta, world_distance maximum_ledge_delta);
