...
#include "screen.hpp"
#include "shell.hpp" // for screen_printf()
#include "weapons.hpp"
...
struct player_powerup_durations_definition player_powerup_durations
        = {70 * TICKS_PER_SECOND, 50 * TICKS_PER_SECOND, 3 * TICKS_PER_MINUTE, 3 * TICKS_PER_MINUTE};
...
static struct player_shape_definitions player_shapes = {
        ...
        9,
        8, /* dying hard, dying soft */
        11,
        10, /* dead hard, dead soft */
        {7, 0, 0, 24, 23}, /* legs: stationary, walking, running, sliding, airborne */
        // LP additions: SMG-wielding/firing shapes (just before last two)
        {1, 3, 20, 26, 14, 12, 31, 16, 28, 33, 5,
                                     18}, /* idle torsos: fists, magnum, fusion, assault, rocket, flamethrower, alien, shotgun, double pistol,
  double shotgun, da ball */
        {1, 3, 21, 26, 14, 12, 31, 16, 28, 33, 5,
                                     18}, /* charging torsos: fists, magnum, fusion, assault, rocket, flamethrower, alien, shotgun, double pistol,
  double shotgun, ball */
        {2, 4, 22, 27, 15, 13, 32, 17, 28, 34, 6,
                                     19}, /* firing torsos: fists, magnum, fusion, assault, rocket, flamethrower, alien, shotgun, double pistol,
  double shotgun, ball */
};

#define NUMBER_OF_PLAYER_INITIAL_ITEMS (sizeof(player_initial_items) / sizeof(short))
static short player_initial_items[] = {_i_magnum, // First weapon is the weapon he will use...
                                       _i_knife, _i_knife, _i_magnum_magazine, _i_magnum_magazine, _i_magnum_magazine,

                                       // LP additions, in case one wants to start very loaded
                                       // AS: if we want to start loaded, shouldn't it be '_i_bong'?
                                       _i_knife, _i_knife, _i_knife, _i_knife, _i_knife,
...
struct player_powerup_definition player_powerups
        = {_i_invincibility_powerup, _i_invisibility_powerup,  _i_infravision_powerup, _i_extravision_powerup,
           _i_triple_energy_powerup, _i_double_energy_powerup, _i_energy_powerup,      _i_oxygen_powerup};
...
player_data* get_player_data(const size_t player_index) {
    ...
    vassert(data, csprintf(temporary, "asked for player #%zu/#%d", player_index, dynamic_world->player_count));
...
#ifdef BETA
    dprintf("#%d players at %p (%x bytes each) ---------------------------------------;g;", MAXIMUM_NUMBER_OF_PLAYERS,
            players, sizeof(struct player_data));
#endif
...
short new_player(short team, short color, short identifier, new_player_flags flags) {
...
void walk_player_list(void) {
    ...
    do {
        if ((player_index += 1) >= dynamic_world->player_count)
            player_index = 0;
        player = get_player_data(player_index);
...
void initialize_players(void) {
    ...
    for (i = 0; i < MAXIMUM_NUMBER_OF_PLAYERS; ++i) { obj_clear(players[i]); }
...
int get_ticks_since_local_player_in_terminal() { return sLocalPlayerTicksSinceTerminal; }

bool m1_solo_player_in_terminal() {
    return (static_world->environment_flags & _environment_terminals_stop_time) && (dynamic_world->player_count == 1)
...
void update_players(ActionQueues* inActionQueuesToUse, bool inPredictive) {
    ...
    for (player_index = 0, player = players; player_index < dynamic_world->player_count; ++player_index, ++player) {
        ...
        if ((((GET_GAME_TYPE() == _game_of_rugby) || (GET_GAME_TYPE() == _game_of_capture_the_flag))
             && (find_player_ball_color(player_index) != NONE))
            && !IsSwimming)
            action_flags &= ~_run_dont_walk;
        ...
        if (!inPredictive) {
            ...
            if (player->reincarnation_delay > 0) {
                ...
                if (((GET_GAME_OPTIONS() & _suicide_is_penalized) || (GET_GAME_OPTIONS() & _dying_is_penalized))
                    && (player_index == message_player_index)) {
                    if (player->reincarnation_delay == 0)
                        screen_printf("You may rise to fight again");
                    else if (player->reincarnation_delay < 4 * TICKS_PER_SECOND
                             && (player->reincarnation_delay % TICKS_PER_SECOND) == 0)
            ...
            // if ((static_world->environment_flags&_environment_vacuum) ||
            // (player->variables.flags&_HEAD_BELOW_MEDIA_BIT)) handle_player_in_vacuum(player_index, action_flags);
            ...
            if (PLAYER_IS_DEAD(player)) {
                ...
                if (PLAYER_IS_TOTALLY_DEAD(player) && (action_flags & _action_trigger_state)
                    && (player->variables.action == _player_stationary || dynamic_world->player_count == 1)) {
...
void damage_player(short monster_index, short aggressor_index, short aggressor_type, struct damage_definition* damage,
...

    {
        ...
        for (i = 0, definition = damage_response_definitions;
             definition->type != damage_type && i < NUMBER_OF_DAMAGE_RESPONSE_DEFINITIONS; ++i, ++definition);
        vwarn(i != NUMBER_OF_DAMAGE_RESPONSE_DEFINITIONS,
              csprintf(temporary, "can't react to damage type #%d", damage_type));
        // vassert(i!=NUMBER_OF_DAMAGE_RESPONSE_DEFINITIONS, csprintf(temporary, "can't react to damage type #%d",
        // damage_type));
    }
    ...
    if (damage_type != _damage_absorbed) {
        ...
        switch (damage->type) {
            case _damage_oxygen_drain: {
                ...
                L_Call_Player_Damaged(player_index, aggressor_player_index, aggressor_index, damage->type,
                                      damage_amount, projectile_index);
                ...
                if (player->suit_energy < 0) {
                    if (damage->type != _damage_energy_drain) {
                        if (!PLAYER_IS_DEAD(player)) {
                            ...
                            if (action == NONE) {
                                action = (damage_amount > PLAYER_MAXIMUM_SUIT_ENERGY / 2) ? _monster_is_dying_hard
                            ...
                            if (aggressor_player_index != NONE) {
                                ...
                                Console::instance()->report_kill(player_index, aggressor_player_index,
                                                                 projectile_index);
                            } else
...

    {
        ...
        if (player_index == current_player_index) {
            if (definition->fade != NONE)
                start_fade((definition->damage_threshhold != NONE && damage_amount > definition->damage_threshhold)
                                   ? (definition->fade + 1)
                                   : definition->fade);
            if (damage_amount)
...
player_shape_definitions* get_player_shape_definitions() { return &player_shapes; }
...
bool legal_player_powerup(short player_index, short item_index) {
    ...
    if ((static_world->environment_flags & _environment_m1_weapons) && film_profile.m1_bce_pickup) {
        return true;
    }
...
static void handle_player_in_vacuum(short player_index, uint32 action_flags) {
    ...
    if (player->suit_oxygen > 0) {
        ...
        while (oxygenChange < 0) {
            ...
            oxygenChange        += 1;
            if (player->suit_oxygen % breathing_frequency == 0 && player_index == current_player_index) {
            ...
            if (offset_o2 < OXYGEN_WARNING_LEVEL && offset_o2 % OXYGEN_WARNING_FREQUENCY == 0
                && player_index == current_player_index) {
    ...
    if (PLAYER_IS_TELEPORTING(player)) {
        switch (player->teleporting_phase += 1) {
            case PLAYER_TELEPORTING_MIDPOINT:
                if (player->teleporting_destination >= 0) /* Intralevel. */
                {
                    ...
                    damage_monsters_in_radius(NONE, NONE, NONE, &destination, destination_polygon_index, WORLD_ONE,
                                              &damage, NONE);
            ...
            case PLAYER_TELEPORTING_MIDPOINT + 1:
                ...
                if (player->teleporting_phase != PLAYER_TELEPORTING_DURATION)
                    break;
        ...
        if (player->teleporting_destination != NO_TELEPORTATION_DESTINATION
            || (((polygon->type == _polygon_is_automatic_exit
                  && calculate_level_completion_state() != _level_unfinished)
                 || polygon->type == _polygon_is_teleporter)
            ...
            if (--player->delay_before_teleport < 0) {
                ...
                if (player->teleporting_destination == NO_TELEPORTATION_DESTINATION) {
                    if (polygon->type == _polygon_is_automatic_exit
                        && calculate_level_completion_state() != _level_unfinished) {
                ...
                } else /* Level change */
                {
                    ...
                    for (other_player_index = 0; other_player_index < dynamic_world->player_count;
                         ++other_player_index) {
...
static void update_player_media(short player_index) {
...

    {
        ...
        if (player->variables.flags & _FEET_BELOW_MEDIA_BIT) {
            ...
            {
                world_distance current_magnitude = (player->variables.old_flags & _HEAD_BELOW_MEDIA_BIT)
                                                           ? media->current_magnitude
                                                           : (media->current_magnitude >> 1);
                world_distance external_magnitude
                        = FIXED_TO_WORLD(GUESS_HYPOTENUSE(std::abs(player->variables.external_velocity.i),
                                                          std::abs(player->variables.external_velocity.j)));
                struct damage_definition* damage = get_media_damage(
                        polygon->media_index,
                        (player->variables.flags & _HEAD_BELOW_MEDIA_BIT) ? FIXED_ONE : FIXED_ONE / 4);

                // apply current if possible
                if (!PLAYER_IS_DEAD(player) && external_magnitude < current_magnitude)
                    accelerate_player(player->monster_index, 0, NORMALIZE_ANGLE(media->current_direction - HALF_CIRCLE),
                                      media->current_magnitude >> 2);
                ...
                if (!(player->variables.old_flags & _FEET_BELOW_MEDIA_BIT))
                    sound_type = _media_snd_feet_entering;
                // head entering media sound
                if (!(player->variables.old_flags & _HEAD_BELOW_MEDIA_BIT)
                    && (player->variables.flags & _HEAD_BELOW_MEDIA_BIT))
                    sound_type = _media_snd_head_entering;
                // head leaving media sound
                if (!(player->variables.flags & _HEAD_BELOW_MEDIA_BIT)
                    && (player->variables.old_flags & _HEAD_BELOW_MEDIA_BIT))
                    sound_type = _media_snd_head_leaving;
            }
        } else {
            // feet leaving media sound
            if (polygon->media_index != NONE && (player->variables.old_flags & _FEET_BELOW_MEDIA_BIT))
                sound_type = _media_snd_feet_leaving;
        }
    ...
    if (!PLAYER_IS_DEAD(player)) {
        ...
        switch (mode) {
            case _shape_weapon_firing:
                torso_shape = player_shapes.firing_torsos[pseudo_weapon_type];
                break;
            case _shape_weapon_idle:
                torso_shape = player_shapes.torsos[pseudo_weapon_type];
                break;
            case _shape_weapon_charging:
                torso_shape = player_shapes.charging_torsos[pseudo_weapon_type];
                break;
            default:
                assert(false);
                break;
        ...
        new_legs_shape  = BUILD_DESCRIPTOR(BUILD_COLLECTION(player_shapes.collection, player->team),
                                           player_shapes.legs[player->variables.action]);
        ...
        if (legs->transfer_mode != transfer_mode)
            legs->transfer_mode = transfer_mode, legs->transfer_period = transfer_period, legs->transfer_phase = 0;
        if (torso->transfer_mode != transfer_mode)
            torso->transfer_mode = transfer_mode, torso->transfer_period = transfer_period, torso->transfer_phase = 0;

        /* stuff in new shapes only if they have changed (and reset phases if they have) */
        if (new_legs_shape != legs->shape)
            legs->shape = new_legs_shape, legs->sequence = 0;
        if (new_torso_shape != torso->shape)
            torso->shape = new_torso_shape, torso->sequence = 0;
    }

    if (animate) {
        /* animate the player only if we’re not airborne and not totally dead */
        if ((variables->action != _player_airborne
             || (PLAYER_IS_TELEPORTING(player) || PLAYER_IS_INTERLEVEL_TELEPORTING(player)))
            && !PLAYER_IS_TOTALLY_DEAD(player))
            animate_object(monster->object_index);
        if (PLAYER_IS_DEAD(player) && !PLAYER_IS_TELEPORTING(player)
            && (GET_OBJECT_ANIMATION_FLAGS(legs) & _obj_last_frame_animated) && !PLAYER_IS_TOTALLY_DEAD(player)) {
...
static void kill_player(short player_index, short aggressor_player_index, short action) {
    ...
    if (GET_GAME_OPTIONS() & _dying_is_penalized)
        player->reincarnation_delay += NORMAL_REINCARNATION_DELAY;
    if (aggressor_player_index == player_index && (GET_GAME_OPTIONS() & _suicide_is_penalized))
        player->reincarnation_delay += SUICIDE_REINCARNATION_DELAY;
    ...
    // START Benad: dec. 3
    {
        ...
        if (ball_color != NONE) {
            ...
            if (((GET_GAME_TYPE() == _game_of_rugby) || (GET_GAME_TYPE() == _game_of_capture_the_flag))
                ...
                drop_the_ball(&player->location, player->supporting_polygon_index, player->monster_index,
        ...
        while ((item_count -= 1) >= 0) {
            ...
            if (!(GET_GAME_OPTIONS() & _burn_items_on_death)
                ...
                if (item_kind != _ammunition || !(global_random() & 1)) {
                    ...
                    do {
                        random_point_on_circle(&player->location, player->supporting_polygon_index, WORLD_ONE_FOURTH,
                                               &random_point, &random_polygon_index);
            ...
            if (film_profile.count_dead_dropped_items_correctly || !dropped)
...
static void get_player_transfer_mode(short player_index, short* transfer_mode, short* transfer_period) {
    ...
    *transfer_mode   = NONE;
    if (PLAYER_IS_TELEPORTING(player)) {
        if (player->teleporting_destination >= 0
            || (player->teleporting_phase < PLAYER_TELEPORTING_MIDPOINT && View_DoInterlevelTeleportOutEffects())
            || (player->teleporting_phase >= PLAYER_TELEPORTING_MIDPOINT && View_DoInterlevelTeleportInEffects())) {
        ...
        if (player->invincibility_duration) {
            *transfer_mode = _xfer_static;
            ...
            if (player->invisibility_duration) {
                *transfer_mode = player->invisibility_duration > kINVISIBILITY_DURATION ? _xfer_subtle_invisibility
                                                                                        : _xfer_invisibility;
            ...
            switch (duration / (TICKS_PER_SECOND / 6)) {
                ...
                case 2:
                    if (player->invincibility_duration && player->invisibility_duration)
                        *transfer_mode = player->invisibility_duration > kINVISIBILITY_DURATION
...
static void adjust_player_physics(monster_data* me) {
    ...
    if (player_settings.PlayerShotsGuided) {
        ...
        // that seems to be OK
        SetPlayerViewAttribs(vacbob->half_visual_arc, vacbob->half_vertical_visual_arc / 3, vacbob->visual_range * 2,
...
uint8* unpack_player_data(uint8* Stream, player_data* Objects, size_t Count) {
    ...
    for (size_t k = 0; k < Count; k++, ObjPtr++) {
        ...
        for (int k = 0; k < MAXIMUM_NUMBER_OF_PLAYERS; k++) StreamToDamRec(S, ObjPtr->damage_taken[k]);
...
uint8* pack_player_data(uint8* Stream, player_data* Objects, size_t Count) {
    ...
    for (size_t k = 0; k < Count; k++, ObjPtr++) {
        ...
        for (int k = 0; k < MAXIMUM_NUMBER_OF_PLAYERS; k++) DamRecToStream(S, ObjPtr->damage_taken[k]);
...
void parse_mml_player(const InfoTree& root) {
    ...
    if (!original_player_settings) {
        original_player_settings
                = (struct player_settings_definition*)malloc(sizeof(struct player_settings_definition));
    ...
    if (!original_damage_response_definitions) {
        original_damage_response_definitions = (struct damage_response_definition*)malloc(
                sizeof(struct damage_response_definition) * NUMBER_OF_DAMAGE_RESPONSE_DEFINITIONS);
        assert(original_damage_response_definitions);
        for (unsigned i = 0; i < NUMBER_OF_DAMAGE_RESPONSE_DEFINITIONS; i++)
            original_damage_response_definitions[i] = damage_response_definitions[i];
    }
    if (!original_player_powerup_durations) {
        original_player_powerup_durations = (struct player_powerup_durations_definition*)malloc(
                sizeof(struct player_powerup_durations_definition));
