...
static media_definition* get_media_definition(const short type);
...
media_definition* get_media_definition(const short type) {
...
short get_media_sound(short media_index, short type) {
    ...
    if (!(type >= 0 && type < NUMBER_OF_MEDIA_SOUNDS))
        return NONE;
...
void reset_mml_liquids() {
    if (original_media_definitions) {
        for (int i = 0; i < NUMBER_OF_MEDIA_TYPES; i++) media_definitions[i] = original_media_definitions[i];
...
void parse_mml_liquids(const InfoTree& root) {
    ...
    if (!original_media_definitions) {
        original_media_definitions
                = (struct media_definition*)malloc(sizeof(struct media_definition) * NUMBER_OF_MEDIA_TYPES);
        assert(original_media_definitions);
        for (int i = 0; i < NUMBER_OF_MEDIA_TYPES; i++) original_media_definitions[i] = media_definitions[i];
    }
    ...
    for (const InfoTree& liquid : root.children_named("liquid")) {
        ...
        for (const InfoTree& dmg : liquid.children_named("damage")) { dmg.read_damage(def.damage); }
    }
}
