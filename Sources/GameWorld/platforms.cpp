...
#include "editor.hpp" // MARATHON_ONE_DATA_VERSION
...
short new_platform(struct static_platform_data* data, short polygon_index) {
    ...
    if (dynamic_world->platform_count < int(MAXIMUM_PLATFORMS_PER_MAP)) {
        ...
        for (i = 0; i < polygon->vertex_count; ++i) {
            calculate_endpoint_polygon_owners(polygon->endpoint_indexes[i],
...
void update_platforms(void) {
    ...
    for (platform_index = 0, platform = platforms; platform_index < dynamic_world->platform_count;
        ...
        if (PLATFORM_IS_ACTIVE(platform)) {
            ...
            if (PLATFORM_IS_MOVING(platform)) {
                ...
                world_distance delta_height = PLATFORM_IS_EXTENDING(platform)
                                                      ? platform->speed
                                                      : (PLATFORM_CONTRACTS_SLOWER(platform) ? (-(platform->speed >> 2))
                                                                                             : -platform->speed);
                ...
                if (PLATFORM_IS_FULLY_EXTENDED(platform)) {
                    if (PLATFORM_COMES_FROM_FLOOR(platform))
                        new_floor_height = platform->maximum_floor_height;
                    if (PLATFORM_COMES_FROM_CEILING(platform))
                        new_ceiling_height = platform->minimum_ceiling_height;
                }
                if (PLATFORM_IS_FULLY_CONTRACTED(platform)) {
                    if (PLATFORM_COMES_FROM_FLOOR(platform))
                        new_floor_height = platform->minimum_floor_height;
                    if (PLATFORM_COMES_FROM_CEILING(platform))
                        new_ceiling_height = platform->maximum_ceiling_height;
                }
                ...
                if (change_polygon_height(platform->polygon_index, new_floor_height, new_ceiling_height,
                                          PLATFORM_CAUSES_DAMAGE(platform) ? &definition->damage
                                                                           : (struct damage_definition*)NULL)) {
                    ...
                    if (PLATFORM_REVERSES_DIRECTION_WHEN_OBSTRUCTED(platform)) {
                        PLATFORM_IS_EXTENDING(platform) ? SET_PLATFORM_IS_CONTRACTING(platform)
                                                        : SET_PLATFORM_IS_EXTENDING(platform);
                    } else {
                ...
                if (PLATFORM_IS_FULLY_EXTENDED(platform) || PLATFORM_IS_FULLY_CONTRACTED(platform)) {
                    ...
                    if (PLATFORM_IS_FULLY_CONTRACTED(platform)) {
                        if (PLATFORM_IS_INITIALLY_CONTRACTED(platform)
...
short monster_can_enter_platform(short platform_index, short source_polygon_index, world_distance height,
    ...
    if (PLATFORM_IS_DOOR(platform)) {
        if (PLATFORM_IS_MONSTER_CONTROLLABLE(platform) && platform->delay >= _short_delay_platform) {
            ...
            result_code
                    = PLATFORM_IS_FULLY_CONTRACTED(platform) ? _platform_is_accessable : _platform_will_be_accessable;
        }
    } else {
        if (PLATFORM_IS_ACTIVE(platform) && PLATFORM_COMES_FROM_FLOOR(platform)
            && !PLATFORM_COMES_FROM_CEILING(platform)) {
            ...
            if (platform->maximum_floor_height != platform->minimum_ceiling_height
                ...
                result_code = (platform->floor_height == source_polygon->floor_height) ? _platform_is_accessable
    ...
    if (delta_height < minimum_ledge_delta || delta_height > maximum_ledge_delta
        || MIN(destination_ceiling_height, source_polygon->ceiling_height)
                           - MAX(destination_floor_height, source_polygon->floor_height)
                   < height) {
                                 ...
                                 world_distance minimum_ledge_delta, world_distance maximum_ledge_delta) /* negative */
{
    ...
    if (PLATFORM_IS_DOOR(platform)) {
        ...
        if (PLATFORM_IS_ACTIVE(platform) && PLATFORM_COMES_FROM_FLOOR(platform)
            ...
            if (platform->minimum_floor_height == destination_polygon->floor_height
                ...
                result_code = (platform->floor_height == destination_polygon->floor_height) ? _exit_is_accessable
    ...
    if (delta_height < minimum_ledge_delta || delta_height > maximum_ledge_delta
        || MIN(destination_polygon->ceiling_height, source_ceiling_height)
                           - MAX(destination_polygon->floor_height, source_floor_height)
                   < height) {
...
void player_touch_platform_state(short player_index, short platform_index) {
    ...
    if (PLATFORM_IS_PLAYER_CONTROLLABLE(platform)) {
        if (PLATFORM_IS_ACTIVE(platform)) {
            ...
            if (definition->key_item_index == NONE
                || try_and_subtract_player_item(player_index, definition->key_item_index)) {
...
bool platform_is_at_initial_state(short platform_index) {
    ...
    return (PLATFORM_HAS_BEEN_ACTIVATED(platform)
            && (!PLATFORM_IS_ACTIVE(platform) || PLATFORM_CANNOT_BE_EXTERNALLY_DEACTIVATED(platform)))
                   ? false
                   : true;
}
        ...
        if (!state || !PLATFORM_ACTIVATES_ONLY_ONCE(platform) || !PLATFORM_HAS_BEEN_ACTIVATED(platform)) {
            if ((state && !PLATFORM_IS_ACTIVE(platform)) || (!state && PLATFORM_IS_ACTIVE(platform))) {
                ...
                if (state) {
                    SET_PLATFORM_HAS_BEEN_ACTIVATED(platform);
                    SET_PLATFORM_IS_MOVING(platform, false);
                    platform->ticks_until_restart = PLATFORM_DELAYS_BEFORE_ACTIVATION(platform) ? platform->delay : 0;
                    ...
                    if (PLATFORM_ACTIVATES_ADJACENT_PLATFORMS_WHEN_ACTIVATING(platform))
                        set_adjacent_platform_states(platform_index, true);
                    if (PLATFORM_DEACTIVATES_ADJACENT_PLATFORMS_WHEN_ACTIVATING(platform))
                        set_adjacent_platform_states(platform_index, false);
                } else {
                    ...
                    if (PLATFORM_ACTIVATES_ADJACENT_PLATFORMS_WHEN_DEACTIVATING(platform))
                        set_adjacent_platform_states(platform_index, true);
                    if (PLATFORM_DEACTIVATES_ADJACENT_PLATFORMS_WHEN_DEACTIVATING(platform))
                        set_adjacent_platform_states(platform_index, false);
...
static void set_adjacent_platform_states(short platform_index, bool state) {
    ...
    for (i = 0; i < polygon->vertex_count; ++i) {
        ...
        if (adjacent_platform_index != NONE) {
            ...
            if (!PLATFORM_DOES_NOT_ACTIVATE_PARENT(platform)
                || platform->parent_platform_index != adjacent_platform_index) {
...
static void play_platform_sound(short platform_index, short type) {
    ...
    switch (type) {
        ...
        case _starting_sound:
            sound_code = PLATFORM_IS_EXTENDING(platform) ? definition->starting_extension
                                                         : definition->starting_contraction;
            break;
        case _stopping_sound:
            sound_code = PLATFORM_IS_FULLY_CONTRACTED(platform) ? definition->stopping_contraction
                                                                : definition->stopping_extension;
            break;
...
static void calculate_platform_extrema(short platform_index, world_distance lowest_level,
    ...
    if (PLATFORM_EXTENDS_FLOOR_TO_CEILING(platform)) {
        if (polygon->ceiling_height > highest_adjacent_floor)
            highest_adjacent_floor = polygon->ceiling_height;
        if (polygon->floor_height < lowest_adjacent_ceiling)
            lowest_adjacent_ceiling = polygon->floor_height;
    }
...
void adjust_platform_sides(platform_data* platform, world_distance old_ceiling_height,
    ...
    for (short i = 0; i < polygon->vertex_count; ++i) {
        ...
        if (adjacent_polygon_index != NONE) {
            side_index = adjacent_polygon_index == line->clockwise_polygon_owner
            ...
            switch (side->type) {
                ...
                case _low_side: /* primary */
                    ...
                    /*
                                        ...
                                        side->primary_texture.y0-= (old_ceiling_height<top_of_side_height &&
                       new_ceiling_height<top_of_side_height) ? delta_height : new_ceiling_height-top_of_side_height;
                    */
                    break;
...
uint8* unpack_platform_data(uint8* Stream, platform_data* Objects, size_t Count) {
    ...
    for (size_t k = 0; k < Count; k++, ObjPtr++) {
        ...
        for (int k = 0; k < MAXIMUM_VERTICES_PER_POLYGON; k++) StreamToEndpointOwner(S, ObjPtr->endpoint_owners[k]);
...
uint8* pack_platform_data(uint8* Stream, platform_data* Objects, size_t Count) {
    ...
    for (size_t k = 0; k < Count; k++, ObjPtr++) {
        ...
        for (int k = 0; k < MAXIMUM_VERTICES_PER_POLYGON; k++) EndpointOwnerToStream(S, ObjPtr->endpoint_owners[k]);
...
void reset_mml_platforms() {
    if (original_platform_definitions) {
        for (int i = 0; i < NUMBER_OF_PLATFORM_TYPES; i++) platform_definitions[i] = original_platform_definitions[i];
...
void parse_mml_platforms(const InfoTree& root) {
    ...
    if (!original_platform_definitions) {
        original_platform_definitions
                = (struct platform_definition*)malloc(sizeof(struct platform_definition) * NUMBER_OF_PLATFORM_TYPES);
        assert(original_platform_definitions);
        for (int i = 0; i < NUMBER_OF_PLATFORM_TYPES; i++) original_platform_definitions[i] = platform_definitions[i];
    }
    ...
    for (const InfoTree& ptree : root.children_named("platform")) {
        ...
        for (const InfoTree& dmg : ptree.children_named("damage")) { dmg.read_damage(def.damage); }
    }
}
