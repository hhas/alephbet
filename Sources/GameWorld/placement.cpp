...
/* private functions */
static void _recreate_objects(short object_type, short max_object_types,
                              struct object_frequency_definition* placement_info, short* object_counts,
                              short* random_counts);
...
void load_placement_data(uint8* _monsters, uint8* _items) {
...
#ifdef DEBUG
    {
        ...
        for (i = 1; i < NUMBER_OF_MONSTER_TYPES; i++) {
            if (monster_placement_info[i].initial_count < 0)
                dprintf("bad monster initial count.;g;");
            if (monster_placement_info[i].minimum_count < 0)
                dprintf("bad monster minimum count.;g;");
            if (monster_placement_info[i].maximum_count < 0)
                dprintf("bad monster maximum count.;g;");
        }

        for (i = 0; i < NUMBER_OF_DEFINED_ITEMS; i++) {
            if (item_placement_info[i].initial_count < 0)
                dprintf("bad item initial count.;g;");
            if (item_placement_info[i].minimum_count < 0)
                dprintf("bad item minimum count.;g;");
            if (item_placement_info[i].maximum_count < 0)
                dprintf("bad item maximum count.;g;");
        }
    }
#endif
 ...
 *
 *************************************************************************************************/
struct object_frequency_definition* get_placement_info(void) { return object_placement_info; }
...
void object_was_just_destroyed(short object_class, short object_type) {
    ...
    switch (object_class) {
        case _object_is_monster:
            dynamic_world->current_monster_count[object_type]--;
            diff = (GET_GAME_OPTIONS() & _monsters_replenish)
                           ? (monster_placement_info + object_type)->minimum_count
                                     - dynamic_world->current_monster_count[object_type]
                           : 0;
            break;
        ...
        case _object_is_item:
            ...
            if (dynamic_world->current_item_count[object_type])
                dynamic_world->current_item_count[object_type]--;
...
short get_random_player_starting_location_and_facing(short max_player_index, short team,
    ...
    for (starting_location_index = 0; starting_location_index < maximum_starting_locations; starting_location_index++) {
        index = get_player_starting_location_and_facing(
                team, (starting_location_index + offset) % maximum_starting_locations, &current_location);

        /* Determine the distances to the nearest monster and player */
        point_is_player_visible(max_player_index, current_location.polygon_index, (world_point2d*)&current_location.p,
                                &player_distance);
        point_is_monster_visible(current_location.polygon_index, (world_point2d*)&current_location.p,
                                 &monster_distance);

        if (monster_distance != 0 && player_distance != 0) {
            uint32 combined_distance = 1LL * player_distance
                                       + (monster_distance >> 1); // [0, ~2^31.6]; weight player distance more heavily.
 ...
 *
 *************************************************************************************************/
static void _recreate_objects(short object_type,      // _object_is_monster or _object_is_item
...
static void add_objects(short object_class, short object_type, short count, bool is_initial_drop) {
    ...
    flags      = (object_class == _object_is_monster) ? (monster_placement_info + object_type)->flags
    ...
    for (i = 0; i < count; i++) {
        ...
        if (need_random_location) {
            if (choose_invisible_random_point(&location.polygon_index, (world_point2d*)&location.p, object_class,
                                              is_initial_drop)) {
...
static bool pick_random_initial_location_of_type(short saved_type, short type, struct object_location* location) {
    ...
    for (i = 0; i < max; i++) {
        ...
        if (saved_object->type == saved_type && saved_object->index == type) {
            ...
            if (polygon_is_valid_for_object_drop((world_point2d*)&location->p, location->polygon_index, actual_type,
                                                 true, false)) {
                found_location = true;
                break;
            }
 ...
 *
 *************************************************************************************************/
static bool choose_invisible_random_point(short* polygon_index, world_point2d* p, short object_type,
                                          bool initial_drop) {
    short retries;
    ...
    switch (polygon->type) {
        ...
        default:
            if (!POLYGON_IS_DETACHED(polygon)) {
                if (!point_is_player_visible(dynamic_world->player_count, polygon_index, location, &distance)
                    || initial_drop) {
