...
static monster_definition* get_monster_definition(const short type);
...
int32 monster_pathfinding_cost_function(short source_polygon_index, short line_index, short destination_polygon_index,
                                        void* data);
int32 monster_m1_trigger_flood_proc(short source_polygon_index, short line_index, short destination_polygon_index,
                                    void* data);
...
static short position_monster_projectile(short aggressor_index, short target_index, struct attack_definition* attack,
                                         world_point3d* origin, world_point3d* destination, world_point3d* _vector,
                                         angle theta);
...
static int32 nearest_goal_cost_function(short source_polygon_index, short line_index, short destination_polygon_index,
                                        void* unused);
...
monster_data* get_monster_data(short monster_index) {
    ...
    vassert(SLOT_IS_USED(monster),
            csprintf(temporary, "monster index #%d (%p) is unused", monster_index, (void*)monster));
...
monster_definition* get_monster_definition_external(const short type) { return get_monster_definition(type); }

/* returns new monster index if successful, NONE otherwise */
short new_monster(struct object_location* location, short monster_type) {
    ...
    switch (dynamic_world->game_information.difficulty_level) {
        case _wuss_level:
            drop_mask = 3;
            break; /* drop every fourth monster */
        case _easy_level:
            drop_mask = 7;
            break; /* drop every eighth monster */
    ...
    if ((definition->flags & _monster_cannot_be_dropped) || !(definition->flags & _monster_is_alien)
        ...
        if (definition->flags & _monster_major) {
            ...
            switch (dynamic_world->game_information.difficulty_level) {
                case _wuss_level:
                    demote_mask = 1;
                    break; /* demote every other major */
                case _easy_level:
                    demote_mask = 3;
                    break; /* demote every fourth major */
                           /* otherwise, demote no monsters */
            }

            if (demote_mask != NONE && !(++dynamic_world->new_monster_mangler_cookie & demote_mask))
                definition = get_monster_definition(monster_type -= 1), flags |= _monster_was_demoted;
        } else {
            if (definition->flags & _monster_minor) {
                ...
                switch (dynamic_world->game_information.difficulty_level) {
                    case _major_damage_level:
                        promote_mask = 1;
                        break; /* promote every other minor */
                    case _total_carnage_level:
                        promote_mask = 0;
                        break; /* promote every minor */
                               /* otherwise, promote no monsters */
                }
                if (promote_mask != NONE && !(++dynamic_world->new_monster_mangler_cookie & promote_mask))
                    definition = get_monster_definition(monster_type += 1), flags |= _monster_was_promoted;
            }
        }

        for (monster_index = 0, monster = monsters; monster_index < MAXIMUM_MONSTERS_PER_MAP;
             ++monster_index, ++monster) {
            if (SLOT_IS_FREE(monster)) {
                short object_index = new_map_object(
                        location, BUILD_DESCRIPTOR(definition->collection, definition->stationary_shape));
                ...
                if (object_index != NONE) {
                    ...
                    if (location->flags & _map_object_is_blind)
                        flags |= _monster_is_blind;
                    if (location->flags & _map_object_is_deaf)
                        flags |= _monster_is_deaf;
                    if (location->flags & _map_object_floats)
                    ...
                    monster->vitality
                            = NONE; /* if a monster is activated with vitality==NONE, it will be properly initialized */
                    monster->object_index         = object_index;
                    monster->flags                = flags;
                    monster->goal_polygon_index   = monster->activation_bias == _activate_on_goal
                                                            ? nearest_goal_polygon_index(location->polygon_index)
                                                            : NONE;
                    ...
                    if (definition->flags & _monster_is_invisible)
                        object->transfer_mode = _xfer_invisibility;
                    if (definition->flags & _monster_is_subtly_invisible)
                        object->transfer_mode = _xfer_subtle_invisibility;
                    if (definition->flags & _monster_is_enlarged)
                        object->flags |= _object_is_enlarged;
                    if (definition->flags & _monster_is_tiny)
    ...
    if ((static_world->mission_flags & _mission_rescue_m1) && monster_index != NONE
            ...
            if (MONSTER_IS_ACTIVE(monster)) {
                if (!OBJECT_IS_INVISIBLE(object)) {
                    ...
                    if ((!monster->external_velocity && !monster->vertical_velocity)
                        || (film_profile.ketchup_fix
                            && (monster->action == _monster_is_attacking_close
                                || monster->action == _monster_is_attacking_far))
                        || ((monster->action != _monster_is_being_hit || !monster->external_velocity)
                            && (definition->flags & (_monster_floats | _monster_flys)))) {
                    ...
                    if (!monster_got_time && monster_index > dynamic_world->last_monster_index_to_get_time
                        && !MONSTER_IS_DYING(monster)) {
                        switch (monster->mode) {
                    ...
                    if (MONSTER_NEEDS_PATH(monster) && !MONSTER_IS_DYING(monster) && !MONSTER_IS_ATTACKING(monster)
                        && ((!monster_built_path && monster_index > dynamic_world->last_monster_index_to_build_path)
                            || monster->path == NONE)) {
                        ...
                        switch (monster->action) {
                            ...
                            case _monster_is_attacking_far:
                                ...
                                if (animation_flags & _obj_last_frame_animated) {
                                    if (((monster->attack_repetitions -= 1) < 0)
                                        || !try_monster_attack(monster_index)) {
                                        ...
                                        set_monster_action(monster_index,
                                                           (monster->attack_repetitions < 0
                                                            && (definition->flags & _monster_waits_with_clear_shot)
                                                            && MONSTER_IS_LOCKED(monster))
                            ...
                            case _monster_is_dying_flaming:
                                ...
                                if ((definition->flags & _monster_has_delayed_hard_death)
                                    && monster->action == _monster_is_dying_soft) {
                                    if (!monster->external_velocity
                                        && object->location.z
                                                   == monster->desired_height) //&& !monster->vertical_velocity)
                                    {
                                        set_monster_action(monster_index, _monster_is_dying_hard);
                                    } else {
                                        if (definition->contrail_effect != NONE)
                                            new_effect(&object->location, object->polygon, definition->contrail_effect,
                                                       object->facing);
                                    }
                                } else {
                                    // LP change: if keyframe is zero, then a monster should not produce shrapnel
                                    // damage. This fixes a side effect of a fix of the keyframe-never-zero bug, which
                                    // is that Hunters injure those nearby when they die a soft death.
                                    if (animation_flags & _obj_keyframe_started
                                        && (!film_profile.keyframe_fix || GET_SEQUENCE_FRAME(object->sequence) != 0))
                ...
                if (!monster_got_time && !MONSTER_IS_BLIND(monster)
                    && monster_index > dynamic_world->last_monster_index_to_get_time) {
                    ...
                    if (MONSTER_HAS_VALID_TARGET(monster))
                        activate_nearby_monsters(monster->target_index, monster_index, _pass_one_zone_border,
                                                 MONSTER_ALERT_ACTIVATION_RANGE);
...
static void load_sound(short sound_index) { SoundManager::instance()->LoadSound(sound_index); }
...
void mark_monster_collections(short monster_type, bool loading) {
    if (monster_type != NONE) {
        struct monster_definition* definition = get_monster_definition(monster_type);
...
void activate_nearby_monsters(
    ...
    if (dynamic_world->tick_count - caller->ticks_since_last_activation > MINIMUM_ACTIVATION_SEPARATION
        ...
        polygon_index = flood_map(polygon_index, max_cost, monster_activation_flood_proc, _flagged_breadth_first,
                                  &flood_flags);
        while (polygon_index != NONE) {
            ...
            for (object_index = get_polygon_data(polygon_index)->first_object; object_index != NONE;
                ...
                if (GET_OBJECT_OWNER(object) == _object_is_monster
                    ...
                    //					bool target_hostile= get_monster_attitude(aggressor_index,
                    //target_index)==_hostile; 					bool caller_hostile= get_monster_attitude(aggressor_index,
                    //caller_index)==_hostile;
                    ...
                    //					!MONSTER_IS_PLAYER(caller) ||
                    //TYPE_IS_FRIEND(get_monster_definition(aggressor->type), caller->type) || caller_hostile
                    ...
                    if (!MONSTER_IS_PLAYER(aggressor) && caller_index != aggressor_index
                        ...
                        && (!(flood_flags & _passed_zone_border)
                            || (!(aggressor->flags & _monster_has_never_been_activated)))
                        && ((flood_flags & _activate_deaf_monsters) || !MONSTER_IS_DEAF(aggressor))
                        && // || !MONSTER_IS_PLAYER(caller) || !TYPE_IS_FRIEND(get_monster_definition(aggressor->type),
                           // caller->type) || !caller_hostile) &&
                        ...
                        if (monster_was_active || !(flags & _use_activation_biases)
                            || (aggressor->activation_bias != _activate_on_goal
                                && aggressor->activation_bias != _activate_randomly)) {
            ...
            polygon_index
                    = flood_map(NONE, max_cost, monster_activation_flood_proc, _flagged_breadth_first, &flood_flags);
        }
...
static int32 monster_activation_flood_proc(short source_polygon_index, short line_index,
    ...
    } else if ((destination_polygon->type == _polygon_is_superglue) && ((*flags) & _cannot_pass_superglue)
               && obey_glue) {
        cost = -1;
    } else if ((destination_polygon->type == _polygon_is_glue) && !((*flags) & _activate_glue_monsters) && obey_glue) {
        cost = -1;
    } else if ((destination_polygon->type == _polygon_is_monster_impassable) && limit_activation) {
        cost = -1;
    } else if ((destination_polygon->type == _polygon_is_platform)
               && (destination_polygon->floor_height == destination_polygon->ceiling_height)
               && !((*flags) & _pass_solid_lines) && limit_activation) {
        cost = -1;
    }
...
void activate_monster(short monster_index) {
    ...
    monster->ticks_since_attack    = (definition->flags & _monster_attacks_immediately)
                                             ...
                                             : global_random() % definition->attack_frequency;
...
bool possible_intersecting_monsters(vector<short>* IntersectedObjectsPtr, unsigned maximum_object_count,
                                    short polygon_index, bool include_scenery) {
                ...
                if (!OBJECT_IS_INVISIBLE(object)) {
                    ...
                    if (solid_object) {
                        ...
                        if (IntersectedObjectsPtr
                            && IntersectedObjectsPtr->size()
                                       < maximum_object_count) /* do we have enough space to add it? */
                        {
                            unsigned j;
...
void monster_moved(short target_index, short old_polygon_index) {
    ...
    if (!MONSTER_IS_PLAYER(monster)) {
        ...
        activate_nearby_monsters(target_index, target_index,
                                 _pass_solid_lines | _activate_deaf_monsters | _activate_invisible_monsters
                                         | _use_activation_biases | _cannot_pass_superglue | _activate_glue_monsters);
    }
    ...
    for (monster_index = 0, monster = monsters; monster_index < MAXIMUM_MONSTERS_PER_MAP; ++monster_index, ++monster) {
        ...
        if (SLOT_IS_USED(monster) && MONSTER_HAS_VALID_TARGET(monster) && monster->target_index == target_index) {
            if (clear_line_of_sight(monster_index, target_index, true)) {
                ...
                if (monster->mode == _monster_losing_lock)
                    monster->changes_until_lock_lost += 1;
                set_monster_mode(monster_index,
                                 (monster->changes_until_lock_lost >= definition->intelligence) ? _monster_lost_lock
...
short legal_player_move(short monster_index, world_point3d* new_location,
        ...
        if (IntersectedObjects[i] != monster->object_index) /* no self-intersection */
        {
            ...
            if (new_distance_squared < separation_squared) {
                ...
                if (old_distance_squared > new_distance_squared) {
                    ...
                    //					dprintf("#%d (%d,%d) hit #%d (%d,%d) moving to (%d,%d)", monster_index,
                    //old_location->x, old_location->y, obstacle->permutation, obstacle_location->x,
                    //obstacle_location->y, new_location->x, new_location->y);
        ...
        if (IntersectedObjects[i] != monster->object_index) /* no self-intersection */
        {
            ...
            if (obstacle_location->z < new_location->z + height
                ...
                if (GET_OBJECT_OWNER(obstacle) != _object_is_scenery && obstacle->permutation > monster_index
                    ...
                    separation = (separation >> 1) + (separation >> 2);
...
void damage_monsters_in_radius(short primary_target_index, short aggressor_index, short aggressor_type,
                               world_point3d* epicenter, short epicenter_polygon_index, world_distance radius,
    ...
    std::vector<short> IntersectedObjects;
    possible_intersecting_monsters(&IntersectedObjects, LOCAL_INTERSECTING_MONSTER_BUFFER_SIZE, epicenter_polygon_index,
                                   false);
    ...
    for (size_t i = 0; i < object_count; ++i) {
        ...
        if (film_profile.damage_aggressor_last_in_tag && GET_GAME_TYPE() == _game_of_tag
            ...
            if (distance < radius + monster_radius) {
                if (epicenter->z + radius - distance > object->location.z
                    ...
                    if (!line_is_obstructed(epicenter_polygon_index, (world_point2d*)epicenter, object->polygon,
    ...
    if (film_profile.damage_aggressor_last_in_tag && aggressor != NULL) {
        ...
        if (distance < radius + monster_radius) {
            if (epicenter->z + radius - distance > aggressor->location.z
                ...
                if (!line_is_obstructed(epicenter_polygon_index, (world_point2d*)epicenter, aggressor->polygon,
                                        (world_point2d*)&aggressor->location)) {
...
void damage_monster(short target_index, short aggressor_index, short aggressor_type, world_point3d* epicenter,
                    struct damage_definition* damage, short projectile_index) {
    struct monster_data* monster          = get_monster_data(target_index);
    struct monster_definition* definition = get_monster_definition(monster->type);
    struct monster_data* aggressor_monster
            = aggressor_index != NONE ? get_monster_data(aggressor_index) : (struct monster_data*)NULL;
    ...
    if (!(definition->immunities & FLAG(damage->type))) {
        ...
        if (aggressor_index != NONE && get_monster_attitude(aggressor_index, target_index) == _friendly) {
            play_object_sound(aggressor_monster->object_index,
                              get_monster_definition(aggressor_monster->type)->apology_sound);
        }
        ...
        if (MONSTER_IS_PLAYER(monster)) {
            ...
            if (monster->vitality > 0) {
                ...
                if (!is_dying) {
                    ...
                    if ((damage_kick_definitions[damage->type].death_action == _monster_is_dying_flaming)
                        && (definition->flags & _monster_can_die_in_flames)) {
                        action = _monster_is_dying_flaming;
                    } else {
                        if ((damage_kick_definitions[damage->type].death_action == _monster_is_dying_hard
                             || ((FLAG(damage->type) & definition->weaknesses)
                                 && !(definition->flags & _monster_weaknesses_cause_soft_death))
                             || definition->soft_dying_shape == UNONE)
                            && definition->hard_dying_shape != UNONE
                            && !(definition->flags & _monster_has_delayed_hard_death)) {
                    ...
                    if (action == _monster_is_dying_flaming
                        || (damage->type == _damage_crushing && (definition->flags & _monster_screams_when_crushed)))
        ...
        if (damage->type >= 0 && damage->type < NUMBER_OF_DAMAGE_TYPES) {
            ...
            external_velocity
                    = (world_distance)(kick_def.base_value + kick_def.delta_vitality_multiplier * delta_vitality);
            vertical_component = kick_def.is_also_vertical;
            if (film_profile.use_vertical_kick_threshold && kick_def.vertical_threshold
        ...
        */
        ...
        if (MONSTER_IS_DYING(monster) && external_velocity < MINIMUM_DYING_EXTERNAL_VELOCITY)
...
bool legal_polygon_height_change(short polygon_index, world_distance new_floor_height,
...
void adjust_monster_for_polygon_height_change(short monster_index, short polygon_index, world_distance new_floor_height,
...
void accelerate_monster(short monster_index, world_distance vertical_velocity, angle direction,
    ...
    if (MONSTER_IS_PLAYER(monster)) {
        ...
        monster->vertical_velocity += PIN(monster->vertical_velocity + vertical_velocity,
                                          -TERMINAL_VERTICAL_MONSTER_VELOCITY, TERMINAL_VERTICAL_MONSTER_VELOCITY);
    }
}
...
static void cause_shrapnel_damage(short monster_index) {
    ...
    if (definition->shrapnel_radius != NONE) {
        damage_monsters_in_radius(NONE, NONE, NONE, &object->location, object->polygon, definition->shrapnel_radius,
                                  &definition->shrapnel_damage, NONE);
    }
}
...
static void update_monster_vertical_physics_model(short monster_index) {
    ...
    struct media_data* media
            = polygon->media_index == NONE ? (struct media_data*)NULL : get_media_data(polygon->media_index);
    uint32 moving_flags    = MONSTER_IS_DYING(monster) ? 0 : (definition->flags & (_monster_flys | _monster_floats));
    world_distance gravity = (static_world->environment_flags & _environment_low_gravity) ? (definition->gravity >> 1)
                                                                                          : definition->gravity;
    ...
    desired_height = (monster->desired_height == NONE || MONSTER_IS_DYING(monster)) ? polygon->floor_height
    ...
    switch (moving_flags) {
        case 0:
            ...
            if (below_ground)
                monster->vertical_velocity = 0, object->location.z = desired_height;
            break;
    ...
    object->location.z = PIN(object->location.z + monster->vertical_velocity, polygon->floor_height,
                             polygon->ceiling_height - definition->height);

    /* if we’re under the floor moving down, put us on the floor and clear our velocity;
        if we’re above the floor moving up, put us on the floor and clear our velocity if we were previously below
       ground */
    switch (moving_flags) {
        case 0:
        case _monster_floats:
            if (object->location.z <= desired_height && monster->vertical_velocity < 0)
                monster->vertical_velocity = 0, object->location.z = desired_height;
            if (object->location.z >= desired_height && monster->vertical_velocity > 0 && below_ground)
                monster->vertical_velocity = 0, object->location.z = desired_height;
            break;
        ...
        default: // LP: added this case to handle "Aqualung" correctly
            if (object->location.z <= desired_height && above_ground)
                monster->vertical_velocity >>= 1, object->location.z = desired_height;
    ...
    if (moving_flags & _monster_flys) {
        ...
        if (MONSTER_HAS_VALID_TARGET(monster)) {
            ...
            monster->desired_height = get_object_data(target->object_index)->location.z
                                      + ((target_definition->height - definition->height) >> 1)
                                      + definition->preferred_hover_height;
            monster->desired_height = PIN(monster->desired_height, floor_height + (definition->height >> 2),
                                          polygon->ceiling_height - definition->height);
        } else {
            if (monster->random_desired_height < floor_height
                || monster->random_desired_height > polygon->ceiling_height) {
            ...
            monster->desired_height
                    = MONSTER_IS_DYING(monster) ? polygon->floor_height : monster->random_desired_height;
        }
    } else {
...
static void update_monster_physics_model(short monster_index) {
    ...
    if (monster->external_velocity) {
        ...
        keep_line_segment_out_of_walls(object->polygon, &object->location, &new_location, 0, definition->height,
...
static void monster_needs_path(short monster_index, bool immediately) {
    ...
    if (monster->path != NONE && immediately)
        delete_path(monster->path), monster->path = NONE;
    if (monster->action == _monster_is_moving && immediately)
        set_monster_action(monster_index, _monster_is_stationary);
    SET_MONSTER_NEEDS_PATH_STATUS(monster, true);
}
...
void set_monster_mode(short monster_index, short new_mode, short target_index) {
    ...
    if (monster->mode == _monster_locked && new_mode != _monster_locked && monster->path == NONE)
        monster_needs_path(monster_index, false);
    ...
    switch (new_mode) {
        case _monster_locked:
            ...
            //			switch (monster->type)
            //			{
            //				case _civilian_crew: case _civilian_engineering: case _civilian_science: case
            //_civilian_security: 				dprintf("monster #%d is locked on new target #%d;g;", monster_index, target_index);
            //			}
            break;
...
static void generate_new_path_for_monster(short monster_index) {
    ...
    //	dprintf("#%d: generating new %spath for monster #%d;g;", dynamic_world->tick_count, destination?"":"random ",
    //monster_index);
    ...
    monster->path = new_path((world_point2d*)&object->location, object->polygon, destination, destination_polygon_index,
                             3 * definition->radius, monster_pathfinding_cost_function, &data);
    if (monster->path == NONE) {
        ...
        if (MONSTER_HAS_VALID_TARGET(monster) && monster->target_index == attacker_index) {
            ...
            if (!MONSTER_IS_DYING(attacker) && !(definition->flags & _monster_cannot_attack)) {
                ...
                if (mTYPE_IS_ENEMY(definition, attacker->type)
                    || (TYPE_IS_NEUTRAL(definition, attacker->type) && delta_vitality) || MONSTER_IS_BERSERK(monster)) {
                    if (!MONSTER_IS_ACTIVE(monster) || MONSTER_IS_IDLE(monster) || monster->mode != _monster_locked
...
static short get_monster_attitude(short monster_index, short target_index) {
    ...
    if (mTYPE_IS_ENEMY(definition, target_type) || MONSTER_IS_BERSERK(monster)
        ...
        || ((definition->_class & _class_human_civilian) && MONSTER_IS_PLAYER(target)
...
short find_closest_appropriate_target(short aggressor_index, bool full_circle) {
    ...
    if (MONSTER_IS_ACTIVE(aggressor)) {
        ...
        /* flood out from the aggressor monster’s polygon, searching through the object lists of all
            polygons we encounter */
        polygon_index = flood_map(polygon_index, INT32_MAX, monster_activation_flood_proc, _flagged_breadth_first,
                                  &flood_flags);
        while (polygon_index != NONE && closest_hostile_target_index == NONE) {
                ...
                if (GET_OBJECT_OWNER(object) == _object_is_monster && OBJECT_IS_VISIBLE(object)) {
                    ...
                    if (!MONSTER_IS_DYING(target_monster) && target_monster_index != aggressor_index) {
                        if (get_monster_attitude(aggressor_index, target_monster_index) == _hostile) {
                            if (((definition->flags & _monster_is_omniscent)
                                 || clear_line_of_sight(aggressor_index, target_monster_index, full_circle))
                                && (MONSTER_IS_ACTIVE(target_monster) || MONSTER_IS_PLAYER(target_monster)
            ...
            polygon_index
                    = flood_map(NONE, INT32_MAX, monster_activation_flood_proc, _flagged_breadth_first, &flood_flags);
        }
    } else {
        short player_index;
...
static bool clear_line_of_sight(short viewer_index, short target_index, bool full_circle) {
...

    {
        ...
        if (!full_circle) {
            ...
            if (std::abs(theta) > viewer_definition->half_visual_arc)
                target_visible = false;
            if (phi >= viewer_definition->half_vertical_visual_arc
                && phi < FULL_CIRCLE - viewer_definition->half_vertical_visual_arc)
                target_visible = false;
        }
        ...
        if (target_visible) {
            if (distance2d
                > viewer_definition
                          ->visual_range) // ||
                                          // (distance2d>viewer_definition->dark_visual_range&&get_object_light_intensity(target->object_index)<=LOW_LIGHT_INTENSITY))
            {
                target_visible = false;
            }
        }
        ...
        if (target_visible) {
            ...
            do {
                line_index = find_line_crossed_leaving_polygon(polygon_index, (world_point2d*)origin,
                                                               (world_point2d*)destination);
                if (line_index != NONE) {
                    ...
                    /* we got to the target’s (x,y) location, but we’re in a different polygon;
                        he’s invisible */
                    if (polygon_index != target_object->polygon)
                        target_visible = false;
                }
...
void change_monster_target(short monster_index, short target_index) {
    ...
    if (monster_index != target_index) {
        ...
        if (target_index != NONE) {
            ...
            if (monster->target_index != target_index
                && TYPE_IS_FRIEND(definition, get_monster_data(target_index)->type)) {
            ...
            if (MONSTER_HAS_VALID_TARGET(monster) && target_index != monster->target_index)
                CLEAR_TARGET_DAMAGE_FLAG(monster);
            monster_needs_path(monster_index, false);
    ...
    if (monster->path == NONE && monster->mode != _monster_locked && monster->action == _monster_is_stationary) {
        ...
        if (monster->action != _monster_is_waiting_to_attack_again) {
            if (translate_monster(monster_index, distance_moved)) {
                ...
                if (monster->action == _monster_is_stationary)
                    set_monster_action(monster_index, _monster_is_moving);
            } else {
                /* we couldn’t move: _monster_is_moving becomes _monster_is_stationary */
                if (monster->action == _monster_is_moving)
                    set_monster_action(monster_index, _monster_is_stationary);
        ...
        if (OBJECT_WAS_ANIMATED(object) && monster->mode == _monster_locked) {
            ...
            if (definition->flags & _monster_is_alien) {
                switch (dynamic_world->game_information.difficulty_level) {
                    case _wuss_level:
                        attack_frequency = 3 * attack_frequency;
                        break;
                    case _easy_level:
                        attack_frequency = 2 * attack_frequency;
                        break;
                    case _major_damage_level:
                        attack_frequency = attack_frequency >> 1;
                        break;
                    case _total_carnage_level:
                        attack_frequency = attack_frequency >> 2;
                        break;
                }
            }
    ...
    if (action == _monster_is_dying_flaming) {
        ...
        switch (action) {
            ...
            case _monster_is_teleporting_out:
                ...
                break;
            default:
                dprintf("what is monster action #%d?", action);
                assert(false);
                break;
        }
...
static void kill_monster(short monster_index) {
    ...
    switch (monster->action) {
        case _monster_is_dying_soft:
            shape = definition->soft_dead_shapes == UNONE
                            ? UNONE
                            : BUILD_DESCRIPTOR(definition->collection, definition->soft_dead_shapes);
            break;
        case _monster_is_dying_hard:
            shape = definition->hard_dead_shapes == UNONE
                            ? UNONE
    ...
    if (monster->flags & _monster_was_promoted)
        monster->type -= 1;
    if (monster->flags & _monster_was_demoted)
        monster->type += 1;
    ...
    if ((obstacle_index = legal_monster_move(monster_index, object->facing, &new_location)) == NONE) {
        ...
        switch (find_obstructing_terrain_feature(monster_index, &feature_index, &relevant_polygon_index)) {
            case _entering_platform_polygon:
                switch (monster_can_enter_platform(feature_index, relevant_polygon_index, definition->height,
                                                   definition->minimum_ledge_delta, definition->maximum_ledge_delta)) {
            ...
            case _leaving_platform_polygon:
                switch (monster_can_leave_platform(feature_index, relevant_polygon_index, definition->height,
                                                   definition->minimum_ledge_delta, definition->maximum_ledge_delta)) {
            ...
            case _flying_or_floating_transition:
                ...
                if (std::abs(object->location.z - monster->desired_height) > MINIMUM_FLOATING_HEIGHT)
                    legal_move = false;
                break;
        ...
        if (GET_OBJECT_OWNER(obstacle_object) == _object_is_monster) {
            ...
            if (!mTYPE_IS_ENEMY(definition, obstacle_monster->type)
                && !(MONSTER_HAS_VALID_TARGET(monster) && monster->target_index == obstacle_object->permutation)
                && !MONSTER_IS_BERSERK(monster)) {
                if (!MONSTER_IS_PLAYER(obstacle_monster)) {
                    if (monster->mode != _monster_locked) {
                        if (!MONSTER_HAS_VALID_TARGET(obstacle_monster)
                            || !switch_target_check(monster_index, obstacle_monster->target_index, 0)) {
                            if (monster->mode == _monster_unlocked && !(global_random() & OBSTRUCTION_DEACTIVATION_MASK)
                                && (monster->goal_polygon_index == NONE
                                    || monster->goal_polygon_index == object->polygon)) {
                                ...
                                if (monster->mode != _monster_locked) {
                                    ...
                                    set_monster_mode(monster_index, _monster_unlocked, NONE);
                                    //								dprintf("monster #%d going unlocked by
                                    //obstruction;g;", monster_index);
                                }
                            }
                        }
                    } else {
                ...
                if ((definition->flags & _monster_is_kamakazi) && object->location.z < key_height) {
...
static bool attempt_evasive_manouvers(short monster_index) {
    ...
    do {
        short line_index
                = find_line_crossed_leaving_polygon(polygon_index, (world_point2d*)&object->location, &destination);
        ...
        if (line_index == NONE) {
            ...
            if (LINE_IS_SOLID(get_line_data(line_index))
                || (polygon_index = find_adjacent_polygon(polygon_index, line_index)) == NONE) {
                polygon_index = NONE;
                successful    = false;
            } else {
                struct polygon_data* polygon = get_polygon_data(polygon_index);
...
void advance_monster_path(short monster_index) {
    ...
    if (monster->path == NONE) {
        ...
        if (monster->mode != _monster_locked
            || object->polygon != get_object_data(get_monster_data(monster->target_index)->object_index)->polygon) {
            monster_needs_path(monster_index, true);
            return;
        }
    } else {
...
static bool try_monster_attack(short monster_index) {
    ...
    if (MONSTER_HAS_VALID_TARGET(monster)) {
        ...
        if (!(definition->flags & _monster_cant_fire_backwards)
            || (delta_theta < QUARTER_CIRCLE + QUARTER_CIRCLE / 2
                || delta_theta > FULL_CIRCLE - QUARTER_CIRCLE - QUARTER_CIRCLE / 2)) {
            switch (monster->action) {
                case _monster_is_attacking_close:
                case _monster_is_attacking_far:
            ...
            if (new_action == _monster_is_attacking_close) {
                ...
                polygon_index
                        = position_monster_projectile(monster_index, monster->target_index, &definition->melee_attack,
                                                      &origin, &destination, &_vector, theta);
                if (preflight_projectile(&origin, polygon_index, &destination, definition->melee_attack.error,
                                         definition->melee_attack.type, monster_index, monster->type,
                                         &obstruction_index)) {
                    if ((obstruction_index != NONE
                         && get_monster_attitude(monster_index, obstruction_index) == _hostile)
                        || !line_is_obstructed(object->polygon, (world_point2d*)&object->location,
                                               target_object->polygon, (world_point2d*)&target_object->location)) {
                ...
                if (new_action == _monster_is_attacking_far) {
                    /* make sure this is a valid projectile, that we don’t hit any walls and that whatever
                        we did hit is _hostile. */
                    polygon_index = position_monster_projectile(monster_index, monster->target_index,
                                                                &definition->ranged_attack, &origin, &destination,
                                                                &_vector, theta);
                    if (preflight_projectile(&origin, polygon_index, &destination, definition->ranged_attack.error,
                                             definition->ranged_attack.type, monster_index, monster->type,
                                             &obstruction_index)) {
                        if ((obstruction_index != NONE
                             && get_monster_attitude(monster_index, obstruction_index) == _hostile)
                            || (obstruction_index == NONE
                                && !line_is_obstructed(object->polygon, (world_point2d*)&object->location,
                                                       target_object->polygon,
                                                       (world_point2d*)&target_object->location))) {
...
static void execute_monster_attack(short monster_index) {
    ...
    if (MONSTER_HAS_VALID_TARGET(monster)) {
        struct monster_definition* definition = get_monster_definition(monster->type);
        struct object_data* object            = get_object_data(monster->object_index);
        struct attack_definition* attack      = (monster->action == _monster_is_attacking_close)
        ...
        projectile_polygon_index = position_monster_projectile(monster_index, monster->target_index, attack, &origin,
                                                               (world_point3d*)NULL, &_vector, object->facing);
        if (projectile_polygon_index != NONE)
            new_projectile(&origin, projectile_polygon_index, &_vector, attack->error, attack->type, monster_index,
                           monster->type, monster->target_index, FIXED_ONE);
        if (definition->flags & _monster_fires_symmetrically) {
            attack->dy = -attack->dy;
            projectile_polygon_index
                    = position_monster_projectile(monster_index, monster->target_index, attack, &origin,
                                                  (world_point3d*)NULL, &_vector, object->facing);
            if (projectile_polygon_index != NONE)
                new_projectile(&origin, projectile_polygon_index, &_vector, attack->error, attack->type, monster_index,
                               monster->type, monster->target_index, FIXED_ONE);
...
int32 monster_pathfinding_cost_function(short source_polygon_index, short line_index, short destination_polygon_index,
    ...
    /* if we’re trying to move into a polygon with an area smaller than MINIMUM_MONSTER_PATHFINDING_POLYGON_AREA,
     * disallow the move */
    if (source_polygon->area < MINIMUM_MONSTER_PATHFINDING_POLYGON_AREA)
        cost = -1;

    // do platform stuff
    if (cost > 0) {
        if (destination_polygon->type == _polygon_is_platform) {
            switch (monster_can_enter_platform(destination_polygon->permutation, source_polygon_index,
                                               definition->height, definition->minimum_ledge_delta,
                                               definition->maximum_ledge_delta)) {
                case _platform_will_never_be_accessable:
                    cost = -1;
                    break;
                default:
        ...
        if (source_polygon->type == _polygon_is_platform) {
            switch (monster_can_leave_platform(source_polygon->permutation, destination_polygon_index,
                                               definition->height, definition->minimum_ledge_delta,
                                               definition->maximum_ledge_delta)) {
                case _exit_will_never_be_accessable:
                    cost = -1;
    ...
    if (respect_polygon_heights) {
        ...
        if (cost > 0)
            cost += delta_height * delta_height; /* prefer not to change heights */
    }
...
static short find_obstructing_terrain_feature(short monster_index, short* feature_index,
                                              short* relevant_polygon_index) {
    struct monster_data* monster          = get_monster_data(monster_index);
    struct monster_definition* definition = get_monster_definition(monster->type);
    struct object_data* object            = get_object_data(monster->object_index);
    ...
    ray_to_line_segment((world_point2d*)&object->location, &p1, object->facing,
                        MONSTER_PLATFORM_BUFFER_DISTANCE + definition->radius);
            ...
            default:
                ...
                if (!(definition->flags & (_monster_floats | _monster_flys)) && polygon->media_index != NONE) {
                    ...
                    if (media) {
                        ...
                        switch (media->type) {
                            case _media_water:
                                if (definition->flags & _monster_is_not_afraid_of_water)
                                    media = (struct media_data*)NULL;
                                break;
                            case _media_jjaro: // LP addition: monsters treat Jjaro goo like sewage
                            case _media_sewage:
                                if (definition->flags & _monster_is_not_afraid_of_sewage)
                                    media = (struct media_data*)NULL;
                                break;
                            case _media_lava: /* height= 0; */
                                if (definition->flags & _monster_is_not_afraid_of_lava)
                                    media = (struct media_data*)NULL;
                                break;
                            case _media_goo: /* height= 0; */
                                if (definition->flags & _monster_is_not_afraid_of_goo)
                                    media = (struct media_data*)NULL;
                                break;
                        }
                    }
...
static short position_monster_projectile(short aggressor_index, short target_index, struct attack_definition* attack,
    ...
    *origin    = aggressor_object->location;
    ...
    return find_new_object_polygon((world_point2d*)&aggressor_object->location, (world_point2d*)origin,
                                   aggressor_object->polygon);
}

short nearest_goal_polygon_index(short polygon_index) {
...
void SetPlayerViewAttribs(int16 half_visual_arc, int16 half_vertical_visual_arc, world_distance visual_range,
...
void parse_mml_damage_kicks(const InfoTree& root) {
    ...
    if (!original_damage_kick_definitions) {
        original_damage_kick_definitions = (struct damage_kick_definition*)malloc(sizeof(struct damage_kick_definition)
