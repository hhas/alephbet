...
#define SET_PRIMARY_WEAPON_IS_VALID(wd, v) \
    ((void)((v) ? ((wd)->flags |= _primary_weapon_is_up) : ((wd)->flags &= ~_primary_weapon_is_up)))
#define SET_SECONDARY_WEAPON_IS_VALID(wd, v) \
    ((void)((v) ? ((wd)->flags |= _secondary_weapon_is_up) : ((wd)->flags &= ~_secondary_weapon_is_up)))

#define SET_WEAPON_WANTS_TWOFIST(wd, v) \
    ((void)((v) ? ((wd)->flags |= _wants_twofist) : ((wd)->flags &= ~_wants_twofist)))
...
#define FLIP_WEAPON_VARIANCE_SIGN(wd) \
    (((wd)->flags & _flip_state) ? ((wd)->flags &= ~_flip_state) : ((wd)->flags |= _flip_state))
...
/*static*/ player_weapon_data* get_player_weapon_data(const short player_index);

static weapon_definition* get_weapon_definition(const short weapon_type);

static shell_casing_definition* get_shell_casing_definition(const short type);

/* -------------- accessors */

player_weapon_data* get_player_weapon_data(const short player_index) {
...
weapon_definition* get_weapon_definition(const short weapon_type) {
...
shell_casing_definition* get_shell_casing_definition(const short type) {
    shell_casing_definition* definition
            = GetMemberWithBounds(shell_casing_definitions, type, NUMBER_OF_SHELL_CASING_TYPES);
    assert(definition);
...
static struct trigger_data* get_player_trigger_data(short player_index, short which_trigger);
struct trigger_data* get_trigger_data(short player_index, short weapon_index, short which_trigger);
static struct weapon_data* get_player_current_weapon(short player_index);
static void fire_weapon(short player_index, short which_trigger, _fixed charged_amount, bool flail_wildly);
static struct trigger_definition* get_trigger_definition(short player_index, short which_weapon, short which_trigger);
...
static struct trigger_definition* get_player_trigger_definition(short player_index, short which_trigger);
...
static void calculate_weapon_position_for_idle(short player_index, short count, short weapon_type, _fixed* height,
                                               _fixed* width, bool use_elevation);
static void add_random_flutter(_fixed flutter_base, _fixed* height, _fixed* width);
static void calculate_weapon_origin_and_vector(short player_index, short which_trigger, world_point3d* origin,
                                               world_point3d* _vector, short* origin_polygon, angle delta_theta);
...
static short get_active_trigger_count_and_states(short player_index, short weapon_index, uint32 action_flags,
                                                 short* first_trigger, bool* triggers_down);
...
static bool get_weapon_data_type_for_count(short player_index, short count, short* type, short* index, short* flags);
...
inline bool CannotWieldWeapons() { return get_weapon_definition(_weapon_fist)->weapon_class == NONE; }
...
void initialize_weapon_manager(void) {
    player_weapons_array
            = (struct player_weapon_data*)malloc(MAXIMUM_NUMBER_OF_PLAYERS * sizeof(struct player_weapon_data));
...
void discharge_charged_weapons(short player_index) {
    ...
    if (player_has_valid_weapon(player_index)) {
        ...
        trigger_count = get_active_trigger_count_and_states(player_index, player_weapons->current_weapon, 0l,
                                                            &first_trigger, NULL);
                ...
                if (definition->item_type == item_type) {
                    ...
                    for (which_trigger = first_trigger; which_trigger < trigger_count; ++which_trigger) {
                        ...
                        if (definition->flags & _weapon_has_random_ammo_on_pickup) {
                            ...
                            player_weapons->weapons[weapon_type].triggers[which_trigger].rounds_loaded = rounds_given;
                    ...
                    if (should_switch_to_weapon(player_index, weapon_type)) {
                        if (!ready_weapon(player_index, weapon_type)) {
                            dprintf("Error! Unable to ready something I should: %d weapon: %d;g", player_index,
                                    weapon_type);
                        }
                    }
        ...
        case _ammunition:
            ...
            if (player_has_valid_weapon(player_index)) {
                for (unsigned weapon_type = 0; weapon_type < NUMBER_OF_WEAPONS; ++weapon_type) {
                    ...
                    trigger_count
                            = get_active_trigger_count_and_states(player_index, weapon_type, 0l, &first_trigger, NULL);
                    for (which_trigger = first_trigger; which_trigger < trigger_count; ++which_trigger) {
                        struct player_weapon_data* player_weapons = get_player_weapon_data(player_index);
                        struct trigger_definition* trigger_definition
                                = get_trigger_definition(player_index, weapon_type, which_trigger);
...
void update_player_weapons(short player_index, uint32 action_flags) {
    ...
    if (player_has_valid_weapon(player_index)) {
        ...
        trigger_count = get_active_trigger_count_and_states(player_index, weapon->weapon_type, action_flags,
                                                            &first_trigger, triggers_down);
        ...
        if (player->weapon_intensity_decay > 0) {
            if (definition->firing_intensity_decay_ticks) {
                player->weapon_intensity = NATURAL_LIGHT_INTENSITY +
                                           ...
                                           ((definition->firing_light_intensity) * player->weapon_intensity_decay)
                                                   / definition->firing_intensity_decay_ticks;
                ...
                switch (trigger->state) {
                    ...
                    case _weapon_lowering:
                        if (definition->item_type == _i_red_ball) {
                            ...
                            if (ball_color != NONE) {
                                ...
                                calculate_weapon_origin_and_vector(player_index, _primary_weapon, &origin, &_vector,
                                                                   &origin_polygon, 0);

                                drop_the_ball(&origin, origin_polygon, player->monster_index, _monster_marine,
                                              item_type);
                    ...
                    case _weapon_firing: {
                        ...
                        if (trigger_definition->recovery_ticks == 0) {
                            ...
                            if (which_trigger == _primary_weapon && TRIGGER_IS_DOWN(weapon)
...
void debug_print_weapon_status(void) {
    ...
    for (unsigned index = 0; index < NUMBER_OF_WEAPONS; ++index) { debug_weapon(index); }
}
...
static void debug_trigger_data(short weapon_type, short which_trigger) {
    ...
    dprintf("%d) Loaded: %d Shot: %d Hit: %d;g", which_trigger, trigger->rounds_loaded, trigger->shots_fired,
            trigger->shots_hit);
    dprintf("%d) Ticks Since Last: %d;g", which_trigger, trigger->ticks_since_last_shot);

    trigger_definition = get_trigger_definition(current_player_index, weapon_type, which_trigger);

    if (weapon_definition->item_type != NONE) {
        dprintf("%d) Has %d items;g", which_trigger, current_player->items[weapon_definition->item_type]);
    } else {
        dprintf("%d) Item type is NONE;g", which_trigger);
    }

    if (trigger_definition->ammunition_type != NONE) {
        dprintf("%d) Player has %d clips;g", which_trigger, current_player->items[trigger_definition->ammunition_type]);
    } else {
...
void* get_weapon_array(void) { return player_weapons_array; }

int32 calculate_weapon_array_length(void) { return dynamic_world->player_count * sizeof(struct player_weapon_data); }
        ...
        if (get_weapon_data_type_for_count(player_index, *count, &type, &which_trigger, &flags)) {
            ...
            if (type == _weapon_type || type == _weapon_ammo_type) {
                ...
                if (definition->flags & _weapon_is_marathon_1) {
                    ...
                    if (weapon->triggers[which_trigger].state == _weapon_idle
                        && !automatic_still_firing(player_index, which_trigger)) {
                        calculate_weapon_position_for_idle(player_index, which_trigger, weapon->weapon_type, &height,
                                                           &width, false);
                    }
                } else {
                    calculate_weapon_position_for_idle(player_index, which_trigger, weapon->weapon_type, &height,
                ...
                switch (weapon->triggers[which_trigger].state) {
                    case _weapon_lowering:
                        ...
                        height = (3 * FIXED_ONE / 2)
                                 - (((3 * FIXED_ONE / 2) - definition->idle_height) * phase) / definition->ready_ticks;
                    ...
                    case _weapon_idle:
                        if (definition->flags & _weapon_is_automatic) {
                            if (which_trigger == _primary_weapon
                                || (which_trigger == _secondary_weapon
                                    && (definition->flags & _weapon_secondary_has_angular_flipping))) {
                    ...
                    case _weapon_sliding_over_to_second_position:
                        if (definition->weapon_class == _twofisted_pistol_class) {
                            if (which_trigger == _primary_weapon) {
                                width -= ((weapon->triggers[which_trigger].phase) * (PISTOL_SEPARATION_WIDTH / 2))
                    ...
                    case _weapon_sliding_over_from_second_position:
                        if (definition->weapon_class == _twofisted_pistol_class) {
                            ...
                            width += sign
                                     * ((definition->ready_ticks - weapon->triggers[which_trigger].phase)
                                        * (PISTOL_SEPARATION_WIDTH / 2))
                                     / definition->ready_ticks;
                        } else {
                            /* Melee weapons stay where they were. */
                        }
                    ...
                    case _weapon_firing:
                        /* We are going up.. */
                        {
                            ...
                            height -= (definition->kick_height * (trigger_definition->ticks_per_round - phase))
                                      / trigger_definition->ticks_per_round;
                            shape_index = definition->firing_shape;
                            if (definition->flags & _weapon_flutters_while_firing) {
                                add_random_flutter(FIXED_ONE, &height, &width);
                            }
                    ...
                    case _weapon_awaiting_reload:
                        if (definition->reloading_shape == NONE) {
                            ...
                            height += (((3 * FIXED_ONE / 2) - definition->idle_height)
                                       * (definition->await_reload_ticks - phase))
                    ...
                    case _weapon_finishing_reload:
                        if (definition->reloading_shape == NONE) {
                            ...
                            height += (((3 * FIXED_ONE / 2) - definition->idle_height) * phase)
                                      / definition->finish_loading_ticks;
                ...
                high_level_data = get_shape_animation_data(BUILD_DESCRIPTOR(definition->collection, shape_index));
                ...
                if (!(*count)) {
                    ...
                    if (owner_transfer_data.collection_code == NONE)
                        return false;
...
void get_player_weapon_mode_and_type(short player_index, short* shape_weapon_type, short* shape_mode) {
    ...
    if (player_has_valid_weapon(player_index)) {
        ...
        *shape_weapon_type = player_weapons->current_weapon;
        ...
        if (definition->weapon_class == _twofisted_pistol_class && PRIMARY_WEAPON_IS_VALID(weapon_data)
            && SECONDARY_WEAPON_IS_VALID(weapon_data)) {
...
/*static*/ void select_next_best_weapon(short player_index) {
    ...
    for (index = current_weapon_index + 1; index < NUMBER_OF_WEAPONS; ++index) {
        if (ready_weapon(player_index, weapon_ordering_array[index]))
            break;
    }
...
static void select_next_weapon(short player_index, bool forward) {
    ...
    if (player_weapons->desired_weapon != _weapon_ball && player_weapons->current_weapon != _weapon_ball) {
        ...
        for (unsigned index = 1; index < NUMBER_OF_WEAPONS; ++index) {
            ...
            if (ready_weapon(player_index, weapon_ordering_array[test_weapon_index]))
                break;
        }
    }
}
...
static void fire_weapon(short player_index, short which_trigger, _fixed charged_amount, bool flail_wildly) {
    ...
    if (trigger->rounds_loaded > 0) {
        ...
        calculate_weapon_origin_and_vector(player_index, which_trigger, &origin, &_vector, &origin_polygon,
                                           delta_theta);
        ...
        while (rounds_to_fire--) {
            ...
            if (definition->weapon_class == _dual_function_class) {
                ...
                if (which_trigger == _secondary_weapon && trigger_definition->charging_ticks) {
                    ...
                    rounds_count = MIN(COST_PER_CHARGED_WEAPON_SHOT - 1,
                                       player_weapons->weapons[player_weapons->current_weapon]
                                               .triggers[which_trigger]
                                               .rounds_loaded);
            ...
            if (trigger_definition->projectile_type != _projectile_ball_dropped) {
                short Target = player_settings.PlayerShotsGuided
                                       ? (ProjectileIsGuided(trigger_definition->projectile_type)
                                                  ...
                                                  : NONE)
                                       : NONE;
                new_projectile(&origin, origin_polygon, &_vector, trigger_definition->theta_error + flailing_bonus,
                               trigger_definition->projectile_type, player->monster_index, _monster_marine, Target,
                               damage_modifier);
        ...
        /* uh oh, no bullets. */
        while (rounds_to_fire--) { play_weapon_sound(player_index, trigger_definition->click_sound, FIXED_ONE); }
    }
}
...
static void calculate_weapon_origin_and_vector(short player_index, short which_trigger, world_point3d* origin,
    ...
    *origin    = player->camera_location;
    ...
    translate_point3d(origin, WEAPON_FORWARD_DISPLACEMENT, player->facing, player->elevation);
    ...
    translate_point2d((world_point2d*)origin, dx_translation_amount, NORMALIZE_ANGLE(player->facing + QUARTER_CIRCLE));

    /* Get a second point to build up a vector.. */
    destination       = *origin;
    projectile_facing = NORMALIZE_ANGLE(player->facing + delta_theta);
    translate_point3d(&destination, WORLD_ONE_HALF, projectile_facing, player->elevation);
    ...
    *origin_polygon
            = find_new_object_polygon((world_point2d*)&player->location, (world_point2d*)origin, object->polygon);
    ...
    if (*origin_polygon == NONE) {
        ...
        while (source_polygon != NONE) {
            line_crossed   = find_line_crossed_leaving_polygon(source_polygon, (world_point2d*)&player->location,
        ...
        find_line_intersection(&get_endpoint_data(line->endpoint_indexes[0])->vertex,
                               &get_endpoint_data(line->endpoint_indexes[1])->vertex, &player->location, origin,
                               origin);
        ...
        *origin = player->location;
        translate_point2d((world_point2d*)origin, distance, NORMALIZE_ANGLE(player->facing + QUARTER_CIRCLE));
...
static bool reload_weapon(short player_index, short which_trigger) {
    ...
    if (!(trigger->state == _weapon_idle && trigger->rounds_loaded == 0))
        return false;
    if (!(trigger_definition->ammunition_type == NONE
          || (trigger_definition->ammunition_type >= 0 && trigger_definition->ammunition_type < NUMBER_OF_ITEMS)))
        return false;
    ...
    if (trigger_definition->ammunition_type != NONE && player->items[trigger_definition->ammunition_type] > 0) {
        ...
        if (definition->weapon_class == _twofisted_pistol_class && !(definition->flags & _weapon_reloads_in_one_hand)
        ...
        switch (definition->weapon_class) {
            ...
            case _twofisted_pistol_class: {
                ...
                if (PRIMARY_WEAPON_IS_VALID(weapon) && SECONDARY_WEAPON_IS_VALID(weapon)) {
                    if (!reload_weapon(player_index, which_trigger)) {
                        ...
                        struct trigger_data* other_trigger_data = get_player_trigger_data(player_index, !which_trigger);
            ...
            case _multipurpose_class:
                ...
                if (!reload_weapon(player_index, which_trigger)) {
                    ...
                    other_trigger_data = get_player_trigger_data(player_index, other_trigger);
...
static bool handle_trigger_down(short player_index, short which_trigger) {
    ...
    if (weapon->triggers[which_trigger].state == _weapon_idle) {
        ...
        if (trigger_definition->charging_ticks) {
            ...
            if (definition->weapon_class != _dual_function_class
                || (definition->weapon_class == _dual_function_class
                ...
                case _twofisted_pistol_class:
                    ...
                    if (PRIMARY_WEAPON_IS_VALID(weapon) && SECONDARY_WEAPON_IS_VALID(weapon)) {
                        if (definition->flags & _weapon_fires_out_of_phase) {
                            ...
                            struct trigger_definition* other_trigger_definition
                                    = get_player_trigger_definition(player_index, !which_trigger);

                            total_delay = other_trigger_definition->recovery_ticks
                                          + other_trigger_definition->ticks_per_round;
                            delay = total_delay - weapon->triggers[!which_trigger].phase;
                            switch (weapon->triggers[!which_trigger].state) {
                                case _weapon_firing:
                                    delay -= trigger_definition->recovery_ticks;
                                case _weapon_recovering:
                                    break;
...
static bool handle_trigger_up(short player_index, short which_trigger) {
    ...
    switch (weapon->triggers[which_trigger].state) {
        case _weapon_charging:
            discharge      = true;
            charged_amount = ((trigger_definition->charging_ticks - weapon->triggers[which_trigger].phase) * FIXED_ONE)
                             / FIXED_ONE;
            break;
    ...
    if (discharge) {
        ...
        if (!PLAYER_IS_TELEPORTING(player) && !PLAYER_IS_INTERLEVEL_TELEPORTING(player)) {
            if (charged_amount > (9 * FIXED_ONE) / 10) {
                ...
                weapon->triggers[which_trigger].sequence = 0;
                SoundManager::instance()->StopSound(player_index == current_player_index ? NONE : player->object_index,
                                                    trigger_definition->charging_sound);
            }
        }
    }
...
*/
bool ready_weapon(short player_index, short weapon_index) {
...
static bool player_weapon_has_ammo(short player_index, short weapon_index) {
    ...
    if (definition->weapon_class == _melee_class) {
        ...
        for (which_trigger = _primary_weapon; !has_ammo && which_trigger < max_triggers; ++which_trigger) {
            struct trigger_data* trigger = get_trigger_data(player_index, weapon_index, which_trigger);
...
static void lower_weapon(short player_index, short weapon_index) {
    ...
    active_trigger_count = get_active_trigger_count_and_states(player_index, weapon_index, 0l, &first_trigger, NULL);
    ...
    for (which_trigger = first_trigger; which_trigger < active_trigger_count; ++which_trigger) {
        ...
        if (trigger->state != _weapon_raising) {
            ...
            if (trigger->state == _weapon_idle) {
                ...
                vassert(trigger->state == _weapon_lowering,
                        csprintf(temporary, "State: %d phase: %d trigger: %d weapon: %d", trigger->state,
                                 trigger->phase, which_trigger, weapon_index));
                trigger->phase = definition->ready_ticks - trigger->phase;
...
static void calculate_weapon_position_for_idle(short player_index, short count, short weapon_type, _fixed* height,
    ...
    if (count == 0) {
        ...
        table            = cosine_table;
        vertical_angle   = NORMALIZE_ANGLE((player->variables.step_phase >> (FIXED_FRACTIONAL_BITS - ANGULAR_BITS))
                                           + HALF_CIRCLE);
        horizontal_phase = player->variables.step_phase;
    }
    ...
    if (graphics_preferences->screen_mode.bobbing_type == BobbingType::none)
        bob_height = 0;
    if (use_elevation)
    ...
    switch (definition->weapon_class) {
        ...
        case _twofisted_pistol_class: {
            ...
            if (PRIMARY_WEAPON_IS_VALID(weapon) && SECONDARY_WEAPON_IS_VALID(weapon)) {
                switch (count) {
                    case 0:
                        *width += PISTOL_SEPARATION_WIDTH / 2;
                        break;
                    case 1:
                        *width -= PISTOL_SEPARATION_WIDTH / 2;
                        break;
...
static void add_random_flutter(_fixed flutter_base, _fixed* height, _fixed* width) {
    ...
    if (delta_height)
        *height += (rand() % delta_height) - (delta_height / 2);
    if (delta_width)
        *width += (rand() % delta_width) - (delta_width / 2);
}
...
static short get_active_trigger_count_and_states(short player_index, short weapon_index, uint32 action_flags,
    ...
    switch (weapon_class) {
        case _normal_class:
            active_count = 1;
            if (triggers_down) {
                if (action_flags & _left_trigger_state || action_flags & _right_trigger_state)
                    triggers_down[0] = true;
            }
            break;
        ...
        case _twofisted_pistol_class: {
            ...
            if (player->items[definition->item_type] > 1 && PRIMARY_WEAPON_IS_VALID(weapon)
                && SECONDARY_WEAPON_IS_VALID(weapon)) {
                if (triggers_down) {
                    if (action_flags & _left_trigger_state)
                        triggers_down[0] = true;
                    if (action_flags & _right_trigger_state)
                        triggers_down[1] = true;
                }
                active_count = 2;
            } else {
                if (SECONDARY_WEAPON_IS_VALID(weapon)) {
...
static void calculate_ticks_from_shapes(void) {
    for (unsigned weapon_type = 0; weapon_type < NUMBER_OF_WEAPONS; ++weapon_type) {
        ...
        if (definition->weapons_by_trigger[_primary_weapon].ticks_per_round == NONE) {
            ...
            high_level_data
                    = get_shape_animation_data(BUILD_DESCRIPTOR(definition->collection, definition->firing_shape));
            ...
            if (definition->flags & _weapon_is_marathon_1) {
                ...
                primary.ticks_per_round = (high_level_data->key_frame + 1) * high_level_data->ticks_per_frame;
                primary.recovery_ticks  = total_ticks - definition->weapons_by_trigger[_primary_weapon].ticks_per_round;
            }
            ...
            if (definition->weapon_class == _melee_class || definition->weapon_class == _twofisted_pistol_class) {
                if (definition->flags & _weapon_is_marathon_1) {
                    ...
                    secondary.ticks_per_round = (high_level_data->key_frame + 1) * high_level_data->ticks_per_frame;
                    secondary.recovery_ticks
                            = total_ticks - definition->weapons_by_trigger[_secondary_weapon].ticks_per_round;
                }
            }
        }
        ...
        if (definition->reloading_shape != NONE) {
            ...
            high_level_data
                    = get_shape_animation_data(BUILD_DESCRIPTOR(definition->collection, definition->reloading_shape));
            // Skip over if the sequence is nonexistent
            if (!high_level_data)
                continue;
            ...
            if (definition->flags & _weapon_is_marathon_1 && film_profile.m1_reload_sound) {
            ...
            definition->finish_loading_ticks = total_ticks - definition->loading_ticks - definition->await_reload_ticks;
        } else {
            /* This shape doesn't have a reloading shape, therefore the reload */
            /*  time is calculated here. */
            short total_ticks
                    = definition->await_reload_ticks + definition->loading_ticks + definition->finish_loading_ticks;
    ...
    if (definition->flags & _weapon_is_automatic) {
        if (which_trigger == _primary_weapon
            ...
            switch (trigger->state) {
                case _weapon_firing:
                case _weapon_recovering:
                    high_level_data = get_shape_animation_data(
                            BUILD_DESCRIPTOR(definition->collection, definition->firing_shape));
                    break;
...
static void UpdateIdleAnimation(short player_index, short which_trigger) {
    ...
    if (animation->frames_per_view >= 1 && animation->number_of_views != _unanimated
        ...
        if (!frame && (!phase || phase >= animation->ticks_per_frame))
            play_weapon_sound(player_index, animation->first_frame_sound, FIXED_ONE);

        /* phase is left unadjusted if it goes over ticks_per_frame until the next call */
        if (phase >= animation->ticks_per_frame)
            phase -= animation->ticks_per_frame;
        if ((phase += 1) >= animation->ticks_per_frame) {
            ...
            if (frame >= animation->frames_per_view) {
                ...
                if (animation->last_frame_sound != NONE)
                    play_weapon_sound(player_index, animation->last_frame_sound, FIXED_ONE);
            }
            ...
            if (frame == animation->key_frame || offset_frame == animation->key_frame) {
                if (animation->key_frame_sound != NONE)
                    play_weapon_sound(player_index, animation->first_frame_sound, FIXED_ONE);
            }
        }
...
static void update_sequence(short player_index, short which_trigger) {
    ...
    switch (trigger->state) {
        ...
        case _weapon_recovering:
            ...
            if ((which_trigger == _primary_weapon && (definition->flags & _weapon_is_automatic))
                || (which_trigger == _secondary_weapon && (definition->flags & _weapon_is_automatic)
                    && (definition->flags & _weapon_secondary_has_angular_flipping))) {
        ...
        case _weapon_charged: {
            ...
            high_level_data
                    = get_shape_animation_data(BUILD_DESCRIPTOR(definition->collection, definition->charged_shape));
        ...
        case _weapon_finishing_reload:
            if (definition->reloading_shape != NONE) {
                high_level_data = get_shape_animation_data(
                        BUILD_DESCRIPTOR(definition->collection, definition->reloading_shape));
                prevent_wrap = true;
            } else {
                trigger->sequence = 0;
            }
...
static bool get_weapon_data_type_for_count(short player_index, short count, short* type, short* index, short* flags) {
    ...
    *index = NONE;
    ...
    if ((definition->flags & _weapon_is_marathon_1) && (definition == get_weapon_definition(_weapon_missile_launcher))
...
/*static*/ bool player_has_valid_weapon(short player_index) {
    // LP change:
    if (CannotWieldWeapons() || !LuaPlayerCanWieldWeapons(player_index))
        return false;
...
static void idle_weapon(short player_index) {
    ...
    if (player_weapons->desired_weapon != player_weapons->current_weapon) {
        if (player_weapons->current_weapon == NONE) {
            ...
            active_triggers = get_active_trigger_count_and_states(player_index, player_weapons->current_weapon, 0l,
                                                                  &first_trigger, NULL);
...
static void test_raise_double_weapon(short player_index, uint32* action_flags) {
    ...
    if (definition->weapon_class == _twofisted_pistol_class
        || (definition->weapon_class == _melee_class && !(definition->flags & _weapon_is_marathon_1))) {
        struct player_weapon_data* player_weapons = get_player_weapon_data(player_index);
        ...
        if (player_weapons->desired_weapon == player_weapons->current_weapon) {
            ...
            if (player->items[definition->item_type] > 1) {
                if (((*action_flags) & _left_trigger_state) && !PRIMARY_WEAPON_IS_VALID(weapon)) {
                    ...
                    if (weapon->triggers[_primary_weapon].rounds_loaded || trigger_definition->ammunition_type == NONE
                        || player->items[trigger_definition->ammunition_type] > 0) {
                        // dprintf("1 twofist;g");
                    ...
                    if (weapon->triggers[_secondary_weapon].rounds_loaded || trigger_definition->ammunition_type == NONE
                        || player->items[trigger_definition->ammunition_type] > 0) {
                        // dprintf("2 twofist;g");
                        SET_WEAPON_WANTS_TWOFIST(weapon, true);
        ...
        case _twofisted_pistol_class:
            ...
            /* we raise the second one. */
            {
                ...
                if (player->items[definition->item_type] == 1) {
                    ...
                    struct trigger_data* trigger
                            = get_trigger_data(player_index, player_weapons->desired_weapon, _primary_weapon);
                    struct trigger_definition* trigger_definition
                            = get_trigger_definition(player_index, player_weapons->desired_weapon, _primary_weapon);
                    ...
                    } else if (get_trigger_data(player_index, player_weapons->desired_weapon, _secondary_weapon)
                                       ->rounds_loaded) {
                        // assert(get_trigger_data(player_index, player_weapons->desired_weapon,
                        // _secondary_weapon)->rounds_loaded);
                        first_trigger = _secondary_weapon;
                        trigger_count = 2;
                    } else {
                        first_trigger = _primary_weapon;
...
static bool automatic_still_firing(short player_index, short which_trigger) {
    ...
    if (which_trigger == _primary_weapon
        ...
        if (definition->flags & _weapon_is_automatic) {
            if (TRIGGER_IS_DOWN(weapon)
                && weapon->triggers[which_trigger].ticks_since_last_shot < AUTOMATIC_STILL_FIRING_DURATION) {
                still_firing = true;
            }
        }
    }
...
static void initialize_shell_casings(short player_index) {
    ...
    for (shell_casing_index = 0, shell_casing = get_player_weapon_data(player_index)->shell_casings;
         shell_casing_index < MAXIMUM_SHELL_CASINGS; ++shell_casing_index, ++shell_casing) {
...
static short new_shell_casing(short player_index, short type, uint16 flags) {
    ...
    if (shell_casing_index == MAXIMUM_SHELL_CASINGS) {
        ...
        shell_casing->vx = SHELL_CASING_IS_REVERSED(shell_casing) ? -definition->vx0 : definition->vx0,
        shell_casing->vy = definition->vy0;
...
static bool get_shell_casing_display_data(struct weapon_display_information* display, short index) {
    ...
    for (shell_casing_index = 0, shell_casing = get_player_weapon_data(current_player_index)->shell_casings;
         shell_casing_index < MAXIMUM_SHELL_CASINGS; ++shell_casing_index, ++shell_casing) {
        if (SLOT_IS_USED(shell_casing)) {
            if ((index -= 1) < 0) {
                if (display) {
                    ...
                    // animate it
                    if ((shell_casing->frame += 1) >= high_level_data->frames_per_view)
                        shell_casing->frame = 0;
                    ...
                    display->vertical_position   = FIXED_ONE - shell_casing->y,
                    display->horizontal_position = shell_casing->x;
...
inline void StreamToWeapData(uint8*& S, weapon_data& Object) {
    ...
    for (int k = 0; k < NUMBER_OF_TRIGGERS; k++) StreamToTrigData(S, Object.triggers[k]);
}
...
inline void WeapDataToStream(uint8*& S, weapon_data& Object) {
    ...
    for (int k = 0; k < NUMBER_OF_TRIGGERS; k++) TrigDataToStream(S, Object.triggers[k]);
}
...
uint8* unpack_player_weapon_data(uint8* Stream, size_t Count) {
    ...
    for (size_t k = 0; k < Count; k++, ObjPtr++) {
        ...
        for (unsigned m = 0; m < NUMBER_OF_WEAPONS; m++) StreamToWeapData(S, ObjPtr->weapons[m]);
        for (unsigned m = 0; m < MAXIMUM_SHELL_CASINGS; m++) StreamToShellData(S, ObjPtr->shell_casings[m]);
    }
...
uint8* pack_player_weapon_data(uint8* Stream, size_t Count) {
    ...
    for (size_t k = 0; k < Count; k++, ObjPtr++) {
        ...
        for (size_t m = 0; m < NUMBER_OF_WEAPONS; m++) WeapDataToStream(S, ObjPtr->weapons[m]);
        for (size_t m = 0; m < MAXIMUM_SHELL_CASINGS; m++) ShellDataToStream(S, ObjPtr->shell_casings[m]);
    }
    assert((S - Stream) == static_cast<ptrdiff_t>(Count * SIZEOF_player_weapon_data));
    return S;
}
...
uint8* unpack_weapon_definition(uint8* Stream, weapon_definition* Objects, size_t Count) {
    ...
    for (size_t k = 0; k < Count; k++, ObjPtr++) {
        ...
        for (int m = 0; m < NUMBER_OF_TRIGGERS; m++) StreamToTrigDefData(S, ObjPtr->weapons_by_trigger[m]);
    }
...
uint8* pack_weapon_definition(uint8* Stream, weapon_definition* Objects, size_t Count) {
    ...
    for (size_t k = 0; k < Count; k++, ObjPtr++) {
        ...
        for (int m = 0; m < NUMBER_OF_TRIGGERS; m++) TrigDefDataToStream(S, ObjPtr->weapons_by_trigger[m]);
    }
    assert((S - Stream) == static_cast<ptrdiff_t>(Count * SIZEOF_weapon_definition));
    return S;
}

void init_weapon_definitions() { memcpy(weapon_definitions, original_weapon_definitions, sizeof(weapon_definitions)); }
...
void reset_mml_weapons() {
    ...
    if (original_weapon_ordering_array) {
        for (unsigned i = 0; i < NUMBER_OF_WEAPONS; i++) weapon_ordering_array[i] = original_weapon_ordering_array[i];
...
void parse_mml_weapons(const InfoTree& root) {
    ...
    if (!original_shell_casing_definitions) {
        original_shell_casing_definitions = (struct shell_casing_definition*)malloc(
                sizeof(struct shell_casing_definition) * NUMBER_OF_SHELL_CASING_TYPES);
    ...
    if (!original_weapon_ordering_array) {
        ...
        for (unsigned i = 0; i < NUMBER_OF_WEAPONS; i++) original_weapon_ordering_array[i] = weapon_ordering_array[i];
    }
