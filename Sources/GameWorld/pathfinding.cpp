...
static void calculate_midpoint_of_shared_line(short polygon1, short polygon2, world_distance minimum_separation,
...
#ifdef VERIFY_PATH_SYNC
    if (path_validation_area)
        delete[] path_validation_area;
...
short new_path(world_point2d* source_point, short source_polygon_index, world_point2d* destination_point,
...
#ifdef VALIDATE_PATH_SPACE
    {
        ...
        for (i = 0; i < MAXIMUM_PATHS; ++i) {
            if (paths[i].step_count != NONE) {
                vassert(paths[i].step_count >= 0 && paths[i].step_count <= MAXIMUM_POINTS_PER_PATH
                                && paths[i].current_step >= 0 && paths[i].current_step <= paths[i].step_count,
    ...
    if (path_index != NONE) {
        ...
        if (destination_polygon_index != NONE) {
            ...
            polygon_index = flood_map(source_polygon_index, INT32_MAX, cost, _breadth_first, data);
            while (polygon_index != NONE) { polygon_index = flood_map(NONE, INT32_MAX, cost, _breadth_first, data); }
        ...
        if (step_count > 0) /* if we have valid steps, extract the path */
        {
            ...
            while ((polygon_index = reverse_flood_map()) != NONE) {
                ...
                //				if (polygon_index!=source_polygon_index&&--step_count<MAXIMUM_POINTS_PER_PATH)
                //find_center_of_polygon(polygon_index, path->points+step_count);
                last_polygon_index = polygon_index;
            }
            assert(!step_count); /* we should be out of points */

            //			dprintf("path from %d to %d (%d steps);dm #%d #%d;g", source_polygon_index,
            //destination_polygon_index, paths[path_index].step_count, 				path_points,
            //paths[path_index].step_count*sizeof(world_point2d));
            ...
            if (path_run_count == 1) {
                ...
                if (memcmp(path_validation_area + path_validation_area_index, path->points,
                           sizeof(world_point2d) * path->step_count)) {
                    dprintf("path #%d at %p didn’t match point list at %p", path_index, path,
                            path_validation_area + path_validation_area_index);
                }
                path_validation_area_index += sizeof(world_point2d*) * path->step_count;
            }
#endif
...
bool move_along_path(short path_index, world_point2d* p) {
    ...
    vassert(path->current_step >= 0 && path->current_step <= path->step_count,
            csprintf(temporary, "invalid current path step: #%d/#%d", path->current_step, path->step_count));
    ...
    if (path->current_step == path->step_count) {
        ...
        *p = path->points[path->current_step++];
        //		vwarn(valid_point2d(p), csprintf(temporary, "step #%d (%d,%d) of path %p looks bad;g;",
        //path->current_step-1, p->x, p->y, path));
    }
...
void delete_path(short path_index) {
    ...
    vassert(paths[path_index].current_step >= 0 && paths[path_index].current_step <= paths[path_index].step_count,
            csprintf(temporary, "invalid current path step: #%d/#%d", paths[path_index].current_step,
                     paths[path_index].step_count));
