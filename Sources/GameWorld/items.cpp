...
#include "player.hpp"
#include "weapons.hpp" /* needed for process_new_item_for_reloading */
...
static item_definition* get_item_definition(const short type);
...
static int32 item_trigger_cost_function(short source_polygon_index, short line_index, short destination_polygon_index,
                                        void* unused);
...
item_definition* get_item_definition_external(const short type) { return get_item_definition(type); }
...
int16 item_definition::get_maximum_count_per_player(bool is_m1, int difficulty_level) const {
    if (has_extended_maximum_count[difficulty_level]) {
        ...
        if (difficulty_level == _total_carnage_level && (is_m1 || item_kind == _ammunition)) {
            return INT16_MAX;
        } else {
...
short new_item(struct object_location* location, short type) {
    ...
    if (dynamic_world->player_count > 1) {
        ...
        if (definition->invalid_environments & _environment_single_player)
            add_item = false;
    }
...
short count_inventory_lines(short player_index) {
    ...
    for (loop = 0; loop < NUMBER_OF_ITEM_TYPES; ++loop) { types[loop] = false; }
            ...
            if (!POLYGON_IS_DETACHED(neighboring_polygon)) {
                ...
                while (next_object != NONE) {
                    ...
                    if (GET_OBJECT_OWNER(object) == _object_is_item && !OBJECT_IS_INVISIBLE(object)) {
                        if (guess_distance2d((world_point2d*)&player->location, (world_point2d*)&object->location)
                            <= MAXIMUM_ARM_REACH) {
                            ...
                            if (object->location.z >= player->location.z - MAXIMUM_ARM_REACH
                                ...
                                && test_item_retrieval(player_object->polygon, &player_object->location,
                                                       &object->location)) {
...
void mark_item_collections(bool loading) { mark_collection(_collection_items, loading); }
    ...
    switch (definition->item_kind) {
        ...
        case _ball:
            ...
            if (find_player_ball_color(player_index) == NONE) {
                ...
                if ((GET_GAME_TYPE() == _game_of_capture_the_flag) && (type - BALL_ITEM_BASE == player->team)) {
        ...
        case _item:
            ...
            if (player->items[type] == NONE) {
                ...
                player->items[type] = 1;
                success             = true;
            } else if (player->items[type] + 1 <= definition->get_maximum_count_per_player(
...
void reset_mml_items() {
    if (original_item_definitions) {
        for (unsigned i = 0; i < NUMBER_OF_DEFINED_ITEMS; i++) item_definitions[i] = original_item_definitions[i];
...
void parse_mml_items(const InfoTree& root) {
    ...
    if (!original_item_definitions) {
        original_item_definitions
                = (struct item_definition*)malloc(sizeof(struct item_definition) * NUMBER_OF_DEFINED_ITEMS);
        assert(original_item_definitions);
        for (unsigned i = 0; i < NUMBER_OF_DEFINED_ITEMS; i++) original_item_definitions[i] = item_definitions[i];
    }
    ...
    for (const InfoTree& itree : root.children_named("item")) {
        ...
        for (const InfoTree& shape : itree.children_named("shape")) shape.read_shape(def.base_shape);
    }
}
