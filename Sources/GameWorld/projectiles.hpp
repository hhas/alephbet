...
#define SET_PROJECTILE_FLYBY_STATUS(p, v) \
    ((void)((v) ? ((p)->flags |= (uint16)0x4000) : ((p)->flags &= (uint16)~0x4000)))

/* only used for persistent projectiles */
#define PROJECTILE_HAS_CAUSED_DAMAGE(p) ((p)->flags & (uint16)0x2000)
#define SET_PROJECTILE_DAMAGE_STATUS(p, v) \
    ((void)((v) ? ((p)->flags |= (uint16)0x2000) : ((p)->flags &= (uint16)~0x2000)))

#define PROJECTILE_HAS_CROSSED_MEDIA_BOUNDARY(p) ((p)->flags & (uint16)0x1000)
#define SET_PROJECTILE_CROSSED_MEDIA_BOUNDARY_STATUS(p, v) \
    ((v) ? ((p)->flags |= (uint16)0x1000) : ((p)->flags &= (uint16)~0x1000))
...
bool preflight_projectile(world_point3d* origin, short polygon_index, world_point3d* _vector, angle delta_theta,
                          short type, short owner, short owner_type, short* target_index);
short new_projectile(world_point3d* origin, short polygon_index, world_point3d* _vector, angle delta_theta, short type,
...
uint16 translate_projectile(short type, world_point3d* old_location, short old_polygon_index,
                            ...
                            short* obstruction_index, short* last_line_index, bool preflight, short projectile_indexx);
