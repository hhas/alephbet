...
#include "shell.hpp" // screen_printf()
// MH: Lua scripting
...
#define NUMBER_OF_CONTROL_PANEL_DEFINITIONS \
    (sizeof(control_panel_definitions) / sizeof(struct control_panel_definition))
...
static struct control_panel_definition control_panel_definitions[] = {
        ...
        {_panel_is_double_shield_refuel,
         0,                                 _collection_walls1,
         2,                                                        3,
         {_snd_energy_refuel, NONE, NONE},
         FIXED_ONE + FIXED_ONE / 8,
         NONE                                                                                                                            },
        {_panel_is_tag_switch,
         0,                                 _collection_walls1,
         0,                                                        1,
         {_snd_chip_insertion, NONE, NONE},
         FIXED_ONE,                                                                                                        _i_uplink_chip},
        {_panel_is_light_switch,
         0,                                 _collection_walls1,
         0,                                                        1,
         {_snd_switch_on, _snd_switch_off, _snd_cant_toggle_switch},
         FIXED_ONE,                                                                                                        NONE          },
        ...
        {_panel_is_light_switch,
         0,                                 _collection_walls5,
         0,                                                        1,
         {_snd_pfhor_switch_on, _snd_pfhor_switch_off, _snd_cant_toggle_switch},
         FIXED_ONE,                                                                                                        NONE          },
        {_panel_is_platform_switch,
         0,                                 _collection_walls5,
         0,                                                        1,
            ...
            switch (definition->_class) {
                ...
                case _panel_is_platform_switch:
                    if (side->control_panel_permutation >= 0) {
                        if (platform_is_on(get_polygon_data(side->control_panel_permutation)->permutation))
                            status = true;
                    } else {
                        SET_SIDE_CONTROL_PANEL(side, false);
                        continue;
...
void update_control_panels(void) {
    ...
    for (player_index = 0, player = players; player_index < dynamic_world->player_count; ++player_index, ++player) {
        ...
        if ((side_index = player->control_panel_side_index) != NONE) {
            ...
            if (definition->_class == _panel_is_pattern_buffer) { // Player was working on a full-auto save
                if (dynamic_world->tick_count - player->ticks_at_last_successful_save
                    > kDoubleClickTicks) { // no double-click - need to safe save
...
bool untoggled_repair_switches_on_level(bool only_last_switch) {
    short side_index;
    struct side_data* side;
    bool untoggled_switch = false;

    for (side_index = 0, side = map_sides;
         side_index < dynamic_world->side_count && (!untoggled_switch || only_last_switch); ++side_index, ++side) {
        if (SIDE_IS_CONTROL_PANEL(side) && SIDE_IS_REPAIR_SWITCH(side)) {
            ...
            switch (definition->_class) {
                case _panel_is_platform_switch:
                    untoggled_switch = platform_is_at_initial_state(
                                               get_polygon_data(side->control_panel_permutation)->permutation)
                                               ? true
                                               : false;
                    break;
...
void assume_correct_switch_position(short switch_type, /* platform or light */
                                    short permutation,
                                    /* platform or light index */ /* ghs: appears to be polygon, not platform */
                                    bool new_state) {
    short side_index;
    struct side_data* side;
                ...
                switch (definition->_class) {
                    case _panel_is_tag_switch:
                        ...
                        if (!side->control_panel_permutation)
                            make_sound = true;
                        if (film_profile.chip_insertion_ignores_tag_state && definition->item != NONE)
                            make_sound = true;
                        if (make_sound) {
...
short find_action_key_target(short player_index, world_distance range, short* target_type, bool perform_panel_actions) {
    ...
    *target_type = _target_is_unrecognized;
    ...
    //	dprintf("#%d(#%d,#%d) --> (#%d,#%d) (#%d along #%d)", current_polygon, player->location.x, player->location.y,
    //destination.x, destination.y, range, player->facing);

    itemhit = NONE;
    while (!done) {
        line_index
                = find_line_crossed_leaving_polygon(current_polygon, (world_point2d*)&player->location, &destination);
        ...
        if (line_index == NONE) {
            ...
            //			dprintf("leaving polygon #%d through line #%d to polygon #%d", original_polygon, line_index,
            //current_polygon);
            ...
            if (current_polygon != NONE) {
                ...
                if (polygon->type == _polygon_is_platform
                    && line_is_within_range(player->monster_index, line_index, MAXIMUM_PLATFORM_ACTIVATION_RANGE)
...
static void change_panel_state(short player_index, short panel_side_index) {
    ...
    switch (definition->_class) {
        ...
        case _panel_is_double_shield_refuel:
        case _panel_is_triple_shield_refuel:
            player->control_panel_side_index
                    = player->control_panel_side_index == panel_side_index ? NONE : panel_side_index;
        ...
        case _panel_is_computer_terminal:
            if (get_game_state() == _game_in_progress && !PLAYER_HAS_CHEATED(player) && !PLAYER_HAS_MAP_OPEN(player)) {
                ...
                enter_computer_interface(player_index, side->control_panel_permutation,
        ...
        case _panel_is_platform_switch:
            state      = !state;
            make_sound = try_and_change_platform_state(get_polygon_data(side->control_panel_permutation)->permutation,
                                                       state);
        ...
        case _panel_is_pattern_buffer:
            if (player_controlling_game() && !PLAYER_HAS_CHEATED(local_player)) {
                if (game_is_networked) {
                    if (player->control_panel_side_index != panel_side_index) {
                        if (dynamic_world->tick_count - player->ticks_at_last_successful_save
                            > MINIMUM_RESAVE_TICKS) { // User pressed "action" - we'll see if they're going to do it
                                                      // again.
...
void set_control_panel_texture(struct side_data* side) {
    ...
    side->primary_texture.texture
            = BUILD_DESCRIPTOR(definition->collection,
                               GET_CONTROL_PANEL_STATUS(side) ? definition->active_shape : definition->inactive_shape);
}

static bool switch_can_be_toggled(short side_index, bool player_hit, bool* should_destroy_switch) {
...
void parse_mml_control_panels(const InfoTree& root) {
    ...
    if (!original_control_panel_settings) {
        original_control_panel_settings
                = (struct control_panel_settings_definition*)malloc(sizeof(struct control_panel_settings_definition));
        assert(original_control_panel_settings);
        *original_control_panel_settings = control_panel_settings;
    }

    if (!original_control_panel_definitions) {
        original_control_panel_definitions = (struct control_panel_definition*)malloc(
                sizeof(struct control_panel_definition) * NUMBER_OF_CONTROL_PANEL_DEFINITIONS);
