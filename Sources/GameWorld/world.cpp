...
world_point2d* rotate_point2d(world_point2d* point, world_point2d* origin, angle theta) {
    ...
    point->x
            = ((temp.i * cosine_table[theta]) >> TRIG_SHIFT) + ((temp.j * sine_table[theta]) >> TRIG_SHIFT) + origin->x;
    point->y
            = ((temp.j * cosine_table[theta]) >> TRIG_SHIFT) - ((temp.i * sine_table[theta]) >> TRIG_SHIFT) + origin->y;
...
void build_trig_tables(void) {
    ...
    for (i = 0; i < NUMBER_OF_ANGLES; ++i) {
        ...
        if (i == 0)
            sine_table[i] = 0, cosine_table[i] = TRIG_MAGNITUDE;
        if (i == QUARTER_CIRCLE)
            sine_table[i] = TRIG_MAGNITUDE, cosine_table[i] = 0;
        if (i == HALF_CIRCLE)
            sine_table[i] = 0, cosine_table[i] = -TRIG_MAGNITUDE;
        if (i == THREE_QUARTER_CIRCLE)
            sine_table[i] = -TRIG_MAGNITUDE, cosine_table[i] = 0;
...
//
static angle a1_arctangent(int32 x, // world_distance x,
...
void set_random_seed(uint16 seed) { random_seed = seed ? seed : DEFAULT_RANDOM_SEED; }

uint16 get_random_seed(void) { return random_seed; }
...
static int32 m2_distance2d_int32(const world_point2d* p0, const world_point2d* p1) {
...
static world_distance m2_distance2d(world_point2d* p0, world_point2d* p1) { return int16(m2_distance2d_int32(p0, p1)); }
...
void overflow_short_to_long_2d(world_point2d& WVec, uint16& flags, long_vector2d& LVec) {
    // Move lower values
    LVec.i = int32(WVec.x) & 0x0000'ffff;
    LVec.j = int32(WVec.y) & 0x0000'ffff;
