...
class ConcreteTickBasedCircularQueue : public WritableTickBasedCircularQueue<tValueType> {
  public:
    ...
    ConcreteTickBasedCircularQueue(int inBufferCapacity) : mBufferSize(inBufferCapacity + 1) {
    ...
    ConcreteTickBasedCircularQueue(const ConcreteTickBasedCircularQueue<tValueType>& o)
        ...
        mFlagsBuffer = new tValueType[mBufferSize];
        for (int32 tick = getReadTick(); tick < getWriteTick(); tick++) elementForTick(tick) = o.peek(tick);
    }
  ...
  protected:
    ...
    //        const uint32& elementForTick(int32 inTick) const { assert(inTick >= mReadTick); assert(inTick <
    //        mWriteTick); return mFlagsBuffer[inTick % mBufferSize]; }
...
class MutableElementsTickBasedCircularQueue : public ConcreteTickBasedCircularQueue<tValueType> {
  public:
    ...
    MutableElementsTickBasedCircularQueue(int inBufferCapacity)
        : ConcreteTickBasedCircularQueue<tValueType>(inBufferCapacity) {}
