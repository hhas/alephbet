...
#define SET_ABSOLUTE_YAW(i, y) \
    (((i) & ~((MAXIMUM_ABSOLUTE_YAW - 1) << (_absolute_yaw_mode_bit + 1))) | ((y) << (_absolute_yaw_mode_bit + 1)))
...
#define SET_ABSOLUTE_PITCH(i, y)                                               \
    (((i) & ~((MAXIMUM_ABSOLUTE_PITCH - 1) << (_absolute_pitch_mode_bit + 1))) \
     | ((y) << (_absolute_pitch_mode_bit + 1)))
...
#define SET_ABSOLUTE_POSITION(i, y)                                                  \
    (((i) & ~((MAXIMUM_ABSOLUTE_POSITION - 1) << (_absolute_position_mode_bit + 1))) \
     | ((y) << (_absolute_position_mode_bit + 1)))
...
#define PLAYER_PERSISTANT_FLAGS \
    (_player_has_cheated_flag | _player_doesnt_auto_switch_weapons_flag | _player_is_zombie_flag)

#define PLAYER_IS_DEAD(p) ((p)->flags & _player_is_dead_flag)
#define SET_PLAYER_DEAD_STATUS(p, v) \
    ((void)((v) ? ((p)->flags |= (uint16)_player_is_dead_flag) : ((p)->flags &= (uint16)~_player_is_dead_flag)))

#define PLAYER_IS_ZOMBIE(p) ((p)->flags & _player_is_zombie_flag)
#define SET_PLAYER_ZOMBIE_STATUS(p, v) \
    ((v) ? ((p)->flags |= (uint16)_player_is_zombie_flag) : ((p)->flags &= (uint16)~_player_is_zombie_flag))

#define PLAYER_IS_PFHORTRAN_CONTROLLED(p) ((p)->flags & _player_is_pfhortran_controlled_flag)
#define SET_PLAYER_IS_PFHORTRAN_CONTROLLED_STATUS(p, v)                 \
    ((v) ? ((p)->flags |= (uint16)_player_is_pfhortran_controlled_flag) \
...
#define SET_PLAYER_TOTALLY_DEAD_STATUS(p, v)                           \
    ((void)((v) ? ((p)->flags |= (uint16)_player_is_totally_dead_flag) \
                : ((p)->flags &= (uint16)~_player_is_totally_dead_flag)))

#define PLAYER_HAS_MAP_OPEN(p) ((p)->flags & _player_has_map_open_flag)
#define SET_PLAYER_MAP_STATUS(p, v)                                 \
    ((void)((v) ? ((p)->flags |= (uint16)_player_has_map_open_flag) \
                : ((p)->flags &= (uint16)~_player_has_map_open_flag)))

#define PLAYER_IS_TELEPORTING(p) ((p)->flags & _player_is_teleporting_flag)
#define SET_PLAYER_TELEPORTING_STATUS(p, v)                           \
    ((void)((v) ? ((p)->flags |= (uint16)_player_is_teleporting_flag) \
...
#define SET_PLAYER_INTERLEVEL_TELEPORTING_STATUS(p, v)                           \
    ((void)((v) ? ((p)->flags |= (uint16)_player_is_interlevel_teleporting_flag) \
...
#define SET_PLAYER_DOESNT_AUTO_SWITCH_WEAPONS_STATUS(p, v)                        \
    ((void)((v) ? ((p)->flags |= (uint16)_player_doesnt_auto_switch_weapons_flag) \
                : ((p)->flags &= (uint16)~_player_doesnt_auto_switch_weapons_flag)))
...
struct player_data {
    ...
    int16 monster_index; /* this player’s entry in the monster list */
    ...
    /* teleporting */
    int16 delay_before_teleport;   /* This is only valid for interlevel teleports (teleporting_destination is a negative
                                      number) */
    ...
    int16 control_panel_side_index; // NONE, or the side index of a control panel the user is using that requires
                                    // passage of time
...
constexpr new_player_flags new_player_make_local = 1u << 0, new_player_make_current = 1u << 1,
